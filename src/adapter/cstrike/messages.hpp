...
{
	std::uint8_t count{0};
	std::array<std::array<char, kMaxCommandTokenBytes>, kMaxCommandTokens> tokens{};
...
using MessageEventSink = void (*)(void* context, const MessageEvent& event) noexcept;

class MessageDecoder final
{
public:
	void configure(UserMessageIds ids, MessageEventSink sink, void* context) noexcept;
	void reset() noexcept;
	...
	MessageDecodeError lastError() const noexcept
	{
		return lastError_;
	}
	const MessageEvent& lastEvent() const noexcept
	{
		return lastEvent_;
	}
