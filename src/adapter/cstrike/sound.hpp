...
{
	std::uint64_t captured{}, processed{}, overflow{}, duplicates{}, invalid{}, unknown{}, expired{}, retired{},
		reentrant{};
...
public:
	...
	void emit(metamod::LifecycleCoordinator&, float time, const edict_t* emitter, const float* origin, int channel,
			  const char* sample, float volume, float attenuation, int flags, int pitch, bool ambient) noexcept;
	void playback(metamod::LifecycleCoordinator&, float time, int flags, const edict_t* invoker, std::uint16_t event,
				  float delay, const float* origin) noexcept;
	void frame(metamod::LifecycleCoordinator&, float time) noexcept;
	const core::world::SoundMemoryModel& memory() const noexcept
	{
		return memory_;
	}
	const SoundAdapterDiagnostics& diagnostics() const noexcept
	{
		return diagnostics_;
	}
	const EventCatalog& events() const noexcept
	{
		return events_;
	}
	...
	void rejectHook() noexcept
	{
		++diagnostics_.hookRejected;
	}
