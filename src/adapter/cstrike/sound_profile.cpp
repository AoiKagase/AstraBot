...
EventKind eventKind(const char* name) noexcept
{
	...
	static constexpr const char* gunshots[]{
		"events/ak47.sc",		 "events/aug.sc",	"events/awp.sc",	   "events/deagle.sc", "events/elite_left.sc",
		"events/elite_right.sc", "events/famas.sc", "events/fiveseven.sc", "events/g3sg1.sc",  "events/galil.sc",
		"events/glock18.sc",	 "events/m249.sc",	"events/m3.sc",		   "events/m4a1.sc",   "events/mac10.sc",
		"events/mp5n.sc",		 "events/p228.sc",	"events/p90.sc",	   "events/scout.sc",  "events/sg550.sc",
		"events/sg552.sc",		 "events/tmp.sc",	"events/ump45.sc",	   "events/usp.sc",	   "events/xm1014.sc"};
	for (const auto* candidate : gunshots)
		if (std::strcmp(name, candidate) == 0)
			return EventKind::Gunshot;
	if (std::strcmp(name, "events/createsmoke.sc") == 0)
		return EventKind::Smoke; // Mapping only; effects are P4-05.
...
core::perception::SoundKind soundSample(const char* name) noexcept
{
	...
	static constexpr const char* prefixes[]{"player/pl_step",	"player/pl_metal", "player/pl_dirt",  "player/pl_duct",
											"player/pl_grate",	"player/pl_tile",  "player/pl_slosh", "player/pl_wade",
											"player/pl_ladder", "player/pl_snow"};
	for (const auto* prefix : prefixes)
	{
		const auto length = std::strlen(prefix);
		if (std::strncmp(name, prefix, length) != 0)
			continue;
	...
	for (const auto* sample :
		 {"weapons/c4_explode1.wav", "weapons/flashbang-1.wav", "weapons/flashbang-2.wav", "weapons/sg_explode.wav"})
		if (std::strcmp(name, sample) == 0)
			return p::SoundKind::Explosion;
	return p::SoundKind::Unknown;
}
bool quantizeSound(p::Point point, p::SoundRegion& region) noexcept
{
	...
	if (!p::finite(point))
		return false;
	const double x = std::floor(point.x / p::SoundRegion::width), y = std::floor(point.y / p::SoundRegion::width),
				 z = std::floor(point.z / p::SoundRegion::width);
	...
	if (x < low || x > high || y < low || y > high || z < low || z > high)
		return false;
	region = {static_cast<std::int32_t>(x), static_cast<std::int32_t>(y), static_cast<std::int32_t>(z)};
	return true;
}
...
EventBinding EventCatalog::bind(std::uint16_t index, const char* name) noexcept
{
	...
	if (prior)
	{
		if (prior->conflict || std::strcmp(prior->name.data(), name) != 0)
		{
			prior->kind = EventKind::Unknown;
			prior->conflict = true;
			return EventBinding::Conflict;
		}
		return EventBinding::Duplicate;
	}
	if (count_ == entries_.size())
		return EventBinding::Full;
	auto& entry = entries_[count_++];
	entry.index = index;
	entry.kind = eventKind(name);
