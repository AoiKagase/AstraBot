...
{
	return connected(e, managed) && e->v.deadflag == DEAD_NO && std::isfinite(e->v.health) && e->v.health > 0 &&
		   e->v.iuser1 == 0 && (e->v.flags & FL_SPECTATOR) == 0;
}
p::Point point(const Vector& v) noexcept
{
	return {v.x, v.y, v.z};
}
p::Point eye(edict_t* e) noexcept
{
	return {double(e->v.origin.x) + e->v.view_ofs.x, double(e->v.origin.y) + e->v.view_ofs.y,
			double(e->v.origin.z) + e->v.view_ofs.z};
}
...
public:
	...
	{
		if (!player.isValid() || player.slot > entities.size())
			return false;
		const auto& b = entities[player.slot - 1U];
		const auto* member = owner.teams().find(player);
		if (member && member->team == p::Team::Spectator)
			return false;
		...
		return owner.registry().isMapActive() && owner.registry().mapGeneration() == stamp.map &&
			   owner.round() == stamp.round && owner.registry().currentTick() == stamp.tick && b.player == player &&
	...
	p::Reason trace(const p::SightRequest& request) noexcept override
	{
		if (!engine.pfnTraceLine)
			return p::Reason::MissingEngine;
		if (!current(request.stamp.observer, request.stamp) || !current(request.target, request.stamp))
			return p::Reason::StaleIdentity;
		if (!p::finite(request.start) || !p::finite(request.end))
			return p::Reason::InvalidGeometry;
		const auto effect = owner.visualEffects().blocked(request.stamp.observer, request.start, request.end);
		if (effect != p::Reason::None)
			return effect;
		const float start[]{static_cast<float>(request.start.x), static_cast<float>(request.start.y),
							static_cast<float>(request.start.z)};
		const float end[]{static_cast<float>(request.end.x), static_cast<float>(request.end.y),
						  static_cast<float>(request.end.z)};
		for (int i = 0; i < 3; ++i)
		...
		if (afterEffect != p::Reason::None)
			return afterEffect;
		if (!std::isfinite(result.flFraction) || result.flFraction < 0 || result.flFraction > 1 ||
			!p::finite(point(result.vecEndPos)) || (result.fStartSolid != 0 && result.fStartSolid != 1) ||
			(result.fAllSolid != 0 && result.fAllSolid != 1))
			return p::Reason::InvalidTrace;
		if (result.fStartSolid || result.fAllSolid)
			return p::Reason::Occluded;
		...
		if (std::hypot(double(result.vecEndPos.x) - (start[0] + (double(end[0]) - start[0]) * fraction),
			...
			return p::Reason::InvalidTrace;
		if (result.pHit == observer)
			return p::Reason::InvalidTrace;
		if (result.pHit == target)
			return p::Reason::None;
		if (result.flFraction < 1 || result.pHit)
			return p::Reason::Occluded;
		...
		if (std::hypot(double(result.vecEndPos.x) - end[0], double(result.vecEndPos.y) - end[1],
					   double(result.vecEndPos.z) - end[2]) > 0.125)
			return p::Reason::InvalidTrace;
		return p::Reason::None;
	}
};
...
bool VisionAdapter::synchronize(metamod::LifecycleCoordinator& owner, enginefuncs_t* engine) noexcept
{
	...
	{
		reset();
		map_ = registry.mapGeneration();
		if (owner.teams_.activate(map_))
		{
			owner.round_ = {1};
			owner.lastRoundTick_ = {};
			owner.lastRoundTime_ = -1;
		}
	}
	...
	bool creating = false;
	for (const auto& client : owner.clients_)
		creating = creating || client.fake.operationActive();
	for (std::uint16_t slot = 1; slot <= registry.clientMax(); ++slot)
	{
		...
		if (revision != revision_ || !registry.isMapActive() || registry.mapGeneration() != map_)
			return false;
		...
		if (prior.player.isValid() && !player.isValid() && valid && prior.entity == entity &&
			prior.serial == entity->serialnumber)
		{
			vision_.forget(prior.player);
			memory_.forget(prior.player);
			world_.forgetReports(prior.player);
			owner.teams_.forget(prior.player);
			continue;
		}
		if (prior.player.isValid() &&
			(!valid || prior.entity != entity || prior.serial != entity->serialnumber || prior.player != player))
		{
			vision_.forget(prior.player);
			memory_.forget(prior.player);
			world_.forgetReports(prior.player);
			...
			{
				(void)registry.disconnectPlayer(player);
				player = {};
			}
			roster_[i] = valid ? EntityBinding{} : prior;
		}
		if (!valid || (managed && (owner.entityFor(player) != entity || owner.removalPending(player))))
			continue;
		if (!player.isValid())
		{
						  ...
						  std::uint64_t frameMicros) noexcept
{
	...
	if (!synchronize(owner, engine))
	{
		...
		vision_.reset();
		error_ = p::Reason::MissingEngine;
		return;
	}
	p::InputFrame input{};
	input.map = map_;
	input.tick = registry.currentTick();
	input.timeMicros = frameMicros;
	...
	for (std::uint16_t slot = 1; slot <= registry.clientMax(); ++slot)
	{
		...
		if (!valid)
			continue;
		if (managed && (owner.entityFor(player) != entity || owner.removalPending(player)))
			continue;
		if (!bound(player, engine))
			continue;
		queries.entities[i] = {entity, entity->serialnumber, player};
		auto& sample = input.players[i];
		sample.player = player;
		sample.alive = alive(entity, managed);
		const auto* member = owner.teams().find(player);
		if (member && member->team == p::Team::Spectator)
			sample.alive = false;
		if (managed)
		{
			...
			if (sample.alive)
				sample.agent = binding.agent;
		}
		if (!sample.alive)
		{
			owner.visualEffects_.forget(player);
			continue;
		}
	...
	memoryFrame.map = input.map;
	memoryFrame.tick = input.tick;
	memoryFrame.timeMicros = input.timeMicros;
	memoryFrame.round = input.round;
	for (const auto& sample : input.players)
	{
		if (!sample.player.isValid())
			continue;
		...
		{
			vision_.forget(sample.player);
			memory_.forget(sample.player);
			world_.forgetReports(sample.player);
		}
		else
		{
	...
	if (!owner.world_.advance(memoryFrame, owner.teams()))
		return;
