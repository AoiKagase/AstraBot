...
struct JoinRequest
{
	...
	{
		return (team == Team::Terrorist || team == Team::CounterTerrorist) && classNumber >= 1U && classNumber <= 4U;
	}
...
struct JoinAction
{
	...
	static constexpr JoinAction noOp() noexcept
	{
		return {};
	}
...
{
public:
	JoinAction begin(host::PlayerId player, host::MapGeneration map, JoinRequest request,
					 host::TickId startTick) noexcept;
	JoinAction onMessage(const MessageEvent& event, host::TickId tick) noexcept;
	...
	host::PlayerId player() const noexcept
	{
		return player_;
	}
	host::MapGeneration map() const noexcept
	{
		return map_;
	}
	JoinRequest request() const noexcept
	{
		return request_;
	}
	host::TickId deadline() const noexcept
	{
		return deadline_;
	}
	...
	{
		return teamConfirmed_;
	}
	bool teamInfoReceived() const noexcept
	{
		return teamInfoReceived_;
	}
	bool classSelectionCompleted() const noexcept
	{
		return classSelectionCompleted_;
	}
	bool postClassFrameAdvanced() const noexcept
	{
		return postClassFrameAdvanced_;
	}
	bool pendingSelection() const noexcept
	{
		return pendingSelection_;
	}
	bool active() const noexcept
	{
		return phase_ != JoinPhase::Idle && phase_ != JoinPhase::Joined && phase_ != JoinPhase::Failed &&
