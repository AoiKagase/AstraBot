...
bool soundAudible(core::perception::Point source, core::perception::Point listener, double volume,
				  double attenuation) noexcept;
...
enum class EventBinding : std::uint8_t
{
	Registered,
	Duplicate,
	Unsupported,
...
public:
	...
	bool supports(EventKind kind) const noexcept
	{
		for (std::size_t i = 0; i < count_; ++i)
			if (entries_[i].kind == kind && !entries_[i].conflict)
				return true;
		return false;
	}
