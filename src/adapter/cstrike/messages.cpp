...
{
	return textEquals(value, "#Team_Select") || textEquals(value, "#IG_Team_Select") ||
		   textEquals(value, "#Terrorist_Select") || textEquals(value, "#CT_Select");
}

} // namespace

CommandParseError tokenizeCommand(const char* command, TokenizedCommand& result) noexcept
{
	result = {};
	if (command == nullptr)
	{
	...
	while (length <= kMaxCommandBytes && command[length] != '\0')
	{
		...
		{
			return command[length] == ';' ? CommandParseError::CommandSeparator : CommandParseError::TokenTooLong;
		}
		++length;
	}
	if (length == 0U)
	...
	while (index < length)
	{
		...
		while (index < length && (!isWhitespace(command[index]) || quoted))
		{
			...
			if (value == ';' || value == '\\')
			{
				return value == ';' ? CommandParseError::CommandSeparator : CommandParseError::TokenTooLong;
			}
			...
			++index;
			if (openedQuote && !quoted && index < length && !isWhitespace(command[index]))
			{
	...
	if (result.count >= 2U)
	{
		for (std::uint8_t index2 = 0; index2 < kMaxCommandTokenBytes && result.tokens[1][index2] != '\0'; ++index2)
		{
...
CommandParseError parseMenuSelect(const char* command, TokenizedCommand& result, std::uint8_t& selection) noexcept
{
	selection = 0;
...
void MessageDecoder::configure(UserMessageIds ids, MessageEventSink sink, void* context) noexcept
{
	ids_ = ids;
	sink_ = sink;
...
void MessageDecoder::begin(int messageType, std::uint16_t recipientSlot) noexcept
{
	if (active_)
	{
...
{
	if (kind_ == MessageKind::ScreenFade)
		return fieldIndex_ < 7 && expected == (fieldIndex_ < 3 ? FieldKind::Short : FieldKind::Byte);
	if (kind_ == MessageKind::Hltv)
		return fieldIndex_ < 2 && expected == FieldKind::Byte;
	...
	if (kind_ == MessageKind::VguiMenu)
	{
		static constexpr FieldKind fields[] = {
			FieldKind::Byte, FieldKind::Short, FieldKind::Char, FieldKind::Byte, FieldKind::String,
		};
		return fieldIndex_ < kVguiMenuFields && fields[fieldIndex_] == expected;
	}
	if (kind_ == MessageKind::ShowMenu)
	{
		...
		return fieldIndex_ < kShowMenuFields && fields[fieldIndex_] == expected;
	}
	if (kind_ == MessageKind::TeamInfo)
	{
		...
		return fieldIndex_ < kTeamInfoFields && fields[fieldIndex_] == expected;
	}
	return false;
}
...
void MessageDecoder::handleShowMenu() noexcept
{
	if (event_.needMore != 0U)
	{
		...
		while (fragmentLength <= kMaxMessageTextBytes && event_.text[fragmentLength] != '\0')
		{
			++fragmentLength;
		}
		...
		showMenuFragment_.length = static_cast<std::uint16_t>(currentLength + fragmentLength);
