...
std::uint8_t utilityCount(const core::economy::EquipmentSnapshot& inventory, core::economy::PurchaseItem item) noexcept
{
...
bool BuyExecutionAdapter::satisfied(const core::economy::EquipmentSnapshot& before,
									...
									core::economy::PurchaseItem item) noexcept
{
	...
	case PurchaseItem::Smg:
		return after.hasUsablePrimary() &&
			   (before.primary == core::economy::WeaponKind::None || before.primary != after.primary);
	...
	case PurchaseItem::Pistol:
		return after.hasUsableSecondary() &&
			   (before.secondary == core::economy::WeaponKind::None || before.secondary != after.secondary);
	...
	case PurchaseItem::Ammo:
		return after.primaryAmmo > before.primaryAmmo || after.secondaryAmmo > before.secondaryAmmo;
...
bool BuyExecutionAdapter::alreadyOwned(const core::economy::EquipmentSnapshot& inventory,
									   core::economy::PurchaseItem item) noexcept
{
	using core::economy::PurchaseItem;
	switch (item)
	{
	case PurchaseItem::Rifle:
		return inventory.primary != core::economy::WeaponKind::Smg && inventory.hasUsablePrimary();
	case PurchaseItem::Galil:
	case PurchaseItem::Famas:
	case PurchaseItem::Smg:
...
core::economy::BuyFailureReason BuyExecutionAdapter::mapFailure(DispatchStatus status) noexcept
{
...
PurchaseExecutionResult BuyExecutionAdapter::execute(const core::economy::BuyPlan& plan, const PurchaseContext& context,
													 IBuyOperations& operations) const noexcept
{
	PurchaseExecutionResult result{};
	if (!plan.valid() || !context.map.isValid() || !context.round.isValid() || !context.tick.isValid() ||
		!context.player.isValid() || !context.inventory.valid() || plan.map != context.map ||
		plan.round != context.round || plan.tick != context.tick || plan.player != context.player)
	{
	...
	for (std::size_t requestIndex = 0; requestIndex < plan.requestCount; ++requestIndex)
	{
		...
		for (std::uint8_t quantity = 0; quantity < request.quantity; ++quantity)
		{
			...
			for (std::size_t candidateIndex = 0; candidateIndex <= request.fallbackCount; ++candidateIndex)
			{
				...
				const auto item = candidateIndex == 0 ? request.item : request.fallbacks[candidateIndex - 1];
				...
				else
				{
					...
					if (status == DispatchStatus::NotInBuyZone || status == DispatchStatus::BuyTimeExpired)
					{
						return result;
					}
				}
