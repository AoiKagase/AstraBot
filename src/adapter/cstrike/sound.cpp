...
bool entityValid(const edict_t* entity) noexcept
{
	return entity && !entity->free;
}
p::Point position(const Vector& value) noexcept
{
	return {value.x, value.y, value.z};
}
bool same(p::Point a, p::Point b) noexcept
{
	return a.x == b.x && a.y == b.y && a.z == b.z;
}
} // namespace
void SoundAdapter::clearQueue() noexcept
{
	head_ = count_ = 0;
...
{
	if (!map.isValid() || map == map_ || (map_.isValid() && map.value < map_.value))
		return;
	if (map_.isValid())
		events_.reset(); // No ServerDeactivate/precache cycle was observed.
	++revision_;
...
{
	if (!round.isValid() || round.value <= round_.value)
		return;
	++revision_;
	round_ = round;
	diagnostics_.retired += count_;
	clearQueue();
	memory_.beginRound(round);
	keyCount_ = keyCursor_ = 0;
}
void SoundAdapter::forget(core::PlayerId player) noexcept
{
	if (!player.isValid() || player.slot > listeners_.size())
		return;
	++revision_;
	memory_.forget(player);
	auto& listener = listeners_[player.slot - 1U];
	if (listener.player == player)
	{
		listener = {};
		++epochs_[player.slot - 1U];
	}
}
void SoundAdapter::precache(int type, const char* name, std::uint16_t index) noexcept
{
	if (type != 1)
	{
		++diagnostics_.eventRejected;
		return;
	}
	const auto result = events_.bind(index, name);
	if (result == EventBinding::Registered)
		++diagnostics_.eventBindings;
	else if (result != EventBinding::Duplicate)
		++diagnostics_.eventRejected;
}
bool SoundAdapter::timestamp(float time, std::uint64_t& micros) noexcept
{
	const double value = static_cast<double>(time) * 1000000;
	if (!std::isfinite(value) || value < 0 || value >= 18446744073709551616.0 ||
		value < static_cast<double>(timeHighWater_))
	{
		++revision_;
		++diagnostics_.invalid;
		diagnostics_.retired += count_;
		clearQueue();
		memory_.invalidate(core::world::SoundReason::InvalidFrame);
		return false;
	}
	micros = static_cast<std::uint64_t>(value);
	timeHighWater_ = micros;
	return true;
}
bool SoundAdapter::synchronize(metamod::LifecycleCoordinator& owner) noexcept
{
	if (!owner.registry().isMapActive())
		return false;
	beginMap(owner.registry().mapGeneration());
	beginRound(owner.round());
	...
	for (std::uint16_t slot = 1; slot <= owner.registry().clientMax(); ++slot)
	{
		...
		if (!binding.isValid() || owner.removalPending(player))
			continue;
		const auto* join = owner.joinState(player);
		auto* entity = owner.entityFor(player);
		if (revision != revision_ || !owner.registry().isMapActive())
			return false;
		const auto* team = owner.teams().find(player);
		if (!join || join->phase() != JoinPhase::Joined || !entityValid(entity) || entity->v.deadflag != DEAD_NO ||
			!std::isfinite(entity->v.health) || entity->v.health <= 0 || entity->v.iuser1 != 0 ||
			(entity->v.flags & FL_SPECTATOR) != 0 || (team && team->team == p::Team::Spectator))
			continue;
	...
	for (std::size_t i = 0; i < current.size(); ++i)
	{
		...
		{
			memory_.forget(listeners_[i].player);
			++epochs_[i];
		}
...
void SoundAdapter::capture(metamod::LifecycleCoordinator& owner, float time, Key key, p::SoundKind kind) noexcept
{
	...
	for (std::size_t i = 0; i < keyCount_; ++i)
	{
		...
		if (key.time == prior.time && key.emitter == prior.emitter && key.serial == prior.serial &&
			key.channel == prior.channel && key.pitch == prior.pitch && key.event == prior.event &&
			key.ambient == prior.ambient && key.sample == prior.sample && same(key.source, prior.source) &&
			key.volume == prior.volume && key.attenuation == prior.attenuation)
		{
	...
	pending.volume = key.volume;
	pending.attenuation = key.attenuation;
	pending.identity = {map_, round_, p::ObservationSource::Sound, ++sequence_, key.time, key.time};
	pending.audience = listeners_;
	++count_;
	++diagnostics_.captured;
	diagnostics_.queued = count_;
	keys_[keyCursor_] = key;
	keyCursor_ = (keyCursor_ + 1) % keys_.size();
	if (keyCount_ < keys_.size())
		++keyCount_;
}
void SoundAdapter::emit(metamod::LifecycleCoordinator& owner, float time, const edict_t* emitter, const float* origin,
						...
						bool ambient) noexcept
{
	const auto kind = soundSample(sample);
	if (kind == p::SoundKind::Unknown)
	{
		++diagnostics_.unknown;
		return;
	}
	if ((!ambient && !entityValid(emitter)) || (ambient && !origin) || flags != 0 || channel < 0 || channel > 7 ||
		pitch < 1 || pitch > 255)
	{
		++diagnostics_.invalid;
		return;
	...
	Key key{};
	key.emitter = reinterpret_cast<std::uintptr_t>(emitter);
	key.serial = emitter ? emitter->serialnumber : 0;
	key.channel = channel;
	key.pitch = pitch;
	key.ambient = ambient;
	key.volume = volume;
	key.attenuation = attenuation;
...
{
	if (events_.find(event) != EventKind::Gunshot)
	{
		++diagnostics_.unknown;
		return;
	}
	...
	if ((flags != 0 && flags != FEV_NOTHOST) || delay != 0 || !origin || !entityValid(invoker))
	{
		++diagnostics_.invalid;
		return;
	}
	Key key{};
	key.event = event;
	key.emitter = reinterpret_cast<std::uintptr_t>(invoker);
	key.serial = invoker->serialnumber;
	key.source = {origin[0], origin[1], origin[2]};
	if (same(key.source, {}))
		key.source = position(invoker->v.origin);
	key.volume = 1;
	key.attenuation = 0.8;
	key.pitch = 100;
...
void SoundAdapter::frame(metamod::LifecycleCoordinator& owner, float time) noexcept
{
	...
	if (!timestamp(time, now))
		return;
	if (!owner.world_.collectingAt(map_, round_, owner.registry().currentTick(), now))
	{
		diagnostics_.retired += count_;
		clearQueue();
		return;
	...
	while (count_ != 0 && diagnostics_.frameEvents < eventsPerFrame)
	{
		...
		if (pending.identity.observedMicros > now)
			break;
		++diagnostics_.frameEvents;
		++diagnostics_.processed;
		if (pending.identity.map != map_ || pending.identity.round != round_)
			++diagnostics_.retired;
		else if (now - pending.identity.observedMicros >= 3000000)
			++diagnostics_.expired;
		else
		{
			p::SoundObservation observation{pending.identity, pending.kind, {}};
			observation.identity.receivedMicros = now;
			if (quantizeSound(pending.source, observation.region))
				for (std::size_t i = 0; i < listeners_.size(); ++i)
				{
					const auto& heard = pending.audience[i];
					const auto& current = listeners_[i];
					if (!heard.player.isValid())
						continue;
					++diagnostics_.frameAudienceChecks;
					if (heard.player != current.player || heard.agent != current.agent || heard.epoch != current.epoch)
					{
						++diagnostics_.recipientRejected;
						continue;
					}
