...
enum class LadderDiscoveryReason
{
	None,
	InvalidInput,
	ScanFailed,
...
struct LadderDiscoveryResult
{
	...
	{
		return reason == LadderDiscoveryReason::None && bool(value);
	}
};
enum class LadderBindingReason
{
	None,
	InvalidInput,
	WrongPublication,
...
struct LadderBindingResult
{
	...
	explicit operator bool() const noexcept
	{
		return reason == LadderBindingReason::None && value.has_value();
	}
};
...
LadderBindingResult bindLadderPlan(LadderWorld, core::MapGeneration, const nav::enrichment::NavMapFingerprint&,
								   const LadderDiscovery&, const nav::enrichment::NavTraversalLink& selected,
								   int maxEntities) noexcept;
...
LadderDiscoveryResult discoverLadderLinks(LadderWorld, core::MapGeneration, const nav::query::NavSpatialIndex&,
										  core::MapGeneration indexMap, const nav::enrichment::NavMapFingerprint&,
										  std::uint64_t generation, int maxEntities,
