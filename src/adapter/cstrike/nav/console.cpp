...
bool sameRouteEdge(const nav::query::NavDirectedEdge& left, const nav::query::NavDirectedEdge& right) noexcept
{
	if (left.source != right.source || left.target != right.target || left.traversal != right.traversal ||
		left.external.has_value() != right.external.has_value())
		return false;
	if (!left.external)
		return left.direction == right.direction;
	return left.external->sourceId == right.external->sourceId &&
		   left.external->generation == right.external->generation && left.external->linkId == right.external->linkId &&
		   left.external->direction == right.external->direction;
}

nav::query::NavCostDecision trafficCost(const nav::query::NavCostContext& input, const void* opaque) noexcept
{
	...
	auto result =
		policy->base.cost
			...
			: nav::query::NavCostDecision{false,
										  {input.geometricDistance, 0.0, 0.0,
										   input.edge.external ? input.edge.external->additionalCost : 0.0, 0.0}};
	if (result.blocked)
		return result;
	...
	if (waitSeconds > 0)
		result.components.danger += waitSeconds * 160.0;
	return result;
}

double trafficHeuristic(const nav::query::NavHeuristicContext& input, const void* opaque) noexcept
{
...
{
	auto& owner = metamod::lifecycleCoordinator();
	owner.navConsole().execute(command, owner);
}
void loadCommand()
{
	run(NavCommand::Load);
...
void statusCommand()
{
	run(NavCommand::Status);
}
void cancelCommand()
{
	run(NavCommand::Cancel);
}
void reportCommand()
{
	run(NavCommand::Report);
}
...
std::optional<core::PlayerId> parsePlayer(std::string_view text) noexcept
{
	...
	if (!slot || !generation || slot->value > host::kMaxClientSlots)
		return {};
...
{
	if (!player.isValid() || player.slot > actors_.size())
		return nullptr;
...
{
	if (inRequest_ || !player.isValid() || player.slot > actors_.size())
		return false;
	auto& actor = actors_[player.slot - 1U];
	try
	{
		if (!actor)
			actor = std::make_unique<ActorState>();
	}
	catch (...)
	{
		line("nav error=AllocationFailure");
		return false;
	}
	current_ = actor.get();
...
const nav::runtime::DecisionTrace* NavConsole::trace(core::PlayerId player) const noexcept
{
	...
	return actor && actor->session_ ? &actor->session_->trace() : nullptr;
}
const MotionTrace* NavConsole::motionTrace(core::PlayerId player) const noexcept
{
	const auto* actor = findActor(player);
	return actor ? &actor->motionTrace_ : nullptr;
}
std::size_t NavConsole::motionHistoryCount(core::PlayerId player) const noexcept
{
	const auto* actor = findActor(player);
	return actor ? actor->motionCount_ : 0;
}
const MotionTrace* NavConsole::motionHistory(core::PlayerId player, std::size_t index) const noexcept
{
	const auto* actor = findActor(player);
	return actor && index < actor->motionCount_
			   ? &actor->motionHistory_[(actor->motionNext_ + motionHistoryLimit - actor->motionCount_ + index) %
										motionHistoryLimit]
			   : nullptr;
}
...
{
	auto* actor = findActor(player);
	if (!actor)
		return;
	ActorScope scope(current_, actor);
	beforeDispatch(owner);
}
							   ...
							   const std::optional<MotionTrace>& ticket) noexcept
{
	...
	if (!actor)
		return;
	ActorScope scope(current_, actor);
	afterDispatch(result, tick, ticket);
}
...
void NavConsole::applyRuntimeNavigation(metamod::LifecycleCoordinator& owner,
										const metamod::RuntimeDecision& decision) noexcept
{
	...
	const bool roamDecision = decision.tactical.intent.type == core::tactical::IntentType::Roam;
	const bool explicitDecision = decision.tactical.intent.type == core::tactical::IntentType::Hold &&
	...
	const auto leaseIdentity = [](const ActorState& actor, const metamod::RuntimeDecision& candidate) noexcept {
		return nav::runtime::RouteGoalIdentity{candidate.agent, candidate.player, candidate.team.shared.map,
	...
	const auto activeLeaseIdentity = [&](const ActorState& actor) noexcept {
		...
		return nav::runtime::RouteGoalIdentity{
			trace ? trace->agent : core::BotAgentId{}, trace ? trace->actor : core::PlayerId{},
			owner.registry().mapGeneration(), owner.round(), trace ? trace->routeGeneration : 0};
	};
	if (!decision.executable || !decision.hasNavigationGoal)
	{
		...
		else
		{
			...
			if (actor && !actor->explicitRoute_ && actor->session_ && actor->session_->executable() &&
	...
	if (!decision.player.isValid() || !decision.agent.isValid() || !decision.navigationGoal.isValid())
	{
	...
	if (owner.registry().mapGeneration() != decision.team.shared.map || owner.round() != decision.team.shared.round ||
		owner.registry().currentTick() != decision.team.shared.tick || owner.registry().currentTick() == core::TickId{})
	{
	...
	if (s.kind != nav::runtime::ActorKind::ManagedBot || s.actor != decision.player || s.agent != decision.agent ||
		s.map != owner.registry().mapGeneration() || s.connected != true || s.alive != true || s.joined != true ||
	...
	if (current_->session_ && current_->session_->executable() && current_->session_->trace().actor == s.actor &&
		current_->session_->trace().agent == s.agent && current_->session_->trace().map == s.map &&
	...
	const bool sameRunningRoute =
		routeIdentity && routeInFlight && current_->session_->trace().goal == decision.navigationGoal;
	const bool preserveLeasedGoal = current_->session_ && current_->session_->executable() &&
	...
	const bool preserveRecoveryGoal =
		!current_->explicitRoute_ && current_->goalLease_.goal().isValid() &&
		current_->goalLease_.holds(activeLeaseIdentity(*current_)) &&
		(current_->execution_.state == nav::runtime::ExecutionState::Recovering ||
		 ...
		 (current_->execution_.state == nav::runtime::ExecutionState::Failed &&
		  (current_->execution_.searchWaiting(current_->navigationTimeUs_) ||
		   current_->execution_.goalCooling(current_->goalLease_.goal(), current_->navigationTimeUs_))));
	const bool preserveAutonomousRoam = roamDecision && !current_->explicitRoute_ && routeIdentity && routeInFlight;
	...
	if (current_->execution_.goalCooling(decision.navigationGoal, current_->navigationTimeUs_) ||
		current_->execution_.saturated() || current_->execution_.searchWaiting(current_->navigationTimeUs_))
	{
		reject(RuntimeNavigationApplyReason::RouteRejected);
		return;
	}
	...
	if (!goalVertex)
	{
		if (roamDecision && current_->roamRejectedGoalCount_ < current_->roamRejectedGoals_.size())
		{
			...
			if (!duplicate)
				current_->roamRejectedGoals_[current_->roamRejectedGoalCount_++] = decision.navigationGoal;
		}
		reject(RuntimeNavigationApplyReason::RouteRejected);
		return;
	}
		...
		current_->execution_.state == nav::runtime::ExecutionState::Running)
	{
		(void)current_->goalLease_.acquire(
			{s.agent, s.actor, s.map, owner.round(), current_->session_->trace().routeGeneration},
...
void NavConsole::configure(enginefuncs_t* engine, mutil_funcs_t* utility, globalvars_t* globals) noexcept
{
	...
	utility_ = utility;
	globals_ = globals;
	if (!engine_ || !engine_->pfnAddServerCommand || !engine_->pfnCmd_Argc || !engine_->pfnCmd_Argv)
		return;
...
void NavConsole::sink(void* ctx, const char* text) noexcept
{
	static_cast<NavConsole*>(ctx)->line(text);
}
void NavConsole::line(const char* text) noexcept
{
	auto& debug = metamod::ConsoleDebug::instance();
	if (!debug.navEnabled() || !text)
		return;
	debug.navLine(text);
}
void NavConsole::printUpdate(const nav::runtime::SessionUpdate& update) noexcept
{
	for (std::size_t i = 0; i < update.count; ++i)
		debug::printNavTrace(update.events[i], &sink, this);
	if (!update.count && !update.accepted)
	{
		char text[96]{};
		std::snprintf(text, sizeof(text), "nav rejected reason=%u", unsigned(update.reason));
		line(text);
	}
}
...
void NavConsole::invalidateActor(core::PlayerId player, nav::runtime::SessionReason reason) noexcept
{
	...
	if (!actor)
		return;
	if (inRequest_ && current_ == actor)
	{
		clearPending();
		deferredInvalidation_ = reason;
		return;
	}
	ActorScope scope(current_, actor);
...
bool NavConsole::applyDeferredInvalidation() noexcept
{
	...
	const bool all = deferredAll_, resetPending = deferredReset_;
	deferredInvalidation_.reset();
	deferredAll_ = deferredReset_ = false;
	if (all)
		invalidate(reason);
	else
		invalidateCurrent(reason);
...
void NavConsole::reset() noexcept
{
	if (inRequest_)
	{
		invalidate(nav::runtime::SessionReason::Cancelled);
		deferredReset_ = true;
		return;
	}
	invalidate(nav::runtime::SessionReason::Cancelled);
											   ...
											   std::shared_ptr<const nav::model::NavMeshSnapshot> mesh) noexcept
{
	if (inRequest_)
	{
		deferredInvalidation_ = nav::runtime::SessionReason::GoalReplaced;
		deferredAll_ = true;
	...
	if (!distributionTopology_)
		return {nav::diagnostics::NavErrorKind::AllocationFailure};
	index_ = *index.value;
	mesh_ = std::move(mesh);
	navigation_ = {map, *graph.value};
	return {};
}
bool NavConsole::loadForMap(const char* path, core::MapGeneration map, metamod::LifecycleCoordinator& owner) noexcept
{
	if (!path || !*path || inRequest_ || deferredInvalidation_ || !map.isValid() || !owner.registry().isMapActive() ||
		owner.registry().mapGeneration() != map)
		return false;
...
bool NavConsole::load(const char* path, core::MapGeneration map, metamod::LifecycleCoordinator& owner) noexcept
{
	...
	try
	{
		...
		if (!input)
		{
			line("nav load=InputUnavailable");
			return false;
		}
		...
		if (!error.isNone())
		{
			...
			std::snprintf(text, sizeof(text), "nav load=Rejected kind=%u record=%u field=%u offset=%llu",
						  unsigned(error.kind), unsigned(error.record), unsigned(error.field),
		...
		loadCurrentLadders(owner);
		return true;
	}
	catch (...)
	{
		line("nav load=AllocationOrInputFailure");
		return false;
	}
}
...
nav::runtime::MovementSnapshot NavConsole::snapshotFor(const metamod::LifecycleCoordinator& owner,
													   core::PlayerId player) const noexcept
{
	...
	s.map = registry.mapGeneration();
	s.tick = registry.currentTick();
	...
	if (!binding.isValid() || binding.player != s.actor || binding.map != s.map || !entity || entity->free ||
		!engine_ || !engine_->pfnIndexOfEdict || engine_->pfnIndexOfEdict(entity) != s.actor.slot ||
		(engine_->pfnPEntityOfEntIndex && engine_->pfnPEntityOfEntIndex(s.actor.slot) != entity) ||
		owner.removalPending(s.actor))
		return s;
	s.kind = nav::runtime::ActorKind::ManagedBot;
	const auto& v = entity->v;
	s.alive = v.deadflag == DEAD_NO;
	s.grounded = (v.flags & FL_ONGROUND) != 0;
	s.ducked = (v.flags & FL_DUCKING) != 0;
	...
	if (std::isfinite(v.maxspeed) && v.maxspeed >= 0)
		s.speedLimit = v.maxspeed;
	if (std::isfinite(v.health) && v.health > 0)
		s.health = v.health;
	return s;
}
std::optional<RuntimeNavigationState> NavConsole::runtimeState(const metamod::LifecycleCoordinator& owner,
															   core::PlayerId player) const noexcept
{
	const auto* actor = findActor(player);
	if (inRequest_ || deferredInvalidation_ || navigation_.map != owner.registry().mapGeneration() || !index_)
		return {};

	RuntimeNavigationState result{};
	result.movement = snapshotFor(owner, player);
	result.roamGeneration = static_cast<std::uint64_t>(result.movement.map.value) * 1'000'003ULL +
	...
	if (actor && actor->session_)
	{
		...
		if (result.roamActive && !result.roamArrived && !result.routeExecutable && result.goal)
		{
			result.roamRejected = actor->execution_.cooling(*result.goal, actor->navigationTimeUs_);
			const auto limit = (std::min)(actor->roamRejectedGoalCount_, actor->roamRejectedGoals_.size());
	...
	if (result.movement.position)
	{
		...
		if (match && *match.value)
		{
			...
			if (result.movement.hull)
				result.currentFloorHeight =
					static_cast<float>(double(result.movement.position->z) + result.movement.hull->minimum.z);
		}
				 ...
				 result.movement.map == navigation_.map)
		{
			...
			const nav::model::NavVector3 support{result.movement.position->x, result.movement.position->y,
	...
	const bool currentAreaFallbackEligible =
		actor && result.movement.connected.value_or(false) && result.movement.joined.value_or(false) &&
		result.movement.alive.value_or(false) && result.movement.tick.isValid() && result.movement.actor.isValid() &&
	...
	if (!result.currentArea && currentAreaFallbackEligible && actor->lastCurrentArea_ &&
		actor->lastCurrentAreaActor_ == result.movement.actor &&
		actor->lastCurrentAreaAgent_ == result.movement.agent && actor->lastCurrentAreaMap_ == result.movement.map &&
	...
	const bool validManagedMovement = result.movement.kind == nav::runtime::ActorKind::ManagedBot &&
	...
	if (validManagedMovement && result.currentArea && navigation_.graph)
	{
		...
		const auto recordExclusion = [&](nav::model::NavAreaId id, RoamExclusionReason reason,
										 core::PlayerId owner = {}, std::uint64_t remainingUs = 0) noexcept {
			if (result.roamExclusionCount < result.roamExclusionSamples.size())
				result.roamExclusionSamples[result.roamExclusionCount++] = {id, reason, owner, remainingUs};
		};
		const auto isListed = [](const auto& values, std::size_t count, nav::model::NavAreaId id) noexcept {
		...
		const auto occupiedByOther = [&](nav::model::NavAreaId id) noexcept -> core::PlayerId {
			for (const auto& other : actors_)
			{
				if (!other || other.get() == actor || !other->session_ || !other->session_->executable() ||
		...
		if (actor && !actor->execution_.canSearch(actor->navigationTimeUs_))
		{
			...
			const auto retry = actor->execution_.retryAtUs > now ? actor->execution_.retryAtUs - now : 0U;
			const auto search = actor->execution_.nextSearchAtUs > now ? actor->execution_.nextSearchAtUs - now : 0U;
			result.roamSearchBackoffRemainingUs = (std::max)(retry, search);
		}
		const auto addCandidate = [&](nav::model::NavAreaId id, bool allowRecent) noexcept {
			const bool preserveCurrentGoal =
				result.roamActive && result.routeExecutable && result.goal && id == *result.goal;
			if (result.roamCandidateCount >= result.roamCandidates.size())
			{
				++result.roamExcludedCapacity;
				recordExclusion(id, RoamExclusionReason::Capacity);
				return;
			}
			if (!id.isValid() || id == current)
			{
				++result.roamExcludedInvalid;
				recordExclusion(id, RoamExclusionReason::Invalid);
				return;
			}
			...
			if (!preserveCurrentGoal && occupiedOwner.isValid())
			{
				...
				recordExclusion(id, RoamExclusionReason::Occupied, occupiedOwner);
				return;
			}
			...
			if (!preserveCurrentGoal && actor && actor->execution_.cooling(id, actor->navigationTimeUs_))
			{
				++result.roamExcludedGoalCooling;
				const auto now = actor->navigationTimeUs_;
				const auto retry = actor->execution_.retryAtUs > now ? actor->execution_.retryAtUs - now : 0U;
				const auto search =
					actor->execution_.nextSearchAtUs > now ? actor->execution_.nextSearchAtUs - now : 0U;
				recordExclusion(id, RoamExclusionReason::GoalCooling, {}, (std::max)(retry, search));
				return;
			}
			if (!preserveCurrentGoal && !result.roamSearchBackoff && actor &&
				isListed(actor->roamRejectedGoals_, actor->roamRejectedGoalCount_, id))
			{
				++result.roamExcludedRejected;
				recordExclusion(id, RoamExclusionReason::Rejected);
				return;
			}
			if (!preserveCurrentGoal && !result.roamSearchBackoff && !allowRecent && actor &&
				isListed(actor->roamRecentGoals_, actor->roamRecentGoalCount_, id))
			{
				++result.roamExcludedRecent;
				recordExclusion(id, RoamExclusionReason::Recent);
				return;
			}
			...
			if (!vertex)
			{
				...
				recordExclusion(id, RoamExclusionReason::Missing);
				return;
			}
			if (!result.movement.hull)
			{
				++result.roamExcludedHull;
				recordExclusion(id, RoamExclusionReason::Hull);
				return;
			}
			const auto point = navigation_.graph->center(*vertex);
			result.roamCandidates[result.roamCandidateCount++] = {{id}, {point.x, point.y, point.z}, id.value};
		};
		...
		const auto seed = static_cast<std::size_t>(result.movement.agent.value) * 2654435761U +
						  static_cast<std::size_t>(owner.round().value);
		if (areaCount != 0)
		{
			const auto offset = seed % areaCount;
			for (std::size_t step = 0; step < areaCount && result.roamCandidateCount < result.roamCandidates.size();
				 ++step)
			{
...
void NavConsole::observe(metamod::LifecycleCoordinator& owner) noexcept
{
	...
	if (current_->session_ && current_->session_->executable())
	{
		...
		if (!current_->session_->executable())
		{
			current_->replan_ = {};
			stopMotion();
		}
...
void NavConsole::execute(NavCommand command, metamod::LifecycleCoordinator& owner) noexcept
{
	if (inRequest_)
		return;
	if (!engine_ || !engine_->pfnCmd_Argc || !engine_->pfnCmd_Argv)
		return;
	if (command == NavCommand::Report)
	{
		if (engine_->pfnCmd_Argc() != 3)
		{
			line("report error=InvalidArguments expected=reporter_slot:generation target_slot:generation");
			return;
		}
		...
		if (!reporter || !target)
		{
			line("report error=InvalidActorArgument expected=slot:generation");
			return;
		}
		const auto result = owner.report(*reporter, *target);
		char text[128]{};
		std::snprintf(text, sizeof(text), "report status=%s recipients=%zu",
					  core::world::reportReasonName(result.reason), result.recipients);
		line(text);
		return;
	}
	const int expected = (command == NavCommand::Load || command == NavCommand::GoTo) ? 2 : 1;
	const auto argc = engine_->pfnCmd_Argc();
	if (argc != expected && (command == NavCommand::Load || argc != expected + 1))
	{
		line("nav error=InvalidArguments");
		return;
	}
	if (command == NavCommand::Load)
	{
		if (!owner.registry().isMapActive())
		{
			line("nav error=NoActiveMap");
			return;
		}
		const auto path = bounded(engine_->pfnCmd_Argv(1), 1024);
		if (path.empty())
		{
			line("nav error=InvalidPath");
			return;
		}
		(void)load(path.data(), owner.registry().mapGeneration(), owner);
		return;
	}
	if (command == NavCommand::GoTo && !owner.registry().isMapActive())
	{
		line("nav error=NoActiveMap");
		return;
	}
	...
	if (argc == expected + 1)
	{
		...
		{
			line("nav error=InvalidActorArgument expected=slot:generation");
			return;
		}
		player = *requested;
	}
	else
	{
		for (std::uint16_t slot = 1; slot <= owner.registry().clientMax(); ++slot)
		{
			...
			if (!owner.agents().findByPlayer(candidate).isValid())
				continue;
			if (player.isValid())
			{
				line("nav error=NoUniqueJoinedManagedActor specify=slot:generation");
				return;
			}
			player = candidate;
		}
	...
	if (!selectActor(player))
		return;
	if (command == NavCommand::Status)
	{
		observe(owner);
		if (current_->session_)
			debug::printNavTrace(current_->session_->trace(), &sink, this);
		else
			line("nav state=Idle");
		printMotion();
		printReplan();
	...
	if (!goal)
	{
		line("nav error=InvalidGoalArgument");
		return;
	}
	const auto s = snapshot(owner);
	...
	{
		observe(owner);
		line("nav error=NoUniqueJoinedManagedActor");
		return;
	}
	if (!navigation_.graph || !s.hull)
	{
		line("nav error=InvalidGoalArea");
		return;
	}
							  ...
							  metamod::LifecycleCoordinator& owner, const nav::runtime::RouteOptions& options) noexcept
{
	...
	inRequest_ = true;
	const auto previousGeneration = current_->session_ ? current_->session_->trace().routeGeneration : 0;
	const auto shortcutGeneration =
		previousGeneration == (std::numeric_limits<std::uint64_t>::max)() ? std::uint64_t{1} : previousGeneration + 1;
	const auto shortcuts = discoverShortcuts(s, shortcutGeneration);
	if (navigation.graph && !shortcuts.links.empty())
	{
		const auto augmented = nav::query::NavGraph::augment(
			navigation.graph, shortcuts, {100000, 1000000, 256U * 1024U * 1024U}, {128, 4U * 1024U * 1024U});
		if (augmented)
			navigation.graph = *augmented.value;
		char text[160]{};
		std::snprintf(text, sizeof(text), "nav shortcuts actor=%u:%u candidates=%zu accepted=%zu generation=%llu",
					  unsigned(s.actor.slot), unsigned(s.actor.generation.value), std::size_t(8),
					  shortcuts.links.size(), static_cast<unsigned long long>(shortcutGeneration));
		line(text);
	}
	...
	if (const auto& state = currentAreaSeed; state && state->currentArea && state->currentFloorHeight &&
											  ...
											  state->currentAreaSource == CurrentAreaSource::Nearest))
	{
		...
		const char* sourceName = observation.source == nav::runtime::CurrentAreaObservationSource::LastKnown
									 ? "LastKnown"
								 : observation.source == nav::runtime::CurrentAreaObservationSource::Nearest ? "Nearest"
																											 : "Exact";
		char text[256]{};
		std::snprintf(text, sizeof(text), "nav current_seed actor=%u:%u area=%u source=%s age_us=%llu floor_z=%.3f",
	...
	for (const auto& other : actors_)
	{
		if (!other || other.get() == current_ || !other->session_ || !other->session_->executable() ||
			other->execution_.state != nav::runtime::ExecutionState::Running || other->session_->trace().map != s.map)
			continue;
	...
	else
	{
		...
		std::snprintf(text, sizeof(text),
					  "nav route_rejected actor=%u:%u goal=%u session_reason=%u nav_error_kind=%u nav_error_field=%u "
					  "nav_error_offset=%llu",
...
void NavConsole::printReplan() noexcept
{
	...
	std::snprintf(text, sizeof(text),
				  "nav replan_actor=%u:%u state=%u attempts=%u max_attempts=%u fact_lifetime_us=%llu reason=%s "
				  "edge=%u:%u direction=%u link=%llu:%llu:%llu",
				  unsigned(current_->actor.slot), unsigned(current_->actor.generation.value),
				  unsigned(current_->replan_.state()), current_->replan_.attempts(),
				  nav::runtime::ReplanAttempt::maxAttempts,
				  static_cast<unsigned long long>(nav::runtime::ReplanAttempt::factLifetimeUs),
				  current_->replan_.state() == nav::runtime::ReplanState::Idle
					  ? "None"
					  : (current_->replan_.isRecovery() ? "Stuck" : "DynamicObstacle"),
...
nav::enrichment::NavTraversalLinkSet NavConsole::discoverShortcuts(const nav::runtime::MovementSnapshot& s,
																   std::uint64_t generation) noexcept
{
	nav::enrichment::NavTraversalLinkSet result{};
	if (!navigation_.graph || !index_ || !s.position || !s.hull || !generation ||
		s.kind != nav::runtime::ActorKind::ManagedBot || s.connected != true || s.alive != true || s.joined != true)
		return result;
	...
	struct Candidate
	{
		nav::model::NavAreaId id{};
		double score{0};
	};
	std::array<Candidate, 8> candidates{};
	std::size_t candidateCount = 0;
	const auto hasNativeEdge = [&](nav::model::NavAreaId id) noexcept {
		for (auto e = navigation_.graph->edgeBegin(*currentVertex); e < navigation_.graph->edgeEnd(*currentVertex); ++e)
			if (navigation_.graph->edge(e).target == id && !navigation_.graph->edge(e).external)
				return true;
		return false;
	};
	for (std::size_t i = 0; i < navigation_.graph->areaCount(); ++i)
	{
		...
		if (id == currentId || hasNativeEdge(id))
			continue;
		...
		for (std::size_t n = 0; n < candidateCount; ++n)
			if (candidate.score < candidates[n].score)
			{
				insert = n;
				break;
			}
		if (candidateCount < candidates.size())
			++candidateCount;
		else if (insert == candidates.size())
			continue;
		if (insert == candidateCount - 1 && candidateCount <= candidates.size())
			candidates[insert] = candidate;
		else
		{
			for (std::size_t n = candidateCount - 1; n > insert; --n)
	...
	for (std::size_t n = 0; n < candidateCount && result.links.size() < 8; ++n)
	{
		...
		const auto targetFloor = nav::query::projectToArea(targetExtent, {s.position->x, s.position->y, s.position->z});
		const nav::model::NavVector3 landing{static_cast<float>(targetFloor.x), static_cast<float>(targetFloor.y),
											 static_cast<float>(targetFloor.z - s.hull->minimum.z)};
		if (shortcutQueriesThisTick_ >= 8)
			break;
		nav::runtime::QueryRequest ground{{s.agent, s.actor, s.map, s.tick, generation, ++shortcutQueriesThisTick_},
										  nav::runtime::QueryKind::GroundedArea,
										  landing,
										  landing,
										  s.hull,
										  18};
		const auto support = query(ground);
		if (support.error != nav::runtime::QueryError::None || !support.ground || !support.ground->floor ||
			!support.ground->area || *support.ground->area != candidates[n].id)
			continue;
		...
		if (shortcutQueriesThisTick_ >= 8)
			break;
		nav::runtime::QueryRequest clear{{s.agent, s.actor, s.map, s.tick, generation, ++shortcutQueriesThisTick_},
										 nav::runtime::QueryKind::SweptHull,
										 *s.position,
										 landing,
										 s.hull,
										 18};
		const auto passage = query(clear);
		const bool directClear = passage.error == nav::runtime::QueryError::None && passage.hull &&
								 !passage.hull->startSolid && passage.hull->fraction == 1;
		if (traversal == nav::model::NavTraversalKind::Walk && !directClear)
			continue;
		if (traversal == nav::model::NavTraversalKind::Drop && !directClear && fall <= 128)
			continue;
		if (!std::isfinite(landing.x) || !std::isfinite(landing.y) || !std::isfinite(landing.z))
			continue;
		const auto linkId = (std::uint64_t(currentId.value) << 32) | candidates[n].id.value;
		result.links.push_back({0x415354524153484FULL,
								generation,
								linkId,
								currentId,
								candidates[n].id,
...
bool NavConsole::runReplan(metamod::LifecycleCoordinator& owner) noexcept
{
	if (current_->replan_.state() != nav::runtime::ReplanState::Pending)
		return false;
	const auto s = snapshot(owner);
	if (!s.tick.isAfter(current_->motionTrace_.decision.tick))
		return true;
	const auto policy =
		current_->replan_.consume(current_->motionTrace_.decision.binding, s.tick, current_->navigationTimeUs_);
	printReplan();
	if (!policy || !current_->session_ || !current_->session_->executable())
	{
		if (current_->recoveryReplan_)
		{
			...
			current_->motionTrace_.decision.terminalEvent =
				false; // The retiring Walk already emitted its terminal event.
	...
	{
		(void)current_->goalLease_.acquire(
			{s.agent, s.actor, s.map, owner.round(), current_->session_->trace().routeGeneration}, goal);
	}
	else
	{
		current_->goalLease_.release();
...
{
	const NavPlayerResolver resolver{
		queryingOwner_, [](const void* context, edict_t* entity) noexcept {
			return context ? static_cast<const metamod::LifecycleCoordinator*>(context)->playerForEntity(entity)
						   : core::PlayerId{};
		}};
	auto result = queryNavWorld(engine_, queryingEntity_, index_.get(), request, globals_ ? globals_->maxEntities : 0,
								queryingPlayers_, resolver);
	if (result.error == nav::runtime::QueryError::Unavailable)
	{
		...
		std::snprintf(unavailable, sizeof(unavailable),
					  "nav query_unavailable actor=%u:%u kind=%u entity=%u index=%u pfn_hull=%u pfn_line=%u "
					  "hull=(%.3g,%.3g,%.3g;%.3g,%.3g,%.3g)",
					  ...
					  hull ? hull->minimum.x : 0.0F, hull ? hull->minimum.y : 0.0F, hull ? hull->minimum.z : 0.0F,
					  hull ? hull->maximum.x : 0.0F, hull ? hull->maximum.y : 0.0F, hull ? hull->maximum.z : 0.0F);
		line(unavailable);
	}
	if (!(result.stamp == request.stamp) || result.kind != request.kind)
	{
		...
		std::snprintf(mismatch, sizeof(mismatch),
					  "nav query_identity_mismatch actor=%u:%u kind=%u expected=(%llu,%u,%llu,%u,%u) "
					  "actual=(%llu,%u,%llu,%u,%u) actual_kind=%u",
					  unsigned(request.stamp.actor.slot), unsigned(request.stamp.actor.generation.value),
					  unsigned(request.kind), static_cast<unsigned long long>(request.stamp.tick.value),
					  unsigned(request.stamp.map.value), static_cast<unsigned long long>(request.stamp.routeGeneration),
					  unsigned(request.stamp.ordinal), unsigned(request.stamp.agent.value),
					  static_cast<unsigned long long>(result.stamp.tick.value), unsigned(result.stamp.map.value),
					  static_cast<unsigned long long>(result.stamp.routeGeneration), unsigned(result.stamp.ordinal),
					  unsigned(result.stamp.agent.value), unsigned(result.kind));
		line(mismatch);
	}
	if (deferredInvalidation_)
	{
