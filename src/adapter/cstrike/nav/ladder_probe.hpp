...
enum class LadderExit
{
	SameFace,
	AcrossTop
};
enum class LadderProbeReason
{
	None,
	InvalidInput,
	Unavailable,
...
struct LadderProbeResult
{
	...
	{
		return reason == LadderProbeReason::None && passage.has_value();
	}
};
...
LadderProbeResult inspectLadderPassage(LadderWorld, core::MapGeneration, const LadderCandidate&, LadderFace, LadderExit,
									   const nav::query::NavSpatialIndex&, core::MapGeneration indexMap,
									   int maxEntities, std::uint32_t maxQueries = 12) noexcept;
