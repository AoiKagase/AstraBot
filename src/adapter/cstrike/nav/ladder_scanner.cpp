									  ...
									  std::uint32_t maxSlots, std::size_t maxCandidates) noexcept
{
	...
	if (!map.isValid() || maxEntities < 1 || maxSlots > 8192 || maxCandidates > 128)
		return result;
	if (static_cast<std::uint32_t>(maxEntities) > maxSlots)
	{
		result.reason = LadderScanReason::EntityLimit;
		return result;
	}
	if (!engine || !engine->pfnPEntityOfEntIndex || !engine->pfnIndexOfEdict || !engine->pfnSzFromIndex)
	{
		result.reason = LadderScanReason::Unavailable;
		return result;
	}
	...
	for (int slot = 1; slot < maxEntities; ++slot)
	{
		...
		if (!entity || entity->free || !entity->v.classname)
			continue;
		...
		if (std::strcmp(name, "func_ladder") != 0)
			continue;
		if (engine->pfnIndexOfEdict(entity) != slot || engine->pfnPEntityOfEntIndex(slot) != entity || entity->free ||
			entity->serialnumber != serial || entity->v.classname != classname)
			return fail(LadderScanReason::InvalidEntity);
		if (result.candidates.count == maxCandidates)
			return fail(LadderScanReason::CandidateLimit);
		...
		if (!low.isFinite() || !high.isFinite() || low.x >= high.x || low.y >= high.y || low.z >= high.z ||
			v.angles.x != 0 || v.angles.y != 0 || v.angles.z != 0 || v.velocity.x != 0 || v.velocity.y != 0 ||
			v.velocity.z != 0 || v.avelocity.x != 0 || v.avelocity.y != 0 || v.avelocity.z != 0)
	...
	result.reason = LadderScanReason::None;
	result.candidates.map = map;
	return result;
}
} // namespace astrabot::adapter::cstrike
