														   ...
														   nav::local::Binding binding, core::TickId tick) noexcept
{
	if (!engine || !engine->pfnCVarGetPointer || !entity || entity->free || !tick.isValid() ||
		...
		!std::isfinite(entity->v.gravity))
		return {};
	const auto* support = entity->v.groundentity;
	if (support &&
		(support->free || support->v.velocity.x != 0 || support->v.velocity.y != 0 || support->v.velocity.z != 0 ||
		 support->v.avelocity.x != 0 || support->v.avelocity.y != 0 || support->v.avelocity.z != 0))
		return {};
	...
	if (!std::isfinite(effective) || effective <= 0 || effective > 4000 || !std::isfinite(jumpHeight) ||
		jumpHeight <= 0 || jumpHeight > 64)
		return {};
	const nav::runtime::HullDimensions standing{{-16, -16, -36}, {16, 16, 36}},
		crouching{{-16, -16, -18}, {16, 16, 32}};
	...
	if (entity->v.mins.x != expected.minimum.x || entity->v.mins.y != expected.minimum.y ||
		...
		entity->v.maxs.y != expected.maximum.y || entity->v.maxs.z != expected.maximum.z)
		return {};
	return nav::local::JumpPhysics{binding,	 tick,		effective, std::sqrt(1600 * jumpHeight),
								   standing, crouching, 1.0 / 3.0};
}

JumpPhysicsAssessment assessStandardJumpPhysics(enginefuncs_t* engine, edict_t* entity, nav::local::Binding binding,
												core::TickId tick) noexcept
{
	...
	if (!engine || !engine->pfnCVarGetPointer)
		return fail(JumpPhysicsReason::MissingHost);
	if (!entity || entity->free)
		return fail(JumpPhysicsReason::MissingActor);
	...
	result.actorHull = nav::runtime::HullDimensions{{entity->v.mins.x, entity->v.mins.y, entity->v.mins.z},
													{entity->v.maxs.x, entity->v.maxs.y, entity->v.maxs.z}};
	if (!tick.isValid())
		return fail(JumpPhysicsReason::InvalidTick);
	if (entity->v.movetype != MOVETYPE_WALK)
		return fail(JumpPhysicsReason::MoveType);
	if (entity->v.waterlevel != 0)
		return fail(JumpPhysicsReason::Water);
	if (entity->v.flags & FL_WATERJUMP)
		return fail(JumpPhysicsReason::WaterJump);
	...
	if (!std::isfinite(entity->v.gravity))
		return fail(JumpPhysicsReason::InvalidGravity);
	const auto* support = entity->v.groundentity;
	result.movingSupport = support && (support->free || support->v.velocity.x != 0 || support->v.velocity.y != 0 ||
									   support->v.velocity.z != 0 || support->v.avelocity.x != 0 ||
									   support->v.avelocity.y != 0 || support->v.avelocity.z != 0);
	if (result.movingSupport)
		return fail(JumpPhysicsReason::MovingSupport);
	const auto* gravity = engine->pfnCVarGetPointer("sv_gravity");
	if (!gravity)
		return fail(JumpPhysicsReason::MissingGravity);
	const double base = gravity->value, multiplier = entity->v.gravity == 0 ? 1 : entity->v.gravity;
	const auto* height = engine->pfnCVarGetPointer("mp_jump_height");
	...
	const auto sameHull = [](const nav::runtime::HullDimensions& a, const nav::runtime::HullDimensions& b) noexcept {
	...
	result.posture = duckFlag == crouchingHull ? (duckFlag ? JumpActorPosture::Crouching : JumpActorPosture::Standing)
											   : JumpActorPosture::Transition;
	result.physics =
		nav::local::JumpPhysics{binding, tick, effective, std::sqrt(1600 * jumpHeight), standing, crouching, 1.0 / 3.0};
	return result;
}
const char* jumpPhysicsReasonName(JumpPhysicsReason reason) noexcept
{
	switch (reason)
	{
	case JumpPhysicsReason::None:
		return "None";
	case JumpPhysicsReason::MissingHost:
		return "MissingHost";
	case JumpPhysicsReason::MissingActor:
		return "MissingActor";
	case JumpPhysicsReason::InvalidTick:
		return "InvalidTick";
	case JumpPhysicsReason::MoveType:
		return "MoveType";
	case JumpPhysicsReason::Water:
		return "Water";
	case JumpPhysicsReason::WaterJump:
		return "WaterJump";
	case JumpPhysicsReason::BaseVelocity:
		return "BaseVelocity";
	case JumpPhysicsReason::MovingSupport:
		return "MovingSupport";
	case JumpPhysicsReason::MissingGravity:
		return "MissingGravity";
	case JumpPhysicsReason::InvalidGravity:
		return "InvalidGravity";
	case JumpPhysicsReason::InvalidJumpHeight:
		return "InvalidJumpHeight";
	case JumpPhysicsReason::NonCanonicalHull:
		return "NonCanonicalHull";
	case JumpPhysicsReason::TicketStale:
		return "TicketStale";
	case JumpPhysicsReason::BindingChanged:
		return "BindingChanged";
	case JumpPhysicsReason::GravityChanged:
		return "GravityChanged";
	case JumpPhysicsReason::ImpulseChanged:
		return "ImpulseChanged";
	case JumpPhysicsReason::CrouchMultiplierChanged:
		return "CrouchMultiplierChanged";
	case JumpPhysicsReason::PostureIncompatible:
		return "PostureIncompatible";
	}
	return "Unknown";
}
const char* jumpActorPostureName(JumpActorPosture posture) noexcept
{
	switch (posture)
	{
	case JumpActorPosture::Unknown:
		return "Unknown";
	case JumpActorPosture::Standing:
		return "Standing";
	case JumpActorPosture::Crouching:
		return "Crouching";
	case JumpActorPosture::Transition:
		return "Transition";
	case JumpActorPosture::Invalid:
		return "Invalid";
	}
	return "Unknown";
}
...
{
public:
	GuardQueries(NavConsole& port, std::uint32_t& count, std::uint64_t& total) noexcept
		: port_(port), count_(count), total_(total)
	{
	}
											...
											core::TickId pendingTick) noexcept
{
	...
	if (ticket.tick != pendingTick)
		return JumpPhysicsReason::TicketStale;
	if (!sameBinding(ticket.binding, binding))
		return JumpPhysicsReason::BindingChanged;
	if (!samePhysicsScalar(current.gravity, ticket.gravity))
		return JumpPhysicsReason::GravityChanged;
	...
	if (!sameHull(current.standingHull, ticket.standingHull) || !sameHull(current.crouchingHull, ticket.crouchingHull))
								   ...
								   const PendingMotion& pending) noexcept
{
	...
	const auto& ticket = *pending.jump;
	...
	const auto reject = [&](JumpGuardReason why, MotionReason reason = MotionReason::JumpChanged) noexcept {
		current_->motionTrace_.jumpGuardReason = why;
		return reason;
	};
	...
	if (!current_->walk_ || current_->walk_->step() != pending.binding.step || !s.velocity || !s.velocity->isFinite() ||
		!s.ducked)
	...
	if (!assessment || !assessment.physics)
		return reject(JumpGuardReason::Physics);
	const auto& physics = *assessment.physics;
	const auto ticketReason = validateJumpPhysicsTicket(ticket.physics, physics, pending.binding, pending.tick);
	if (ticketReason != JumpPhysicsReason::None)
		return rejectPhysics(ticketReason);
	if (!sameHull(assessment.actorHull, s.hull))
		return rejectPhysics(JumpPhysicsReason::PostureIncompatible);
	const auto hints =
		constraints(nav::model::NavTraversalKind::Jump, ticket.plan.sourceAttributes, ticket.plan.targetAttributes);
	const auto capability = deriveJumpLimits(jumpLimits.motion, physics, s, hints);
	if (!capability)
		return reject(JumpGuardReason::Capability);
	const auto motion = *capability;
	if (!sameHull(ticket.plan.flightHull, motion.flightHull))
		return reject(JumpGuardReason::FlightHull);
	const bool duck = (command.buttons & static_cast<core::ButtonMask>(core::Button::Duck)) != 0;
	if (ticket.state == JumpState::Complete)
	{
		if (press || speed != 0 || s.grounded != true)
			return reject(JumpGuardReason::CompletionState);
		if (duck || s.ducked == false)
			return accept();
		if (!physics.standingHull || !s.hull || !s.position || current_->guardQueries_ >= 21)
			return reject(current_->guardQueries_ >= 21 ? JumpGuardReason::QueryBudget
														: JumpGuardReason::StandClearance);
		...
		{
			return reject(JumpGuardReason::StandClearance);
		}
		return r.stamp == q.stamp && r.kind == q.kind && r.error == QueryError::None && r.clearance &&
					   r.clearance->clear
				   ? accept()
				   : reject(r.error == QueryError::BudgetExceeded ? JumpGuardReason::QueryBudget
																  : JumpGuardReason::StandClearance);
	}
	const bool airbornePhase =
		s.grounded == false || ticket.state == JumpState::Airborne || ticket.state == JumpState::Recover;
	const bool expectedDuck = airbornePhase ? ticket.plan.flightHull.has_value() : hints.sourceDuck;
	const bool sourcePostureMismatch =
		press && *s.ducked != hints.sourceDuck && assessment.posture != JumpActorPosture::Transition;
	if (duck != expectedDuck || sourcePostureMismatch)
		return reject(JumpGuardReason::Posture);
	const auto delta = movement_->frameDeltaUs();
	if (!delta || delta > 120000)
		return reject(JumpGuardReason::Timing, MotionReason::StaleCommand);
	...
	if (press)
	{
		if (ticket.state != JumpState::Takeoff || ticket.pressTick != pending.tick || s.grounded != true || !entity ||
			!std::isfinite(entity->v.fuser2) || entity->v.fuser2 > 0 ||
		...
		auto flight = jumpLimits.flight;
		flight.maxQueries = 20 - current_->guardQueries_; // Reserve the actual frame sweep.
		const auto launch = JumpProbe::launch(s, pending.binding, ticket.plan, motion, physics, flight, *index_,
											  navigation_.map, queries);
		if (!launch)
			return reject(JumpGuardReason::LaunchProbe);
	}
	else if (s.grounded == true)
	{
		if (ticket.state == JumpState::Takeoff)
			return speed == 0 ? accept() : reject(JumpGuardReason::TakeoffState);
		if (ticket.state == JumpState::Airborne || ticket.state == JumpState::Recover)
		{
			if (speed != 0)
				return reject(JumpGuardReason::LandingProbe);
			return JumpProbe::land(s, pending.binding, ticket.plan, motion, ground, *index_, navigation_.map, queries)
					   ? accept()
					   : reject(JumpGuardReason::LandingProbe);
		}
		...
		const auto path = GroundProbe::inspect(s, pending.binding.routeGeneration, ticket.plan.source, x, y, *index_,
											   navigation_.map, queries, ground);
		return path && path.target->area == ticket.plan.source
				   ? accept()
				   : reject(path.reason == ProbeReason::BudgetExceeded ? JumpGuardReason::QueryBudget
																	   : JumpGuardReason::GroundProbe);
	}
	...
	const nav::model::NavVector3 end{
		static_cast<float>(s.position->x + double(s.velocity->x) * dt),
	...
	if (!end.isFinite())
		return reject(JumpGuardReason::FlightPrediction);
	const QueryRequest q{{s.agent, s.actor, s.map, s.tick, pending.binding.routeGeneration, 1},
						 QueryKind::SweptHull,
						 *s.position,
						 end,
						 (duck && s.grounded == false) ? ticket.plan.flightHull : s.hull};
	WorldQueryResult r;
	try
	{
		r = queries.query(q);
	...
	if (!(r.stamp == q.stamp) || r.kind != q.kind || r.error != QueryError::None || !r.hull || r.hull->startSolid ||
		!std::isfinite(r.hull->fraction) || r.hull->fraction < 0 || r.hull->fraction > 1 || !r.hull->end.isFinite() ||
		!r.hull->normal.isFinite())
		return reject(r.error == QueryError::BudgetExceeded ? JumpGuardReason::QueryBudget
															: JumpGuardReason::FlightSweep);
	...
	auto contact = s;
	contact.position = hit.end;
	contact.grounded = true;
	if (duck && ticket.plan.flightHull)
	{
		contact.hull = ticket.plan.flightHull;
		contact.ducked = true;
	}
