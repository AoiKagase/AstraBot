...
bool ready(const nav::runtime::MovementSnapshot& s, bool airborne = false) noexcept
{
	return s.kind == nav::runtime::ActorKind::ManagedBot && s.connected == true && s.alive == true &&
		   s.joined == true && (s.grounded == true || (airborne && s.grounded == false)) && s.position &&
		   s.position->isFinite() && s.view && s.view->isFinite() && s.hull && s.hull->minimum.isFinite() &&
		   s.hull->maximum.isFinite() && s.speedLimit && std::isfinite(*s.speedLimit) && *s.speedLimit >= 0;
}
bool segmentAllows(nav::model::NavVector3 start, nav::model::NavVector3 end, nav::model::NavVector3 position,
...
const char* walkState(nav::local::WalkState state) noexcept
{
	switch (state)
	{
	case nav::local::WalkState::Running:
		return "Running";
	case nav::local::WalkState::Arrived:
		return "Arrived";
	case nav::local::WalkState::Failed:
		return "Failed";
	case nav::local::WalkState::Aborted:
		return "Aborted";
	}
	return "Unknown";
}
const char* motionDisposition(nav::local::MotionDisposition disposition) noexcept
{
	switch (disposition)
	{
	case nav::local::MotionDisposition::Execute:
		return "Execute";
	case nav::local::MotionDisposition::Hold:
		return "Hold";
	case nav::local::MotionDisposition::Recovery:
		return "Recovery";
	}
	return "Unknown";
}
const char* obstacleClassName(nav::local::ObstacleClass obstacle) noexcept
{
	switch (obstacle)
	{
	case nav::local::ObstacleClass::None:
		return "None";
	case nav::local::ObstacleClass::Step:
		return "Step";
	case nav::local::ObstacleClass::Jumpable:
		return "Jumpable";
	case nav::local::ObstacleClass::TooHigh:
		return "TooHigh";
	case nav::local::ObstacleClass::Dynamic:
		return "Dynamic";
	case nav::local::ObstacleClass::Unknown:
		return "Unknown";
	}
	return "Unknown";
}
const char* jumpCandidateRegionModeName(nav::local::JumpCandidateRegionMode mode) noexcept
{
	switch (mode)
	{
	case nav::local::JumpCandidateRegionMode::Margin:
		return "Margin";
	case nav::local::JumpCandidateRegionMode::PhysicalHull:
		return "PhysicalHull";
	case nav::local::JumpCandidateRegionMode::CentreInset:
		return "CentreInset";
	case nav::local::JumpCandidateRegionMode::SuccessorCentreInset:
		return "SuccessorCentreInset";
	case nav::local::JumpCandidateRegionMode::Collapsed:
		return "Collapsed";
	case nav::local::JumpCandidateRegionMode::NoBallisticLanding:
		return "NoBallisticLanding";
	}
	return "Unknown";
}
const char* jumpCandidateSwitchReasonName(nav::local::JumpCandidateSwitchReason reason) noexcept
{
	switch (reason)
	{
	case nav::local::JumpCandidateSwitchReason::None:
		return "None";
	case nav::local::JumpCandidateSwitchReason::PlannedLandingStaticFailure:
		return "PlannedLandingStaticFailure";
	case nav::local::JumpCandidateSwitchReason::BallisticLandingMismatch:
		return "BallisticLandingMismatch";
	}
	return "Unknown";
}
const char* jumpProofPhaseName(nav::local::JumpProofPhase phase) noexcept
{
	switch (phase)
	{
	case nav::local::JumpProofPhase::None:
		return "None";
	case nav::local::JumpProofPhase::Prepare:
		return "Prepare";
	case nav::local::JumpProofPhase::Launch:
		return "Launch";
	case nav::local::JumpProofPhase::Land:
		return "Land";
	}
	return "Unknown";
}
const char* jumpProofDeferReasonName(nav::local::JumpProofDeferReason reason) noexcept
{
	switch (reason)
	{
	case nav::local::JumpProofDeferReason::None:
		return "None";
	case nav::local::JumpProofDeferReason::ReservedQueries:
...
const char* probeReasonName(nav::local::ProbeReason reason) noexcept
{
	switch (reason)
	{
	case nav::local::ProbeReason::None:
		return "None";
	case nav::local::ProbeReason::AllSolid:
		return "AllSolid";
	case nav::local::ProbeReason::StartSolid:
		return "StartSolid";
	case nav::local::ProbeReason::TraceNoHit:
		return "TraceNoHit";
	case nav::local::ProbeReason::NoSupport:
		return "NoSupport";
	case nav::local::ProbeReason::ActorGroundFlagMismatch:
		return "ActorGroundFlagMismatch";
	case nav::local::ProbeReason::FloorHeightMismatch:
		return "FloorHeightMismatch";
	case nav::local::ProbeReason::NavContainmentMissing:
		return "NavContainmentMissing";
	case nav::local::ProbeReason::BudgetExceeded:
		return "BudgetExceeded";
	case nav::local::ProbeReason::StaleQuery:
		return "StaleQuery";
	case nav::local::ProbeReason::QueryUnavailable:
		return "QueryUnavailable";
	case nav::local::ProbeReason::QueryFailed:
		return "QueryFailed";
	default:
		return "Other";
	}
...
const char* jumpProbeReasonName(nav::local::JumpProbeReason reason) noexcept
{
	switch (reason)
	{
	case nav::local::JumpProbeReason::None:
		return "None";
	case nav::local::JumpProbeReason::NoSupport:
		return "NoSupport";
	case nav::local::JumpProbeReason::CannotLand:
		return "CannotLand";
	case nav::local::JumpProbeReason::OutsideTakeoff:
		return "OutsideTakeoff";
	case nav::local::JumpProbeReason::BudgetExceeded:
		return "BudgetExceeded";
	case nav::local::JumpProbeReason::StaleQuery:
		return "StaleQuery";
	case nav::local::JumpProbeReason::QueryUnavailable:
		return "QueryUnavailable";
	case nav::local::JumpProbeReason::StalePhysics:
		return "StalePhysics";
	case nav::local::JumpProbeReason::Blocked:
		return "Blocked";
	default:
		return "Other";
	}
...
const char* jumpSupportRoleName(nav::local::JumpSupportRole role) noexcept
{
	switch (role)
	{
	case nav::local::JumpSupportRole::Source:
		return "Source";
	case nav::local::JumpSupportRole::PlannedLanding:
		return "PlannedLanding";
	case nav::local::JumpSupportRole::PredictedLanding:
...
const char* jumpLandingFailureName(nav::local::JumpLandingFailure failure) noexcept
{
	switch (failure)
	{
	case nav::local::JumpLandingFailure::None:
		return "None";
	case nav::local::JumpLandingFailure::PlannedSupport:
		return "PlannedSupport";
	case nav::local::JumpLandingFailure::PredictedSupport:
		return "PredictedSupport";
	case nav::local::JumpLandingFailure::BallisticSolution:
		return "BallisticSolution";
	case nav::local::JumpLandingFailure::LandingRadius:
		return "LandingRadius";
	case nav::local::JumpLandingFailure::MaximumDistance:
		return "MaximumDistance";
	case nav::local::JumpLandingFailure::FloorHeightMismatch:
		return "FloorHeightMismatch";
	}
	return "Unknown";
}
...
{
	const bool traversalRejected = reason == MotionReason::JumpChanged || reason == MotionReason::DropChanged ||
								   reason == MotionReason::LadderChanged || reason == MotionReason::PostureChanged;
	if (event == MotionEvent::Rejected && !traversalRejected)
		current_->recovery_.pause(current_->motionTrace_.decision.binding);
	if (event == MotionEvent::Decision)
		current_->motionTrace_.selectedEdge.reset();
	if (current_->session_ && current_->session_->trace().route &&
		current_->session_->trace().routeGeneration == current_->motionTrace_.decision.binding.routeGeneration &&
		current_->motionTrace_.decision.binding.step < current_->session_->trace().route->steps.size())
		current_->motionTrace_.selectedEdge =
			current_->session_->trace().route->steps[current_->motionTrace_.decision.binding.step].edge;
	...
	if (current_->walk_)
	{
		if (const auto* transition = current_->walk_->activeTransition())
		{
			...
			current_->motionTrace_.sourceExtentWidth =
				double(transition->sourceExtent.southEast.x) - transition->sourceExtent.northWest.x;
			current_->motionTrace_.sourceExtentHeight =
				double(transition->sourceExtent.southEast.y) - transition->sourceExtent.northWest.y;
			current_->motionTrace_.targetExtentWidth =
				double(transition->targetExtent.southEast.x) - transition->targetExtent.northWest.x;
			current_->motionTrace_.targetExtentHeight =
				double(transition->targetExtent.southEast.y) - transition->targetExtent.northWest.y;
		}
	...
	current_->motionTrace_.event = event;
	current_->motionTrace_.reason = reason;
	current_->motionSequence_ = add(current_->motionSequence_, 1);
	current_->motionTrace_.sequence = current_->motionSequence_;
	current_->motionHistory_[current_->motionNext_] = current_->motionTrace_;
	current_->motionNext_ = (current_->motionNext_ + 1) % motionHistoryLimit;
	...
	if (event == MotionEvent::Rejected || event == MotionEvent::Cancelled ||
		(event == MotionEvent::Decision && current_->motionTrace_.decision.terminalEvent))
		printMotion();
}
void NavConsole::printMotion() noexcept
{
	...
	if (d.recovery.cause != nav::local::StuckCause::None)
	{
		...
		std::snprintf(recovery, sizeof(recovery),
					  "recovery state=%u cause=%u symptom=%u attempts=%u commanded_us=%llu deadline_us=%llu "
					  "displacement=%.6g projected=%.6g travel=%.6g windows_us=(500000,1000000) stage_us=250000",
					  unsigned(r.state), unsigned(r.cause), unsigned(r.symptom), r.attempts,
					  static_cast<unsigned long long>(r.commandedUs), static_cast<unsigned long long>(r.deadlineUs),
					  r.displacement, r.projected, r.travel);
		line(recovery);
	}
	...
	std::snprintf(
		text, sizeof(text),
		"walk actor=%u:%u map=%u route=%llu step=%zu tick=%llu state=%s disposition=%s reason=%u probe=%u event=%u "
		"motion_reason=%u corridor=%u portal_reason=%u transition=%zu micro_area=%u extent=(%.6g,%.6g)->(%.6g,%.6g) "
		"hull=(%.6g,%.6g) transport=%u command_tick=%llu dispatch_tick=%llu age_us=%llu speed=%.6g "
		"direction=(%.6g,%.6g) target_present=%u target=(%.6g,%.6g,%.6g) support=%u queries=%u samples=%u "
		"step_probes=%u queued=%llu dispatched=%llu rejected=%llu missed=%llu history=%zu omitted=%llu edge=%u:%u "
		"command_forward=%.6g command_side=%.6g command_msec=%u door=%llu door_state=%u door_reason=%u use_checks=%llu "
		"contact_pulse=%u contact_guards=%llu clearance=(%.6g,%.6g) narrow=%u avoiding=%u lateral=%.6g",
		...
		static_cast<unsigned long long>(d.tick.value), walkState(d.state), motionDisposition(d.disposition),
		unsigned(d.reason), unsigned(d.probeReason), unsigned(current_->motionTrace_.event),
		unsigned(current_->motionTrace_.reason), unsigned(current_->motionTrace_.corridorError),
		unsigned(current_->motionTrace_.portalReason), current_->motionTrace_.corridorTransition,
		...
		static_cast<unsigned long long>(current_->motionTrace_.intentAgeUs), d.intent.speed, d.intent.direction.x,
		d.intent.direction.y, unsigned(d.target.has_value()), double(target.x), double(target.y), double(target.z),
		d.support ? unsigned(d.support->area.value) : 0U, d.queries, d.samples, d.steps,
		static_cast<unsigned long long>(current_->motionTrace_.queued),
		static_cast<unsigned long long>(current_->motionTrace_.dispatched),
		static_cast<unsigned long long>(current_->motionTrace_.rejected),
		static_cast<unsigned long long>(current_->motionTrace_.missedDecisions), current_->motionCount_,
		static_cast<unsigned long long>(current_->motionSequence_ - current_->motionCount_),
		...
		double(current_->motionTrace_.command.movement.forward), double(current_->motionTrace_.command.movement.side),
		unsigned(current_->motionTrace_.command.msec), static_cast<unsigned long long>(d.doorId),
		d.doorState ? unsigned(*d.doorState) : 0U, unsigned(d.doorReason),
	...
	std::snprintf(text + used, sizeof(text) - used,
				  " blocker=%llu blocker_kind=%u blocker_action=%u blocker_reason=%u blocker_player=%u:%u",
	...
	std::snprintf(
		route, sizeof(route),
		"nav route_progress actor=%u:%u route=%llu step=%zu projection=%.6g target_projection=%.6g progress=%.6g "
		"stuck_us=%llu forward=(%.6g,%.6g) retained=%u lease_goal=%u detour_first=%u:%d:%u detour_second=%u:%d:%u",
		unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
		static_cast<unsigned long long>(d.binding.routeGeneration), d.binding.step, d.routeProjection,
		d.targetProjection, d.routeProgress, static_cast<unsigned long long>(d.stuckElapsedUs), d.routeForward.x,
		d.routeForward.y, unsigned(d.retainedTarget), unsigned(current_->goalLease_.goal().value),
	...
	if (d.obstacleClass != nav::local::ObstacleClass::None || d.obstacleHull || d.blocker)
	{
		...
		std::snprintf(obstacle, sizeof(obstacle),
					  "nav obstacle actor=%u:%u route=%llu step=%zu class=%s blocker=%u hit=(%.3f,%.3f,%.3f) "
					  "normal=(%.3f,%.3f,%.3f) fraction=%.6g start_solid=%u all_solid=%u floor_start=%.3f "
					  "floor_next=%.3f delta=%.3f cumulative_drop=%.3f max_step=%.3f",
					  unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
					  static_cast<unsigned long long>(d.binding.routeGeneration), d.binding.step,
					  obstacleClassName(d.obstacleClass), unsigned(d.blocker.has_value()), hull ? hull->end.x : 0.0F,
					  hull ? hull->end.y : 0.0F, hull ? hull->end.z : 0.0F, hull ? hull->normal.x : 0.0F,
					  hull ? hull->normal.y : 0.0F, hull ? hull->normal.z : 0.0F, hull ? hull->fraction : 0.0F,
					  hull ? unsigned(hull->startSolid) : 0U, hull ? unsigned(hull->allSolid) : 0U, d.startFloorHeight,
					  d.lastFloorHeight, d.floorDelta, d.cumulativeDownDrop, d.maxDownStep);
		line(obstacle);
	}
	if (d.avoiding || d.avoidanceReason != nav::local::AvoidanceReason::None)
	{
		...
		std::snprintf(
			avoidance, sizeof(avoidance),
			"nav avoidance actor=%u:%u route=%llu step=%zu side=%d reason=%u distance=%.6g candidate_present=%u "
			"candidate=(%.6g,%.6g,%.6g) left=%.6g right=%.6g forward_probe=%u queries=%u budget=%u",
			unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
			static_cast<unsigned long long>(d.binding.routeGeneration), d.binding.step, d.avoidanceSide,
	...
	std::snprintf(timing, sizeof(timing),
				  "motion_timing actor=%u:%u elapsed_us=%llu frame_delta_us=%llu locomotion_mode=%u intent_speed=%.6g "
				  "decision_speed_limit=%.6g dispatch_speed_limit=%.6g resolved_speed=%.6g validated_distance=%.6g "
				  "intent_lifetime_us=%llu pending_remaining_us=%llu motion_reason=%u diagnostic_suppressed=%llu",
				  unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
	...
	if (current_->motionTrace_.groundGuardReason != nav::local::ProbeReason::None)
	{
		...
		std::snprintf(ground, sizeof(ground), "ground_guard actor=%u:%u reason=%s(%u) queries=%u proof_valid=%u",
					  unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
					  probeReasonName(current_->motionTrace_.groundGuardReason),
					  unsigned(current_->motionTrace_.groundGuardReason), current_->motionTrace_.groundGuardQueries,
	...
	const auto printGroundTrace = [&](const char* kind,
		...
		std::snprintf(detail, sizeof(detail),
					  "ground_probe actor=%u:%u kind=%s start=(%.3f,%.3f,%.3f) end=(%.3f,%.3f,%.3f) "
					  "hit=(%.3f,%.3f,%.3f) normal=(%.3f,%.3f,%.3f) fraction=%.6g startsolid=%u allsolid=%u",
					  unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value), kind, trace->start.x,
					  trace->start.y, trace->start.z, trace->end.x, trace->end.y, trace->end.z, trace->hitEnd.x,
					  trace->hitEnd.y, trace->hitEnd.z, trace->hitNormal.x, trace->hitNormal.y, trace->hitNormal.z,
	...
	if (current_->motionTrace_.groundGuardStep)
	{
		...
		std::snprintf(detail, sizeof(detail),
					  "ground_step actor=%u:%u start=(%.3f,%.3f,%.3f) lifted=(%.3f,%.3f,%.3f) across=(%.3f,%.3f,%.3f) "
					  "landing_area=%u",
					  unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value), step.start.x,
					  step.start.y, step.start.z, step.lifted.x, step.lifted.y, step.lifted.z, step.across.x,
					  step.across.y, step.across.z, unsigned(step.landing.area.value));
		line(detail);
	}
	if (current_->motionTrace_.dispatchPhysicalValid)
	{
		...
		std::snprintf(
			...
			"movement_dispatch actor=%u:%u sequence=%llu command_tick=%llu dispatch_tick=%llu physical_valid=%u "
			"before_origin=%.3f,%.3f,%.3f after_origin=%.3f,%.3f,%.3f before_velocity=%.3f,%.3f,%.3f "
			"after_velocity=%.3f,%.3f,%.3f onground=%u->%u horizontal_displacement=%.3f no_progress=%u",
			unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
			static_cast<unsigned long long>(current_->motionTrace_.dispatchSequence),
			static_cast<unsigned long long>(current_->motionTrace_.commandTick.value),
			static_cast<unsigned long long>(current_->motionTrace_.dispatchTick.value),
			unsigned(current_->motionTrace_.dispatchPhysicalValid), current_->motionTrace_.dispatchBeforeOriginX,
			current_->motionTrace_.dispatchBeforeOriginY, current_->motionTrace_.dispatchBeforeOriginZ,
			current_->motionTrace_.dispatchAfterOriginX, current_->motionTrace_.dispatchAfterOriginY,
			current_->motionTrace_.dispatchAfterOriginZ, current_->motionTrace_.dispatchBeforeVelocityX,
			current_->motionTrace_.dispatchBeforeVelocityY, current_->motionTrace_.dispatchBeforeVelocityZ,
			current_->motionTrace_.dispatchAfterVelocityX, current_->motionTrace_.dispatchAfterVelocityY,
			current_->motionTrace_.dispatchAfterVelocityZ, unsigned(current_->motionTrace_.dispatchBeforeOnGround),
			unsigned(current_->motionTrace_.dispatchAfterOnGround),
			current_->motionTrace_.dispatchHorizontalDisplacement, unsigned(current_->motionTrace_.dispatchNoProgress));
		line(dispatch);
	}
	if (d.posture || d.constraintReason != nav::local::ConstraintReason::None)
	{
		static char posture[128]{};
		std::snprintf(posture, sizeof(posture),
					  "walk posture_present=%u posture=%u posture_reason=%u constraint_reason=%u duck=%u",
					  unsigned(d.posture.has_value()), d.posture ? unsigned(*d.posture) : 0U, unsigned(d.postureReason),
					  unsigned(d.constraintReason), unsigned(d.intent.duck));
		line(posture);
	}
	if (d.jumpState)
	{
		static char jump[384]{};
		std::snprintf(jump, sizeof(jump),
					  "jump state=%u reason=%u probe=%s(%u) support_reason=%s(%u) support_initial=%s(%u) "
					  "fallback_attempted=%u fallback_accepted=%u landing_failure=%s(%u) geometry=%u guard_reason=%u "
					  "press_tick=%llu gravity=%.6g impulse=%.6g guard_queries=%llu profile=standard-cs",
					  unsigned(*d.jumpState), unsigned(d.jumpReason), jumpProbeReasonName(d.jumpProbeReason),
					  unsigned(d.jumpProbeReason), probeReasonName(d.jumpSupportReason), unsigned(d.jumpSupportReason),
					  probeReasonName(d.jumpSupportInitialReason), unsigned(d.jumpSupportInitialReason),
					  ...
					  unsigned(d.jumpGeometryReason), unsigned(current_->motionTrace_.jumpGuardReason),
					  static_cast<unsigned long long>(d.jumpPressTick.value),
					  d.jumpPhysics ? d.jumpPhysics->gravity : 0, d.jumpPhysics ? d.jumpPhysics->verticalImpulse : 0,
					  static_cast<unsigned long long>(current_->motionTrace_.jumpGuardQueries));
		line(jump);
		const auto printFloorTrace = [&](const nav::local::JumpSupportEvidence& evidence, const char* kind,
										 nav::local::ProbeReason reason,
			...
			if (!trace)
			{
				std::snprintf(floor, sizeof(floor),
							  "jump_floor actor=%u:%u role=%s kind=%s present=0 reason=%s(%u) origin=(%.3f,%.3f,%.3f) "
							  "expected_area=%u fallback_attempted=%u fallback_accepted=%u hull_present=%u",
							  unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
							  jumpSupportRoleName(evidence.role), kind, probeReasonName(reason), unsigned(reason),
							  evidence.origin.x, evidence.origin.y, evidence.origin.z,
							  unsigned(evidence.expectedArea.value), unsigned(evidence.fallbackAttempted),
							  unsigned(evidence.fallbackAccepted), unsigned(hull != nullptr));
			}
			else
			{
				std::snprintf(floor, sizeof(floor),
							  "jump_floor actor=%u:%u role=%s kind=%s present=1 reason=%s(%u) origin=(%.3f,%.3f,%.3f) "
							  "expected_area=%u start=(%.3f,%.3f,%.3f) end=(%.3f,%.3f,%.3f) hit=(%.3f,%.3f,%.3f) "
							  "normal=(%.3f,%.3f,%.3f) fraction=%.6g start_solid=%u all_solid=%u fallback_attempted=%u "
							  "fallback_accepted=%u",
							  unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
							  jumpSupportRoleName(evidence.role), kind, probeReasonName(reason), unsigned(reason),
							  evidence.origin.x, evidence.origin.y, evidence.origin.z,
							  unsigned(evidence.expectedArea.value), trace->start.x, trace->start.y, trace->start.z,
							  trace->end.x, trace->end.y, trace->end.z, trace->hitEnd.x, trace->hitEnd.y,
							  trace->hitEnd.z, trace->hitNormal.x, trace->hitNormal.y, trace->hitNormal.z,
							  trace->fraction, unsigned(trace->startSolid), unsigned(trace->allSolid),
							  unsigned(evidence.fallbackAttempted), unsigned(evidence.fallbackAccepted));
			}
			line(floor);
		};
		...
		std::snprintf(
			...
			"candidate=%u/%u candidate_region=%s candidate_switch=%s candidate_area=%u candidate_advance=%u "
			"landing=(%.4f,%.4f,%.4f) predicted=(%.4f,%.4f,%.4f) landing_error=%.4f trajectory_ready=%u proof_phase=%s "
			"proof_queries=%u reserved_queries=%u available_queries=%u deferred=%u defer_reason=%s guard_suppressed=%u "
			"landing_failure=%u "
			...
			jumpCandidateRegionModeName(d.jumpCandidateRegionMode),
			jumpCandidateSwitchReasonName(d.jumpCandidateSwitchReason), unsigned(d.jumpCandidateArea.value),
			unsigned(d.jumpCandidateAdvance), d.jumpCandidateLanding ? d.jumpCandidateLanding->x : 0.0F,
			d.jumpCandidateLanding ? d.jumpCandidateLanding->y : 0.0F,
			d.jumpCandidateLanding ? d.jumpCandidateLanding->z : 0.0F, d.jumpPredictedLanding.x,
			d.jumpPredictedLanding.y, d.jumpPredictedLanding.z, d.jumpLandingError, unsigned(d.jumpTrajectoryReady),
			jumpProofPhaseName(d.jumpProofPhase), d.jumpLaunchRequiredQueries, d.jumpReservedQueries,
			d.jumpLaunchAvailableQueries, unsigned(d.jumpLaunchDeferred),
			jumpProofDeferReasonName(d.jumpProofDeferReason), unsigned(d.jumpGuardSuppressed),
			unsigned(d.jumpLandingFailure), unsigned(d.jumpTakeoffProof), unsigned(d.jumpFlightProof),
			unsigned(d.jumpLandingProof), unsigned(d.jumpProofProvenance), unsigned(d.jumpReadiness),
			unsigned(d.jumpRecoveryDisposition), double(d.jumpAxis.x), double(d.jumpAxis.y), d.jumpFromTakeoff,
			d.jumpAlong, d.jumpLateral, d.jumpMinimumSpeed, d.jumpMaximumSpeed, d.jumpSpeedLimit, d.jumpDesiredSpeed,
		...
		if (d.jumpLandingEnvelope)
		{
			...
			std::snprintf(
				...
				"jump_envelope actor=%u:%u attempt=%llu target_area=%u advance=%u kind=%u "
				"extent=(%.3f,%.3f)-(%.3f,%.3f) bounds=(%.3f,%.3f,%.3f,%.3f) successor_area=%u successor_advance=%u "
				"successor_kind=%u successor_extent=(%.3f,%.3f)-(%.3f,%.3f) successor_bounds=(%.3f,%.3f,%.3f,%.3f)",
				unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
				static_cast<unsigned long long>(d.jumpAttemptId), unsigned(landingTarget.area.value),
				unsigned(landingTarget.cursorAdvance), unsigned(landingTarget.kind), landingTarget.extent.northWest.x,
				landingTarget.extent.northWest.y, landingTarget.extent.southEast.x, landingTarget.extent.southEast.y,
				landingTarget.lowX, landingTarget.highX, landingTarget.lowY, landingTarget.highY,
		...
		const auto printPhysics = [&](const char* phase, const JumpPhysicsAssessment& p) {
			...
			std::snprintf(
				...
				"jump_physics phase=%s reason=%s(%u) posture=%s(%u) model_valid=%u model_tick=%llu model_route=%llu "
				"model_step=%zu gravity=%.9g impulse=%.9g crouch_multiplier=%.9g movetype=%d water=%d flags=%d "
				"base=(%.6g,%.6g,%.6g) moving_support=%u hull_valid=%u hull=(%.6g,%.6g,%.6g)->(%.6g,%.6g,%.6g)",
				phase, jumpPhysicsReasonName(p.reason), unsigned(p.reason), jumpActorPostureName(p.posture),
				unsigned(p.posture), unsigned(model != nullptr),
				...
				model ? model->binding.step : 0, model ? model->gravity : 0, model ? model->verticalImpulse : 0,
				model ? model->crouchSpeedMultiplier : 0, p.moveType, p.waterLevel, p.flags, p.baseVelocity.x,
				p.baseVelocity.y, p.baseVelocity.z, unsigned(p.movingSupport), unsigned(hull != nullptr),
				hull ? hull->minimum.x : 0, hull ? hull->minimum.y : 0, hull ? hull->minimum.z : 0,
				hull ? hull->maximum.x : 0, hull ? hull->maximum.y : 0, hull ? hull->maximum.z : 0);
			line(detail);
		};
	...
	if (d.dropState)
	{
		...
		std::snprintf(
			drop, sizeof(drop),
			"drop actor=%u:%u route=%llu step=%zu state=%u reason=%u fall=%.6g gap=%.6g predicted_damage=%.6g",
			unsigned(d.binding.actor.slot), unsigned(d.binding.actor.generation.value),
			static_cast<unsigned long long>(d.binding.routeGeneration), d.binding.step, unsigned(*d.dropState),
			unsigned(d.dropReason), d.dropPlan ? d.dropPlan->fall : 0, d.dropPlan ? d.dropPlan->gap : 0,
	...
	if (d.ladderState)
	{
		...
		std::snprintf(ladder, sizeof(ladder),
					  "ladder state=%u reason=%u source=%llu generation=%llu link=%llu press_tick=%llu bind=%u "
					  "frame=%u guard_queries=%llu profile=standard-cs",
					  unsigned(*d.ladderState), unsigned(d.ladderReason),
					  static_cast<unsigned long long>(link ? link->sourceId : 0),
					  static_cast<unsigned long long>(link ? link->generation : 0),
					  static_cast<unsigned long long>(link ? link->linkId : 0),
					  static_cast<unsigned long long>(d.ladderPressTick.value),
					  unsigned(current_->motionTrace_.ladderBindingReason),
					  unsigned(current_->motionTrace_.ladderFrameReason),
					  static_cast<unsigned long long>(current_->motionTrace_.ladderGuardQueries));
		line(ladder);
	}
}
void NavConsole::clearPending() noexcept
{
	if (current_->pendingMotion_ && movement_)
		(void)movement_->cancel(current_->pendingMotion_->binding.actor, current_->pendingMotion_->binding.map,
								current_->pendingMotion_->tick);
...
void NavConsole::stopMotion() noexcept
{
	...
	if (current_->walk_ || current_->pendingMotion_)
	...
	if (current_->walk_ && current_->walk_->state() == nav::local::WalkState::Running)
	{
		...
		recordMotion(MotionEvent::Cancelled, MotionReason::Cancelled);
		current_->neutralDuck_ = current_->motionTrace_.decision.intent.duck == core::ActionRequest::Hold;
	}
	current_->walk_.reset();
	current_->pump_.reset();
	current_->segment_.reset();
	current_->intentWallAgeUs_ = 0;
}
void NavConsole::failExecution(nav::runtime::ExecutionFailure reason, bool structural, bool bindFailedEdge,
							   bool aggregateSource) noexcept
{
	...
	auto edge = bindFailedEdge ? current_->motionTrace_.failedEdge : std::optional<nav::query::NavDirectedEdge>{};
	if (bindFailedEdge && !edge)
		edge = current_->motionTrace_.selectedEdge;
	...
	std::snprintf(execution, sizeof(execution),
				  "execution actor=%u:%u agent=%llu map=%u route=%llu tick=%llu state=Failed reason=%u "
				  "failed_edge=%u:%u failed_transition=%zu structural=%u retry_at_us=%llu",
...
{
	current_->motionTrace_ = {};
	current_->motionNext_ = current_->motionCount_ = 0;
	current_->motionSequence_ = 0;
	current_->requestTick_ = s.tick;
	...
	const auto fail = [&](MotionReason reason) {
		...
		failExecution(
			reason == MotionReason::MissingObservation ? nav::runtime::ExecutionFailure::Observation
													   : nav::runtime::ExecutionFailure::Corridor,
	...
	if (!ready(s) || !movement_ || !activeGraph || !index_)
	{
		fail(MotionReason::MissingObservation);
		return;
	}
	...
	const auto corridor =
		nav::corridor::Corridor::build(*activeGraph, *route.route, clearance, {100000, 256U * 1024U * 1024U, 1000000},
									   nav::corridor::PortalPolicy::AllowMicroTransit);
	if (!corridor)
	{
		...
		fail(corridor.error == nav::corridor::Error::InvalidGoalArea ? MotionReason::InvalidGoal
																	 : MotionReason::InvalidCorridor);
		return;
	}
	...
	const nav::model::NavVector3 xy{
		static_cast<float>(std::clamp(double(s.position->x), lowX + insetX, highX - insetX)),
	...
	current_->pump_.emplace(current_->motionTrace_.decision.binding);
	current_->intentWallAgeUs_ = 0;
	current_->neutralBinding_.reset();
}
nav::local::ProbeResult NavConsole::guardGround(metamod::LifecycleCoordinator& owner,
												const nav::runtime::MovementSnapshot& s,
												const PendingMotion& pending) noexcept
{
	...
	if (!index_ || !current_->walk_ || !current_->session_ || !current_->session_->executable() ||
		current_->walk_->step() != pending.binding.step || !s.position || !pending.segment)
		return proof;
	const auto& route = current_->session_->trace();
	if (route.routeGeneration != pending.binding.routeGeneration || route.actor != pending.binding.actor ||
		route.agent != pending.binding.agent || route.map != pending.binding.map ||
		navigation_.map != pending.binding.map)
		return proof;
	if (current_->guardQueries_ >= 21)
	{
	...
	struct Queries final : nav::runtime::IWorldQueries
	{
		...
		nav::runtime::WorldQueryResult query(const nav::runtime::QueryRequest& original) override
		{
			...
			if (count >= 21)
			{
				nav::runtime::WorldQueryResult r;
				r.stamp = q.stamp;
				r.kind = q.kind;
				r.error = nav::runtime::QueryError::BudgetExceeded;
				return r;
			}
			...
			if (!(r.stamp == q.stamp) || r.kind != q.kind)
			{
				...
				std::snprintf(mismatch, sizeof(mismatch),
							  "nav guard_query_mismatch actor=%u:%u kind=%u expected=(%llu,%u,%llu,%u,%u) "
							  "actual=(%llu,%u,%llu,%u,%u) actual_kind=%u error=%u",
							  unsigned(q.stamp.actor.slot), unsigned(q.stamp.actor.generation.value), unsigned(q.kind),
							  ...
							  unsigned(q.stamp.agent.value), static_cast<unsigned long long>(r.stamp.tick.value),
							  unsigned(r.stamp.map.value), static_cast<unsigned long long>(r.stamp.routeGeneration),
							  unsigned(r.stamp.ordinal), unsigned(r.stamp.agent.value), unsigned(r.kind),
							  unsigned(r.error));
				port.line(mismatch);
			}
			if (sameQueryContext(r.stamp, q.stamp))
				r.stamp = original.stamp;
			return r;
		}
	...
	inRequest_ = true;
	queryingEntity_ = owner.entityFor(s.actor);
	queryingPlayers_ = &owner.registry();
	queryingOwner_ = &owner;
	proof = nav::local::inspectGroundFrame(s, pending.binding.routeGeneration, source, target, x, y, *index_,
										   navigation_.map, queries, limits);
	inRequest_ = false;
	queryingEntity_ = nullptr;
	queryingPlayers_ = nullptr;
	queryingOwner_ = nullptr;
	return proof;
}
MotionReason NavConsole::guardDrop(metamod::LifecycleCoordinator& owner, const nav::runtime::MovementSnapshot& s,
								   const PendingMotion& pending) noexcept
{
	using namespace nav::runtime;
	if (!pending.drop || !s.position || !s.velocity || !s.hull || !s.grounded || !index_ || !current_->walk_ ||
		current_->walk_->step() != pending.binding.step)
	...
	if (!physics || !physics.physics || physics.physics->gravity != pending.dropGravity || !s.velocity->isFinite() ||
		plan.fall <= 0 || plan.fall > 128 || plan.gap < 0 || plan.gap > 32 || s.velocity->z < -580 ||
		s.ducked != false ||
		(pending.command.buttons &
		 (static_cast<core::ButtonMask>(core::Button::Jump) | static_cast<core::ButtonMask>(core::Button::Duck))))
		return MotionReason::DropChanged;
	const auto delta = movement_->frameDeltaUs();
	if (!delta || delta > 120000 || current_->guardQueries_ > 18)
		return MotionReason::StaleCommand;
	const double dt =
		double(std::clamp(delta / 1000 + (delta % 1000 >= 500 ? 1U : 0U), std::uint64_t{1}, std::uint64_t{255})) / 1000;
	const double yaw = pending.command.view.yaw * 3.14159265358979323846 / 180;
	...
	if (std::hypot(vx, vy) > 100.001 || pending.command.movement.up != 0 || !plan.landing.isFinite() ||
		!plan.takeoff.isFinite())
		return MotionReason::DropChanged;
	const auto ask = [&](QueryKind kind, nav::model::NavVector3 a, nav::model::NavVector3 b,
		...
		if (!(r.stamp == q.stamp) || r.kind != q.kind || r.error != QueryError::None)
			return {};
		return r;
	};
	...
	if (!floor || !floor->floor || !floor->floor->supported || !std::isfinite(floor->floor->height) ||
		!floor->floor->normal.isFinite() || floor->floor->normal.z < 0.7f ||
		std::abs(floor->floor->height - floorZ) > 4)
		return MotionReason::DropChanged;
	const auto area = index_->containing({plan.landing.x, plan.landing.y, floor->floor->height}, 18);
	if (!area || !*area.value || (**area.value).areaId != plan.target)
		return MotionReason::DropChanged;
	const double vertical =
		s.grounded == true ? 0.0 : double(s.velocity->z) * dt - 0.5 * physics.physics->gravity * dt * dt;
	// Air movement uses observed velocity; ground movement uses the submitted wish.
	const nav::model::NavVector3 end{static_cast<float>(s.position->x + (s.grounded == true ? vx : s.velocity->x) * dt),
	...
	if (!sweep || !sweep->hull || sweep->hull->startSolid || !sweep->hull->end.isFinite() ||
		!sweep->hull->normal.isFinite() || !std::isfinite(sweep->hull->fraction) || sweep->hull->fraction < 0 ||
		sweep->hull->fraction > 1)
		return MotionReason::DropChanged;
	...
	if (!closeCoordinate(hit.end.x, s.position->x + (end.x - s.position->x) * hit.fraction) ||
		...
		!closeCoordinate(hit.end.z, s.position->z + (end.z - s.position->z) * hit.fraction))
		return MotionReason::DropChanged;
	if (hit.fraction < 1)
	{
		if (vertical >= 0 || hit.normal.z < 0.7f || !navigation_.graph)
			return MotionReason::DropChanged;
...
{
	observe(owner);
	if (current_->guardTick_ != owner.registry().currentTick())
	{
		current_->guardTick_ = owner.registry().currentTick();
		current_->guardQueries_ = 0;
	}
	if (!current_->pendingMotion_ || !movement_)
		return;
	...
	current_->motionTrace_.dispatchDurationUs =
		1000 * std::clamp(duration / 1000 + (duration % 1000 >= 500 ? 1U : 0U), std::uint64_t{1}, std::uint64_t{255});
	...
	current_->motionTrace_.intentSpeed =
		std::hypot(double(pending.command.movement.forward), double(pending.command.movement.side));
	...
	current_->motionTrace_.intentLifetimeUs =
		nav::local::IntentPump::freshnessLimit(current_->motionTrace_.decision.intent);
	...
	if (pending.envelope && current_->walk_ && current_->walk_->native())
	{
		...
		if (ready(s, true) && s.elapsedUs && s.elapsedUs <= pending.remainingFreshUs && s.tick.isAfter(pending.tick) &&
			pending.envelope->matches(s, s.elapsedUs) && index_ &&
			std::hypot(double(pending.command.movement.forward), double(pending.command.movement.side)) <=
				double(*s.speedLimit) + 0.001)
		{
			...
			nav::model::NavVector3 destination{
				...
				static_cast<float>(s.position->y + (m.forward * std::sin(yaw) - m.side * std::cos(yaw)) * dt),
				s.position->z};
			inRequest_ = true;
			queryingEntity_ = owner.entityFor(s.actor);
			queryingPlayers_ = &owner.registry();
			...
			{
				(void)applyDeferredInvalidation();
				return;
			}
		}
		if (!proof)
		{
			...
			if (press)
				(void)current_->walk_->reportJumpDispatch({binding, queued, s.tick, false});
			...
			const bool observationGap =
				...
				(proof.reason == nav::local::ProbeReason::NoArea && current_->motionTrace_.decision.avoiding);
			if (observationGap)
			{
			...
			current_->motionTrace_.commandTick = queued;
			current_->motionTrace_.dispatchTick = s.tick;
			clearPending();
			if (current_->pump_)
				current_->pump_->submissionRejected();
	...
	const auto reportTraversalRejection = [&](MotionReason rejected) noexcept {
		if ((rejected != MotionReason::JumpChanged && rejected != MotionReason::LadderChanged &&
			 rejected != MotionReason::PostureChanged) ||
			!pending.observation.position)
			return;
		const auto expected = (pending.command.buttons & static_cast<core::ButtonMask>(core::Button::Duck))
								  ? nav::local::ExpectedProgress::Crouch
								  : nav::local::ExpectedProgress::Walk;
		(void)current_->recovery_.report({pending.binding, pending.tick, s.tick, *pending.observation.position,
										  current_->motionTrace_.decision.progressDirection,
	...
	if (!ready(s, nativeOwned || pending.jump.has_value() || pending.drop.has_value() || pending.ladder.has_value() ||
					  stationary) ||
		s.actor != pending.binding.actor || s.agent != pending.binding.agent || s.map != pending.binding.map ||
		!s.tick.isAfter(pending.tick) ||
		(!pending.jump && (s.hull->minimum != pending.observation.hull->minimum ||
						   s.hull->maximum != pending.observation.hull->maximum)))
		reason = MotionReason::MissingObservation;
		...
		reason = MotionReason::StaleCommand;
			 ...
			 add(add(current_->navigationTimeUs_, elapsed), current_->motionTrace_.dispatchDurationUs) >
				 current_->motionTrace_.decision.recovery.deadlineUs)
		reason = MotionReason::StaleCommand;
	else
	{
		const auto& m = pending.command.movement;
		...
		const auto delta = movement_->frameDeltaUs();
		const auto msec =
			std::clamp(delta / 1000 + (delta % 1000 >= 500 ? 1U : 0U), std::uint64_t{1}, std::uint64_t{255});
		if (speed > double(*s.speedLimit) + 0.001 ||
			(!nativeOwned && speed > 0 && !pending.jump && !pending.drop && !pending.ladder && !pending.contact &&
			 (!pending.segment ||
	...
	if (reason != MotionReason::None)
	{
		...
		if (pending.ladder)
			reportLadderTransport(current_->motionTrace_.decision, pending.tick, s.tick, false);
		...
		current_->motionTrace_.commandTick = pending.tick;
		current_->motionTrace_.dispatchTick = s.tick;
		current_->motionTrace_.failedEdge = edge;
		clearPending();
		if (current_->pump_)
			current_->pump_->submissionRejected();
		current_->motionTrace_.rejected = add(current_->motionTrace_.rejected, 1);
		recordMotion(MotionEvent::Rejected, reason);
		if (nativeOwned)
			return;
		if (reason == MotionReason::StaleCommand && current_->walk_ && current_->session_ &&
			current_->session_->executable() && binding.actor == current_->actor && binding.agent.isValid() &&
			binding.map == navigation_.map)
		{
			...
			const bool scheduled =
				edge ? current_->replan_.schedule(binding, *edge, s.tick, current_->navigationTimeUs_)
		...
		{
			failExecution(reason == MotionReason::MissingObservation ? nav::runtime::ExecutionFailure::Observation
																	 : nav::runtime::ExecutionFailure::Transport);
		}
		return;
	}
	if (pending.ladder)
	...
	if (pending.jump)
	{
		...
		if (preserveFlightBudget)
			current_->motionTrace_.decision.jumpGuardSuppressed = true;
		inRequest_ = true;
		queryingEntity_ = owner.entityFor(current_->actor);
		queryingPlayers_ = &owner.registry();
		queryingOwner_ = &owner;
	...
	if (!pending.contact && (pending.command.movement.forward != 0 || pending.command.movement.side != 0))
	{
		...
		if (!proof)
		{
			const bool provenDetour =
				...
				current_->motionTrace_.decision.detourFirstReason == nav::local::ProbeReason::None &&
				proof.reason == nav::local::ProbeReason::AllSolid;
			const bool detourRecoverable = current_->motionTrace_.decision.detourFirstAttempted ||
										   current_->motionTrace_.decision.detourSecondAttempted;
			const auto observationGap = proof.reason == nav::local::ProbeReason::BudgetExceeded ||
										proof.reason == nav::local::ProbeReason::StaleQuery ||
										proof.reason == nav::local::ProbeReason::QueryUnavailable ||
										proof.reason == nav::local::ProbeReason::QueryFailed ||
										proof.reason == nav::local::ProbeReason::InvalidResult ||
										...
										proof.reason == nav::local::ProbeReason::NoSupport ||
										proof.reason == nav::local::ProbeReason::FloorHeightMismatch ||
										(detourRecoverable && (proof.reason == nav::local::ProbeReason::StartSolid ||
															   proof.reason == nav::local::ProbeReason::AllSolid));
	...
	if (s.ducked == true && (pending.command.buttons & static_cast<core::ButtonMask>(core::Button::Duck)) == 0)
	{
		...
		if (result.error != nav::runtime::QueryError::None || !result.clearance || !result.clearance->clear)
		{
			...
			if (current_->pump_)
				current_->pump_->submissionRejected();
			current_->motionTrace_.rejected = add(current_->motionTrace_.rejected, 1);
			recordMotion(MotionEvent::Rejected, MotionReason::PostureChanged);
			return;
		}
	}
	if (pending.contact)
	{
		const auto contact = *pending.contact;
		const auto queued = pending.tick;
		const auto command = pending.command;
		bool valid = false;
		if (!current_->guardQueries_)
		{
			++current_->guardQueries_;
			current_->motionTrace_.contactGuardQueries = add(current_->motionTrace_.contactGuardQueries, 1);
			nav::runtime::QueryRequest q{{s.agent, s.actor, s.map, s.tick, pending.binding.routeGeneration, 1},
										 nav::runtime::QueryKind::Door,
										 *s.position,
										 contact.end,
										 s.hull,
				...
				result.door->canTouch && !result.door->open && result.hull && !result.hull->startSolid)
			{
				...
				const double length = std::hypot(dx, dy),
							 distance = std::hypot(double(h.end.x) - s.position->x, double(h.end.y) - s.position->y);
				const auto delta = movement_->frameDeltaUs();
				const auto msec =
					std::clamp(delta / 1000 + (delta % 1000 >= 500 ? 1U : 0U), std::uint64_t{1}, std::uint64_t{255});
				...
				valid = length > 0 && length <= 0.751 && distance <= 0.125 && h.fraction >= 0 && h.fraction < 1 &&
						travel >= distance + 0.001 && travel <= 0.75 && command.buttons == 0 &&
						command.movement.up == 0 && std::abs(double(h.end.z) - s.position->z) <= 0.1 &&
						std::abs(double(contact.end.z) - s.position->z) <= 0.1 &&
						std::abs((double(h.end.x) - s.position->x) * dy - (double(h.end.y) - s.position->y) * dx) /
								length <=
							0.01 &&
						std::abs(distance - length * h.fraction) <= 0.02 &&
						h.normal.x * dx / length + h.normal.y * dy / length <= -0.7 && std::abs(h.normal.z) <= 0.2 &&
						speed > 0 && (vx * dx + vy * dy) / (speed * length) > 0.999;
			}
		}
		if (!valid)
		{
			...
			if (current_->pump_)
				current_->pump_->submissionRejected();
			current_->motionTrace_.rejected = add(current_->motionTrace_.rejected, 1);
			recordMotion(MotionEvent::Rejected, MotionReason::DoorChanged);
		}
		return;
	}
	if ((pending.command.buttons & static_cast<core::ButtonMask>(core::Button::Use)) != 0)
	{
		...
		const auto queued = pending.tick;
		inRequest_ = true;
		const auto view = doorUseView(engine_, owner.entityFor(current_->actor), current_->motionTrace_.decision.doorId,
									  globals_ ? globals_->maxEntities : 0);
		inRequest_ = false;
		if (deferredInvalidation_)
		{
							   ...
							   const std::optional<MotionTrace>& ticket) noexcept
{
	...
	if (!sameRoute)
		current_->motionTrace_ = *ticket; // reentrant goto must not receive old-route feedback
	if (movement_ != nullptr)
	{
		...
		if (transport.physical.valid)
		{
			...
			trace.dispatchNoProgress = (transport.forward != 0.0F || transport.side != 0.0F || transport.up != 0.0F ||
										transport.buttons != 0U) &&
									   trace.dispatchHorizontalDisplacement < 1.0;
			const double corridorProgress =
				dx * trace.decision.progressDirection.x + dy * trace.decision.progressDirection.y;
	...
	if (sameRoute && current_->walk_ && current_->walk_->native())
	{
		...
		current_->walk_->feedback(
			{b,
			 ticket->commandTick,
			 tick,
			 result.dispatched(),
			 trace.dispatchPhysicalValid,
			 ...
			 trace.dispatchBeforeOnGround != 0,
			 trace.dispatchAfterOnGround != 0});
	}
	if (sameRoute)
		reportLadderTransport(ticket->decision, ticket->commandTick, tick, result.dispatched());
	if (sameRoute && ticket->dispatchOrigin)
	{
		const auto& d = ticket->decision;
		auto expected = nav::local::ExpectedProgress::Pause;
		if (d.state == nav::local::WalkState::Running && !d.jumpState && !d.dropState && !d.ladderState &&
			!d.doorState && !d.contact && d.blockerAction != nav::local::BlockerAction::Yield &&
	...
	if (current_->pendingMotion_ && current_->pendingMotion_->tick == ticket->commandTick &&
		current_->pendingMotion_->binding.actor == b.actor && current_->pendingMotion_->binding.map == b.map &&
		current_->pendingMotion_->binding.routeGeneration == b.routeGeneration)
		current_->pendingMotion_.reset();
	current_->motionTrace_.commandTick = ticket->commandTick;
	current_->motionTrace_.dispatchTick = tick;
	current_->motionTrace_.transportError = result.error;
	if (result.dispatched())
	{
		current_->motionTrace_.dispatched = add(current_->motionTrace_.dispatched, 1);
		recordMotion(MotionEvent::Dispatched);
	}
	else
	{
		if (sameRoute && current_->pump_)
			current_->pump_->submissionRejected();
		current_->motionTrace_.rejected = add(current_->motionTrace_.rejected, 1);
	...
	if (!sameRoute)
		current_->motionTrace_ = saved;
}
void NavConsole::moveFrame(metamod::LifecycleCoordinator& owner) noexcept
{
	observe(owner);
	const auto composePendingCombat = [&]() noexcept {
		if (inRequest_ || !movement_ || current_->pendingMotion_)
			return;
		const auto s = snapshot(owner);
		const auto* runtimeDecision = owner.runtimeOrchestrator().decision(s.actor);
		if (!ready(s, true) || !s.elapsedUs || !runtimeDecision || !runtimeDecision->executable ||
			!runtimeDecision->combat.hasAttackInput())
			return;
		...
		if (s.ducked == true)
			neutral.duck = core::ActionRequest::Hold;
	...
	{
		const auto s = snapshot(owner);
		if (!ready(s, true) || s.actor != current_->neutralBinding_->actor ||
			s.agent != current_->neutralBinding_->agent || s.map != current_->neutralBinding_->map)
		{
			current_->neutralBinding_.reset();
			return;
		}
		if (!s.elapsedUs || !s.tick.isAfter(current_->motionTrace_.commandTick))
			return;
		current_->motionTrace_.decision.binding = *current_->neutralBinding_;
		current_->neutralBinding_.reset();
		core::MovementIntent neutral;
		if (current_->neutralDuck_ || s.ducked == true)
			neutral.duck = core::ActionRequest::Hold;
		submitMotion(s, owner, neutral, true, 0);
		return;
	}
	if (inRequest_ || !current_->walk_ || !current_->pump_ || !movement_ || !current_->session_ ||
		!current_->session_->executable() || current_->walk_->state() != nav::local::WalkState::Running)
		return;
	const auto s = snapshot(owner);
	...
	if (!s.elapsedUs || !movement_->frameDeltaUs())
		return;
	if (!s.tick.isAfter(current_->requestTick_))
		return; // Route request owns ordinal 1 on its tick.
	...
	if (schedule.decisionDue)
	{
		...
		auto binding = current_->motionTrace_.decision.binding;
		binding.step = current_->walk_->step();
		std::optional<nav::local::RecoveryEdge> recoveryEdge;
		...
		const auto recovery =
			current_->walk_->native()
				? nav::local::RecoveryDecision{}
				: (s.position ? current_->recovery_.observe(binding, s.tick, current_->navigationTimeUs_, *s.position)
							  : current_->recovery_.decision());
		nav::local::WalkDecision decision;
		if (current_->walk_->native())
		{
			const auto* step =
				engine_ && engine_->pfnCVarGetPointer ? engine_->pfnCVarGetPointer("sv_stepsize") : nullptr;
			current_->walk_->stepHeight(step ? double(step->value) : 0);
		}
		if (recovery.state == nav::local::RecoveryState::Monitoring)
		{
			...
			decision = current_->walk_->update(s, *index, navigation_.map, *this, current_->pump_->timeUs(), reserved,
											   physics.physics, ladder);
			decision.recovery = recovery;
		}
		else
			decision = current_->walk_->recover(s, *index, navigation_.map, *this, recovery, reserved);
		inRequest_ = false;
		queryingEntity_ = nullptr;
		queryingPlayers_ = nullptr;
		queryingOwner_ = nullptr;
		...
		current_->motionTrace_.missedDecisions = add(current_->motionTrace_.missedDecisions, schedule.missedDeadlines);
			...
			current_->motionTrace_.commandTick.isValid() && s.tick.isAfter(current_->motionTrace_.commandTick))
		{
			const auto expected = decision.intent.duck == core::ActionRequest::Hold
									  ? nav::local::ExpectedProgress::Crouch
									  : nav::local::ExpectedProgress::Walk;
			(void)current_->recovery_.report({decision.binding, current_->motionTrace_.commandTick, s.tick, *s.position,
											  decision.progressDirection, current_->intentWallAgeUs_, expected, false,
											  true});
		}
		if (decision.state == nav::local::WalkState::Arrived)
		{
			...
			if (wasRoam)
			{
				...
				if (goal.isValid())
				{
					for (std::size_t i = (std::min)(current_->roamRecentGoalCount_, current_->roamRecentGoals_.size());
						 i > 0; --i)
					{
						if (i < current_->roamRecentGoals_.size())
							current_->roamRecentGoals_[i] = current_->roamRecentGoals_[i - 1U];
					}
					current_->roamRecentGoals_[0] = goal;
					if (current_->roamRecentGoalCount_ < current_->roamRecentGoals_.size())
						++current_->roamRecentGoalCount_;
				}
			}
			stopMotion();
			if (current_->session_)
				printUpdate(current_->session_->cancel());
			return;
		}
		current_->segment_ = decision.target && s.position
								 ? std::optional<Segment>{{*s.position, decision.target->origin}}
								 : std::nullopt;
		current_->intentWallAgeUs_ = 0;
		...
		if (decision.terminalEvent && decision.reason == nav::local::WalkReason::RecoveryReplan)
		{
			...
			current_->recoveryReplan_ =
				geometryEdge
					? current_->replan_.schedule(decision.binding, *current_->motionTrace_.selectedEdge, s.tick,
		...
		if (decision.terminalEvent && decision.reason == nav::local::WalkReason::DynamicBlocked &&
			...
			decision.blockerReason == nav::local::BlockerReason::TimedOut && decision.blocker && decision.blocker->id &&
			current_->motionTrace_.selectedEdge)
		{
			const auto& blocker = *decision.blocker;
			const bool player = blocker.kind == nav::runtime::BlockerKind::Player ||
								blocker.kind == nav::runtime::BlockerKind::Teammate ||
								blocker.kind == nav::runtime::BlockerKind::Enemy;
			...
			{
				(void)current_->replan_.schedule(decision.binding, *current_->motionTrace_.selectedEdge, s.tick,
												 current_->navigationTimeUs_);
				printReplan();
			}
		}
		if (decision.terminalEvent && decision.state == nav::local::WalkState::Failed)
		{
			...
			else
			{
				...
				const bool jumpStructural = jumpFailure && decision.jumpRecoveryDisposition ==
															   nav::local::RecoveryDisposition::StructuralEdgeExclusion;
				const bool structural = decision.reason == nav::local::WalkReason::UnsupportedTraversal ||
										(jumpStructural && !decision.blocker) ||
										(decision.reason == nav::local::WalkReason::ProbeFailed &&
										 decision.probeReason == nav::local::ProbeReason::Blocked && !decision.blocker);
				...
				const bool geometryFailure =
					decision.blocker && decision.blocker->kind == nav::runtime::BlockerKind::Geometry;
				failExecution(nav::runtime::ExecutionFailure::Motion, current_->walk_->native() ? false : structural,
		...
		if (!current_->pump_->publish(decision.binding, s.tick, decision.intent))
			return;
	}
	const auto output = current_->pump_->take();
	if (!output.emit || !ready(s, current_->walk_->native() || current_->motionTrace_.decision.jumpState.has_value() ||
									  current_->motionTrace_.decision.dropState.has_value() ||
									  current_->motionTrace_.decision.ladderState.has_value()))
		return;
	const auto age = (std::max)(output.intentAgeUs, current_->intentWallAgeUs_);
	if (age > output.intentLifetimeUs)
	{
		current_->pump_->stop(nav::local::PumpReason::StaleIntent);
		return;
	}
							  ...
							  const core::MovementIntent& intent, bool firstFrame, std::uint64_t age) noexcept
{
	...
	{
		effective = {};
		effective.jump = core::ActionRequest::Release;
	}
	...
	if (s.ducked == true && effective.duck == core::ActionRequest::None)
		effective.duck = core::ActionRequest::Hold;
	const auto command =
		core::Motor::command(effective, {s.view->x, s.view->y, s.view->z}, *s.speedLimit, s.elapsedUs, firstFrame);
	if (!command)
	{
		if (!frameCommandOnly && current_->pump_)
			current_->pump_->submissionRejected();
		current_->motionTrace_.rejected = add(current_->motionTrace_.rejected, 1);
		recordMotion(MotionEvent::Rejected, MotionReason::MotorRejected);
		if (!frameCommandOnly)
	...
	if (const auto combat = owner.takeRuntimeCombatDecision(s.actor, s.agent, s.map, owner.round(), s.tick))
	{
		const auto composed = owner.submitCombatDecision(s.actor, s.map, s.tick, *combat, submittedCommand);
		if (composed.composition)
			submittedCommand = composed.composition.command;
		result = composed.transport;
		if (!composed.accepted && result.error == metamod::MovementError::None)
			result = metamod::MovementResult::rejectedResult(metamod::MovementError::RegistryRejected);
	}
	else
	{
	...
	current_->motionTrace_.commandTick = s.tick;
	current_->motionTrace_.dispatchTick = {};
	current_->motionTrace_.intentAgeUs = age;
	current_->motionTrace_.transportError = result.error;
	if (result.queued())
	{
		...
		const auto pendingFreshness =
			intent.locomotion == core::LocomotionMode::Explicit ? add(remaining, frameDelta) : remaining;
		current_->motionTrace_.elapsedUs = s.elapsedUs;
		...
		current_->motionTrace_.validatedDistance = decision.target && s.position
													   ? std::hypot(double(decision.target->origin.x) - s.position->x,
																	double(decision.target->origin.y) - s.position->y)
													   : 0.0;
		if (frameCommandOnly)
		{
		...
		pending.tick = s.tick;
		pending.remainingFreshUs = pendingFreshness;
		pending.observation = s;
		pending.command = submittedCommand;
		pending.segment = current_->segment_;
		pending.contact = contact;
		if (current_->walk_ && current_->walk_->native())
			pending.envelope = current_->walk_->envelope();
		...
		if (decision.jumpState && decision.jumpPlan && decision.jumpPhysics)
		{
			...
			current_->pendingMotion_->jump =
				JumpTicket{*decision.jumpPlan, queuedPhysics, *decision.jumpState, decision.jumpPressTick};
		}
		...
		{
			pending.ladder = decision.ladderPlan;
			pending.ladderState = *decision.ladderState;
			...
			pending.ladderTarget = current_->walk_ ? current_->walk_->ladderTarget(*decision.ladderPlan, *s.position)
												   : decision.ladderPlan->end;
		}
		current_->motionTrace_.queued = add(current_->motionTrace_.queued, 1);
		recordMotion(MotionEvent::Queued);
	}
