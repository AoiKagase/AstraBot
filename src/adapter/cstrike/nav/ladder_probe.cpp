...
bool closeValue(float a, float b) noexcept
{
	return std::abs(double(a) - b) <= 0.05;
}
bool same(V a, V b) noexcept
{
	return closeValue(a.x, b.x) && closeValue(a.y, b.y) && closeValue(a.z, b.z);
}
class Probe
{
public:
	...
	const LadderCandidate& candidate;
	int maximum;
	std::uint32_t limit;
	LadderProbeResult result{};
	edict_t* entity{};
	int modelIndex{}, modelName{};
	bool fail(LadderProbeReason reason) noexcept
	{
	...
	bool fresh() noexcept
	{
		if (world.currentMap(world.context) != map)
			return fail(LadderProbeReason::StaleMap);
		const auto slot = static_cast<std::uint32_t>(candidate.entityId);
		auto* e = world.engine->pfnPEntityOfEntIndex(static_cast<int>(slot));
		if (!e || e->free || (entity && e != entity))
			return fail(LadderProbeReason::StaleEntity);
		...
		if (world.engine->pfnIndexOfEdict(e) != static_cast<int>(slot) || !name || std::strcmp(name, "func_ladder") ||
			!model || model[0] != '*' || model[1] < '0' || model[1] > '9' || e->v.modelindex <= 0 ||
			e->v.modelindex >= 512 || e->v.skin != CONTENTS_LADDER ||
			(modelIndex && (e->v.modelindex != modelIndex || e->v.model != modelName)) ||
			static_cast<std::uint32_t>(e->serialnumber) != serial || e->free ||
			value(e->v.absmin) != candidate.minimum || value(e->v.absmax) != candidate.maximum || e->v.angles.x != 0 ||
			e->v.angles.y != 0 || e->v.angles.z != 0 || e->v.velocity.x != 0 || e->v.velocity.y != 0 ||
			e->v.velocity.z != 0 || e->v.avelocity.x != 0 || e->v.avelocity.y != 0 || e->v.avelocity.z != 0 ||
			world.currentMap(world.context) != map)
			return fail(LadderProbeReason::StaleEntity);
		entity = e;
		modelIndex = e->v.modelindex;
		modelName = e->v.model;
		return true;
	}
	bool trace(V from, V to, bool model, TraceResult& hit, bool contact = false) noexcept
	{
		if (!fresh())
			return false;
		if (result.queries >= limit)
			return fail(LadderProbeReason::BudgetExceeded);
		if (!from.isFinite() || !to.isFinite())
			return fail(LadderProbeReason::InvalidInput);
		...
		if (model)
			world.engine->pfnTraceModel(a, b, contact ? 1 : 0, entity, &hit);
		else
			world.engine->pfnTraceHull(a, b, 0, 1, nullptr, &hit);
		if (!fresh())
			return false;
		const float f = hit.flFraction;
		if (!std::isfinite(f) || f < 0 || f > 1 || !value(hit.vecEndPos).isFinite() ||
			!value(hit.vecPlaneNormal).isFinite() ||
			!same(value(hit.vecEndPos),
				  {from.x + (to.x - from.x) * f, from.y + (to.y - from.y) * f, from.z + (to.z - from.z) * f}))
			return fail(LadderProbeReason::InvalidTrace);
		if (!contact && (hit.fStartSolid || hit.fAllSolid))
			return fail(LadderProbeReason::Blocked);
		return true;
	}
	bool contact(V at) noexcept
	{
		TraceResult t{};
		return trace(at, at, true, t, true) &&
			   ((t.pHit == entity && (t.fStartSolid || t.fAllSolid)) || fail(LadderProbeReason::NoFace));
	}
	bool clear(V from, V to) noexcept
	{
		TraceResult t{};
		return trace(from, to, false, t) && (t.flFraction == 1 || fail(LadderProbeReason::Blocked));
	}
	bool endpoint(V point, float bottom, float top, const nav::query::NavSpatialIndex& index,
				  LadderEndpoint& out) noexcept
	{
		TraceResult t{};
		if (!trace({point.x, point.y, top + 36}, {point.x, point.y, bottom + 36}, false, t))
			return false;
		if (t.flFraction == 1 || t.vecPlaneNormal.z < 0.7f)
			return fail(LadderProbeReason::NoSupport);
...
LadderProbeResult inspectLadderPassage(LadderWorld world, core::MapGeneration map, const LadderCandidate& c,
									   LadderFace face, LadderExit exit, const nav::query::NavSpatialIndex& index,
									   core::MapGeneration indexMap, int maximum, std::uint32_t maxQueries) noexcept
{
	...
	if (!map.isValid() || maximum < 1 || maximum > 8192 || !slot || slot >= static_cast<std::uint32_t>(maximum) ||
		maxQueries > 12 || !c.minimum.isFinite() || !c.maximum.isFinite() || c.minimum.x >= c.maximum.x ||
		c.minimum.y >= c.maximum.y || double(c.maximum.z) - c.minimum.z < 72 ||
		(face != LadderFace::MinX && face != LadderFace::MaxX && face != LadderFace::MinY &&
		 face != LadderFace::MaxY) ||
		(exit != LadderExit::SameFace && exit != LadderExit::AcrossTop))
		return invalid;
	if (map != indexMap)
	{
		invalid.reason = LadderProbeReason::StaleMap;
		return invalid;
	}
	...
	{
		invalid.reason = LadderProbeReason::Unavailable;
		return invalid;
	}
	...
	p.face = face;
	p.exit = exit;
	p.normal = face == LadderFace::MinX	  ? V{-1, 0, 0}
			   : face == LadderFace::MaxX ? V{1, 0, 0}
			   : face == LadderFace::MinY ? V{0, -1, 0}
										  : V{0, 1, 0};
	V center{static_cast<float>((double(c.minimum.x) + c.maximum.x) / 2),
			 static_cast<float>((double(c.minimum.y) + c.maximum.y) / 2), 0};
	V edge = center;
	if (p.normal.x)
		edge.x = p.normal.x < 0 ? c.minimum.x : c.maximum.x;
	else
		edge.y = p.normal.y < 0 ? c.minimum.y : c.maximum.y;
	for (int sample = 0; sample < 3; ++sample)
	{
		const float z = sample == 0	  ? c.minimum.z + 18
						: sample == 2 ? c.maximum.z - 18
									  : static_cast<float>((double(c.minimum.z) + c.maximum.z) / 2);
		TraceResult t{};
		if (!probe.trace({edge.x + p.normal.x * 33, edge.y + p.normal.y * 33, z}, {center.x, center.y, z}, true, t))
			return probe.result;
		const auto n = value(t.vecPlaneNormal);
		...
		{
			probe.fail(LadderProbeReason::NoFace);
			return probe.result;
		}
	...
	if (!probe.endpoint(bottom, c.minimum.z - 64, c.minimum.z + 18, index, p.bottom) ||
		!probe.endpoint(top, c.maximum.z - 18, c.maximum.z + 18, index, p.top))
		return probe.result;
	if (p.top.origin.z <= p.bottom.origin.z || p.top.area == p.bottom.area)
	{
		probe.fail(LadderProbeReason::NoArea);
		return probe.result;
	}
	...
	auto from = p.bottom.origin, to = p.top.origin;
	from.z += 0.05f;
	to.z += 0.05f;
	if (!probe.contact(p.mount) || !probe.contact({p.dismount.x, p.dismount.y, p.dismount.z - 18}))
		return probe.result;
	if (!probe.clear(from, p.mount) || !probe.clear(p.mount, p.dismount) || !probe.clear(p.dismount, to))
		return probe.result;
	p.candidate = c;
	p.modelIndex = probe.modelIndex;
	p.modelName = probe.modelName;
	probe.result.reason = LadderProbeReason::None;
	probe.result.passage = p;
	return probe.result;
}
bool ladderPassageCurrent(LadderWorld world, const LadderPassage& p, int maximum) noexcept
{
	...
	if (!world.engine || !world.currentMap || !world.engine->pfnPEntityOfEntIndex || !world.engine->pfnIndexOfEdict ||
		...
		slot >= static_cast<std::uint32_t>(maximum) || p.entityId != p.candidate.entityId || p.modelIndex <= 0 ||
		p.modelIndex >= 512)
		return false;
	Probe probe{world, p.map, p.candidate, maximum, 0};
	probe.modelIndex = p.modelIndex;
	probe.modelName = p.modelName;
	return probe.fresh();
}
} // namespace astrabot::adapter::cstrike
