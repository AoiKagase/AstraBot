...
struct LadderFrameResult
{
	...
	{
		return reason == LadderFrameReason::None && value.has_value();
	}
};
...
LadderFrameResult inspectLadderFrame(LadderFrameWorld, edict_t*, nav::local::Binding,
									 const nav::runtime::MovementSnapshot&, const BoundLadderPlan&,
									 nav::model::NavVector3 target, const nav::query::NavSpatialIndex&,
									 core::MapGeneration indexMap, int maxEntities, std::uint32_t maxQueries = 4,
									 std::optional<core::BotCommand> command = {},
