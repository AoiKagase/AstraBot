...
JoinAction JoinState::begin(host::PlayerId player, host::MapGeneration map, JoinRequest request,
...
JoinAction JoinState::onMessage(const MessageEvent& event, host::TickId tick) noexcept
{
	...
	if (event.kind == MessageKind::TeamInfo)
	{
		...
		if (isExpectedTeam(event.text.data()))
		{
			...
			teamConfirmed_ = true;
			if (phase_ == JoinPhase::WaitingConfirmation && classSelectionCompleted_ && postClassFrameAdvanced_)
			{
	...
	if (phase_ == JoinPhase::WaitingTeamMenu && teamConfirmed_ && isClassMenu(event))
	{
	...
	if (phase_ == JoinPhase::WaitingTeamMenu && isTeamMenu(event))
	{
		...
		if (pendingSelection_ && pendingValue_ == request_.teamSelection() && pendingTick_.isValid() &&
			pendingTick_ == tick)
		{
			return JoinAction::noOp();
		}
	...
	if (phase_ == JoinPhase::WaitingClassMenu && isClassMenu(event))
	{
		...
		if (pendingSelection_ && pendingValue_ == request_.classNumber && pendingTick_.isValid() &&
			pendingTick_ == tick)
		{
			return JoinAction::noOp();
		}
	...
	if ((phase_ == JoinPhase::ClassCommandPending || phase_ == JoinPhase::WaitingConfirmation) && isClassMenu(event))
	{
		if (hasSelection(event, request_.classNumber))
		{
			repeatedPrompt_ = true;
...
{
	if (!active() || !classSelectionCompleted_ || phase_ != JoinPhase::WaitingConfirmation)
	{
		return JoinAction::noOp();
	}
...
JoinAction JoinState::commandCompleted(bool dispatched) noexcept
{
	...
	{
		pendingSelection_ = true;
		pendingValue_ = phase_ == JoinPhase::TeamCommandPending ? request_.teamSelection() : request_.classNumber;
		pendingTick_ = host::TickId::invalid();
		phase_ = phase_ == JoinPhase::TeamCommandPending ? JoinPhase::WaitingTeamMenu : JoinPhase::WaitingClassMenu;
		repeatedPrompt_ = false;
		promptGraceFrames_ = 0;
		return {JoinActionKind::None, JoinError::None, 0, true};
	}
...
bool JoinState::isTeamMenu(const MessageEvent& event) const noexcept
{
	...
	return event.kind == MessageKind::ShowMenu &&
		   (textEquals(event.text.data(), "#Team_Select") || textEquals(event.text.data(), "#Team_Select_Spect") ||
...
bool JoinState::isClassMenu(const MessageEvent& event) const noexcept
{
	if (event.kind == MessageKind::VguiMenu)
	{
		const std::uint8_t expected = request_.team == Team::Terrorist ? kVguiClassTMenu : kVguiClassCtMenu;
	...
	return request_.team == Team::Terrorist ? textEquals(event.text.data(), "#Terrorist_Select")
											: textEquals(event.text.data(), "#CT_Select");
}

bool JoinState::hasSelection(const MessageEvent& event, std::uint8_t value) const noexcept
{
...
{
	return request_.team == Team::Terrorist ? textEqualsIgnoreCase(value, "TERRORIST")
											: textEqualsIgnoreCase(value, "CT");
}

bool JoinState::isOppositeTeam(const char* value) const noexcept
{
	return request_.team == Team::Terrorist ? textEqualsIgnoreCase(value, "CT")
...
bool JoinState::textEqualsIgnoreCase(const char* left, const char* right) noexcept
{
	if (left == nullptr || right == nullptr)
	{
		return false;
