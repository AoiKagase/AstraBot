...
RuntimeActorStaleReason staleReason(const LifecycleCoordinator& owner, const RuntimeFrame& frame, core::PlayerId player,
									core::BotAgentId agent, const edict_t* entity) noexcept
{
	...
	if (!owner.registry().isMapActive())
		return RuntimeActorStaleReason::MapInactive;
	...
	if (owner.round() != frame.round)
		return RuntimeActorStaleReason::RoundMismatch;
	if (owner.registry().currentPlayer(player.slot) != player)
		return RuntimeActorStaleReason::PlayerGenerationMismatch;
	if (!binding.isValid())
		return RuntimeActorStaleReason::BindingInvalid;
	if (binding.agent != agent)
		return RuntimeActorStaleReason::BindingAgentMismatch;
	if (binding.map != frame.map)
		return RuntimeActorStaleReason::BindingMapMismatch;
	if (!entity || owner.entityFor(player) != entity)
		return RuntimeActorStaleReason::MissingEntity;
	if (entity->free)
		return RuntimeActorStaleReason::EntityFree;
	if (owner.removalPending(player))
		return RuntimeActorStaleReason::RemovalPending;
	if (!join || join->phase() != cstrike::JoinPhase::Joined)
		return RuntimeActorStaleReason::NotJoined;
	if (entity->v.deadflag != DEAD_NO)
		return RuntimeActorStaleReason::Dead;
	if (!std::isfinite(entity->v.health) || entity->v.health <= 0)
		return RuntimeActorStaleReason::InvalidHealth;
	if (entity->v.iuser1 != 0)
		return RuntimeActorStaleReason::SpectatorState;
	if (entity->v.flags & FL_SPECTATOR)
		return RuntimeActorStaleReason::SpectatorFlag;
	return RuntimeActorStaleReason::None;
}

bool current(const LifecycleCoordinator& owner, const RuntimeFrame& frame, core::PlayerId player,
			 core::BotAgentId agent, const edict_t* entity) noexcept
{
	return staleReason(owner, frame, player, agent, entity) == RuntimeActorStaleReason::None;
}

bool readWeapon(const LifecycleCoordinator& owner, const RuntimeFrame& frame, core::PlayerId player,
				core::BotAgentId agent, DLL_FUNCTIONS* dll, edict_t* entity, cstrike::WeaponObservation& result,
				RuntimeInputBuildReason* failure = nullptr, RuntimeInputBuildStatus* status = nullptr) noexcept
{
	...
	if (status)
		status->updateClientDataCalled = true;
	...
	if (status)
		status->weaponDataCalled = true;
	if (!dll->pfnGetWeaponData(entity, weapons.data()) || !current(owner, frame, player, agent, entity))
		return fail(RuntimeInputBuildReason::InvalidWeaponObservation);
	...
	if (active.m_iId != client.m_iId || !std::isfinite(client.vuser4[1]) || client.vuser4[1] < 0 ||
		client.vuser4[1] > core::combat::kMaxAmmo || std::floor(client.vuser4[1]) != client.vuser4[1] ||
		!std::isfinite(client.m_flNextAttack) || !std::isfinite(active.m_flNextPrimaryAttack))
		return fail(RuntimeInputBuildReason::InvalidWeaponObservation);
	result.map = frame.map;
	result.round = frame.round;
	...
	if (active.m_iClip < cstrike::protocol::kNoClip ||
		(active.m_iClip < 0 && result.activeClass != WeaponClass::Unknown && result.activeClass != WeaponClass::Melee))
		return fail(RuntimeInputBuildReason::InvalidWeaponObservation);
	...
	result.canReload = result.activeClass != WeaponClass::Unknown && result.activeClass != WeaponClass::Melee &&
					   result.reserveAmmo > 0;
	...
	for (std::size_t i = 1; i < weapons.size(); ++i)
	{
		...
		hasSwitchableAlternative = hasSwitchableAlternative || (i != static_cast<std::size_t>(client.m_iId) &&
																isSwitchableWeapon(static_cast<int>(i)));
	}
	...
	const double delay =
		(std::max)(0.0, static_cast<double>((std::max)(client.m_flNextAttack, active.m_flNextPrimaryAttack)));
	...
	if (!(client.iuser3 & cstrike::protocol::kCanShoot) || (client.iuser3 & cstrike::protocol::kFreezePeriod) ||
		result.activeClass == WeaponClass::Unknown)
...
bool runtimeActorReady(const LifecycleCoordinator& owner, const RuntimeFrame& frame, DLL_FUNCTIONS* dll,
					   core::PlayerId player, core::combat::WeaponId expected, bool attack) noexcept
{
	...
	return current(owner, frame, player, agent, entity) && nav && nav->currentArea &&
		   readWeapon(owner, frame, player, agent, dll, entity, weapon) && weapon.activeWeapon == expected.value &&
		   (!attack ||
			(!weapon.reloading && weapon.clipAmmo > 0 && weapon.primaryAttackReadyMicros <= frame.nowMicros));
}

std::size_t buildRuntimeInputs(const LifecycleCoordinator& owner, const RuntimeFrame& frame, DLL_FUNCTIONS* dll,
							   RuntimeActorInput* output, std::size_t capacity, RuntimeInputBuildStatus* status,
							   std::size_t statusCapacity, RuntimeInputSources sources) noexcept
{
	const auto statusFor = [&](core::PlayerId player) noexcept -> RuntimeInputBuildStatus* {
		if (!status || !player.isValid() || player.slot == 0 || player.slot > statusCapacity)
			return nullptr;
		return &status[player.slot - 1U];
	};
	const auto initializeStatus = [&](RuntimeInputBuildStatus* target, core::PlayerId player,
	...
	std::size_t count = 0;
	const auto clientMax = (std::min)(owner.registry().clientMax(), static_cast<std::uint16_t>(host::kMaxClientSlots));
	for (std::uint16_t slot = 1; slot <= clientMax && count < capacity; ++slot)
	{
		const auto player = owner.registry().currentPlayer(slot);
		const auto binding = owner.agents().findByPlayer(player);
		if (!player.isValid() || !binding.isValid() || binding.player != player || binding.map != frame.map)
			continue;
		...
		if (!current(owner, frame, player, input.agent, entity))
		{
			if (actorStatus)
			{
				...
				actorStatus->staleReason = staleReason(owner, frame, player, input.agent, entity);
			}
			continue;
		}
		...
		{
			if (actorStatus)
				actorStatus->reason = RuntimeInputBuildReason::MissingWorld;
			continue;
		}
		if (!nav)
		{
			if (actorStatus)
				actorStatus->reason = RuntimeInputBuildReason::MissingNav;
			continue;
		}
		if (actorStatus)
			actorStatus->currentAreaHeld = nav->currentAreaHeld;
		if (!nav->currentArea)
		{
			if (actorStatus)
				actorStatus->reason = RuntimeInputBuildReason::MissingCurrentArea;
			continue;
		}
		if (!nav->movement.position)
		{
			if (actorStatus)
				actorStatus->reason = RuntimeInputBuildReason::MissingPosition;
			continue;
		}

		cstrike::WeaponObservation weapon{};
		RuntimeInputBuildReason weaponFailure = RuntimeInputBuildReason::WeaponUnavailable;
		if (!readWeapon(owner, frame, player, input.agent, dll, entity, weapon, &weaponFailure, actorStatus))
		{
			if (actorStatus)
				actorStatus->reason = weaponFailure;
			continue;
		}
		if (actorStatus)
		{
		...
		{
			if (actorStatus)
				actorStatus->reason = RuntimeInputBuildReason::UnknownTeam;
			continue;
		}
		...
		{
			if (actorStatus)
				actorStatus->reason = RuntimeInputBuildReason::CombatConversionFailed;
			continue;
		}
		...
		{
			(void)sources.objective(sources.context, owner, frame, player, entity, objective, economy);
		}
		if (sources.experience != nullptr)
		{
			input.experienceEventCount =
				(std::min)(sources.experience(sources.context, owner, frame, player, entity,
		...
		input.tactical.navigation.roamCandidateCount =
			(std::min)(nav->roamCandidateCount, input.tactical.navigation.roamCandidates.size());
		...
		if (nav->routeExecutable && nav->goal && nav->goalPosition && nav->movement.speedLimit &&
			*nav->movement.speedLimit > 0)
		{
			auto& route = input.tactical.navigation.routes[0];
			route.target = {*nav->goal, *nav->goalPosition, nav->goal->value};
			route.style = nav->explicitRoute ? core::tactical::RouteStyle::Hold : core::tactical::RouteStyle::Roam;
			const auto& goal = *nav->goalPosition;
			const double eta = std::hypot(std::hypot(goal.x - position.x, goal.y - position.y), goal.z - position.z) /
