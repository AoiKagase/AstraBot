...
public:
	...
	using WeaponSelectionHandler = bool (*)(edict_t*, core::WeaponSelection) noexcept;

	void configure(enginefuncs_t* engineFunctions, host::PlayerRegistry* registry,
	...
	std::uint64_t frameDeltaUs() const noexcept
	{
		return frameDeltaUs_;
	}

	MovementResult submit(core::PlayerId player, core::MapGeneration mapGeneration, core::TickId tick,
						  const core::BotCommand& command) noexcept;
	MovementResult rejectIngress(MovementError error, core::PlayerId player, core::MapGeneration mapGeneration,
								 core::TickId tick, std::uint8_t originalMsec) noexcept;

	void beginFrame(std::optional<std::uint64_t> engineFrameDeltaUs = std::nullopt) noexcept;
	...
	MovementResult dispatchAtFrameEnd(cstrike::JoinPhase joinPhase, core::PlayerId activePlayer, edict_t* entity,
									  core::MapGeneration mapGeneration, core::TickId dispatchTick,
									  bool removalPending = false, bool suppressRuntimeInput = false) noexcept;
	...
	bool dispatchJoinProgress(core::PlayerId activePlayer, edict_t* entity, core::MapGeneration mapGeneration,
...
private:
	...
	MovementResult reject(MovementError error, core::PlayerId player, core::MapGeneration mapGeneration,
						  core::TickId commandTick, std::uint8_t originalMsec) noexcept;
	MovementResult dispatchOne(const PendingCommand& pending, cstrike::JoinPhase joinPhase, core::PlayerId activePlayer,
							   edict_t* entity, core::MapGeneration mapGeneration, core::TickId dispatchTick) noexcept;
	bool dispatchNeutral(core::PlayerId activePlayer, edict_t* entity, core::MapGeneration mapGeneration,
						 debug::MovementTraceSource source, bool requireDead) noexcept;

	void emit(MovementOutcome outcome, MovementError error, core::MapGeneration mapGeneration, core::PlayerId player,
			  core::TickId commandTick, core::TickId dispatchTick, std::uint8_t originalMsec, bool engineCall,
			  std::optional<std::uint64_t> dispatchDelta = std::nullopt,
			  debug::MovementTraceSource source = debug::MovementTraceSource::None, std::uint64_t callCount = 0,
			  std::uint32_t edictSerial = 0, float forward = 0.0F, float side = 0.0F, float up = 0.0F,
			  std::uint16_t buttons = 0, std::uint8_t impulse = 0,
			  const debug::MovementPhysicalSample* physical = nullptr) noexcept;
