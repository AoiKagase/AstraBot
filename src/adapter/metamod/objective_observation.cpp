...
bool validBomb(const ObjectiveAuthoritySnapshot& snapshot) noexcept {
    ...
    if (bomb.position.known && !core::perception::finite(bomb.position.value))
        return false;
    if (!bomb.state.known) return true;
    if (!validBombState(bomb.state.value)) return false;

    if (bomb.state.value == core::tactical::BombState::Carried) {
        if (!bomb.carrier.known || !bomb.carrier.value.isValid()) return false;
    } else if (bomb.carrier.known && bomb.carrier.value.isValid()) {
        return false;
    }

    if (bomb.actorHasC4.known && bomb.actorHasC4.value &&
        (bomb.state.value != core::tactical::BombState::Carried ||
         !bomb.carrier.known || bomb.carrier.value != snapshot.stamp.player)) {
        return false;
    }

    if (bomb.state.value == core::tactical::BombState::Planted) {
        return bomb.position.known && bomb.remainingMicros.known &&
            bomb.remainingMicros.value != 0 && bomb.site.known &&
            bomb.site.value != 0;
    }
    return true;
}
...
bool validVip(const ObjectiveAuthoritySnapshot& snapshot) noexcept {
    if (snapshot.vip.vip.known && !snapshot.vip.vip.value.isValid()) return false;
    if (!snapshot.family.known ||
        snapshot.family.value != ObjectiveFamily::VipEscort) return true;
    return snapshot.vip.vip.known && snapshot.vip.vip.value.isValid() &&
        snapshot.vip.alive.known && snapshot.vip.escaped.known;
}

ObjectiveObservationError validateHostages(
    const HostageAuthorityObservation& hostages) noexcept {
    if (hostages.count > hostages.values.size())
        return ObjectiveObservationError::InvalidHostage;
    if (!hostages.known) return hostages.count == 0
        ? ObjectiveObservationError::None
        : ObjectiveObservationError::InvalidHostage;
...
bool validRound(const RoundAuthorityObservation& round) noexcept {
    ...
    if (round.roundNumber.known && round.roundNumber.value == 0) return false;
    return true;
}

bool validEconomy(const EconomyAuthorityObservation& economy) noexcept {
    if (economy.money.known &&
        (economy.money.value < 0 || economy.money.value > core::economy::kMaxMoney))
        return false;
    return !economy.weaponValue.known || economy.weaponValue.value >= 0;
}

template <typename T>
void clearUnknown(ObservedValue<T>& value) noexcept {
...
void normalize(ObjectiveEconomyObservation& value) noexcept {
    ...
    if (!value.hostages.known) value.hostages = {};
...
ObjectiveObservationResult ObjectiveObservationAdapter::observe(
    const ObjectiveObservationStamp& expected,
    ...
    if ((authority.family.known && !validFamily(authority.family.value)) ||
        !validBomb(authority) || !validVip(authority)) {
        result.error = ObjectiveObservationError::InvalidObjective;
        return result;
    }
    if (const auto error = validateHostages(authority.hostages);
        error != ObjectiveObservationError::None) {
        result.error = error;
        return result;
    }
