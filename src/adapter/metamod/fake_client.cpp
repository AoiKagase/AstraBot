...
void FakeClientCoordinator::queuePrimaryCreate(cstrike::JoinRequest request) noexcept
{
	primaryJoinRequest_ = request;
	primaryQueued_ = true;
}

void FakeClientCoordinator::configure(enginefuncs_t* engineFunctions, mutil_funcs_t* utilityFunctions,
									  DLL_FUNCTIONS* hookedGameDllFunctions, host::PlayerRegistry* players,
...
FakeClientResult FakeClientCoordinator::create(const char* name, cstrike::JoinRequest request) noexcept
{
	...
	if (entity == nullptr)
	{
		return rejected(debug::FakeClientError::CreateFailed, debug::FakeClientStage::Rejected);
	}
	trace(debug::FakeClientStage::Allocated, debug::FakeClientError::None);

	const int index = engineFunctions_->pfnIndexOfEdict(entity);
	if (index < 1 || index > static_cast<int>(players_->clientMax()) || index > static_cast<int>(host::kMaxClientSlots))
	{
		cleanup(entity, false);
		return rejected(debug::FakeClientError::InvalidSlot, debug::FakeClientStage::RolledBack);
	}
	...
	if (players_->isConnected(slot))
	{
		...
		return rejected(debug::FakeClientError::SlotOccupied, debug::FakeClientStage::RolledBack);
	}

	if (!utilityFunctions_->pfnCallGameEntity(PLID, "player", &entity->v))
	{
		cleanup(entity, false);
		return rejected(debug::FakeClientError::PlayerFactoryFailed, debug::FakeClientStage::RolledBack);
	}
	...
	{
		cleanup(entity, false);
		return rejected(debug::FakeClientError::InfoBufferFailed, debug::FakeClientStage::RolledBack);
	}
	char modelKey[] = "model";
	char modelValue[] = "";
	char rateKey[] = "rate";
	...
	char botKey[] = "*bot";
	char botValue[] = "1";
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, modelKey, modelValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, rateKey, rateValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, updateRateKey, updateRateValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, trackerKey, trackerValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, downloadMaxKey, downloadMaxValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, leftHandKey, leftHandValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, friendsKey, friendsValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, dmKey, dmValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, autoHelpKey, autoHelpValue);
	engineFunctions_->pfnSetClientKeyValue(index, infoBuffer, vguiKey, vguiValue);
	...
	if (!hookedGameDllFunctions_->pfnClientConnect(entity, nameBuffer, kLoopbackAddress, rejectReason))
	{
		cleanup(entity, false);
		return rejected(debug::FakeClientError::ConnectRejected, debug::FakeClientStage::RolledBack);
	}
	...
	{
		cleanup(entity, false);
		return rejected(debug::FakeClientError::PlayerRegistrationFailed, debug::FakeClientStage::RolledBack);
	}
	...
	const host::BotAgentResult binding = agents_->bind(registration.event.player, registration.event.map);
	if (!binding)
	{
		cleanup(entity, true);
		const host::LifecycleResult rollback = players_->disconnectPlayer(registration.event.player);
		FakeClientResult result = rejected(debug::FakeClientError::AgentBindingFailed,
										   debug::FakeClientStage::RolledBack, registration.event.player);
	...
	result.changed = true;
	trace(debug::FakeClientStage::Published, debug::FakeClientError::None, result.player, result.agent, true, true);
	return result;
}

void FakeClientCoordinator::trace(debug::FakeClientStage stage, debug::FakeClientError error, host::PlayerId player,
								  core::BotAgentId agent, bool accepted, bool changed) noexcept
{
	const host::MapGeneration map = players_ == nullptr ? host::MapGeneration::invalid() : players_->mapGeneration();
	const host::EventSequence sequence = players_ != nullptr && player.isValid() ? players_->eventSequence() : 0;
...
FakeClientResult FakeClientCoordinator::rejected(debug::FakeClientError error, debug::FakeClientStage stage,
...
{
	return engineFunctions_ != nullptr && utilityFunctions_ != nullptr && hookedGameDllFunctions_ != nullptr &&
		   players_ != nullptr && agents_ != nullptr && engineFunctions_->pfnCreateFakeClient != nullptr &&
		   engineFunctions_->pfnIndexOfEdict != nullptr && engineFunctions_->pfnGetInfoKeyBuffer != nullptr &&
		   engineFunctions_->pfnSetClientKeyValue != nullptr && engineFunctions_->pfnRemoveEntity != nullptr &&
		   utilityFunctions_->pfnCallGameEntity != nullptr && hookedGameDllFunctions_->pfnClientConnect != nullptr &&
...
{
	auto* entity = activeEntity_;
	auto* engine = engineFunctions_;
	const auto player = activePlayer_;
	const auto map = activeMap_;
	const auto serial = activeSerial_;
	if (!entity || entity->free || entity->serialnumber != serial || !engine || !engine->pfnIndexOfEdict ||
		engine->pfnIndexOfEdict(entity) != player.slot ||
		(engine->pfnPEntityOfEntIndex && engine->pfnPEntityOfEntIndex(player.slot) != entity))
		return false;
...
edict_t* FakeClientCoordinator::entityFor(host::PlayerId player) const noexcept
{
	if (!player.isValid() || player != activePlayer_ || !players_ || !players_->isMapActive() ||
		players_->mapGeneration() != activeMap_ || players_->currentPlayer(player.slot) != player || !sameEntity())
		return nullptr;
	return players_ && players_->isMapActive() && players_->mapGeneration() == activeMap_ &&
				   players_->currentPlayer(player.slot) == player
			   ? activeEntity_
			   : nullptr;
}
...
RemovalResult FakeClientCoordinator::requestRemoval() noexcept
{
	...
	if (removalPending_)
	{
		return {debug::RemovalOutcome::NoOp, debug::RemovalError::None, activePlayer_, true, false};
	}
	if (!sameEntity())
		return {debug::RemovalOutcome::Rejected, debug::RemovalError::DirectCleanupFailed, activePlayer_, false, false};
	if (!configured())
	{
		emitRemoval(debug::RemovalOutcome::Rejected, debug::RemovalError::NotConfigured, activePlayer_, true, true);
		return {debug::RemovalOutcome::Rejected, debug::RemovalError::NotConfigured, activePlayer_, false, false};
	}
	...
	{
		if (activePlayer_ == requestedPlayer)
			removalPending_ = false;
		emitRemoval(debug::RemovalOutcome::Rejected, error, activePlayer_, true, true);
		return {debug::RemovalOutcome::Rejected, error, activePlayer_, false, false};
	}

	emitRemoval(debug::RemovalOutcome::KickQueued, debug::RemovalError::None, requestedPlayer, true, true);
	return {debug::RemovalOutcome::KickQueued, debug::RemovalError::None, requestedPlayer, true, true};
}
...
bool FakeClientCoordinator::cleanupActiveDirect(bool connected) noexcept
{
	...
	if (engineFunctions_ == nullptr || engineFunctions_->pfnRemoveEntity == nullptr ||
		(connected && (hookedGameDllFunctions_ == nullptr || hookedGameDllFunctions_->pfnClientDisconnect == nullptr)))
	{
		return false;
	}
...
void FakeClientCoordinator::acknowledgeDisconnect(host::PlayerId player) noexcept
{
	if (player.isValid() && activePlayer_ == player)
	{
		activeEntity_ = nullptr;
...
bool FakeClientCoordinator::kickAndCleanup(host::PlayerId player) noexcept
{
	...
	{
		emitRemoval(debug::RemovalOutcome::Rejected, debug::RemovalError::NoActiveClient, player, false, false);
		return false;
	}
	...
	if (kicked)
	{
		emitRemoval(debug::RemovalOutcome::KickQueued, debug::RemovalError::None, player, true, true);
	}
	else
	{
		const bool canCleanup = engineFunctions_ != nullptr && engineFunctions_->pfnRemoveEntity != nullptr &&
								hookedGameDllFunctions_ != nullptr &&
								hookedGameDllFunctions_->pfnClientDisconnect != nullptr;
		cleanup(entity, true);
		emitRemoval(canCleanup ? debug::RemovalOutcome::Cleaned : debug::RemovalOutcome::Rejected,
					canCleanup ? error : debug::RemovalError::DirectCleanupFailed, player, true, true);
	}
	forget(player);
	return kicked;
}

bool FakeClientCoordinator::copyName(const char* source, char* destination, std::uint16_t capacity) noexcept
{
	...
	if (length == 0U || ((length + 1U >= capacity) && source[length] != '\0'))
	{
		return false;
	}
...
void FakeClientCoordinator::cleanup(edict_t* entity, bool connected) noexcept
{
	...
	if (connected && hookedGameDllFunctions_ != nullptr && hookedGameDllFunctions_->pfnClientDisconnect != nullptr)
	{
		hookedGameDllFunctions_->pfnClientDisconnect(entity);
	}
	if (!entity->free && entity->serialnumber == serial && engineFunctions_ != nullptr &&
...
bool FakeClientCoordinator::issueKick(edict_t* entity, debug::RemovalError& error) noexcept
{
	error = debug::RemovalError::None;
	if (entity == nullptr || engineFunctions_ == nullptr || engineFunctions_->pfnGetPlayerUserId == nullptr ||
		engineFunctions_->pfnServerCommand == nullptr || engineFunctions_->pfnServerExecute == nullptr)
	{
...
void FakeClientCoordinator::emitRemoval(debug::RemovalOutcome outcome, debug::RemovalError error, host::PlayerId player,
										bool mappingPresent, bool entityPresent) noexcept
{
	const host::MapGeneration map = players_ == nullptr ? host::MapGeneration::invalid() : players_->mapGeneration();
	const host::TickId tick = players_ == nullptr ? host::TickId::invalid() : players_->currentTick();
	const host::EventSequence sequence = players_ == nullptr ? 0 : players_->eventSequence();
	debug::emitRemoval(
		{
			outcome,
			error,
