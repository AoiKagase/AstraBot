...
namespace astrabot
{
	namespace metamod
	{
		namespace
		{
			const char *const kNativeServerCommands[] = {"bot_about",
														 "bot_add",
														 "bot_add_t",
														 "bot_add_ct",
														 "bot_kill",
		...
		NativeBotGuardDecision
		NativeBotGuard::evaluate(const NativeBotObservation &observation) const
		{
			if (!observation.controlsAvailable || !observation.controlsWritable)
			{
				if (!observation.clientObservationAvailable)
				{
					return {NativeBotGuardState::Unsupported,
							NativeBotGuardReason::ControlsUnavailable, false};
				}
				for (std::size_t index = 0U; index < NativeBotObservation::kClientSlotCount;
					 ++index)
				{
					if (observation.fakeClientSlots[index] &&
						!observation.managedClientSlots[index])
					{
						return {NativeBotGuardState::Conflict,
								NativeBotGuardReason::UnmanagedFakeClient, false};
					}
				}
				return {NativeBotGuardState::Clean, NativeBotGuardReason::None, true};
			}

			if (!std::isfinite(observation.botEnable) || !std::isfinite(observation.botQuota))
			{
				return {NativeBotGuardState::Unsupported, NativeBotGuardReason::InvalidObservation,
						false};
			}

			if (observation.botEnable != 0.0f || observation.botQuota != 0.0f)
			{
				return {NativeBotGuardState::Conflict, NativeBotGuardReason::NativeControlsActive,
						false};
			}
			...
			return {observation.suppressionApplied ? NativeBotGuardState::Suppressed
												   : NativeBotGuardState::Clean,
					NativeBotGuardReason::None, true};
		}
