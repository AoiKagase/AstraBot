...
{
	return gameDllFunctions != nullptr && gameDllFunctions->dllapi_table != nullptr &&
		   gameDllFunctions->newapi_table != nullptr;
}

bool hasRequiredUtilityTable(const mutil_funcs_t* utilityFunctions) noexcept
{
	return utilityFunctions != nullptr && utilityFunctions->pfnLogConsole != nullptr &&
		   utilityFunctions->pfnGetHookTables != nullptr && utilityFunctions->pfnGetUserMsgID != nullptr;
}

bool hasRequiredFakeClientEngine(const enginefuncs_t* engineFunctions) noexcept
{
	return engineFunctions != nullptr && engineFunctions->pfnIndexOfEdict != nullptr &&
		   engineFunctions->pfnRunPlayerMove != nullptr && engineFunctions->pfnCreateFakeClient != nullptr &&
		   engineFunctions->pfnGetInfoKeyBuffer != nullptr && engineFunctions->pfnSetClientKeyValue != nullptr &&
		   engineFunctions->pfnRemoveEntity != nullptr && engineFunctions->pfnGetPlayerUserId != nullptr &&
		   engineFunctions->pfnServerCommand != nullptr && engineFunctions->pfnServerExecute != nullptr;
}

bool hasRequiredFakeClientHookTable(const DLL_FUNCTIONS* hookedGameDllFunctions) noexcept
{
	return hookedGameDllFunctions != nullptr && hookedGameDllFunctions->pfnClientConnect != nullptr &&
...
bool hasRequiredFakeClientUtility(const mutil_funcs_t* utilityFunctions) noexcept
{
	return utilityFunctions != nullptr && utilityFunctions->pfnCallGameEntity != nullptr;
}
...
plugin_info_t Plugin_info = {
	kPluginInterfaceVersion, kPluginName, kPluginVersion, kPluginDate, kPluginAuthor, kPluginUrl,
	kPluginLogTag,			 PT_ANYTIME,  PT_ANYPAUSE,
};
...
C_DLLEXPORT FORCE_STACK_ALIGN int Meta_Query(char* interfaceVersion, plugin_info_t** pluginInfo,
											 mutil_funcs_t* metaUtilFunctions)
{
	if (interfaceVersion == nullptr || pluginInfo == nullptr || metaUtilFunctions == nullptr)
	{
		return 0;
	}
...
C_DLLEXPORT FORCE_STACK_ALIGN int Meta_Attach(PLUG_LOADTIME /* now */, META_FUNCTIONS* functionTable,
											  meta_globals_t* metaGlobals, gamedll_funcs_t* gameDllFunctions)
{
	...
	if (!gEngineGlobals || !gPluginEngine.pfnAddServerCommand || !gPluginEngine.pfnCmd_Argc ||
		!gPluginEngine.pfnCmd_Argv)
	{
	...
	gpMetaUtilFuncs->pfnGetHookTables(PLID, &engineFunctions, &hookDllFunctions, &hookNewDllFunctions);
	if (engineFunctions == nullptr || engineFunctions->pfnIndexOfEdict == nullptr)
	{
	...
	const GET_ENGINE_FUNCTIONS_FN previousEngineFunctions = functionTable->pfnGetEngineFunctions;
	...
	astrabot::adapter::metamod::lifecycleCoordinator().configure(engineFunctions, gpMetaUtilFuncs, hookDllFunctions, {},
																 gEngineGlobals);
	...
	astrabot::adapter::metamod::lifecycleCoordinator().navConsole().configure(&gPluginEngine, gpMetaUtilFuncs,
																			  gEngineGlobals);
	astrabot::adapter::metamod::ConsoleDebug::instance().configure(&gPluginEngine, gpMetaUtilFuncs,
...
C_DLLEXPORT FORCE_STACK_ALIGN int Meta_Detach(PLUG_LOADTIME /* now */, PL_UNLOAD_REASON /* reason */)
{
	...
	if (gState.attached && gState.functionTable != nullptr)
	{
		...
		gState.functionTable->pfnGetEngineFunctions = gState.previousEngineFunctions;
...
C_DLLEXPORT FORCE_STACK_ALIGN int GetEntityAPI2(DLL_FUNCTIONS* functionTable, int* interfaceVersion)
{
	...
	functionTable->pfnServerActivate = &astrabot::adapter::metamod::serverActivateHook;
	functionTable->pfnServerDeactivate = &astrabot::adapter::metamod::serverDeactivateHook;
	functionTable->pfnClientDisconnect = &astrabot::adapter::metamod::clientDisconnectHook;
	functionTable->pfnStartFrame = &astrabot::adapter::metamod::startFrameHook;
	return 1;
}

C_DLLEXPORT FORCE_STACK_ALIGN int GetEngineFunctions(enginefuncs_t* engineFunctions, int* interfaceVersion)
{
	...
	engineFunctions->pfnMessageBegin = &astrabot::adapter::metamod::messageBeginHook;
	engineFunctions->pfnMessageEnd = &astrabot::adapter::metamod::messageEndHook;
	engineFunctions->pfnWriteByte = &astrabot::adapter::metamod::writeByteHook;
	engineFunctions->pfnWriteChar = &astrabot::adapter::metamod::writeCharHook;
	engineFunctions->pfnWriteShort = &astrabot::adapter::metamod::writeShortHook;
	engineFunctions->pfnWriteString = &astrabot::adapter::metamod::writeStringHook;
	engineFunctions->pfnCmd_Args = &astrabot::adapter::metamod::commandArgsHook;
	engineFunctions->pfnCmd_Argv = &astrabot::adapter::metamod::commandArgvHook;
	engineFunctions->pfnCmd_Argc = &astrabot::adapter::metamod::commandArgcHook;
	return 1;
}
