...
struct ObjectiveObservationStamp final {
    ...
    constexpr bool valid() const noexcept {
        return map.isValid() && round.isValid() && tick.isValid() &&
            nowMicros != 0 && player.isValid() && agent.isValid();
    }
    friend constexpr bool operator==(const ObjectiveObservationStamp& left,
        ...
        return left.map == right.map && left.round == right.round &&
            left.tick == right.tick && left.nowMicros == right.nowMicros &&
...
struct HostageAuthorityObservation final {
    std::array<core::team::ObjectiveTarget,
               core::team::kMaxObjectiveTargets> values{};
    std::size_t count{0};
    bool known{false};
};
...
public:
    ...
    virtual bool read(const ObjectiveObservationStamp& expected,
                      ObjectiveAuthoritySnapshot& output) const noexcept = 0;
};
...
public:
    static ObjectiveObservationResult observe(
