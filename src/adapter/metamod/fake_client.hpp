...
struct FakeClientResult
{
	...
	static constexpr FakeClientResult noOp() noexcept
	{
		return {{}, {}, {}, {}, {}, true, false};
	}
...
public:
	...
	FakeClientResult create(const char* name, cstrike::JoinRequest request) noexcept;

	edict_t* activeEntity() const noexcept
	{
		return entityFor(activePlayer_);
	}
	edict_t* entityFor(host::PlayerId) const noexcept;
	host::PlayerId activePlayer() const noexcept
	{
	...
	bool removalPending() const noexcept
	{
		return removalPending_;
	}
	bool operationActive() const noexcept
	{
		return operationActive_;
	}
...
private:
	...
	cstrike::JoinRequest primaryJoinRequest_{cstrike::Team::Terrorist, 1};
	cstrike::JoinRequest activeJoinRequest_{cstrike::Team::Terrorist, 1};
	...
	void trace(debug::FakeClientStage stage, debug::FakeClientError error,
			   host::PlayerId player = host::PlayerId::invalid(), core::BotAgentId agent = core::BotAgentId::invalid(),
			   bool accepted = false, bool changed = false) noexcept;
	FakeClientResult rejected(debug::FakeClientError error,
