...
{
	if (text.empty() || text.size() > 10 || text.front() < '0' || text.front() > '9')
		return {};
	std::uint32_t value{};
	const auto r = std::from_chars(text.data(), text.data() + text.size(), value);
	if (r.ec != std::errc{} || r.ptr != text.data() + text.size() || !value)
		return {};
...
#define RNAME(n)                                                                                                       \
	case R::n:                                                                                                         \
		...
		RNAME(ActorChanged)
		RNAME(MapChanged)
		RNAME(Disconnected) RNAME(Dead) RNAME(NotJoined) RNAME(InvalidSnapshot) RNAME(StaleSnapshot) RNAME(MissingGraph)
			RNAME(InvalidGoal) RNAME(QueryBudgetExceeded) RNAME(StaleQuery) RNAME(QueryFailed) RNAME(UnknownGround)
				RNAME(NoCurrentArea) RNAME(Unreachable) RNAME(ExpansionLimit) RNAME(NavFailure) RNAME(AllocationFailure)
...
void printNavTrace(const nav::runtime::DecisionTrace& t, NavLineSink sink, void* ctx) noexcept
{
	...
	std::snprintf(
		line, sizeof(line),
		"nav actor=%u:%u agent=%u map=%u tick=%llu route=%llu current=%u current_source=%s current_age_us=%llu goal=%u "
		"state=%s reason=%s terminal=%u arrival=unverified nav_error=%u field=%u offset=%llu",
		...
		static_cast<unsigned long long>(t.currentAreaAgeUs), unsigned(t.goal.value), stateName(t.state),
		reasonName(t.reason), unsigned(t.terminal), unsigned(t.navError.kind), unsigned(t.navError.field),
		static_cast<unsigned long long>(t.navError.offset));
	sink(ctx, line);
	if (!t.route)
		return;
	...
	const char* status = route.status == nav::query::NavRouteStatus::Complete	   ? "Complete"
						 : route.status == nav::query::NavRouteStatus::Unreachable ? "Unreachable"
																				   : "ExpansionLimit";
	const auto count = route.steps.size() < 64 ? route.steps.size() : 64;
	std::snprintf(line, sizeof(line), "nav cost=%s edges=%zu shown=%zu omitted=%zu expansions=%zu status=%s", cost,
				  route.steps.size(), count, route.steps.size() - count, route.metrics.expansions, status);
	sink(ctx, line);
	for (std::size_t i = 0; i < count; ++i)
	{
		const auto& e = route.steps[i].edge;
		std::snprintf(line, sizeof(line),
					  "nav edge=%zu from=%u to=%u direction=%u traversal=%u source=%llu generation=%llu link=%llu", i,
					  unsigned(e.source.value), unsigned(e.target.value), unsigned(e.direction), unsigned(e.traversal),
