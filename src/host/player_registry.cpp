...
LifecycleResult PlayerRegistry::disconnectPlayer(PlayerId player) noexcept
{
	...
	state.lastCommandTick = TickId::invalid();
	const LifecycleResult result = event(LifecycleEventKind::PlayerDisconnected, state.player);
...
CommandResult PlayerRegistry::submitCommand(PlayerId player, TickId tick, const BotCommand& command) noexcept
{
	if (!mapActive_)
	{
...
{
	return slot >= kFirstClientSlot && slot <= clientMax_ && slot <= kMaxClientSlots;
}
...
LifecycleResult PlayerRegistry::event(LifecycleEventKind kind, PlayerId player) noexcept
{
	++eventSequence_;
