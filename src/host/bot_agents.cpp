...
BotAgentResult BotAgentRegistry::bind(PlayerId player, MapGeneration map) noexcept
{
	if (!player.isValid())
	{
	...
	if (mappingCount_ >= kMaxBotAgents || nextAgentValue_ == std::numeric_limits<std::uint32_t>::max())
	{
...
BotAgentBinding BotAgentRegistry::findByPlayer(PlayerId player) const noexcept
{
	...
	const std::uint16_t index = entryIndex(player);
	if (index >= entries_.size() || !entries_[index].occupied || entries_[index].binding.player != player)
	{
		return {};
	}
...
{
	return player.slot == 0 ? kMaxBotAgents : static_cast<std::uint16_t>(player.slot - 1U);
}
