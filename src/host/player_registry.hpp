...
{
public:
	LifecycleResult activateMap(std::uint16_t clientMax = kMaxClientSlots) noexcept;
	...
	CommandResult submitCommand(PlayerId player, TickId tick, const BotCommand& command) noexcept;
	...
	bool isFrameStarted() const noexcept
	{
		return frameStarted_;
	}
	std::uint16_t clientMax() const noexcept
	{
		return clientMax_;
	}
	MapGeneration mapGeneration() const noexcept
	{
	...
	EventSequence eventSequence() const noexcept
	{
...
private:
	...
	LifecycleResult event(LifecycleEventKind kind, PlayerId player = PlayerId::invalid()) noexcept;
