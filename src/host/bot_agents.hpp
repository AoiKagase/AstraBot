...
struct BotAgentBinding
{
	...
	friend constexpr bool operator==(BotAgentBinding left, BotAgentBinding right) noexcept
	{
		return left.agent == right.agent && left.player == right.player && left.map == right.map;
	}
	friend constexpr bool operator!=(BotAgentBinding left, BotAgentBinding right) noexcept
	{
		return !(left == right);
...
struct BotAgentResult
{
	...
	static constexpr BotAgentResult acceptedBinding(BotAgentBinding value) noexcept
	{
		return {value, BotAgentError::None, true, true};
	}
	static constexpr BotAgentResult acceptedNoOp(BotAgentBinding value = {}) noexcept
	{
...
{
public:
	BotAgentResult bind(PlayerId player, MapGeneration map) noexcept;
	...
	std::uint16_t mappingCount() const noexcept
	{
		return mappingCount_;
	}
	std::uint32_t nextAgentValue() const noexcept
	{
		return nextAgentValue_;
	}
