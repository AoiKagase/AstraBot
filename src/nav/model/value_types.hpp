...
struct NavAreaId final
{
	...
	static constexpr NavAreaId invalid() noexcept
	{
		return {};
	}
...
struct NavExtent final
{
	...
	{
		return left.northWest == right.northWest && left.southEast == right.southEast &&
			   left.northEastZ == right.northEastZ && left.southWestZ == right.southWestZ;
	}
	friend bool operator!=(const NavExtent& left, const NavExtent& right) noexcept
	{
		return !(left == right);
