...
public:
	...
	struct PolicySnapshot
	{
		...
		static query::NavCostDecision cost(const query::NavCostContext& c, const void* context)
		{
			...
			if (self.blocked && sameEdge(*self.blocked, c.edge))
				return {true, {}};
			return self.base.cost
					   ? self.base.cost(c, self.base.context)
					   : query::NavCostDecision{
							 false,
							 {c.geometricDistance, c.edge.external ? c.edge.external->additionalCost : 0, 0, 0, 0}};
		}
	...
	bool schedule(local::Binding binding, query::NavDirectedEdge edge, core::TickId tick, std::uint64_t nowUs) noexcept
	{
	...
	bool isRecovery() const noexcept
	{
		return recovery_;
	}
...
private:
	...
	{
		if (state_ == ReplanState::Pending)
			return false;
		if (attempts_)
		{
			state_ = ReplanState::Exhausted;
			return false;
		}
		if (!binding.agent.isValid() || !binding.actor.isValid() || !binding.map.isValid() ||
			!binding.routeGeneration || !tick.isValid() ||
			(edge && (!edge->source.isValid() || !edge->target.isValid())))
		{
			state_ = ReplanState::Invalid;
			return false;
		}
		binding_ = binding;
		edge_ = edge;
...
public:
	std::optional<PolicySnapshot> consume(local::Binding binding, core::TickId tick, std::uint64_t nowUs) noexcept
	{
		...
		if (!tick.isAfter(tick_))
			return {};
		if (nowUs - observedUs_ >= factLifetimeUs)
		{
			state_ = ReplanState::Expired;
			edge_.reset();
			return {};
		}
		++attempts_;
		state_ = ReplanState::Consumed;
		return PolicySnapshot{edge_};
	}
	PolicySnapshot snapshot(core::MapGeneration map, std::uint64_t nowUs) const noexcept
	{
		return {map == binding_.map && nowUs >= observedUs_ && nowUs - observedUs_ < factLifetimeUs ? edge_
																									: std::nullopt};
	}
	ReplanState state() const noexcept
	{
		return state_;
	}
...
private:
	...
	{
		if (a.source != b.source || a.target != b.target || a.traversal != b.traversal ||
