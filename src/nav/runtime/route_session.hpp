...
enum class CurrentAreaObservationSource : std::uint8_t
{
	Exact,
	Nearest,
	LastKnown
...
public:
	...
	SessionUpdate request(const MovementSnapshot&, model::NavAreaId goal, const NavigationSnapshot&, IWorldQueries&,
						  const RouteOptions&) noexcept;
	SessionUpdate observe(const MovementSnapshot&) noexcept;
	SessionUpdate cancel() noexcept;
	bool executable() const noexcept;
	const DecisionTrace& trace() const noexcept
	{
		return trace_;
	}
	std::shared_ptr<const query::NavGraph> graph() const noexcept
	{
		return graph_;
	}
	...
	{
		return 0;
	} // Portal cursor advancement belongs to P3-02.
private:
