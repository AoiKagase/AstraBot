...
{
	if (a.source != b.source || a.target != b.target || a.direction != b.direction || a.traversal != b.traversal ||
		a.external.has_value() != b.external.has_value())
		return false;
	if (!a.external)
		return true;
	const auto& x = *a.external;
	const auto& y = *b.external;
	return x.sourceId == y.sourceId && x.generation == y.generation && x.linkId == y.linkId && x.from == y.from &&
		   x.to == y.to && same(x.entry, y.entry) && same(x.exit, y.exit) && x.traversal == y.traversal &&
		   x.direction == y.direction && x.additionalCost == y.additionalCost;
}
...
{
	return double(e.southEast.x) - e.northWest.x >= 2 * h.halfX && double(e.southEast.y) - e.northWest.y >= 2 * h.halfY;
}
...
PortalFailureReason portal(Transition& t, HullClearance hull, PortalPolicy policy) noexcept
{
	...
	const auto& a = t.sourceExtent;
	const auto& b = t.targetExtent;
	if (!a.isFinite() || !b.isFinite() || a.southEast.x <= a.northWest.x || a.southEast.y <= a.northWest.y ||
	...
	if ((!sourceFits || !targetFits) && policy == PortalPolicy::Strict)
		return !sourceFits ? PortalFailureReason::SourceHullFit : PortalFailureReason::TargetHullFit;
	if (t.edge.external)
	{
		const auto& e = *t.edge.external;
		const bool explicitSpecial =
			e.traversal == model::NavTraversalKind::Jump || e.traversal == model::NavTraversalKind::Drop ||
			(e.traversal == model::NavTraversalKind::Walk &&
			 (e.direction == enrichment::NavLinkDirection::Up || e.direction == enrichment::NavLinkDirection::Down));
		...
		t.sourceLow = t.sourceHigh = entry;
		t.targetLow = t.targetHigh = exit;
	...
	if (hints)
		t.effectiveTraversal = hints.kind;
	const auto d = t.edge.direction;
	double boundary = 0, opposite = 0;
	switch (d)
	{
	case 0:
		boundary = a.northWest.y;
		opposite = b.southEast.y;
		break;
	case 1:
		boundary = a.southEast.x;
		opposite = b.northWest.x;
		break;
	case 2:
		boundary = a.southEast.y;
		opposite = b.northWest.y;
		break;
	case 3:
		boundary = a.northWest.x;
		opposite = b.southEast.x;
		break;
	default:
		return PortalFailureReason::UnsupportedTraversal;
	}
	...
	double low = std::max(sourceLow + (sourceFits ? margin : 0.0), targetLow + (targetFits ? margin : 0.0));
	double high = std::min(sourceHigh - (sourceFits ? margin : 0.0), targetHigh - (targetFits ? margin : 0.0));
	if (!(low < high))
	{
	...
	t.sourceLow = support(a, x0, y0);
	t.sourceHigh = support(a, x1, y1);
	...
	if (gap != 0)
	{
		...
		if (!hints || hints.kind != model::NavTraversalKind::Walk || lowFall <= 18 || highFall <= 18 || lowFall > 128 ||
			highFall > 128)
			return PortalFailureReason::UnsupportedTraversal;
		t.effectiveTraversal = model::NavTraversalKind::Drop;
	}
...
Point project(const Transition& t, Point p) noexcept
{
	...
	if (std::abs(denominator) <= 0.000001)
		return a;
	const double f =
		vertical ? std::clamp((p.y - a.y) / denominator, 0.0, 1.0) : std::clamp((p.x - a.x) / denominator, 0.0, 1.0);
	return {a.x + (b.x - a.x) * f, a.y + (b.y - a.y) * f, a.z + (b.z - a.z) * f};
}
} // namespace
BuildResult Corridor::build(const query::NavGraph& graph, const query::NavRouteResult& route, HullClearance hull,
							Limits limits, PortalPolicy policy) noexcept
{
	...
	if (route.status != query::NavRouteStatus::Complete || route.areas.empty() || count != route.areas.size() - 1)
		return {{}, Error::InvalidRoute, 0, PortalFailureReason::None};
	if (count > limits.maxTransitions || limits.maxBytes < sizeof(Corridor) ||
		count > (limits.maxBytes - sizeof(Corridor)) / sizeof(Transition))
		return {{}, Error::LimitExceeded, 0, PortalFailureReason::None};
	...
	if (!goal.extent.isFinite() || goal.extent.southEast.x <= goal.extent.northWest.x ||
		goal.extent.southEast.y <= goal.extent.northWest.y ||
		(policy == PortalPolicy::Strict && !fits(goal.extent, hull)))
	...
	try
	{
		...
		result->start_ = route.areas.front();
		result->goal_ = route.areas.back();
		...
		for (; index < count; ++index)
		{
			...
			{
				if (checks == limits.maxEdgeChecks)
					return {{}, Error::LimitExceeded, index, PortalFailureReason::None};
				++checks;
				if (same(edge, graph.edge(e)))
				{
					found = true;
					break;
				}
			...
			transition.sourceExtent = graph.area(*from).extent;
			transition.targetExtent = graph.area(*to).extent;
	...
	{
		return {{}, Error::AllocationFailure, index, PortalFailureReason::None};
	}
	catch (...)
	{
		return {{}, Error::LimitExceeded, index, PortalFailureReason::None};
...
{
	if (cursor >= transitions_.size() || lookAhead == 0)
	...
	while (stop + 1 < end && !transitions_[stop].edge.external && !transitions_[stop + 1].edge.external &&
		   ...
		   transitions_[stop + 1].effectiveTraversal == model::NavTraversalKind::Walk)
		++stop;
	auto aim = project(transitions_[stop], position);
	while (stop > cursor)
		aim = project(transitions_[--stop], aim);
...
bool Cursor::advance(std::size_t expected, model::NavAreaId area, bool supportVerified) noexcept
{
	if (!corridor_ || index_ >= corridor_->transitions().size() || expected != index_ || !supportVerified ||
		area != corridor_->transitions()[index_].edge.target)
		return false;
	++index_;
	return true;
}
bool Cursor::advanceLanding(std::size_t expected, model::NavAreaId area, std::uint8_t transitions,
							bool supportVerified) noexcept
{
	if (!corridor_ || expected != index_ || !supportVerified || (transitions != 1 && transitions != 2) ||
		index_ >= corridor_->transitions().size())
		return false;
	const auto end = index_ + transitions;
	if (end > corridor_->transitions().size())
		return false;
	const auto& active = corridor_->transitions()[index_];
	if (transitions == 1)
	{
		if (area != active.edge.target)
			return false;
	}
	else
	{
		...
		if (active.edge.target != next.edge.source || next.edge.external ||
			(next.effectiveTraversal != model::NavTraversalKind::Walk &&
			 next.effectiveTraversal != model::NavTraversalKind::Crouch) ||
			area != next.edge.target)
			return false;
	}
	index_ = end;
	return true;
