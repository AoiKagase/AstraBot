...
public:
	...
	build(std::shared_ptr<const model::NavMeshSnapshot> snapshot, const NavGraphLimits& limits) noexcept;
	...
	compose(std::shared_ptr<const model::NavMeshSnapshot> snapshot,
			const enrichment::NavMapFingerprint& expectedFingerprint, const enrichment::NavTraversalLinkSet& links,
	...
	static diagnostics::ReadResult<std::shared_ptr<const NavGraph>>
	augment(std::shared_ptr<const NavGraph> base, const enrichment::NavTraversalLinkSet& links,
			const NavGraphLimits& limits, const enrichment::NavEnrichmentLimits& enrichmentLimits) noexcept;
	...
	{
		return vertices_[vertex].center;
	}
	...
	{
		return edges_[edgeIndex].selected;
	}
	std::size_t targetIndex(std::size_t edgeIndex) const noexcept
	{
		return edges_[edgeIndex].target;
	}

private:
	static diagnostics::ReadResult<std::shared_ptr<const NavGraph>>
	buildImpl(std::shared_ptr<const model::NavMeshSnapshot> snapshot, const NavGraphLimits& limits,
