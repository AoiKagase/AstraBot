...
inline diagnostics::NavError charge(std::size_t& used, std::size_t count, std::size_t elementSize,
...
inline diagnostics::NavError validateEdge(model::NavTraversalKind traversal) noexcept
{
	if (!model::isKnownTraversalKind(traversal))
		return graphError(diagnostics::NavErrorKind::UnsupportedValue, diagnostics::NavField::GraphTraversal);
	return {};
}
} // namespace astrabot::nav::query::detail
