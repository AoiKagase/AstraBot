...
{
public:
	static std::shared_ptr<const DistributionTopology> build(core::MapGeneration, std::shared_ptr<const NavGraph>,
															 std::shared_ptr<const NavSpatialIndex>) noexcept;
...
public:
	...
	const DistributionDiagnostics& diagnostics() const noexcept
	{
		return diagnostics_;
	}
