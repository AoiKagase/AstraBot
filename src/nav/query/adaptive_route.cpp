...
{
	return team == core::perception::Team::Unknown || team == core::perception::Team::Terrorist ||
		   team == core::perception::Team::CounterTerrorist || team == core::perception::Team::Spectator;
}
...
StyleWeights styleWeights(AdaptiveRouteStyle style) noexcept
{
	switch (style)
	{
	case AdaptiveRouteStyle::Fast:
		return {1.0, 1.0, 0.35, 0.75, 0.5, 0.35};
	case AdaptiveRouteStyle::Safe:
		return {1.1, 1.25, 3.0, 1.5, 2.0, 1.0};
	case AdaptiveRouteStyle::LowExposure:
		return {1.1, 1.1, 2.0, 1.0, 3.0, 1.0};
	case AdaptiveRouteStyle::LowTraffic:
		return {1.05, 1.1, 1.25, 1.0, 1.5, 3.0};
	case AdaptiveRouteStyle::Flank:
		return {1.05, 1.0, 1.5, 1.0, 2.0, 2.5};
	case AdaptiveRouteStyle::ObjectiveFast:
		return {0.75, 0.85, 0.5, 0.75, 0.5, 0.5};
	}
	return {};
}
...
double personalityDanger(AdaptiveRoutePersonality personality) noexcept
{
	switch (personality)
	{
	case AdaptiveRoutePersonality::Aggressive:
		return 0.5;
	case AdaptiveRoutePersonality::Balanced:
		return 1.0;
	case AdaptiveRoutePersonality::Cautious:
		return 2.0;
	}
	return 0.0;
}

double teamDanger(const core::experience::AreaExperience& area, core::perception::Team team) noexcept
{
	switch (team)
	{
	case core::perception::Team::Terrorist:
		return area.dangerT;
	case core::perception::Team::CounterTerrorist:
		return area.dangerCT;
	case core::perception::Team::Unknown:
	case core::perception::Team::Spectator:
		return (std::max)(area.dangerT, area.dangerCT);
	}
	return 0.0;
}
...
double traversalBase(model::NavTraversalKind kind) noexcept
{
	switch (kind)
	{
	case model::NavTraversalKind::Walk:
		return 0.0;
	case model::NavTraversalKind::Crouch:
		return 0.25;
	case model::NavTraversalKind::Jump:
		return 1.0;
	case model::NavTraversalKind::Ladder:
		return 1.0;
	case model::NavTraversalKind::Drop:
		return 1.5;
	}
	return 0.0;
}
...
{
	if (!cost.edge.external || !context.traversalExperience)
		return nullptr;
	const auto id = cost.edge.external->linkId;
	for (std::size_t i = 0; i < context.traversalExperienceCount; ++i)
	{
		if (context.traversalExperience[i].linkId == id)
			return &context.traversalExperience[i];
	}
	return nullptr;
}
...
NavCostDecision cost(const NavCostContext& input, const void* opaque) noexcept
{
	...
	const auto* area = context->experience && context->experience->active()
						   ? context->experience->area(input.target.id.value)
						   : nullptr;
	...
	const double familiarity =
		area ? 1.0 / (1.0 + area->visits) : (context->experience && context->experience->active() ? 1.0 : 0.0);

	double traversal = traversalBase(input.edge.traversal);
	if (input.edge.external)
		traversal += input.edge.external->additionalCost;
	...
	const double dangerWeight =
		context->settings.dangerWeight * weights.danger * personalityDanger(context->settings.personality);
	const double contextualDanger =
		context->contextualDanger
			? context->contextualDanger->risk({input.target.id.value, context->settings.team,
											   context->settings.approachDirection, context->settings.enemyWeaponClass,
											   context->settings.likelyEnemyArea})
			: 0.0;
	const double learnedDanger = area ? danger + area->encounterRate + area->grenadeThreat + area->deathRate : 0.0;
	const double dangerCost =
		(learnedDanger + contextualDanger) * dangerWeight + traffic * context->settings.trafficWeight * weights.traffic;
	const double experienceCost =
		(familiarity + traversalExperience) * context->settings.experienceWeight * weights.experience;
	const double exposureCost = exposure * context->settings.exposureWeight * weights.exposure;
	return {false,
			{input.geometricDistance * context->settings.distanceWeight * weights.distance,
			 traversal * context->settings.traversalWeight * weights.traversal, dangerCost, experienceCost,
			 exposureCost}};
}
...
bool AdaptiveTraversalExperience::valid() const noexcept
{
	return linkId != 0U && finiteNonNegative(humanAttempts) && finiteNonNegative(humanSuccess) &&
		   finiteNonNegative(botAttempts) && finiteNonNegative(botSuccess) && finiteNonNegative(failures) &&
		   humanSuccess <= humanAttempts && botSuccess <= botAttempts;
}
...
double AdaptiveTraversalExperience::successRate() const noexcept
{
	...
	return finiteNonNegative(attempts) && finiteNonNegative(successes) && std::isfinite(attempts + failures) &&
				   attempts + failures + 1.0 > 0.0
			   ? successes / (attempts + failures + 1.0)
			   : 0.0;
}
...
double AdaptiveTraversalExperience::failureRisk() const noexcept
{
	...
	return finiteNonNegative(attempts) && finiteNonNegative(failures) && std::isfinite(denominator) && denominator > 0.0
			   ? (failedAttempts + failures) / denominator
			   : 0.0;
}
...
{
	return valid() && minimumHumanAttempts != 0U && finiteNonNegative(minimumHumanSuccessRate) &&
		   minimumHumanSuccessRate <= 1.0 && humanAttempts >= static_cast<double>(minimumHumanAttempts) &&
...
bool AdaptiveRouteSettings::valid() const noexcept
{
	...
	return styleValue <= static_cast<std::uint8_t>(AdaptiveRouteStyle::ObjectiveFast) &&
		   personalityValue <= static_cast<std::uint8_t>(AdaptiveRoutePersonality::Cautious) && known(team) &&
		   finiteNonNegative(distanceWeight) && finiteNonNegative(traversalWeight) && finiteNonNegative(dangerWeight) &&
		   finiteNonNegative(experienceWeight) && finiteNonNegative(exposureWeight) &&
		   finiteNonNegative(trafficWeight) && finiteNonNegative(traversalRiskWeight) &&
		   minHumanTraversalAttempts != 0U && minHumanTraversalAttempts <= 1'000'000U &&
		   finiteNonNegative(minHumanTraversalSuccessRate) && minHumanTraversalSuccessRate <= 1.0 &&
...
{
	if (!settings.valid() || (traversalExperienceCount != 0U && !traversalExperience))
		return false;
	if (!traversalExperience)
		return traversalExperienceCount == 0U;
	for (std::size_t i = 0; i < traversalExperienceCount; ++i)
	{
		if (!traversalExperience[i].valid())
			return false;
		for (std::size_t j = 0; j < i; ++j)
		{
			if (traversalExperience[j].linkId == traversalExperience[i].linkId)
				return false;
		}
	}
	return true;
