...
{
	const auto it =
		std::lower_bound(vertices_.begin(), vertices_.end(), id, [this](const Vertex& vertex, model::NavAreaId key) {
...
NavGraph::build(std::shared_ptr<const model::NavMeshSnapshot> snapshot, const NavGraphLimits& limits) noexcept
{
	return buildImpl(std::move(snapshot), limits, nullptr, nullptr, {});
}

diagnostics::ReadResult<std::shared_ptr<const NavGraph>>
NavGraph::compose(std::shared_ptr<const model::NavMeshSnapshot> snapshot, const enrichment::NavMapFingerprint& expected,
				  const enrichment::NavTraversalLinkSet& links, const NavGraphLimits& limits,
...
diagnostics::ReadResult<std::shared_ptr<const NavGraph>>
NavGraph::augment(std::shared_ptr<const NavGraph> base, const enrichment::NavTraversalLinkSet& links,
				  const NavGraphLimits& limits, const enrichment::NavEnrichmentLimits& enrichmentLimits) noexcept
{
	...
	auto charge = [&](std::size_t count, std::size_t size) {
		if (count > std::numeric_limits<std::size_t>::max() / size)
			return false;
		if (bytes > std::numeric_limits<std::size_t>::max() - count * size)
			return false;
		bytes += count * size;
		return true;
	};
	if (!charge(base->areaCount(), sizeof(Vertex)) || !charge(total, sizeof(Edge)) || bytes > limits.maxGraphBytes)
	...
	try
	{
		...
		for (const auto& link : links.links)
		{
			...
			if (!target)
				return Result::failure(detail::graphError(K::DanglingReference, F::LinkTo));
		...
		std::sort(graph->edges_.begin(), graph->edges_.end(), [](const Edge& a, const Edge& b) {
			const auto& x = a.selected;
			const auto& y = b.selected;
			if (x.source != y.source)
				return x.source < y.source;
			if (x.external.has_value() != y.external.has_value())
				return !x.external;
			if (x.external)
				return enrichment::detail::identity(*x.external) < enrichment::detail::identity(*y.external);
			return std::tie(x.direction, x.target, x.traversal) < std::tie(y.direction, y.target, y.traversal);
		});
		std::size_t cursor = 0;
		for (auto& vertex : graph->vertices_)
		{
			...
			while (cursor < graph->edges_.size() && graph->edges_[cursor].selected.source == id)
				++cursor;
			vertex.end = cursor;
		}
...
diagnostics::ReadResult<std::shared_ptr<const NavGraph>>
NavGraph::buildImpl(std::shared_ptr<const model::NavMeshSnapshot> snapshot, const NavGraphLimits& limits,
					const enrichment::NavMapFingerprint* expected, const enrichment::NavTraversalLinkSet* links,
					const enrichment::NavEnrichmentLimits& enrichmentLimits) noexcept
{
	using Result = diagnostics::ReadResult<std::shared_ptr<const NavGraph>>;
	using K = diagnostics::NavErrorKind;
	...
	try
	{
		...
		for (std::size_t i = 0; i < areaCount; ++i)
		{
			...
			vertex.center = {(double(e.northWest.x) + double(e.southEast.x)) * 0.5,
							 ...
							 double(e.northWest.z) * 0.25 + double(e.northEastZ) * 0.25 + double(e.southWestZ) * 0.25 +
								 double(e.southEast.z) * 0.25};
		}
		std::sort(graph->vertices_.begin(), graph->vertices_.end(), [&graph](const Vertex& a, const Vertex& b) {
			return graph->snapshot_->areas()[a.snapshotIndex].id < graph->snapshot_->areas()[b.snapshotIndex].id;
		});
		...
		for (auto& vertex : graph->vertices_)
		{
			...
			for (std::uint8_t direction = 0; direction < 4; ++direction)
			{
				for (const auto& connection : area.connections[direction])
				{
					...
					if (!target)
						return Result::failure(detail::graphError(K::DanglingReference, F::ConnectionAreaId));
					graph->edges_.push_back({{area.id, connection.target, direction, connection.traversal}, *target});
				}
			}
		}
		if (links)
