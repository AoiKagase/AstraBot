...
NavSpatialIndex::Node NavSpatialIndex::bounds(std::size_t area) const noexcept
{
	...
	result.low = {e.northWest.x, e.northWest.y,
				  std::min({double(e.northWest.z), double(e.northEastZ), double(e.southWestZ), double(e.southEast.z)})};
	result.high = {
		e.southEast.x, e.southEast.y,
		std::max({double(e.northWest.z), double(e.northEastZ), double(e.southWestZ), double(e.southEast.z)})};
	return result;
}
std::size_t NavSpatialIndex::buildRange(std::size_t first, std::size_t last)
{
	...
	for (std::size_t i = 1; i < 3; ++i)
		if (node.high[i] - node.low[i] > node.high[axis] - node.low[axis])
			axis = i;
	std::sort(order_.begin() + static_cast<std::ptrdiff_t>(first), order_.begin() + static_cast<std::ptrdiff_t>(last),
			  [this, axis](std::size_t a, std::size_t b) {
				  const auto left = bounds(a), right = bounds(b);
				  const double x = left.low[axis] + left.high[axis], y = right.low[axis] + right.high[axis];
...
void NavSpatialIndex::visit(std::size_t index, model::NavVector3 point, NavQueryLimits limits, bool containment,
							std::optional<NavAreaMatch>& best) const noexcept
{
	...
	if (node.area != absent)
	{
		...
		if (std::abs(projected.z - z) > limits.maxVerticalDistance)
			return;
		if (!containment && std::hypot(projected.x - x, projected.y - y, projected.z - z) > limits.maxRadius)
			return;
		const double distance = squaredDistance(projected, point);
		if (!best || distance < best->distanceSquared || (distance == best->distanceSquared && area.id < best->areaId))
...
NavQueryResult NavSpatialIndex::query(model::NavVector3 point, NavQueryLimits limits, bool containment) const noexcept
{
	if (!point.isFinite() || !std::isfinite(limits.maxVerticalDistance) || limits.maxVerticalDistance < 0 ||
		!std::isfinite(limits.maxRadius) || limits.maxRadius < 0)
...
NavQueryResult NavSpatialIndex::containing(model::NavVector3 point, double maxVerticalDistance) const noexcept
{
	return query(point, {0, maxVerticalDistance}, true);
}
NavQueryResult NavSpatialIndex::nearestGeometry(model::NavVector3 point, NavQueryLimits limits) const noexcept
{
