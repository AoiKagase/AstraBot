...
bool valid(double value) noexcept
{
...
{
	total = c.distance;
	return valid(c.distance) && valid(c.traversal) && valid(c.danger) && valid(c.experience) && valid(c.exposure) &&
		   add(total, c.traversal) && add(total, c.danger) && add(total, c.experience) && add(total, c.exposure);
}
...
Error preflight(const NavGraph& graph, const NavRouteLimits& limits, const Query& query,
				const NavRouteResult& result) noexcept
{
	...
	for (const auto& part : {std::pair<std::size_t, std::size_t>{1, sizeof(Query)},
							 {1, sizeof(NavRouteResult)},
							 {count, sizeof(Record)},
							 {count, sizeof(std::size_t)},
							 {count, sizeof(model::NavAreaId)},
	...
	if (count > query.records.max_size() || count > query.open.max_size() || count > result.areas.max_size() ||
		(count != 0 && count - 1 > result.steps.max_size()))
		return routeError(K::AllocationFailure, F::RouteBytes);
	return {};
}

Error heuristic(const NavGraph& graph, Query& query, std::size_t vertex, std::size_t goal, NavRoutePolicy policy,
				F& activeField)
{
...
Error reconstruct(const NavGraph& graph, const Query& query, std::size_t start, std::size_t target,
				  NavRouteResult& result)
{
	...
	for (const auto& step : result.steps)
	{
		if (!add(result.components.distance, step.components.distance) ||
			...
			!add(result.components.exposure, step.components.exposure) || !add(result.total, step.total))
			return routeError(K::InvalidValue, F::RouteCost);
	}
	return {};
}
...
diagnostics::ReadResult<NavRouteResult> NavRouteSearch::search(const NavGraph& graph, const NavRouteRequest& request,
			...
			for (auto edgeIndex = graph.edgeBegin(vertex); edgeIndex < graph.edgeEnd(vertex); ++edgeIndex)
			{
				...
				const auto decision =
					policy.cost
						...
						: NavCostDecision{false, {geometric, edge.external ? edge.external->additionalCost : 0, 0, 0}};
				if (decision.blocked)
					continue;
				double total = 0;
