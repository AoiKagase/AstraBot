...
bool support(const GroundedTarget& p, const runtime::MovementSnapshot& s, model::NavAreaId area,
...
{
	return key.agent.isValid() && key.actor.isValid() && key.map.isValid() && key.source.isValid() &&
		   key.target.isValid() && key.source != key.target && key.traversal == model::NavTraversalKind::Jump;
}
std::uint64_t add(std::uint64_t value, std::uint64_t amount) noexcept
{
	return value > (std::numeric_limits<std::uint64_t>::max)() - amount ? (std::numeric_limits<std::uint64_t>::max)()
																		: value + amount;
}
} // namespace
...
{
	for (auto& item : attempts_)
		if (validKey(item.key) && item.key == key)
			return item;
	...
	if (!slot)
		slot = &*std::min_element(attempts_.begin(), attempts_.end(),
...
void JumpAttemptRegistry::clear() noexcept
{
	attempts_ = {};
}
...
JumpDecision SimpleJump::result(JumpReason reason) const noexcept
{
	...
	out.attemptId = attempt->attemptId;
	out.attemptStartedUs = attempt->startedUs;
	out.minimumSpeed = limits_.minimumSpeed;
	out.maximumSpeed = limits_.maximumSpeed;
	out.intent.jump = ActionRequest::Release;
	const bool duck = ((plan_.sourceAttributes & 1U) != 0) ||
					  (plan_.flightHull && (state_ == JumpState::Airborne || state_ == JumpState::Recover ||
											state_ == JumpState::Complete));
...
JumpDecision SimpleJump::update(const JumpFeedback& f) noexcept
{
	...
	const auto fail = [&](JumpReason reason, RecoveryDisposition disposition = RecoveryDisposition::EdgeCooldown) {
		auto out = finish(JumpState::Failed, reason);
		out.recoveryDisposition = disposition;
		return out;
	};
	...
	if (!binding_.agent.isValid() || !binding_.actor.isValid() || !binding_.map.isValid() ||
		...
		limits_.minimumSpeed > limits_.maximumSpeed || limits_.maximumSpeed > 400 ||
		limits_.approachSpeed > limits_.maximumSpeed || !positive(limits_.takeoffRadius) ||
		!positive(limits_.landingRadius) || !positive(limits_.facingDegrees) || limits_.facingDegrees > 45 ||
		!positive(limits_.maximumDistance) || !positive(limits_.maximumRise) || !positive(limits_.supportTolerance) ||
		!limits_.maxQueries || !limits_.approachTimeoutUs || !limits_.takeoffTimeoutUs || !limits_.airborneTimeoutUs ||
		!limits_.cooldownUs)
	...
	if (!s.tick.isValid() || (started_ && !s.tick.isAfter(tick_)))
		return result(JumpReason::StaleTick);
	...
	if (!attempt_)
	{
		localAttempt_.key = {binding_.agent, binding_.actor, binding_.map,
							 plan_.source,	 plan_.target,	 model::NavTraversalKind::Jump};
	...
	{
		started_ = true;
		if (!attempt_->startedUs)
			attempt_->startedUs = f.nowUs;
		startedUs_ = attempt_->startedUs;
		phaseUs_ = f.nowUs;
		if (attempt_->pressed)
		{
			pressTick_ = attempt_->pressTick;
			state_ = JumpState::Takeoff;
		}
	}
	if (!s.position || !s.position->isFinite() || !s.velocity || !s.velocity->isFinite() || !s.view ||
		!s.view->isFinite() || !s.hull || !s.hull->minimum.isFinite() || !s.hull->maximum.isFinite() ||
		s.hull->minimum.x >= s.hull->maximum.x || s.hull->minimum.y >= s.hull->maximum.y ||
		s.hull->minimum.z >= s.hull->maximum.z || !s.grounded || !s.ducked || !s.speedLimit || !positive(*s.speedLimit))
	...
	auto out = result();
	out.accepted = true;
	out.jumpAxis = {static_cast<float>(ux), static_cast<float>(uy), 0};
	out.speedLimit = *s.speedLimit;
	out.takeoffProof = f.takeoffProof;
	out.flightProof = f.flightProof;
	out.landingProof = f.landingProof;
	out.proofProvenance = f.proofProvenance;
	const bool flight =
		state_ == JumpState::Airborne || state_ == JumpState::Recover || (state_ == JumpState::Takeoff && !*s.grounded);
	...
	const auto moving = [&](double speed) {
		...
		out.intent.speed = (std::min)(speed, double(*s.speedLimit));
	...
	if (state_ == JumpState::Takeoff && f.dispatch)
	{
		...
		if (!dispatch.dispatched)
			return fail(JumpReason::DispatchRejected, RecoveryDisposition::Retry);
		dispatched_ = true;
	}
	if (state_ == JumpState::Takeoff)
	{
		...
		{
			if (!dispatched_)
				return fail(JumpReason::MissingDispatch, RecoveryDisposition::Retry);
			state_ = JumpState::Airborne;
			phaseUs_ = f.nowUs;
			out.state = state_;
			return moving(flightSpeed_);
		}
	...
	if (state_ == JumpState::Airborne || state_ == JumpState::Recover)
	{
		...
		if (!*s.grounded)
			return state_ == JumpState::Recover ? fail(JumpReason::LostSupport, RecoveryDisposition::EdgeCooldown)
												: moving(flightSpeed_);
		if (!proof || !proof->support ||
			!support(*proof->support, s, jumpLandingArea(plan_), limits_.supportTolerance) ||
		...
		if (f.nowUs - phaseUs_ >= limits_.cooldownUs)
			return finish(JumpState::Complete, JumpReason::None);
		return out;
	}

	const bool expired = f.nowUs >= attempt_->startedUs && f.nowUs - attempt_->startedUs >= limits_.approachTimeoutUs;
	...
	if (!proof)
	{
		...
		out.reason = JumpReason::StaleInspection;
		out.recoveryDisposition = RecoveryDisposition::Retry;
		if (expired)
			return fail(JumpReason::ProofTimeout, RecoveryDisposition::EdgeCooldown);
		return out;
	}
	if (proof->stamp.agent != s.agent || proof->stamp.actor != s.actor || proof->stamp.map != s.map ||
		...
		(proof->velocity && *proof->velocity != *s.velocity) || proof->hull.minimum != s.hull->minimum ||
		proof->hull.maximum != s.hull->maximum || proof->takeoff != plan_.takeoff || proof->landing != plan_.landing)
	{
		out.reason = JumpReason::StaleInspection;
		out.recoveryDisposition = RecoveryDisposition::Retry;
	...
	out.takeoffProof = proof->takeoffProof;
	out.flightProof = proof->flightProof;
	out.landingProof = proof->landingProof;
	out.proofProvenance = proof->provenance;
	out.validatedDistance = proof->validatedDistance;
	out.validForUs = proof->validForUs;
	...
	if (state_ == JumpState::Approach)
	{
		...
		else
		{
			...
			out.commandDirection = out.intent.direction;
			out.validatedDistance = range;
	...
	if (state_ == JumpState::Align)
	{
		if (aligned)
		{
			state_ = JumpState::Accelerate;
			out.state = state_;
		}
		if (expired)
			return fail(JumpReason::VelocityTimeout, RecoveryDisposition::EdgeCooldown);
		return out;
	}
	if (!aligned)
	{
	...
	if (proof->trajectoryReady && (proof->takeoffProof != JumpProof::Passed ||
	...
	if (along < limits_.minimumSpeed)
		out.readiness = JumpReadiness::UnderSpeed;
	else if (along > cap)
		out.readiness = JumpReadiness::OverSpeed;
	else if (std::abs(lateral) > limits_.minimumSpeed * 0.1)
		out.readiness = JumpReadiness::LateralError;
	else
		out.readiness = JumpReadiness::Ready;

	const double ex = ux * desired - s.velocity->x, ey = uy * desired - s.velocity->y;
	const double error = std::hypot(ex, ey);
	if (error > 1e-3)
		out.commandDirection = {static_cast<float>(ex / error), static_cast<float>(ey / error), 0};
	else
		out.commandDirection = {static_cast<float>(ux), static_cast<float>(uy), 0};
	...
	if (!core::Motor::bindLocomotion(out.intent, core::LocomotionMode::Run, *s.speedLimit, proof->validatedDistance) ||
		out.intent.validForUs < s.elapsedUs)
		return fail(JumpReason::Blocked, RecoveryDisposition::Retry);
	out.validForUs = out.intent.validForUs;
	...
	if (!attempt_->pressed)
	{
		attempt_->pressed = true;
		attempt_->pressTick = s.tick;
		pressTick_ = s.tick;
		phaseUs_ = f.nowUs;
		state_ = JumpState::Takeoff;
		out.state = state_;
