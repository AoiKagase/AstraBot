...
public:
	...
	CrouchDecision update(const runtime::MovementSnapshot&, bool required, std::uint64_t nowUs, runtime::IWorldQueries&,
						  std::uint32_t reservedQueries, std::uint32_t maxQueries) noexcept;
	CrouchDecision abort() noexcept;
	CrouchState state() const noexcept
	{
		return state_;
	}
