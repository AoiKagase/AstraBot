...
{
	return std::abs(t.sourceHigh.x - t.sourceLow.x) <= 1e-4 && std::abs(t.sourceHigh.y - t.sourceLow.y) <= 1e-4;
}
...
bool crosses(Point from, Point to, const corridor::Transition& t, double& previous) noexcept
{
	...
	if (f < previous - 1e-7 || f < -1e-7 || f > 1.0 + 1e-7)
		return false;
	...
	if (tangent < low - 1e-7 || tangent > high + 1e-7)
		return false;
	previous = f;
	return true;
}
...
PathFollower::PathFollower(std::shared_ptr<const corridor::Corridor> corridor, Point goal) noexcept
	: corridor_(std::move(corridor)), goal_(goal)
{
}

const corridor::Transition* PathFollower::activeTransition() const noexcept
{
	return corridor_ && step_ < corridor_->transitions().size() ? &corridor_->transitions()[step_] : nullptr;
}

FollowResult PathFollower::update(Point position, model::NavAreaId area, bool supportVerified, double lookAhead,
								  double goalTolerance) noexcept
{
	...
	const auto* goalExtent =
		transitionsPtr && !transitionsPtr->empty() ? &transitionsPtr->back().targetExtent : nullptr;
	if (!corridor_ || !finite(position) || !finite(goal_) || !std::isfinite(lookAhead) || lookAhead <= 0 ||
		!std::isfinite(goalTolerance) || goalTolerance < 0 || (goalExtent && !contains(*goalExtent, goal_)))
		return result;
	if (!supportVerified)
	{
		result.status = FollowStatus::MissingSupport;
		return result;
	}
	...
	const auto diagnose = [&]() {
		...
		if (step_ < steps.size() && ordinary(steps[step_]))
		{
			...
			if (area == active.edge.source)
				routeProgress_ = std::max(routeProgress_,
		...
		else if (step_ == steps.size())
		{
			...
			if (d > 1e-9)
				result.routeForward = {(goal_.x - position.x) / d, (goal_.y - position.y) / d, 0};
	...
	const auto recoverBoundary = [&](const model::NavExtent& extent) {
		...
		const double patchWidth =
			std::min(double(extent.southEast.x) - extent.northWest.x, double(extent.southEast.y) - extent.northWest.y);
		const double tolerance = patchWidth < 2.0 * boundaryHalfWidth ? supportBoundaryTolerance : boundaryHalfWidth;
		if (std::abs(x - position.x) > tolerance || std::abs(y - position.y) > tolerance)
			return false;
		const auto projected = query::projectToArea(extent, {static_cast<float>(x), static_cast<float>(y), 0});
		...
		if (projection(projected, position, result.routeForward) < -1e-7)
			return false;
	...
	for (auto i = step_; i < end && ordinary(steps[i]); ++i)
	{
		...
		const Point targetCenter{(targetExtent.northWest.x + targetExtent.southEast.x) * 0.5,
								 (targetExtent.northWest.y + targetExtent.southEast.y) * 0.5, position.z};
		const bool nearTarget =
			distance(position, targetCenter) <=
			boundaryHalfWidth + std::max(1.0, std::min(std::abs(targetExtent.southEast.x - targetExtent.northWest.x),
													   std::abs(targetExtent.southEast.y - targetExtent.northWest.y)) *
												  0.5);
	...
	if (ordinary(active))
	{
		...
		if (length > 1e-3)
		{
			...
			if (cardinalAhead <= 1e-7 || projection(segmentEnd, position, segmentDirection) > 1e-7)
			{
	...
	const bool backDrift = step_ > 0 && ordinary(steps[step_ - 1]) && area == steps[step_ - 1].edge.source &&
						   contains(steps[step_ - 1].sourceExtent, position);
	if (!inSource && !backDrift)
	{
		...
		const Point center{(extent.northWest.x + extent.southEast.x) * 0.5,
						   (extent.northWest.y + extent.southEast.y) * 0.5, position.z};
	...
	if (!ordinary(active))
	{
		if (backDrift)
		{
			...
			const auto entry = active.edge.external ? active.sourceLow : midpoint(active.sourceLow, active.sourceHigh);
			result.routeForward = forward(previous);
			result.routeProjection =
				projection(position, midpoint(previous.sourceLow, previous.sourceHigh), result.routeForward);
			double intersection = 0;
		...
		result.status = FollowStatus::SpecialTransition;
		result.specialTransition = true;
	...
	const Point center{(e.northWest.x + e.southEast.x) * 0.5, (e.northWest.y + e.southEast.y) * 0.5, portal.z};
	...
	const auto reachable = [&](Point candidate, std::size_t through) {
		...
		for (auto j = begin; j <= through; ++j)
			if (!crosses(position, candidate, steps[j], previous))
				return false;
		return true;
	};
	...
	const auto immediate = microPortal(active) ? portal : toward(portalStep, center, std::min(lookAhead, 24.0));
	if (reachable(immediate, step_))
		best = immediate;
	...
	const bool microTransit =
		active.sourceFit == corridor::AreaFit::MicroTransit || active.targetFit == corridor::AreaFit::MicroTransit;
	const auto nearSource = [&] {
		if (contains(active.sourceExtent, position))
			return true;
	...
	if (!best && microTransit && area == active.edge.source && nearSource() &&
	...
	for (auto i = step_; i < horizon && ordinary(steps[i]); ++i)
	{
		...
		if (i + 1 == steps.size())
			candidate = goal_;
		else if (!ordinary(steps[i + 1]))
			candidate = midpoint(steps[i + 1].sourceLow, steps[i + 1].sourceHigh);
		else
		{
			const auto& next = steps[i].targetExtent;
			candidate = toward(
				candidate,
				{(next.northWest.x + next.southEast.x) * 0.5, (next.northWest.y + next.southEast.y) * 0.5, candidate.z},
				24.0);
		}
		...
		if (valid && distance(position, candidate) >= lookAhead)
			break;
	}
	...
	if (!best && ordinary(active) && area == active.edge.source && nearSource())
	{
		...
		if (std::isfinite(forwardLength) && forwardLength > 0.0 && std::isfinite(result.routeForward.x) &&
			std::isfinite(result.routeForward.y))
		{
	...
	confirmedTarget_ = *best;
	confirmedStep_ = step_;
	confirmedThrough_ = bestThrough;
	if (!setTarget(*best))
	{
		result.status = FollowStatus::OutsideCorridor;
		result.target.reset();
...
bool PathFollower::completeSpecial(std::size_t expectedStep, model::NavAreaId area, bool verified,
								   std::uint8_t transitions) noexcept
{
	const auto* active = activeTransition();
	if (!active || ordinary(*active) || !verified || expectedStep != step_ || (transitions != 1 && transitions != 2))
		return false;
	const auto& steps = corridor_->transitions();
	if (transitions == 1)
	{
		if (active->edge.target != area)
			return false;
	}
	else
	{
		if (step_ + 1 >= steps.size() || !ordinary(steps[step_ + 1]) ||
			steps[step_ + 1].edge.source != active->edge.target || steps[step_ + 1].edge.target != area)
			return false;
	}
	step_ += transitions;
	confirmedTarget_.reset();
