...
ProbeReason floorReason(const runtime::FloorObservation& f, double minimumNormal) noexcept
{
	...
	case runtime::FloorObservationStatus::NavContainmentMissing:
		break; // NAV is advisory; validate the physical floor below.
				  ...
				  runtime::IWorldQueries& port, GroundProbeLimits limits, bool locateOnly) noexcept
{
	...
	const auto clearance = [&](model::NavVector3 a, model::NavVector3 b) {
		...
		if (!std::isfinite(hit.fraction) || hit.fraction < 0 || hit.fraction > 1 || !hit.end.isFinite() ||
			!hit.normal.isFinite())
			result.reason = ProbeReason::InvalidResult;
		else if (hit.startSolid || hit.allSolid)
	...
	if (!ground)
		return result;
	if (!ground->ground || !ground->ground->floor)
	...
	if (reason == ProbeReason::AllSolid || reason == ProbeReason::StartSolid)
	{
		...
		if (!representable(top) || !representable(bottom))
			return fail(ProbeReason::InvalidInput);
		const auto fallback = fetch(runtime::QueryKind::Floor, {s.position->x, s.position->y, static_cast<float>(top)},
	...
	if (reason != ProbeReason::None)
		return fail(reason);
	if (!actorGrounded)
		return fail(ProbeReason::ActorGroundFlagMismatch);
	const double feet = double(s.position->z) + s.hull->minimum.z;
	if (std::abs(double(floor.height) - feet) > limits.supportTolerance)
	...
	if (observedArea && !observedArea->isValid())
	...
	const bool boundaryCandidate =
		sourceMatch.projectedPoint.x != s.position->x || sourceMatch.projectedPoint.y != s.position->y;
	...
	if (boundaryCandidate && !clearance(*s.position, *s.position))
	...
	for (std::uint32_t i = 0; i < samples; ++i)
	{
		...
		if (reason != ProbeReason::None)
			return fail(reason);
		if (next.height > start.z || next.height < end.z)
		...
		if (intervalDrop > limits.maxDrop)
		...
		if (!clearance(position, destination))
		{
			...
			if (extra > limits.maxQueries - result.queries)
				return fail(ProbeReason::BudgetExceeded);
			const double liftedZ = double(position.z) + limits.maxStepUp;
			if (!representable(liftedZ))
				return fail(ProbeReason::InvalidInput);
			...
			if (!clearance(position, lifted) || !clearance(lifted, across) || !clearance(across, destination))
				return result;
			result.lastStep = StepEvidence{position, lifted, across, GroundedTarget{destination, area, next}};
			++result.steps;
		}
