...
namespace
{
bool sameHull(const std::optional<runtime::HullDimensions>& a, const std::optional<runtime::HullDimensions>& b) noexcept
{
	return a.has_value() == b.has_value() && (!a || (a->minimum == b->minimum && a->maximum == b->maximum));
}
bool same(Binding a, Binding b) noexcept
{
	return a.agent == b.agent && a.actor == b.actor && a.map == b.map && a.routeGeneration == b.routeGeneration &&
		   a.step == b.step;
}
								...
								JumpGeometryLimits geometry, JumpCandidateRegionMode& regionMode) noexcept
{
	...
	const auto bounds = [&](double margin) noexcept {
		return std::array<double, 4>{
			double(target.northWest.x) - hull.minimum.x + margin, double(target.southEast.x) - hull.maximum.x - margin,
			double(target.northWest.y) - hull.minimum.y + margin, double(target.southEast.y) - hull.maximum.y - margin};
	};
	auto inner = bounds(geometry.clearanceMargin);
	const auto valid = [&](const std::array<double, 4>& value) noexcept {
		return finite(value[0]) && finite(value[1]) && finite(value[2]) && finite(value[3]) && value[0] <= value[1] &&
			   value[2] <= value[3];
	};
	...
	const auto append = [&](double depth, double offset) noexcept {
		...
		if (!landing.isFinite() || !inside(target, landing, hull) ||
			std::hypot(x - double(base.takeoff.x), y - double(base.takeoff.y)) > limits.maximumDistance)
			return;
		for (std::size_t index{}; index < count; ++index)
			if (candidates[index].landing == landing)
				return;
...
std::size_t buildEnvelopeCandidates(std::array<JumpPlan, 5>& candidates, const JumpPlan& base,
									const JumpLandingEnvelope& envelope, JumpLimits limits, JumpPhysics physics,
									double speedLimit, JumpCandidateRegionMode& regionMode) noexcept
{
	const auto finite = [](double value) noexcept { return std::isfinite(value); };
	...
	if (length <= 0)
	{
		regionMode = JumpCandidateRegionMode::NoBallisticLanding;
		return 0;
	}
	...
	const auto appendRegion = [&](const JumpLandingRegion& region) noexcept {
		const double lateral =
			(std::min)(16.0, (std::min)((region.highX - region.lowX) * 0.5, (region.highY - region.lowY) * 0.5));
		const auto append = [&](double depth, double offset) noexcept {
			if (count == candidates.size())
				return;
			...
			if (std::hypot(x - double(base.takeoff.x), y - double(base.takeoff.y)) > limits.maximumDistance)
				return;
			model::NavVector3 landing{static_cast<float>(x), static_cast<float>(y), 0};
			const double z =
				query::projectToArea(region.extent, landing).z - (base.flightHull ? base.flightHull->minimum.z : 0.0F);
			if (!landing.isFinite() || !finite(z))
				return;
			landing.z = static_cast<float>(z);
			for (std::size_t i = 0; i < count; ++i)
				if (candidates[i].landingArea == region.area &&
					std::hypot(double(candidates[i].landing.x) - landing.x,
							   double(candidates[i].landing.y) - landing.y) < 0.01)
					return;
			const double range = std::hypot(double(landing.x) - base.takeoff.x, double(landing.y) - base.takeoff.y);
			if (range <= 0 || !finite(speedLimit) || speedLimit <= 0)
				return;
			const double desired = (std::min)((std::max)(limits.approachSpeed, limits.minimumSpeed),
											  (std::min)(limits.maximumSpeed, speedLimit));
			if (desired < limits.minimumSpeed)
				return;
			...
			candidate.landingArea = region.area;
			candidate.landingAdvance = region.cursorAdvance;
			if (!solveJumpTrajectory(candidate.takeoff, velocity, candidate.landing, limits, physics))
				return;
	...
	if (envelope.successor)
		appendRegion(*envelope.successor);
	if (!count)
	{
		regionMode = JumpCandidateRegionMode::Collapsed;
		return 0;
	...
	regionMode = candidates[0].landingAdvance == 2 ? JumpCandidateRegionMode::SuccessorCentreInset
												   : (envelope.target.kind == JumpLandingRegionKind::HullMargin
														  ? JumpCandidateRegionMode::Margin
...
public:
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& request) override
	{
		if (issued == maximum_)
		{
			...
			r.stamp = request.stamp;
			r.kind = request.kind;
			r.error = runtime::QueryError::BudgetExceeded;
			return r;
		}
		auto wire = request;
		wire.stamp.ordinal = ++issued;
		auto reply = port_.query(wire);
		if (reply.stamp == wire.stamp)
...
WalkDecision Walk::updateDrop(WalkDecision out, const runtime::MovementSnapshot& s, const query::NavSpatialIndex& index,
							  core::MapGeneration indexMap, runtime::IWorldQueries& port, std::uint64_t now,
							  std::uint32_t reserved, std::optional<JumpPhysics> physics) noexcept
{
	out.queries = reserved;
	out.dropState = dropState_;
	out.dropPlan = dropPlan_;
	out.jumpPhysics = physics;
	const auto fail = [&](DropReason reason) {
		out.dropReason = reason;
		out.dropState = DropState::Failed;
		out.intent = {};
		return finish(out, WalkState::Failed, WalkReason::ProbeFailed);
	};
	if (!limits_.drop)
		return fail(DropReason::Disabled);
	const auto& l = *limits_.drop;
	const auto positive = [](double v) { return std::isfinite(v) && v > 0; };
	if (cursor_.exhausted() || !positive(l.maximumFall) || l.maximumFall > 256 || !std::isfinite(l.maximumGap) ||
		l.maximumGap < 0 || l.maximumGap > 64 || !positive(l.speed) || l.speed > 400 || !positive(l.arrivalTolerance) ||
		l.arrivalTolerance > 8 || !std::isfinite(l.maximumDamage) || l.maximumDamage < 0 || l.maximumDamage > 100 ||
		!std::isfinite(l.minimumLandingHealth) || l.minimumLandingHealth < 0 || l.minimumLandingHealth > 100 ||
		!std::isfinite(l.maximumHealthFraction) || l.maximumHealthFraction < 0 || l.maximumHealthFraction > 1 ||
		!l.approachTimeoutUs || !l.airborneTimeoutUs || !l.maxSegments || l.maxSegments > 12 || !l.maxQueries ||
		l.maxQueries > 21 || reserved >= l.maxQueries || reserved >= limits_.probe.maxQueries)
		return fail(DropReason::InvalidInput);
	if (!s.position || !s.position->isFinite() || !s.velocity || !s.velocity->isFinite() || !s.hull ||
		!s.hull->minimum.isFinite() || !s.hull->maximum.isFinite() || s.hull->minimum.x >= s.hull->maximum.x ||
		s.hull->minimum.y >= s.hull->maximum.y || s.hull->minimum.z >= s.hull->maximum.z || !s.grounded ||
		s.ducked != false || !s.speedLimit || !positive(*s.speedLimit) || !s.elapsedUs || s.elapsedUs > 250000)
		return fail(DropReason::MissingObservation);
	if (!physics || !same(physics->binding, out.binding) || physics->tick != s.tick || !positive(physics->gravity) ||
		(dropPlan_ && dropGravity_ != physics->gravity))
	...
	if (t.effectiveTraversal != model::NavTraversalKind::Drop || !hints || hints.kind != model::NavTraversalKind::Walk)
	...
	auto groundLimits = limits_.probe;
	groundLimits.maxQueries = maximum - reserved;
	const auto doneFail = [&](DropReason reason) {
	...
	if (!dropPlan_)
	{
		...
		else
		{
			...
			const double targetInset = (vertical ? (sign > 0 ? -s.hull->minimum.x : s.hull->maximum.x)
									   ...
									   l.arrivalTolerance;
			const double landingNormal =
				std::clamp(targetBoundary + sign * targetInset,
		...
		if (!takeoff.isFinite() || !landing.isFinite() || fall <= limits_.probe.maxStepUp || fall > l.maximumFall ||
			gap < 0 || gap > l.maximumGap || !query::containsXY(t.targetExtent, landing))
			return doneFail(DropReason::UnsafeGeometry);
		const double impactSpeed = std::sqrt((std::max)(0.0, 2.0 * physics->gravity * fall));
		const double predictedDamage = (std::max)(0.0, impactSpeed - 500.0) * (100.0 / (1100.0 - 500.0)) * 1.25;
		if (!std::isfinite(predictedDamage) || predictedDamage > l.maximumDamage ||
			(predictedDamage > 0 &&
			 (!s.health || !std::isfinite(*s.health) || *s.health - predictedDamage < l.minimumLandingHealth ||
	...
	if (s.grounded == false)
	{
		if (dropState_ == DropState::Approach)
			return doneFail(DropReason::MissingSupport);
		if (dropState_ != DropState::Airborne)
		{
			dropState_ = DropState::Airborne;
			dropAirborneUs_ = now;
		}
		if (now - dropAirborneUs_ >= l.airborneTimeoutUs)
			return doneFail(DropReason::Timeout);
	}
	else
	{
		...
		if (dropState_ == DropState::Airborne)
		{
			...
			out.dropState = DropState::Landed;
			out.primitiveEvent = PrimitiveEvent::Complete;
			dropPlan_.reset();
			dropState_ = DropState::Approach;
			primitive_ = Primitive{};
			return out;
		}
		if (supported.target->area != plan.source)
			return doneFail(DropReason::WrongLanding);
	}
	...
	if (dropState_ == DropState::Approach && distance > l.arrivalTolerance)
	{
		...
		const auto proof = GroundProbe::inspect(s, binding_.routeGeneration, plan.source, plan.takeoff.x,
												plan.takeoff.y, index, indexMap, queries, groundLimits);
		out.queries = queries.issued;
		out.probeReason = proof.reason;
		if (!proof || proof.target->area != plan.source)
			return doneFail(DropReason::MissingSupport);
		...
		if (range <= 0)
			return doneFail(DropReason::Blocked);
	...
	const auto fetch = [&](runtime::QueryKind kind, model::NavVector3 a,
						   model::NavVector3 b) -> std::optional<runtime::WorldQueryResult> {
		if (queries.issued >= maximum)
		{
			queryFailure = DropReason::BudgetExceeded;
			return {};
		}
		runtime::QueryRequest request{
			{binding_.agent, binding_.actor, binding_.map, s.tick, binding_.routeGeneration, 1},
			kind,
			a,
			b,
			s.hull,
			limits_.probe.navTolerance};
		if (kind == runtime::QueryKind::SweptHull)
			request.hull = sweptHull;
		try
		{
			const auto r = queries.query(request);
			out.queries = queries.issued;
			if (!(r.stamp == request.stamp) || r.kind != kind)
				queryFailure = DropReason::StaleQuery;
			else if (r.error != runtime::QueryError::None)
				queryFailure = DropReason::QueryFailed;
			else
				return r;
		}
		catch (...)
	...
	const auto sweep = [&](model::NavVector3 a, model::NavVector3 b) {
		...
		if (!r)
			return false;
		if (!r->hull || r->hull->startSolid || r->hull->fraction != 1 || !r->hull->end.isFinite() ||
			!r->hull->normal.isFinite() ||
			std::hypot(double(r->hull->end.x) - b.x, double(r->hull->end.y) - b.y) > 0.001 ||
			std::abs(double(r->hull->end.z) - b.z) > 0.001)
		{
			queryFailure = DropReason::Blocked;
			return false;
		}
		return true;
	};
	...
	if (height < 0 || height > l.maximumFall || vz > 1)
		return doneFail(DropReason::UnsafeGeometry);
	...
	if (forward < 0 || forward > l.speed || forward > *s.speedLimit || (airborne && lateral > 4))
	...
	if (!airborne && (forward < desired * 0.8 || forward > desired * 1.2 || lateral > 4))
	{
		...
		if (!query::containsXY(t.sourceExtent, {x, y, p.z}))
			return doneFail(DropReason::UnsafeVelocity);
		groundLimits.maxQueries = maximum - queries.issued;
		const auto proof = GroundProbe::inspect(s, binding_.routeGeneration, plan.source, x, y, index, indexMap,
												queries, groundLimits);
		if (!proof || proof.target->area != plan.source)
			return doneFail(DropReason::MissingSupport);
	...
	if (!landing.isFinite() || !query::containsXY(t.targetExtent, landing))
		return doneFail(DropReason::UnsafeVelocity);
	const auto support = fetch(runtime::QueryKind::GroundedArea, landing, landing);
	if (!support)
		return doneFail(queryFailure);
	if (!support->ground || support->ground->area != plan.target || !support->ground->floor)
		return doneFail(DropReason::MissingSupport);
	const auto floor = *support->ground->floor;
	const double normal = double(floor.normal.x) * floor.normal.x + double(floor.normal.y) * floor.normal.y +
						  double(floor.normal.z) * floor.normal.z;
	...
	if (!floor.supported || !floor.normal.isFinite() || normal < 0.99 || normal > 1.01 ||
		...
		std::abs(double(landing.z) + s.hull->minimum.z - floor.height) > limits_.probe.supportTolerance || !area ||
		!*area.value || (**area.value).areaId != plan.target)
		return doneFail(DropReason::MissingSupport);
	const auto clear = fetch(runtime::QueryKind::Clearance, landing, landing);
	if (!clear)
		return doneFail(queryFailure);
	if (!clear->clearance || !clear->clearance->clear)
		return doneFail(DropReason::Blocked);
	if (!airborne && !sweep(p, release))
		return doneFail(queryFailure);
	const double count = std::ceil(flight / 0.05);
	if (count < 1 || count > l.maxSegments)
		return doneFail(DropReason::BudgetExceeded);
	...
	sweptHull.maximum.z =
		static_cast<float>(sweptHull.maximum.z + physics->gravity * chordSeconds * chordSeconds / 8 + 0.01);
	if (!sweptHull.maximum.isFinite())
		return doneFail(DropReason::UnsafeGeometry);
	...
	for (std::uint32_t i = 1; i <= segments; ++i)
	{
		...
		if (!next.isFinite() || !sweep(previous, next))
			return doneFail(queryFailure);
		previous = next;
	}
	if (!airborne)
...
{
	auto expected = binding_;
	expected.step = cursor_.index();
	if (!jump_ || jump_->state() != JumpState::Takeoff || !jumpPressTick_.isValid() || jumpDispatchSeen_ ||
		!same(dispatch.binding, expected) || dispatch.commandTick != jumpPressTick_)
		return false;
	jumpDispatch_ = dispatch;
	jumpDispatchSeen_ = true;
	return true;
}
...
{
	out.queries = reserved;
	out.jumpState = jump_ ? jump_->state() : JumpState::Approach;
	out.jumpPlan = jumpPlan_;
	out.jumpPhysics = physics;
	out.jumpPressTick = jumpPressTick_;
	...
	const bool obstacleStampMatches = observedObstacleHull_ && observedObstacleHull_->stamp.agent == s.agent &&
	...
	const auto fail = [&](JumpReason reason) {
		...
		if (jumpAttemptKey_)
			jumpAttempts_->erase(*jumpAttemptKey_);
		jumpAttempt_ = nullptr;
		jumpAttemptKey_.reset();
		jumpCandidateCount_ = 0;
		jumpCandidateIndex_ = 0;
		out.jumpReason = reason;
		out.jumpState = JumpState::Failed;
		return finish(out, WalkState::Failed, WalkReason::JumpFailed);
	};
	if (!limits_.jump || cursor_.exhausted())
	...
	if (!same(physics->binding, out.binding) || physics->tick != s.tick || !std::isfinite(physics->gravity) ||
		...
		(jumpPhysics_ &&
		 (jumpPhysics_->gravity != physics->gravity || jumpPhysics_->verticalImpulse != physics->verticalImpulse ||
	...
	profile.geometry.preferredDistance =
		(std::min)(profile.geometry.preferredDistance, profile.motion.maximumDistance * 0.9);
	const auto maximum =
		(std::min)({limits_.probe.maxQueries, profile.motion.maxQueries, profile.flight.maxQueries, 21U});
	...
	{
		if (!s.position || !s.hull || s.grounded != true)
			return fail(JumpReason::WrongLanding);
		if (!crouch_)
			crouch_.emplace(binding_, limits_.crouch);
		const auto pose = crouch_->update(s, hints.targetDuck, nowUs, port, reserved, maximum);
		out.queries += pose.queries;
		posture_ = pose.state;
		postureReason_ = pose.reason;
		postureAction_ = pose.intent.duck;
		out.intent = pose.intent;
		out.intent.jump = ActionRequest::Release;
		if (pose.terminalEvent)
			return finish(out, WalkState::Failed, WalkReason::PostureFailed);
		if (!pose.movementAllowed)
			return out;
		auto ground = limits_.probe;
		ground.maxQueries = maximum - out.queries;
		...
		const auto completed =
			primitive_.update({out.binding, s.tick, Progress::Complete, {}, out.support->area, true});
		if (!completed.accepted || completed.state != PrimitiveState::Complete ||
			!cursor_.advance(out.binding.step, out.support->area, true))
			return fail(JumpReason::WrongLanding);
		out.primitiveEvent = completed.event;
		completedJumpStep_ = out.binding.step;
		observedJumpCandidate_ = false;
		if (jumpAttemptKey_)
			jumpAttempts_->erase(*jumpAttemptKey_);
		jumpAttempt_ = nullptr;
	...
	if (!jump_)
	{
		...
		const auto candidate =
			JumpGeometry::derive(*corridor_, out.binding, s, profile.motion, profile.geometry, observedJumpCandidate_);
		out.jumpGeometryReason = candidate.reason;
		if (!candidate)
			return fail(JumpReason::InvalidInput);
		if (observedJumpCandidate_ &&
			(primitive_.state() != PrimitiveState::Running || !same(primitive_.binding(), out.binding)))
			return fail(JumpReason::InvalidInput);
		if (!observedJumpCandidate_)
		{
		...
		out.jumpLandingEnvelope = jumpLandingEnvelope_;
		if (!jumpLandingEnvelope_)
			return fail(JumpReason::InvalidInput);
		jumpCandidateCount_ =
			buildEnvelopeCandidates(jumpCandidates_, *candidate.plan, *jumpLandingEnvelope_, profile.motion, *physics,
									*s.speedLimit, jumpCandidateRegionMode_);
		out.jumpCandidateRegionMode = jumpCandidateRegionMode_;
		...
		if (jumpPlan_)
		{
			...
			const double length = std::hypot(dx, dy);
			if (length > 0)
				out.progressDirection = {dx / length, dy / length, 0};
		}
		return out; // Primitive entry owns its own tick.
	}
	JumpFeedback feedback{out.binding, s, nowUs, {}, jumpDispatch_};
	jumpDispatch_.reset();
	...
	if (s.grounded == true && state != JumpState::Takeoff)
	{
		out.jumpProofPhase = (state == JumpState::Airborne || state == JumpState::Recover)
								 ? JumpProofPhase::Land
								 : (state == JumpState::Accelerate ? JumpProofPhase::Launch : JumpProofPhase::Prepare);
	}
	...
	{
		if (reserved == maximum)
			proof.reason = JumpProbeReason::BudgetExceeded;
		else
		{
			auto ground = limits_.probe;
			ground.maxQueries = maximum;
			...
			else if (state == JumpState::Accelerate)
			{
				...
				flight.maxQueries = maximum;
				flight.supportTolerance = profile.motion.supportTolerance;
				flight.supportProbeDepth = limits_.probe.probeDepth;
				proof = JumpProbe::launch(s, out.binding, *jumpPlan_, profile.motion, *physics, flight, index, indexMap,
										  queries);
			}
			else
				proof =
					JumpProbe::prepare(s, out.binding, *jumpPlan_, profile.motion, ground, index, indexMap, queries);
		}
		...
		if (proof)
		{
			...
			out.support = proof.inspection->support;
			out.target = proof.inspection->approach;
		}
		...
		if (!proof && proof.reason == JumpProbeReason::BudgetExceeded)
			feedback.proofProvenance = JumpProofProvenance::QueryBudget;
				 ...
				 (proof.reason == JumpProbeReason::StaleNavigation || proof.reason == JumpProbeReason::StalePhysics ||
	...
	const auto plannedStaticFailure = [&]() noexcept {
		if (proof.landingFailure != JumpLandingFailure::PlannedSupport ||
			...
			proof.provenance == JumpProofProvenance::Stale || proof.provenance == JumpProofProvenance::DynamicBlocker)
			return false;
		...
		return support.reason == ProbeReason::AllSolid || support.reason == ProbeReason::StartSolid ||
			   support.reason == ProbeReason::FloorHeightMismatch ||
			   support.reason == ProbeReason::NavContainmentMissing;
	};
	...
	out.jumpState = decision.state;
	out.jumpReason = decision.reason;
	out.jumpPressTick = decision.pressTick;
	out.jumpAttemptId = decision.attemptId;
	out.jumpAttemptStartedUs = decision.attemptStartedUs;
	out.jumpTakeoffProof = decision.takeoffProof;
	out.jumpFlightProof = decision.flightProof;
	out.jumpLandingProof = decision.landingProof;
	out.jumpProofProvenance = decision.proofProvenance;
	out.jumpReadiness = decision.readiness;
	out.jumpRecoveryDisposition = decision.recoveryDisposition;
	out.jumpAxis = decision.jumpAxis;
	out.jumpCommandDirection = decision.commandDirection;
	out.jumpFromTakeoff = decision.fromTakeoff;
	out.jumpAlong = decision.along;
	out.jumpLateral = decision.lateral;
	out.jumpMinimumSpeed = decision.minimumSpeed;
	out.jumpMaximumSpeed = decision.maximumSpeed;
	out.jumpSpeedLimit = decision.speedLimit;
	out.jumpDesiredSpeed = decision.desiredSpeed;
	out.jumpValidatedDistance = decision.validatedDistance;
	...
	postureAction_ = decision.intent.duck;
	if (jumpPlan_)
	{
		const double dx = double(jumpPlan_->landing.x) - jumpPlan_->takeoff.x;
		const double dy = double(jumpPlan_->landing.y) - jumpPlan_->takeoff.y;
	...
	const bool zeroMovement = out.intent.speed <= 0 && out.intent.direction.x == 0 && out.intent.direction.y == 0 &&
							  out.intent.jump != ActionRequest::Press;
	if (zeroMovement && (decision.state == JumpState::Approach || decision.state == JumpState::Align ||
						 decision.state == JumpState::Accelerate || decision.state == JumpState::Takeoff))
	{
		out.disposition = decision.recoveryDisposition == RecoveryDisposition::Retry ||
							  ...
							  ? MotionDisposition::Recovery
							  : MotionDisposition::Hold;
	}
	if (decision.intent.jump == ActionRequest::Press)
		jumpPressTick_ = decision.pressTick;
	if (decision.state == JumpState::Failed || decision.state == JumpState::Aborted)
	{
		if (decision.state == JumpState::Failed && jumpAttemptKey_)
			jumpAttempts_->erase(*jumpAttemptKey_);
		jumpAttempt_ = nullptr;
		jumpAttemptKey_.reset();
		jumpCandidateCount_ = 0;
		jumpCandidateIndex_ = 0;
		return finish(out, decision.state == JumpState::Aborted ? WalkState::Aborted : WalkState::Failed,
					  WalkReason::JumpFailed);
	}
	if (decision.state == JumpState::Complete && !limits_.crouch.transitionTimeoutUs)
	{
		if (!jumpPlan_ || !out.support || !s.position || !s.hull || out.support->area != jumpLandingArea(*jumpPlan_))
			return fail(JumpReason::WrongLanding);
		const auto extentFor = [&]() -> const JumpLandingRegion* {
			if (!jumpLandingEnvelope_)
				return nullptr;
			if (jumpLandingEnvelope_->target.area == jumpLandingArea(*jumpPlan_))
				return &jumpLandingEnvelope_->target;
		...
		if (!landingRegion || !query::containsXY(landingRegion->extent, *s.position) ||
			(landingRegion->kind == JumpLandingRegionKind::HullMargin &&
			 !inside(landingRegion->extent, *s.position, *s.hull)))
			return fail(JumpReason::WrongLanding);
		const auto completed =
			primitive_.update({out.binding, s.tick, Progress::Complete, {}, out.support->area, true});
		if (!completed.accepted || completed.state != PrimitiveState::Complete ||
