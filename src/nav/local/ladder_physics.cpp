...
constexpr auto kDirectionalButtons =
	static_cast<core::ButtonMask>(core::Button::Forward) | static_cast<core::ButtonMask>(core::Button::Back);
bool commandValid(const core::BotCommand& c) noexcept
{
	return c.msec > 0 && c.msec <= 120 && !c.impulse && !(c.buttons & ~kDirectionalButtons) &&
		   std::isfinite(c.view.pitch) && std::abs(c.view.pitch) <= 89 && std::isfinite(c.view.yaw) &&
		   c.view.roll == 0 && std::isfinite(c.movement.forward) && std::isfinite(c.movement.side) &&
		   c.movement.up == 0;
}
bool positive(double v, double cap) noexcept
{
	return std::isfinite(v) && v > 0 && v <= cap;
}
} // namespace
std::optional<model::NavVector3> ladderVelocity(const core::BotCommand& c, model::NavVector3 n, double maximum,
												bool floorSolid) noexcept
{
	if (!commandValid(c) || !positive(maximum, 2000) || !n.isFinite() || n.z != 0 ||
		std::abs(double(n.x) * n.x + double(n.y) * n.y - 1) > 0.001 || c.buttons == kDirectionalButtons)
		return {};
	...
	if (!forward && !back)
		return model::NavVector3{};
	const double speed = (forward ? 1 : -1) * (std::min)(maximum, 200.0);
	const double pitch = c.view.pitch * kRadiansPerDegree, yaw = c.view.yaw * kRadiansPerDegree;
	...
	return model::NavVector3{static_cast<float>(-n.y * tangent + n.x * kick),
							 static_cast<float>(n.x * tangent + n.y * kick),
							 static_cast<float>(-speed * std::sin(pitch) - into)};
}
std::optional<LadderAirStep> ladderJumpAirStep(const core::BotCommand& c, model::NavVector3 n,
											   LadderAirPhysics physics) noexcept
{
	constexpr auto jump = static_cast<core::ButtonMask>(core::Button::Jump);
	if (!(c.buttons & jump) || !n.isFinite() || n.z != 0 || std::abs(double(n.x) * n.x + double(n.y) * n.y - 1) > 0.001)
		return {};
	auto airborne = c;
	airborne.buttons = static_cast<core::ButtonMask>(c.buttons & ~jump);
...
std::optional<LadderAirStep> ladderAirStep(const core::BotCommand& c, model::NavVector3 v, LadderAirPhysics p) noexcept
{
	if (!commandValid(c) || !v.isFinite() || !positive(p.gravity, 4000) || !positive(p.airAcceleration, 1000) ||
		...
		std::abs(v.x) > 10000 || std::abs(v.y) > 10000 || std::abs(v.z) > 10000)
		return {};
	const double yaw = c.view.yaw * kRadiansPerDegree, dt = double(c.msec) / 1000;
	...
	result.velocity = {static_cast<float>(clamp(vx)), static_cast<float>(clamp(vy)),
					   static_cast<float>(clamp(halfZ - 0.5 * p.gravity * dt))};
	if (!result.displacement.isFinite() || !result.velocity.isFinite())
		return {};
	return result;
}
} // namespace astrabot::nav::local
