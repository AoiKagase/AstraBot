...
{
	return c.msec > 0 && c.msec <= 120 && !c.impulse && !(c.buttons & ~directional) && std::isfinite(c.view.pitch) &&
		   std::abs(c.view.pitch) <= 89 && std::isfinite(c.view.yaw) && c.view.roll == 0 &&
												...
												bool floorSolid) noexcept
{
	if (!commandValid(c) || !positive(maximum, 2000) || !n.isFinite() || n.z != 0 ||
		std::abs(double(n.x) * n.x + double(n.y) * n.y - 1) > 0.001 || c.buttons == directional)
		return {};
