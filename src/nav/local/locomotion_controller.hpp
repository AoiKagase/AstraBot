...
public:
    ...
    LocomotionController(Binding,std::shared_ptr<const corridor::Corridor>,
        model::NavVector3,WalkLimits,JumpAttemptRegistry* =nullptr) noexcept;
    WalkDecision update(const runtime::MovementSnapshot&,const query::NavSpatialIndex&,
        core::MapGeneration,runtime::IWorldQueries&,std::uint64_t=0,std::uint32_t=0,
        std::optional<JumpPhysics> ={},std::optional<LadderObservation> ={}) noexcept;
    WalkDecision recover(const runtime::MovementSnapshot&,const query::NavSpatialIndex&,
        core::MapGeneration,runtime::IWorldQueries&,const RecoveryDecision&,std::uint32_t=0) noexcept;
    ...
    bool native() const noexcept { return !comparison_ && !ladderOwned_; }
    void stepHeight(double value) noexcept { limits_.probe.maxStepUp=value; }
    const std::optional<MotionEnvelope>& envelope() const noexcept { return envelope_; }
    void feedback(const MovementFeedback&) noexcept;
    ProbeResult guard(const runtime::MovementSnapshot&,const MotionEnvelope&,
        model::NavVector3,const query::NavSpatialIndex&,core::MapGeneration,
        runtime::IWorldQueries&,std::uint32_t,std::optional<JumpPhysics>) noexcept;
private:
    Binding binding_{};
    ...
    void issue(WalkDecision&,const runtime::MovementSnapshot&,const GroundedTarget&,
        core::IntentVector,LocomotionPhase) noexcept;
    WalkDecision airborne(WalkDecision,const runtime::MovementSnapshot&,
        const query::NavSpatialIndex&,core::MapGeneration,runtime::IWorldQueries&,std::uint32_t);
    bool beginJump(WalkDecision&,const runtime::MovementSnapshot&,const GroundedTarget&,
        core::IntentVector,const query::NavSpatialIndex&,core::MapGeneration,
        runtime::IWorldQueries&,std::uint32_t&,JumpPhysics,bool,
        const runtime::WorldQueryResult*);
};
}
