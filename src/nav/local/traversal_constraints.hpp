...
struct TraversalConstraints
{
	...
	{
		return reason == ConstraintReason::None;
	}
};
...
inline TraversalConstraints constraints(model::NavTraversalKind edge, std::uint8_t source, std::uint8_t target) noexcept
{
	...
	TraversalConstraints out;
	out.noJump = (hints & 8U) != 0;
	out.precise = (hints & 4U) != 0;
	if (hints & ~15U)
	{
		out.reason = ConstraintReason::UnknownAttributes;
		return out;
	}
	if (edge != model::NavTraversalKind::Walk && edge != model::NavTraversalKind::Crouch &&
		edge != model::NavTraversalKind::Jump)
	{
		out.reason = ConstraintReason::UnsupportedTraversal;
		return out;
	}
	...
	if (jump && out.noJump)
	{
		out.reason = ConstraintReason::ConflictingJump;
		return out;
	}
	out.sourceDuck = (source & 1U) != 0;
	out.targetDuck = (target & 1U) != 0;
	out.kind = jump	  ? model::NavTraversalKind::Jump
			   : duck ? model::NavTraversalKind::Crouch
