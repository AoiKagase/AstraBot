...
{
	return a.sourceId == b.sourceId && a.generation == b.generation && a.linkId == b.linkId && a.from == b.from &&
		   a.to == b.to && a.entry.x == b.entry.x && a.entry.y == b.entry.y && a.entry.z == b.entry.z &&
...
WalkDecision Walk::updateLadder(WalkDecision out, const runtime::MovementSnapshot& s,
								const query::NavSpatialIndex& index, std::uint64_t nowUs, std::uint32_t reserved,
								const std::optional<LadderObservation>& observation) noexcept
{
	out.queries = reserved;
	out.ladderState = ladder_ ? ladder_->state() : LadderState::Approach;
	out.ladderPlan = ladderPlan_;
	const auto fail = [&](LadderReason reason) {
		out.ladderReason = reason;
		out.ladderState = LadderState::Failed;
		return finish(out, WalkState::Failed, WalkReason::LadderFailed);
	};
	const auto selected = selectedLadderLink();
	if (!selected || !limits_.ladder)
	...
	if (!same(*selected, incoming.link) || (ladderPlan_ && !same(*ladderPlan_, incoming)))
	...
	if (reserved > maximum || observation->inspection.queries > maximum - reserved)
		return fail(LadderReason::StaleInspection);
	...
	if (entering)
	{
		ladderPlan_ = incoming;
		ladder_.emplace(out.binding, incoming, *limits_.ladder);
	}
	auto movement = s;
	movement.ladder = observation->contact;
	const auto decision =
		ladder_->update({out.binding, movement, nowUs, observation->inspection, observation->climbing});
	out.ladderState = decision.state;
	out.ladderReason = decision.reason;
	out.ladderPlan = ladderPlan_;
	out.ladderPressTick = decision.pressTick;
	out.intent = decision.intent;
	out.support = observation->inspection.support;
	if (decision.state == LadderState::Failed || decision.state == LadderState::Aborted)
		return finish(out, decision.state == LadderState::Aborted ? WalkState::Aborted : WalkState::Failed,
					  WalkReason::LadderFailed);
	if (entering)
	{
	...
	{
		if (!out.support || out.support->area != selected->to)
		...
		if (!complete.accepted || complete.state != PrimitiveState::Complete ||
			!cursor_.advance(out.binding.step, out.support->area, true))
			return fail(LadderReason::WrongLanding);
		out.primitiveEvent = complete.event;
