...
{
	return !t.edge.external &&
		   (t.effectiveTraversal == model::NavTraversalKind::Walk ||
...
{
	return s.agent == b.agent && s.actor == b.actor && s.map == b.map && s.kind == runtime::ActorKind::ManagedBot &&
		   s.connected == true && s.alive == true && s.joined == true;
}
struct Queries final : runtime::IWorldQueries
{
	runtime::IWorldQueries& port;
	std::uint32_t used, maximum;
	...
	{
		runtime::WorldQueryResult out;
		out.stamp = original.stamp;
		out.kind = original.kind;
		if (used >= maximum)
		{
			out.error = runtime::QueryError::BudgetExceeded;
			return out;
		}
		auto request = original;
		request.stamp.ordinal = ++used;
		try
		{
			out = port.query(request);
		}
		catch (...)
		{
			out.error = runtime::QueryError::Unavailable;
			return out;
		}
		if (sameQueryContext(out.stamp, request.stamp))
			out.stamp = original.stamp;
		else
		{
			out.stamp = {};
			out.error = runtime::QueryError::InvalidResult;
		}
		return out;
	}
};
runtime::WorldQueryResult ask(Queries& q, const runtime::MovementSnapshot& s, Binding b, runtime::QueryKind kind,
							  Vec from, Vec to)
{
	...
	if (!(result.stamp == request.stamp) || result.kind != kind)
		result.error = runtime::QueryError::InvalidResult;
	return result;
}
bool clear(const runtime::WorldQueryResult& r) noexcept
{
	return r.error == runtime::QueryError::None && r.hull && !r.hull->startSolid && !r.hull->allSolid &&
		   std::isfinite(r.hull->fraction) && r.hull->fraction == 1;
}
bool unknown(ProbeReason r) noexcept
{
	return r == ProbeReason::BudgetExceeded || r == ProbeReason::QueryUnavailable || r == ProbeReason::QueryFailed ||
		   r == ProbeReason::StaleQuery || r == ProbeReason::InvalidResult;
}
} // namespace
LocomotionController::LocomotionController(Binding b, std::shared_ptr<const corridor::Corridor> corridor, Vec goal,
										   WalkLimits limits, JumpAttemptRegistry* attempts) noexcept
...
WalkState LocomotionController::state() const noexcept
{
	return comparison_ ? legacy_.state() : state_;
}
std::size_t LocomotionController::step() const noexcept
{
	return comparison_ ? legacy_.step() : follower_.step();
}
...
{
	if (comparison_ || ladderOwned_)
		return legacy_.reportJumpDispatch(d);
	if (d.binding.agent != binding_.agent || d.binding.actor != binding_.actor || d.binding.map != binding_.map ||
		...
		(d.dispatched && !d.dispatchTick.isAfter(d.commandTick)))
		return false;
	pressDispatched_ = d.dispatched;
	dispatchTick_ = d.dispatchTick;
	return true;
}
bool LocomotionController::reportLadderDispatch(const LadderDispatch& d) noexcept
{
	return legacy_.reportLadderDispatch(d);
}
...
Vec LocomotionController::ladderTarget(const LadderPlan& p, Vec origin) const noexcept
{
	return legacy_.ladderTarget(p, origin);
}
...
{
	if (comparison_)
		return legacy_.abort();
	if (ladderOwned_)
		(void)legacy_.abort();
	WalkDecision out;
	out.binding = binding_;
	out.binding.step = step();
	out.tick = tick_;
	out.accepted = true;
...
bool LocomotionController::beginJump(WalkDecision& out, const runtime::MovementSnapshot& s,
									 const GroundedTarget& support, core::IntentVector direction,
									 const query::NavSpatialIndex& index, core::MapGeneration map,
									 runtime::IWorldQueries& port, std::uint32_t& used, JumpPhysics physics, bool drop,
									 const runtime::WorldQueryResult* obstacle)
{
	...
	if (!drop && (attributes & 8U))
		return false;
	const bool plannedJump = !drop && t && t->effectiveTraversal == model::NavTraversalKind::Jump;
	if (!s.grounded.value_or(false) || !s.velocity || !s.speedLimit || !s.hull || !std::isfinite(physics.gravity) ||
	...
	if (!drop && !plannedJump)
	{
		...
		const double supportNormalLength =
			std::sqrt(double(normal.x) * normal.x + double(normal.y) * normal.y + double(normal.z) * normal.z);
		const double slopeRise =
			normal.z > 0.001 ? -(double(normal.x) * direction.x + double(normal.y) * direction.y) / normal.z : 0.0;
		if (!normal.isFinite() || supportNormalLength < 0.99 || supportNormalLength > 1.01 || normal.z < 0.7f ||
			slopeRise > 0.05)
			return false;
		if (obstacle == nullptr || obstacle->error != runtime::QueryError::None || !obstacle->blocker ||
			obstacle->blocker->kind != runtime::BlockerKind::Geometry || !obstacle->hull)
			return false;
		const auto& hit = *obstacle->hull;
		const double normalLength =
			std::sqrt(double(hit.normal.x) * hit.normal.x + double(hit.normal.y) * hit.normal.y +
		...
		if (hit.startSolid || hit.allSolid || !hit.end.isFinite() || !hit.normal.isFinite() ||
			!std::isfinite(hit.fraction) || hit.fraction < 0 || hit.fraction >= 1 || normalLength < 0.99 ||
			normalLength > 1.01 || facing >= -0.25 || std::abs(double(hit.normal.z)) > 0.25 || takeoff > 32.5)
			return false;
	}
	...
	if (!std::isfinite(seconds) || seconds <= 0 || seconds > 1.5 || horizontal * seconds > 256)
		return false;
	...
	if (!drop)
	{
		...
		if (!clear(overhead))
		{
			out.probeReason = overhead.error == runtime::QueryError::BudgetExceeded ? ProbeReason::BudgetExceeded
																					: ProbeReason::Blocked;
			return reject();
		}
		...
		if (!clear(upperEdge))
		{
			out.probeReason = upperEdge.error == runtime::QueryError::BudgetExceeded ? ProbeReason::BudgetExceeded
																					 : ProbeReason::Blocked;
			return reject();
		}
	}
	...
	{
		out.probeReason = candidate.error == runtime::QueryError::BudgetExceeded ? ProbeReason::BudgetExceeded
																				 : ProbeReason::QueryUnavailable;
		return reject();
	}
	const double height = candidate.floor->height;
	const double fall = feet - height;
	if (fall > 64 || (drop && fall < 0) || (!drop && height - feet > apex))
		return reject();
	const double discriminant = impulse * impulse + 2 * physics.gravity * fall;
	if (discriminant < 0)
		return reject();
	seconds = (impulse + std::sqrt(discriminant)) / physics.gravity;
	if (seconds <= 0 || seconds > 1.5 || horizontal * seconds > 256)
		return reject();
	target = {static_cast<float>(s.position->x + s.velocity->x * seconds),
			  static_cast<float>(s.position->y + s.velocity->y * seconds),
			  static_cast<float>(height - s.hull->minimum.z)};
	auto landing = s;
	landing.position = target;
	landing.grounded = true;
	auto allowance = limits_.probe;
	allowance.maxQueries = queries.maximum - queries.used;
	...
	{
		out.probeReason = supported.reason;
		return reject();
	}
	const unsigned segments = static_cast<unsigned>(std::ceil(seconds / 0.05));
	if (queries.maximum - queries.used < segments)
	{
	...
	airborneTarget_ = supported.target->origin;
	airborneArea_ = supported.target->area;
	flightPhysics_ = physics;
	phase_ = drop ? LocomotionPhase::Drop : LocomotionPhase::Takeoff;
	phaseStartedUs_ = nowUs_;
	pressDispatched_ = false;
	airborneSeen_ = false;
	...
	out.support = support;
	issue(out, s, *supported.target, direction, phase_);
	if (!envelope_)
	{
	...
	if (s.ducked == true)
		out.intent.duck = core::ActionRequest::Hold;
	out.jumpState = drop ? JumpState::Airborne : JumpState::Takeoff;
	out.jumpPhysics = physics;
	out.jumpPressTick = pressTick_;
	out.jumpTrajectoryReady = true;
	out.jumpPredictedLanding = airborneTarget_;
	if (!drop)
		jumpCooldownUntilUs_ = nowUs_ + 750000;
...
WalkDecision LocomotionController::airborne(WalkDecision out, const runtime::MovementSnapshot& s,
											const query::NavSpatialIndex& index, core::MapGeneration map,
											runtime::IWorldQueries& port, std::uint32_t reserved)
{
	out.jumpState = phase_ == LocomotionPhase::Takeoff ? JumpState::Takeoff : JumpState::Airborne;
	out.jumpPhysics = flightPhysics_;
	out.jumpPressTick = pressTick_;
	if (nowUs_ - phaseStartedUs_ > 2000000)
		return finish(out, WalkState::Failed, WalkReason::RecoveryReplan);
	if (phase_ == LocomotionPhase::Takeoff)
	{
		...
		if (s.grounded == false)
		{
			phase_ = LocomotionPhase::Airborne;
			airborneSeen_ = true;
		}
		else if (nowUs_ - phaseStartedUs_ > 200000)
		{
			...
			phase_ = LocomotionPhase::Ground;
			return hold(out, ProbeReason::Blocked);
		}
	}
	else if (s.grounded == true && airborneSeen_)
	{
		...
		if (!support)
			return hold(out, support.reason);
		if (!reachableArea(support.target->area))
			return finish(out, WalkState::Failed, WalkReason::RecoveryReplan);
		const auto* t = follower_.activeTransition();
		...
		phase_ = LocomotionPhase::Ground;
		out.jumpState = JumpState::Complete;
...
{
	ProbeResult out;
	out.reason = ProbeReason::StaleNavigation;
	...
	{
		out.reason = ProbeReason::QueryUnavailable;
		return out;
	}
	if (reserved >= queryLimit)
	{
		out.reason = ProbeReason::BudgetExceeded;
		return out;
	}
	Queries queries(port, reserved, queryLimit);
	if (envelope.phase == LocomotionPhase::Ground)
	{
		auto limits = limits_.probe;
		...
		limits.maxQueries = queryLimit - queries.used;
		out = TerrainSampler::inspect(s, binding_.routeGeneration, support.target->area, destination.x, destination.y,
									  index, map, queries, limits);
		if (out && !reachableArea(out.target->area))
		{
			out.reason = ProbeReason::NoArea;
			out.target.reset();
		}
	}
	else
	{
		if (!physics || !flightPhysics_ || !s.velocity || physics->gravity != flightPhysics_->gravity ||
			physics->verticalImpulse != flightPhysics_->verticalImpulse)
		{
			out.reason = ProbeReason::QueryUnavailable;
			return out;
		}
		...
		if (dt <= 0 || dt > 0.12)
		{
			out.reason = ProbeReason::InvalidInput;
			return out;
		}
		...
		const double vertical =
			envelope.phase == LocomotionPhase::Takeoff && !pressDispatched_ ? physics->verticalImpulse : s.velocity->z;
		destination.z = leavingEdge
							? s.position->z
		...
		out.reason = clear(sweep) ? ProbeReason::None
								  : (sweep.error == runtime::QueryError::BudgetExceeded ? ProbeReason::BudgetExceeded
																						: ProbeReason::Blocked);
		if (out.reason == ProbeReason::None)
			out.target = GroundedTarget{destination, airborneArea_, {}};
	}
	out.queries = queries.used;
	return out;
}
...
{
	if (!corridor_ || !area.isValid())
		return false;
	if (!follower_.activeTransition())
		return area == corridor_->goal();
	auto i = follower_.step();
	const auto& transitions = corridor_->transitions();
	if (area == transitions[i].edge.source)
		return true;
	...
	{
		if (area == transitions[i].edge.target)
			return true;
		if (!ordinary(transitions[i]))
			break;
	}
	return false;
}
void LocomotionController::issue(WalkDecision& out, const runtime::MovementSnapshot& s, const GroundedTarget& target,
								 core::IntentVector direction, LocomotionPhase phase) noexcept
{
	out.target = target;
	out.intent.direction = direction;
	...
	envelope_ = MotionEnvelope{out.binding,
							   ...
							   *s.position,
							   target.origin,
							   out.support ? out.support->area : target.area,
							   phase,
							   out.intent.validForUs,
							   64};
}
void LocomotionController::feedback(const MovementFeedback& f) noexcept
{
	if (!native() || f.binding.agent != binding_.agent || f.binding.actor != binding_.actor ||
		...
		!f.dispatchTick.isAfter(f.commandTick))
		return;
	if (f.commandTick == pressTick_)
	{
		pressDispatched_ = f.dispatched;
		dispatchTick_ = f.dispatchTick;
	}
...
WalkDecision LocomotionController::recover(const runtime::MovementSnapshot& s, const query::NavSpatialIndex& index,
										   core::MapGeneration map, runtime::IWorldQueries& port,
										   const RecoveryDecision& recovery, std::uint32_t reserved) noexcept
{
	if (comparison_)
		return legacy_.recover(s, index, map, port, recovery, reserved);
...
WalkDecision LocomotionController::update(const runtime::MovementSnapshot& s, const query::NavSpatialIndex& index,
										  core::MapGeneration map, runtime::IWorldQueries& port, std::uint64_t now,
										  std::uint32_t reserved, std::optional<JumpPhysics> physics,
										  std::optional<LadderObservation> ladder) noexcept
{
	if (comparison_)
		return legacy_.update(s, index, map, port, now, reserved, physics, ladder);
	WalkDecision out;
	out.binding = binding_;
	out.binding.step = step();
	out.tick = s.tick;
	out.state = state_;
	if (state_ != WalkState::Running)
		return out;
	if (!validActor(s, binding_) || s.map != map)
		return finish(out, WalkState::Aborted, WalkReason::InvalidActor);
	if (!s.tick.isAfter(tick_) || now < nowUs_)
	{
		out.reason = WalkReason::StaleTick;
		return out;
	}
	...
	if (!std::isfinite(limits_.probe.maxStepUp) || limits_.probe.maxStepUp <= 0)
		return hold(out, ProbeReason::QueryUnavailable);
	if (!s.position || !s.position->isFinite() || !s.hull || !s.speedLimit || !s.velocity || !s.velocity->isFinite() ||
		!s.elapsedUs || !s.grounded.has_value())
		return hold(out, ProbeReason::QueryUnavailable);
	if (!progressInitialized_)
	{
		progressInitialized_ = true;
		progressAnchor_ = *s.position;
		progressUs_ = now;
	}
	if (ladderOwned_)
	{
		out = legacy_.update(s, index, map, port, now, reserved, physics, ladder);
		if (out.state != WalkState::Running)
		{
			state_ = out.state;
			return out;
		}
	...
	{
		const auto reason = contact.error == runtime::QueryError::BudgetExceeded ? ProbeReason::BudgetExceeded
																				 : ProbeReason::QueryUnavailable;
		...
		if (phase_ == LocomotionPhase::Ground && contact.error == runtime::QueryError::BudgetExceeded)
			return hold(out, reason);
	}
	else if (contact.blocker)
	{
		out.blocker = contact.blocker;
		out.obstacleHull = contact.hull;
		const auto& observed = *contact.blocker;
		const bool player = observed.kind == runtime::BlockerKind::Player ||
							observed.kind == runtime::BlockerKind::Teammate ||
							observed.kind == runtime::BlockerKind::Enemy;
					   ...
					   observed.player->sameSlot(binding_.actor)))
		{
			if (phase_ == LocomotionPhase::Ground)
				return hold(out, ProbeReason::InvalidResult);
		}
		else if (player)
		{
			...
			{
				const auto policy =
					limits_.blocker.timeoutUs ? limits_.blocker : BlockerLimits{120000, 200000, 1000000};
				blockerWait_.emplace(binding_, policy);
				blockerStep_ = step();
			}
			...
			out.blockerAction = decision.action;
			out.blockerReason = decision.reason;
			if (phase_ == LocomotionPhase::Ground)
			{
				if (decision.action == BlockerAction::Yield)
					return hold(out, ProbeReason::Blocked);
	...
	if (phase_ != LocomotionPhase::Ground)
		return airborne(out, s, index, map, port, queries.used);
	if (queries.used >= queries.maximum)
		return hold(out, ProbeReason::BudgetExceeded);
	auto budget = limits_.probe;
	budget.maxQueries = queries.maximum - queries.used;
	const auto ground = TerrainSampler::locate(s, binding_.routeGeneration, index, map, queries, budget);
	out.queries = queries.used;
	out.support = ground.target;
	out.probeReason = ground.reason;
	if (!ground)
	{
		...
		const bool ordinaryGround =
			transition && ordinary(*transition) && ground.reason != ProbeReason::BudgetExceeded &&
			ground.reason != ProbeReason::StaleQuery && ground.reason != ProbeReason::QueryUnavailable &&
			ground.reason != ProbeReason::QueryFailed && ground.reason != ProbeReason::InvalidResult;
		if (ordinaryGround && s.position && s.hull && s.grounded == true && transition->edge.source.isValid() &&
			queries.maximum - queries.used >= 2)
		{
			...
			if (axisLength > 0.001)
			{
				...
				ay /= axisLength;
				const double feet = double(s.position->z) + s.hull->minimum.z;
				GroundedTarget fallback{
					*s.position, transition->edge.source,
					runtime::FloorObservation{
						static_cast<float>(feet), {0, 0, 1}, true, runtime::FloorObservationStatus::Supported, {}}};
				for (int side : {-1, 1})
				{
					const Vec candidateEnd{static_cast<float>(s.position->x - ay * side * 16.0),
										   static_cast<float>(s.position->y + ax * side * 16.0), s.position->z};
					const auto detour =
						ask(queries, s, binding_, runtime::QueryKind::SweptHull, *s.position, candidateEnd);
				...
				if (queries.maximum - queries.used >= 1)
				{
					...
					const auto forwardSweep =
						ask(queries, s, binding_, runtime::QueryKind::SweptHull, *s.position, end);
					if (clear(forwardSweep))
					{
						...
						out.probeReason = ProbeReason::None;
						issue(out, s, GroundedTarget{end, fallback.area, fallback.floor}, core::IntentVector{ax, ay, 0},
							  LocomotionPhase::Ground);
						out.queries = queries.used;
						return out;
					}
				}
		...
		const bool transientGap =
			ground.reason == ProbeReason::BudgetExceeded || ground.reason == ProbeReason::StaleQuery ||
			ground.reason == ProbeReason::QueryUnavailable || ground.reason == ProbeReason::QueryFailed ||
			ground.reason == ProbeReason::InvalidResult || ground.reason == ProbeReason::NavContainmentMissing ||
			ground.reason == ProbeReason::NoArea;
		if (transientGap && transition && ordinary(*transition) && s.position && s.grounded == true &&
			follower_.confirmedTarget())
		{
			...
			if (length > 1e-3 && std::isfinite(length))
			{
				...
				const auto sweep = ask(queries, s, binding_, runtime::QueryKind::SweptHull, *s.position, end);
				if (clear(sweep))
				{
	...
	out.stuckElapsedUs = progressInitialized_ && now >= progressUs_ ? now - progressUs_ : 0U;
	out.binding.step = follower_.step();
	binding_.step = out.binding.step;
	if (jumpSuppressedStep_ && (follower_.step() != *jumpSuppressedStep_ ||
	...
	if (s.grounded != true && crouch != (s.ducked == true))
		return hold(out, ProbeReason::QueryUnavailable);
	if (limits_.crouch.transitionTimeoutUs && s.grounded == true)
	{
		if (!posture_)
			posture_.emplace(binding_, limits_.crouch);
		const auto pose = posture_->update(s, crouch, now, queries, queries.used, queries.maximum);
		out.posture = pose.state;
		out.postureReason = pose.reason;
		if (!pose.movementAllowed)
		{
			out = hold(out, ProbeReason::QueryUnavailable);
			out.intent = pose.intent;
			out.queries = queries.used;
			return out;
		}
	}
	std::optional<GroundedTarget> contactDetour;
	const bool playerBlocker =
		out.blocker && out.blocker->player && out.blocker->player->isValid() &&
		(out.blocker->kind == runtime::BlockerKind::Player || out.blocker->kind == runtime::BlockerKind::Teammate ||
	...
	{
		if (side_ == 0)
			side_ = (*out.blocker->player < binding_.actor) ? 1 : -1;
		const int first = side_, second = -side_;
		for (int candidateSide : {first, second})
		{
			const Vec lateral{static_cast<float>(-direction.y * static_cast<double>(candidateSide)),
	...
	if (contactDetour)
	{
		...
		if (progressAxis_.x == 0 && progressAxis_.y == 0)
			progressAxis_ = direction;
		out.probeReason = ProbeReason::None;
	...
	const double projected =
		(s.position->x - progressAnchor_.x) * progressAxis_.x + (s.position->y - progressAnchor_.y) * progressAxis_.y;
	if (projected >= progressBest_ + 4)
	{
		progressBest_ = projected;
		progressUs_ = now;
		blockedUs_ = 0;
	}
	if (now - progressUs_ >= 5000000)
		return finish(out, WalkState::Failed, WalkReason::RecoveryReplan);
	const bool isDrop = transition && transition->effectiveTraversal == model::NavTraversalKind::Drop;
	const bool isJump = transition && transition->effectiveTraversal == model::NavTraversalKind::Jump;
	if (isJump && physics)
	{
		...
		queries.used = used;
		out.queries = used;
		if (unknown(out.probeReason))
			return hold(out, out.probeReason);
	}
	if (isDrop && physics)
	{
		auto used = queries.used;
		if (beginJump(out, s, *ground.target, direction, index, map, port, used, *physics, true, nullptr))
			return out;
		queries.used = used;
		out.queries = used;
		if (unknown(out.probeReason))
	...
	if (!precise && queries.maximum - queries.used >= 2)
	{
		...
		out.leftClearance = fractions[0] * length;
		out.rightClearance = fractions[1] * length;
		...
		direction = {x / norm, y / norm, 0};
	}
	const double travel = (std::min)(distance, limits_.probe.maxDistance);
	const auto inspect = [&](core::IntentVector axis) {
		auto allowance = limits_.probe;
		allowance.maxQueries = queries.maximum - queries.used;
		return TerrainSampler::inspect(
			s, binding_.routeGeneration, ground.target->area, static_cast<float>(s.position->x + axis.x * travel),
			static_cast<float>(s.position->y + axis.y * travel), index, map, queries, allowance);
	};
	auto proof = inspect(direction);
	if (proof && !reachableArea(proof.target->area))
		proof.reason = ProbeReason::NoArea;
	const bool geometryContact = out.blocker && out.obstacleHull && out.blocker->kind == runtime::BlockerKind::Geometry;
	...
	if (!proof && !unknown(proof.reason))
	{
		...
		auto used = queries.used;
		const auto door =
			door_.update(s, out.binding, target, index, map, limits_.probe, port, now, used, queries.maximum);
		queries.used = used;
		if (door)
		{
			if (door->target && !door->contact && door->intent.speed > 0)
				envelope_ = MotionEnvelope{door->binding,
										   s.tick,
										   *s.hull,
										   *s.position,
										   door->target->origin,
										   ground.target->area,
										   LocomotionPhase::Ground,
										   door->intent.validForUs,
										   64};
			return *door;
		}
		if (geometryContact && physics && !jumpSuppressedStep_ && nowUs_ >= jumpCooldownUntilUs_ &&
			beginJump(out, s, *ground.target, direction, index, map, port, used, *physics, false, &contact))
			return out;
		queries.used = used;
		if (!side_ || now >= sideUntilUs_)
		{
			side_ = fractions[1] >= fractions[0] ? 1 : -1;
			sideUntilUs_ = now + 500000;
		}
		...
		if (!precise || geometryContact)
		{
			...
			const bool localBlock = proof.reason == ProbeReason::Blocked || proof.reason == ProbeReason::NoArea ||
									proof.reason == ProbeReason::AllSolid || proof.reason == ProbeReason::StartSolid;
	...
	out.queries = queries.used;
	out.probeReason = proof.reason;
	out.samples = proof.samples;
	out.steps = proof.steps;
	if (!proof)
		return hold(out, proof.reason);
	door_.reset();
	issue(out, s, contactDetour ? *contactDetour : *proof.target, direction, LocomotionPhase::Ground);
	if (crouch || s.ducked == true)
		out.intent.duck = core::ActionRequest::Hold;
	return out;
}
} // namespace astrabot::nav::local
