...
{
	if (terminal(state_))
		return {state_, reason_, false, false, {}};
...
DoorWaitDecision DoorWait::update(const DoorWaitFeedback& f) noexcept
{
	...
	if (!same(binding_, f.binding) || q.agent != binding_.agent || q.actor != binding_.actor || q.map != binding_.map ||
		q.routeGeneration != binding_.routeGeneration)
	...
	if (!q.ordinal || !(r.stamp == q) || r.kind != runtime::QueryKind::Door || r.error != runtime::QueryError::None ||
		!r.door || !r.door->id)
	...
	if (door.open)
		return finish(DoorWaitState::Clear, DoorWaitReason::None);
	if (state_ == DoorWaitState::Waiting)
		return {state_, DoorWaitReason::None, true, false, {}};
	if (f.passive && door.canTouch && !door.canUse)
	{
		startedUs_ = f.nowUs;
		doorId_ = door.id;
		state_ = DoorWaitState::Waiting;
		return {state_, DoorWaitReason::None, true, false, {}};
	}
	if (!door.canUse)
		return finish(DoorWaitState::Failed, DoorWaitReason::Unusable);
	MovementIntent intent;
	intent.view = f.useView;
	intent.use = ActionRequest::Press;
	if (!intent.view || !core::Motor::valid(intent) || intent.view->x < core::kMinPitch ||
		intent.view->x > core::kMaxPitch || intent.view->y < core::kMinYaw || intent.view->y > core::kMaxYaw ||
