...
inline bool groundSegmentAllows(model::NavVector3 start, model::NavVector3 end, model::NavVector3 position,
								double travel) noexcept
{
	if (!start.isFinite() || !end.isFinite() || !position.isFinite() || !std::isfinite(travel) || travel < 0)
		return false;
...
inline ProbeResult inspectGroundFrame(const runtime::MovementSnapshot& s, std::uint64_t generation,
									  model::NavAreaId source, model::NavAreaId target, float x, float y,
									  ...
									  runtime::IWorldQueries& port, GroundProbeLimits limits) noexcept
{
	...
	struct CorridorQueries final : runtime::IWorldQueries
	{
		...
		bool outside{};
		std::uint32_t issued{};
		CorridorQueries(runtime::IWorldQueries& p, const query::NavSpatialIndex& i, model::NavAreaId a,
						model::NavAreaId b)
			: port(p), index(i), source(a), target(b)
		{
		}
		runtime::WorldQueryResult query(const runtime::QueryRequest& original) override
		{
			auto q = original;
			q.stamp.ordinal = ++issued;
			auto r = port.query(q);
			...
			if (runtime::sameQueryContext(r.stamp, q.stamp))
				r.stamp = original.stamp;
			return r;
		}
	} queries(port, index, source, target);
	ProbeResult result;
	if (limits.maxQueries < 2)
	{
	...
	if (queries.outside && (!result.target || (result.target->area != source && result.target->area != target)))
	{
