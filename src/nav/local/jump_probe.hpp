...
std::optional<JumpLimits> deriveJumpLimits(JumpLimits, const JumpPhysics&, const runtime::MovementSnapshot&,
										   TraversalConstraints) noexcept;
...
struct JumpTrajectoryResult
{
	...
	{
		return reason == JumpTrajectoryReason::None;
	}
};
...
struct JumpProbeResult
{
	...
	explicit operator bool() const noexcept
	{
		return reason == JumpProbeReason::None && inspection.has_value();
	}
};
...
public:
	...
	static JumpProbeResult land(const runtime::MovementSnapshot&, Binding, JumpPlan, JumpLimits, GroundProbeLimits,
	...
	static JumpProbeResult prepare(const runtime::MovementSnapshot&, Binding, JumpPlan, JumpLimits, GroundProbeLimits,
								   const query::NavSpatialIndex&, core::MapGeneration indexMap,
								   runtime::IWorldQueries&) noexcept;
	...
	static JumpProbeResult launch(const runtime::MovementSnapshot&, Binding, JumpPlan, JumpLimits, JumpPhysics,
								  JumpProbeLimits, const query::NavSpatialIndex&, core::MapGeneration indexMap,
								  runtime::IWorldQueries&) noexcept;
};
} // namespace astrabot::nav::local
