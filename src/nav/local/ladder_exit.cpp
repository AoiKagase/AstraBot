...
namespace
{
LadderExitResult planExit(const LadderPlan& p, const runtime::MovementSnapshot& s, bool touching, double releaseZ,
						  LadderAirPhysics physics, std::uint8_t msec, const query::NavSpatialIndex& index,
						  core::MapGeneration indexMap, bool jumping,
						  std::optional<core::BotCommand> firstCommand) noexcept
{
	LadderExitResult result;
	const auto fail = [&](LadderExitReason reason) {
		result.reason = reason;
		return result;
	};
	if (!s.map.isValid() || !s.tick.isValid() || !s.position || !s.position->isFinite() || !s.velocity ||
		!s.velocity->isFinite() || !s.hull || s.hull->minimum != model::NavVector3{-16, -16, -36} ||
		s.hull->maximum != model::NavVector3{16, 16, 36} || s.ducked != false || !p.end.isFinite() ||
		!p.normal.isFinite() || p.normal.z != 0 ||
		...
		(firstCommand && firstCommand->msec != msec) || (jumping && s.grounded != false) ||
		std::abs(double(s.position->z) - p.end.z) > 96 ||
		std::hypot(double(s.position->x) - p.end.x, double(s.position->y) - p.end.y) > 96)
		return result;
	if (s.map != indexMap)
		return fail(LadderExitReason::StaleNavigation);
	if (p.link.traversal != model::NavTraversalKind::Ladder ||
		(p.link.direction != enrichment::NavLinkDirection::Up &&
		 (!jumping || p.link.direction != enrichment::NavLinkDirection::Down)) ||
		!p.link.to.isValid())
		return fail(LadderExitReason::Unsupported);
	core::BotCommand neutral;
	neutral.msec = msec;
	if (!ladderAirStep(neutral, *s.velocity, physics))
		return result;
	...
	const auto flush = [&]() {
		...
		if (candidate.columnCount + nx * ny > candidate.columns.size())
			return false;
	...
	for (unsigned frame = 0; frame < 256 && (frame + 1) * unsigned(msec) <= 2000; ++frame)
	{
		...
		auto motor = core::Motor::command(intent, {}, static_cast<float>(physics.maximumSpeed),
										  std::uint64_t(msec) * 1000, true);
		if (!motor)
			return result;
		if (frame == 0 && firstCommand)
			motor.command = firstCommand;
		if (frame == 0)
		{
			candidate.suppliedFirstCommand = firstCommand.has_value();
			candidate.intent = firstCommand ? MovementIntent{} : intent;
			candidate.command = *motor.command;
		}
		...
		else if (attached)
		{
			...
			displacement = {static_cast<float>(v->x * dt), static_cast<float>(v->y * dt),
							static_cast<float>(v->z * dt)};
		}
		else
		{
		...
		if (!next.isFinite() || double(next.z) - p.end.z > 96)
			return fail(LadderExitReason::Unsupported);
		...
		if (landing)
		{
			...
			next = {static_cast<float>(position.x + displacement.x * fraction),
					static_cast<float>(position.y + displacement.y * fraction), p.end.z};
		}
		...
		{
			if (!flush())
				return fail(LadderExitReason::BudgetExceeded);
			low = high = position;
		}
		include(next);
		++candidate.simulatedFrames;
		if (landing)
		{
			const auto area = index.containing({next.x, next.y, next.z - 36}, 2);
			if (!area || !area.value->has_value() || (**area.value).areaId != p.link.to)
				return fail(LadderExitReason::NoLanding);
			if (!flush())
				return fail(LadderExitReason::BudgetExceeded);
			candidate.landing = next;
			result.reason = LadderExitReason::None;
...
LadderExitResult planUpperLadderExit(const LadderPlan& p, const runtime::MovementSnapshot& s, bool touching,
									 double releaseZ, LadderAirPhysics physics, std::uint8_t msec,
