...
enum class LocomotionPhase : std::uint8_t { Ground, Takeoff, Airborne, Drop, Hold };
...
struct MotionEnvelope {
    ...
    bool matches(const runtime::MovementSnapshot& s,std::uint64_t age) const noexcept {
        if(s.agent!=binding.agent || s.actor!=binding.actor || s.map!=binding.map ||
           !binding.routeGeneration || !tick.isValid() || !s.tick.isAfter(tick) ||
           s.kind!=runtime::ActorKind::ManagedBot || s.connected!=true ||
           s.alive!=true || s.joined!=true || !s.position || !s.position->isFinite() ||
           !s.hull || s.hull->minimum!=hull.minimum || s.hull->maximum!=hull.maximum ||
           !origin.isFinite() || !target.isFinite() || !validForUs ||
           validForUs>120000 || age>validForUs || !std::isfinite(maximumDisplacement) ||
           maximumDisplacement<=0) return false;
        return std::hypot(double(s.position->x)-origin.x,double(s.position->y)-origin.y)
            <=maximumDisplacement;
    }
};
