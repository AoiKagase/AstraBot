...
{
public:
	PathFollower(std::shared_ptr<const corridor::Corridor> corridor, query::NavQueryPoint goal) noexcept;
	FollowResult update(query::NavQueryPoint position, model::NavAreaId supportedArea, bool supportVerified,
						double lookAhead, double goalTolerance = 12.0) noexcept;
	bool completeSpecial(std::size_t expectedStep, model::NavAreaId supportedArea, bool supportVerified,
						 std::uint8_t transitions = 1) noexcept;
	std::size_t step() const noexcept
	{
		return step_;
	}
	const std::shared_ptr<const corridor::Corridor>& corridor() const noexcept
	{
		return corridor_;
	}
