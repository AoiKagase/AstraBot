...
ProbeReason floorReason(const runtime::FloorObservation& f, double minimumNormal) noexcept {
    ...
    case runtime::FloorObservationStatus::NavContainmentMissing: break; // NAV is advisory; validate physical floor below.
...
ProbeResult probe(const runtime::MovementSnapshot& s, std::uint64_t generation,
    ...
    if(distance>limits.maxDistance || count>limits.maxSamples || limits.maxQueries<1) return fail(ProbeReason::BudgetExceeded);
    ...
    const auto clearance=[&](model::NavVector3 a, model::NavVector3 b) {
        ...
        if(!reply->hull) { result.reason=ProbeReason::InvalidResult; return false; }
        const auto& hit=*reply->hull;
        if(!std::isfinite(hit.fraction) || hit.fraction<0 || hit.fraction>1 ||
           !hit.end.isFinite() || !hit.normal.isFinite()) {
            result.reason=ProbeReason::InvalidResult; return false;
        }
        if(hit.startSolid || hit.allSolid || hit.fraction<1) {
            result.reason=ProbeReason::Blocked; return false;
        }
        if(std::abs(double(hit.end.x)-b.x)>0.001 || std::abs(double(hit.end.y)-b.y)>0.001 ||
           std::abs(double(hit.end.z)-b.z)>0.001) {
            result.reason=ProbeReason::InvalidResult; return false;
        }
        result.reason=ProbeReason::None; return true;
    };
    // Candidate sensing never proves that the player's footprint or head fits.
    const auto support=[&](runtime::QueryKind kind, float px,float py,double top,double bottom)
        ->std::optional<runtime::FloorObservation> {
        if(!representable(top) || !representable(bottom) || top<=bottom) {
            result.reason=ProbeReason::InvalidInput; return {};
        }
        auto reply=fetch(kind,{px,py,static_cast<float>(top)},{px,py,static_cast<float>(bottom)});
        if(!reply) return {};
        if(!reply->floor) { result.reason=ProbeReason::QueryUnavailable; return {}; }
        auto f=*reply->floor;
    ...
    auto source=support(runtime::QueryKind::HullSupport,s.position->x,s.position->y,
                        feet+limits.supportTolerance,feet-limits.probeDepth);
    ...
    if(result.reason==ProbeReason::StartSolid || result.reason==ProbeReason::AllSolid) {
        ...
        source=support(runtime::QueryKind::HullSupport,s.position->x,s.position->y,
                       feet+0.03125,feet-limits.probeDepth);
        if(!source) return result;
        result.fallbackTrace=source->trace;
        if(result.reason==ProbeReason::None) result.supportFallbackAccepted=true;
    }
    if(result.reason!=ProbeReason::None) return fail(result.reason);
    auto floor=*source;
    ...
    if(!representable(sourceZ)) return fail(ProbeReason::InvalidInput);
    ...
    if(!clearance(*s.position,position)) return fail(result.reason);
    ...
    for(std::uint32_t i=0;i<samples;++i) {
        ...
        if(!candidate) return fail(result.reason);
        ...
        const bool rescue=candidateReason==ProbeReason::StartSolid ||
                          candidateReason==ProbeReason::AllSolid;
        if(rescue) result.supportFallbackAttempted=true;
        if(candidateReason!=ProbeReason::None && !rescue) return fail(candidateReason);
        auto landing=support(runtime::QueryKind::HullSupport,tx,ty,top,bottom);
        if(!landing) return fail(result.reason);
        if(result.reason!=ProbeReason::None) return fail(result.reason);
        const auto next=*landing;
        const double delta=double(next.height)-floor.height;
        if(delta>limits.maxStepUp) return fail(ProbeReason::Blocked);
        if(-delta>limits.maxDrop) return fail(ProbeReason::UnsafeDrop);
        ...
        if(!nextMatch) return fail(ProbeReason::QueryUnavailable);
        const auto nextArea=nextMatch.value && *nextMatch.value
            ? (**nextMatch.value).areaId : area;
        ...
        bool direct=false;
        if(!rescue) direct=clearance(position,destination);
        if(!direct) {
            if(!rescue && result.reason!=ProbeReason::Blocked) return fail(result.reason);
            const double liftZ=double(position.z)+limits.maxStepUp;
            if(!representable(liftZ)) return fail(ProbeReason::InvalidInput);
            ...
            ++result.steps;
            if(rescue) {
                result.supportFallbackAttempted=true; result.supportFallbackAccepted=true;
        ...
        position=destination; floor=next; area=nextArea; ++result.samples;
    }
