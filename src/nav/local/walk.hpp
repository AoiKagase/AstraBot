...
enum class MotionDisposition : std::uint8_t
{
	Execute,
	Hold,
	Recovery
};
enum class WalkReason
{
	None,
	InvalidInput,
	StaleTick,
...
enum class AvoidanceReason
{
	...
	CandidateCollapsed,
	CandidateBlocked,
	CandidateOffCorridor,
	BudgetExceeded
};
enum class ObstacleClass : std::uint8_t
{
	None,
	Step,
	Jumpable,
...
{
	None,
	PlannedLandingStaticFailure,
	BallisticLandingMismatch
};
enum class JumpProofPhase : std::uint8_t
{
	None,
	Prepare,
	Launch,
...
enum class JumpProofDeferReason : std::uint8_t
{
	None,
	ReservedQueries
};
struct WalkJumpLimits
{
	JumpLimits motion{};
	JumpGeometryLimits geometry{};
	JumpProbeLimits flight{};
};
struct WalkLimits
{
...
struct WalkDecision
{
	...
	double jumpFromTakeoff{}, jumpAlong{}, jumpLateral{}, jumpMinimumSpeed{}, jumpMaximumSpeed{}, jumpSpeedLimit{},
		jumpDesiredSpeed{}, jumpValidatedDistance{};
...
{
public:
	Walk(Binding, std::shared_ptr<const corridor::Corridor>, model::NavVector3 goal, WalkLimits,
		 JumpAttemptRegistry* jumpAttempts = nullptr) noexcept;
	WalkDecision update(const runtime::MovementSnapshot&, const query::NavSpatialIndex&, core::MapGeneration indexMap,
						runtime::IWorldQueries&, std::uint64_t nowUs = 0, std::uint32_t reservedQueries = 0,
	...
	WalkDecision recover(const runtime::MovementSnapshot&, const query::NavSpatialIndex&, core::MapGeneration,
						 runtime::IWorldQueries&, const RecoveryDecision&, std::uint32_t reservedQueries = 0) noexcept;
	...
	const corridor::Transition* activeTransition() const noexcept
	{
		return corridor_ && cursor_.index() < corridor_->transitions().size()
				   ? &corridor_->transitions()[cursor_.index()]
				   : nullptr;
	}
...
private:
	...
	WalkDecision updateJump(WalkDecision, const runtime::MovementSnapshot&, const query::NavSpatialIndex&,
							core::MapGeneration, runtime::IWorldQueries&, std::uint64_t, std::uint32_t,
	...
	WalkDecision updateDrop(WalkDecision, const runtime::MovementSnapshot&, const query::NavSpatialIndex&,
							core::MapGeneration, runtime::IWorldQueries&, std::uint64_t, std::uint32_t,
							std::optional<JumpPhysics>) noexcept;
	WalkDecision updateMotion(const runtime::MovementSnapshot&, const query::NavSpatialIndex&, core::MapGeneration,
							  runtime::IWorldQueries&, std::uint64_t, std::uint32_t, std::optional<JumpPhysics>,
							  std::optional<LadderObservation>) noexcept;
	WalkDecision updateDoor(WalkDecision, const runtime::MovementSnapshot&, const query::NavSpatialIndex&,
							core::MapGeneration, runtime::IWorldQueries&, model::NavVector3 end,
							std::uint64_t nowUs) noexcept;
	WalkDecision approachDoor(WalkDecision, const runtime::MovementSnapshot&, const query::NavSpatialIndex&,
							  core::MapGeneration, runtime::IWorldQueries&, const runtime::WorldQueryResult&) noexcept;
