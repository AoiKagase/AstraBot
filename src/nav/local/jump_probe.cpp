										   ...
										   const runtime::MovementSnapshot& s, TraversalConstraints hints) noexcept
{
	if (!hints || hints.noJump || physics.tick != s.tick || physics.binding.agent != s.agent ||
		physics.binding.actor != s.actor || physics.binding.map != s.map || !s.hull || !s.speedLimit ||
		!std::isfinite(*s.speedLimit) || *s.speedLimit <= 0 || !std::isfinite(physics.gravity) ||
	...
	{
		if (!physics.standingHull || !physics.crouchingHull || !s.ducked || !validHull(*physics.standingHull) ||
			!validHull(*physics.crouchingHull))
			return {};
		const auto& expected = *s.ducked ? *physics.crouchingHull : *physics.standingHull;
		if (s.hull->minimum != expected.minimum || s.hull->maximum != expected.maximum)
			return {};
	}
	...
	if (!std::isfinite(motion.maximumRise) || motion.maximumRise <= 0)
		return {};
	...
	if (hints.sourceDuck)
	{
		if (!std::isfinite(physics.crouchSpeedMultiplier) || physics.crouchSpeedMultiplier <= 0 ||
			physics.crouchSpeedMultiplier > 1)
			return {};
		const double speed = *s.speedLimit * physics.crouchSpeedMultiplier;
		motion.minimumSpeed = (std::min)(motion.minimumSpeed, speed * 0.8);
		motion.maximumDistance =
			(std::min)(motion.maximumDistance, speed * 2 * physics.verticalImpulse / physics.gravity);
	}
...
ProbeReason floorProbeReason(const runtime::FloorObservation& floor) noexcept
{
	...
	case runtime::FloorObservationStatus::NavContainmentMissing:
		return ProbeReason::NavContainmentMissing;
	...
	if (!floor.normal.isFinite() || floor.normal.z < 0.7f)
		return ProbeReason::UnsupportedFloor;
	return ProbeReason::None;
}
bool positive(double n) noexcept
{
...
{
public:
	PreparationQueries(runtime::IWorldQueries& port, const query::NavSpatialIndex& index, model::NavAreaId source,
					   double tolerance, std::uint32_t& issued) noexcept
	...
	{
		auto actual = request;
		actual.stamp.ordinal = ++issued_;
		auto reply = port_.query(actual);
		if (!(reply.stamp == actual.stamp))
		{
			reply.stamp = {};
			return reply;
		}
		...
		if (reply.error == runtime::QueryError::None && reply.kind == runtime::QueryKind::Floor && reply.floor)
		{
			...
			if (!match || !*match.value || (**match.value).areaId != source_)
				reply.floor.reset();
		}
		return reply;
	}
...
JumpProbeReason preparationReason(ProbeReason reason) noexcept
{
	switch (reason)
	{
	case ProbeReason::None:
		return JumpProbeReason::None;
	case ProbeReason::StaleNavigation:
		return JumpProbeReason::StaleNavigation;
	case ProbeReason::BudgetExceeded:
		return JumpProbeReason::BudgetExceeded;
	case ProbeReason::StaleQuery:
		return JumpProbeReason::StaleQuery;
	case ProbeReason::QueryUnavailable:
		return JumpProbeReason::QueryUnavailable;
	case ProbeReason::QueryFailed:
		return JumpProbeReason::QueryFailed;
	case ProbeReason::InvalidResult:
		return JumpProbeReason::InvalidResult;
	case ProbeReason::ActorNotGrounded:
		return JumpProbeReason::ActorNotGrounded;
	case ProbeReason::FloorHeightMismatch:
		return JumpProbeReason::FloorHeightMismatch;
	case ProbeReason::NavContainmentMissing:
		return JumpProbeReason::NavContainmentMissing;
	...
	case ProbeReason::UnsupportedFloor:
		return JumpProbeReason::NoSupport;
	case ProbeReason::NoSupport:
	case ProbeReason::UnsafeDrop:
		return JumpProbeReason::NoSupport;
	case ProbeReason::NoArea:
	case ProbeReason::WrongStartArea:
		return JumpProbeReason::WrongArea;
	case ProbeReason::Blocked:
		return JumpProbeReason::Blocked;
	default:
		return JumpProbeReason::InvalidInput;
	}
}
} // namespace
										 ...
										 model::NavVector3 landing, JumpLimits motion, JumpPhysics physics) noexcept
{
	...
	if (!origin.isFinite() || !velocity.isFinite() || !landing.isFinite() || !positive(physics.gravity) ||
		!positive(physics.verticalImpulse) || !positive(motion.maximumDistance) || !positive(motion.maximumRise) ||
		!positive(motion.landingRadius) || !motion.airborneTimeoutUs)
		return result;
	...
	if ((rise < 0 && !motion.flightHull) || rise > motion.maximumRise || !std::isfinite(discriminant) ||
		discriminant <= 0)
	{
		result.reason = JumpTrajectoryReason::BallisticSolution;
		return result;
	}
	...
	if (result.landingError > motion.landingRadius)
	{
		result.reason = JumpTrajectoryReason::LandingRadius;
		return result;
	}
	if (std::hypot(double(result.touchdown.x) - origin.x, double(result.touchdown.y) - origin.y) >
		motion.maximumDistance)
	{
		result.reason = JumpTrajectoryReason::MaximumDistance;
		return result;
	}
	result.reason = JumpTrajectoryReason::None;
	return result;
}
JumpProbeResult JumpProbe::land(const runtime::MovementSnapshot& s, Binding binding, JumpPlan plan, JumpLimits motion,
								GroundProbeLimits limits, const query::NavSpatialIndex& index,
								core::MapGeneration indexMap, runtime::IWorldQueries& port) noexcept
{
	...
	if (s.ducked != plan.flightHull.has_value() ||
		(plan.flightHull &&
		 (!s.hull || s.hull->minimum != plan.flightHull->minimum || s.hull->maximum != plan.flightHull->maximum)))
	...
	if (s.agent != binding.agent || s.actor != binding.actor || s.map != binding.map || !s.ducked ||
		...
		!positive(motion.landingRadius) || !positive(motion.supportTolerance) || !limits.maxQueries ||
		limits.maxQueries > 21 || limits.maxQueries > motion.maxQueries)
		return fail(JumpProbeReason::InvalidInput);
	...
	if (!support)
		return fail(preparationReason(support.reason));
	if (support.target->area != jumpLandingArea(plan))
		return fail(JumpProbeReason::WrongArea);
	...
	JumpInspection proof;
	proof.stamp = support.stamp;
	proof.step = binding.step;
	proof.queries = result.queries;
	proof.origin = *s.position;
	proof.hull = *s.hull;
	proof.support = support.target;
	proof.takeoff = plan.takeoff;
	proof.landing = plan.landing;
	result.inspection = proof;
	result.touchdown = *s.position;
	return result;
}
JumpProbeResult JumpProbe::prepare(const runtime::MovementSnapshot& s, Binding binding, JumpPlan plan,
								   JumpLimits motion, GroundProbeLimits limits, const query::NavSpatialIndex& index,
								   core::MapGeneration indexMap, runtime::IWorldQueries& port) noexcept
{
	JumpProbeResult result;
	const auto fail = [&](JumpProbeReason reason) {
		result.reason = reason;
		result.inspection.reset();
		if (result.provenance == JumpProofProvenance::None)
		{
			if (reason == JumpProbeReason::BudgetExceeded)
				result.provenance = JumpProofProvenance::QueryBudget;
			else if (reason == JumpProbeReason::StaleNavigation || reason == JumpProbeReason::StalePhysics ||
					 reason == JumpProbeReason::StaleQuery)
				result.provenance = JumpProofProvenance::Stale;
	...
	if (s.ducked != constraints(model::NavTraversalKind::Jump, plan.sourceAttributes, plan.targetAttributes).sourceDuck)
		return fail(JumpProbeReason::InvalidInput);
	if (s.agent != binding.agent || s.actor != binding.actor || s.map != binding.map || !s.ducked || !s.position ||
		!s.position->isFinite() || !plan.takeoff.isFinite() || !plan.landing.isFinite() || !plan.source.isValid() ||
		!plan.target.isValid() || plan.source == plan.target || !positive(motion.takeoffRadius) ||
		!positive(motion.approachSpeed) || !positive(motion.maximumSpeed) ||
		motion.approachSpeed > motion.maximumSpeed || motion.maximumSpeed > 400 || !positive(motion.maximumDistance) ||
		!positive(limits.maxDistance) || !limits.maxQueries || limits.maxQueries > 21 ||
		limits.maxQueries > motion.maxQueries)
		return fail(JumpProbeReason::InvalidInput);
	...
	if (length <= 0 || length > motion.maximumDistance)
		return fail(JumpProbeReason::InvalidInput);
	const auto support = GroundProbe::locate(s, binding.routeGeneration, index, indexMap, port, limits);
	result.supportReason = support.reason;
	result.supportInitialReason = support.initialReason;
	...
	if (!support)
		return fail(preparationReason(support.reason));
	if (support.target->area != plan.source)
		return fail(JumpProbeReason::WrongArea);
	double x = 0, y = 0;
	...
	{
		const double ux = (double(plan.landing.x) - plan.takeoff.x) / length,
					 uy = (double(plan.landing.y) - plan.takeoff.y) / length;
		...
		const double remaining =
			std::sqrt((std::max)(0.0, motion.takeoffRadius * motion.takeoffRadius - side * side)) - along;
		const double travel =
			(std::min)({motion.approachSpeed * 0.120, (std::max)(0.0, remaining), limits.maxDistance});
		...
		x = s.position->x + ux * travel;
		y = s.position->y + uy * travel;
	}
	...
	const auto path = GroundProbe::inspect(s, binding.routeGeneration, plan.source, static_cast<float>(x),
										   static_cast<float>(y), index, indexMap, queries, limits);
	result.supportReason = path.reason;
	if (!path)
		return fail(preparationReason(path.reason));
	if (path.target->area != plan.source)
		return fail(JumpProbeReason::WrongArea);
	JumpInspection proof;
	proof.stamp = support.stamp;
	proof.step = binding.step;
	...
	proof.hull = *s.hull;
	proof.support = support.target;
	proof.approach = path.target;
	proof.takeoff = plan.takeoff;
	proof.landing = plan.landing;
	proof.approachClear = proof.takeoffClear = true;
	...
	proof.validForUs =
		static_cast<std::uint64_t>((std::min)(120'000.0, proof.validatedDistance / motion.maximumSpeed * 1'000'000.0));
	result.inspection = proof;
	result.takeoffProof = JumpProof::Passed;
	result.provenance = JumpProofProvenance::GroundProbe;
	return result;
}
JumpProbeResult JumpProbe::launch(const runtime::MovementSnapshot& s, Binding binding, JumpPlan plan, JumpLimits motion,
								  JumpPhysics physics, JumpProbeLimits limits, const query::NavSpatialIndex& index,
								  core::MapGeneration indexMap, runtime::IWorldQueries& port) noexcept
{
	JumpProbeResult result;
	...
	if (!binding.agent.isValid() || !binding.actor.isValid() || !binding.map.isValid() || !binding.routeGeneration ||
		...
		s.grounded != true ||
		s.ducked !=
			constraints(model::NavTraversalKind::Jump, plan.sourceAttributes, plan.targetAttributes).sourceDuck ||
		!s.position || !s.position->isFinite() || !s.velocity || !s.velocity->isFinite() || !s.view ||
		!s.view->isFinite() || !s.speedLimit || !positive(*s.speedLimit) || !s.hull || !s.hull->minimum.isFinite() ||
		!s.hull->maximum.isFinite() || s.hull->minimum.x >= s.hull->maximum.x ||
		s.hull->minimum.y >= s.hull->maximum.y || s.hull->minimum.z >= s.hull->maximum.z || !plan.source.isValid() ||
		!plan.target.isValid() || plan.source == plan.target || !plan.takeoff.isFinite() || !plan.landing.isFinite())
		return fail(JumpProbeReason::InvalidInput);
	if (indexMap != s.map)
		return fail(JumpProbeReason::StaleNavigation);
	if (!same(binding, physics.binding) || physics.tick != s.tick)
		return fail(JumpProbeReason::StalePhysics);
	if (plan.flightHull && (!physics.crouchingHull || plan.flightHull->minimum != physics.crouchingHull->minimum ||
							plan.flightHull->maximum != physics.crouchingHull->maximum))
		return fail(JumpProbeReason::StalePhysics);
	if (!constraints(model::NavTraversalKind::Jump, plan.sourceAttributes, plan.targetAttributes))
		return fail(JumpProbeReason::UnsupportedConstraints);
	...
	if (length <= 0 || length > motion.maximumDistance || (plan.landing.z < plan.takeoff.z && !plan.flightHull) ||
		double(plan.landing.z) - plan.takeoff.z > motion.maximumRise)
		return fail(JumpProbeReason::InvalidInput);
	...
	const auto fetch =
		[&](runtime::QueryKind kind, model::NavVector3 start, model::NavVector3 end,
			std::optional<runtime::HullDimensions> hull = {}) -> std::optional<runtime::WorldQueryResult> {
		if (!start.isFinite() || !end.isFinite())
		{
			result.reason = JumpProbeReason::InvalidInput;
			return {};
		}
		if (result.queries == limits.maxQueries)
		{
		...
		runtime::QueryRequest q{stamp, kind, start, end, hull ? hull : s.hull, limits.navTolerance};
		q.stamp.ordinal = ++result.queries;
		try
		{
	...
	const auto ground = [&](JumpSupportRole role, model::NavVector3 origin, model::NavAreaId area,
		...
		if (!r->ground || !r->ground->floor)
		{
			...
			if (role == JumpSupportRole::Source)
				result.supportReason = ProbeReason::QueryUnavailable;
			result.reason = JumpProbeReason::QueryUnavailable;
			return {};
		}
		...
		if (role == JumpSupportRole::Source)
			result.supportInitialTrace = floor.trace;
		...
		if (role == JumpSupportRole::Source)
			result.supportInitialReason = supportReason;
		if (supportReason == ProbeReason::AllSolid || supportReason == ProbeReason::StartSolid)
		{
			...
			const auto fallback =
				fetch(runtime::QueryKind::Floor, {origin.x, origin.y, static_cast<float>(feet + tolerance)},
			...
			if (fallback && fallback->floor)
			{
				...
				if (role == JumpSupportRole::Source)
					result.supportFallbackTrace = fallbackFloor.trace;
				...
				if (evidence.fallbackReason == ProbeReason::None)
				{
					...
					supportReason = initialSupportReason;
			}
		}
		evidence.reason = supportReason;
		if (role == JumpSupportRole::Source)
			result.supportReason = supportReason;
		...
		if (!floor.supported || floor.normal.z < 0.7f ||
			std::abs(double(origin.z) + (hull ? hull->minimum.z : s.hull->minimum.z) - floor.height) >
				motion.supportTolerance)
		{
			evidence.reason = ProbeReason::FloorHeightMismatch;
			if (role == JumpSupportRole::Source)
				result.supportReason = ProbeReason::FloorHeightMismatch;
			result.reason = JumpProbeReason::FloorHeightMismatch;
			return {};
		}
		...
		if (!r->ground->area || !match || !*match.value)
		{
			...
			if (role == JumpSupportRole::Source)
				result.supportReason = ProbeReason::NavContainmentMissing;
			result.reason = JumpProbeReason::NavContainmentMissing;
			return {};
		}
		if (*r->ground->area != area || (**match.value).areaId != area)
		{
			...
			if (role == JumpSupportRole::Source)
				result.supportReason = ProbeReason::WrongStartArea;
			result.reason = JumpProbeReason::WrongArea;
			return {};
		}
	...
	const auto sweep = [&](model::NavVector3 start, model::NavVector3 end,
		...
		const auto r = fetch(runtime::QueryKind::SweptHull, start, end, hull);
		if (!r)
			return false;
		...
		else if (r->hull->startSolid || r->hull->fraction != 1)
		{
			...
			else
			{
				...
				result.provenance =
					r->blocker ? JumpProofProvenance::DynamicBlocker : JumpProofProvenance::QueryUnavailable;
			}
		}
	...
	const auto source = ground(JumpSupportRole::Source, *s.position, plan.source);
	if (!source)
		return fail(result.reason);
	const auto destination =
		ground(JumpSupportRole::PlannedLanding, plan.landing, jumpLandingArea(plan), plan.flightHull);
	if (!destination)
	{
		result.landingFailure = JumpLandingFailure::PlannedSupport;
		return fail(result.reason);
	}
	const double landingZ =
		double(destination->floor.height) - (plan.flightHull ? plan.flightHull->minimum.z : s.hull->minimum.z);
	const double rise = landingZ - s.position->z;
	const double discriminant = physics.verticalImpulse * physics.verticalImpulse - 2 * physics.gravity * rise;
	if (!representable(landingZ) || (rise < 0 && !plan.flightHull) || rise > motion.maximumRise ||
		!std::isfinite(discriminant) || discriminant <= 0)
	{
	...
	const auto trajectory =
		solveJumpTrajectory(*s.position, {s.velocity->x, s.velocity->y, 0}, expectedLanding, motion, physics);
	if (!trajectory)
	{
		result.landingFailure =
			trajectory.reason == JumpTrajectoryReason::LandingRadius
				? JumpLandingFailure::LandingRadius
		...
		if (trajectory.reason == JumpTrajectoryReason::LandingRadius)
		{
			...
			pending.stamp = stamp;
			pending.step = binding.step;
			pending.queries = result.queries;
			pending.origin = *s.position;
			pending.hull = *s.hull;
			pending.velocity = *s.velocity;
			pending.support = source;
			pending.approach = source;
			pending.takeoff = plan.takeoff;
			pending.landing = expectedLanding;
			pending.predictedLanding = trajectory.touchdown;
			pending.landingError = trajectory.landingError;
			pending.validatedDistance = motion.takeoffRadius;
			pending.validForUs = 120'000;
			pending.provenance = JumpProofProvenance::GroundProbe;
			result.inspection = pending;
			result.provenance = JumpProofProvenance::GroundProbe;
			result.touchdown = trajectory.touchdown;
			result.flightSeconds = trajectory.seconds;
			result.landingError = trajectory.landingError;
			result.trajectoryReady = false;
			result.reason = JumpProbeReason::None;
			return result;
		}
		return fail(JumpProbeReason::CannotLand);
	}
	...
	if (!std::isfinite(count) || count < 1 || count > limits.maxSegments || remaining < 2 ||
		count > (remaining - 2) / 2)
		return fail(JumpProbeReason::BudgetExceeded);
	...
	if (!positive(bulge) || bulge > limits.maxChordRise || bulge > hullHeight)
		return fail(JumpProbeReason::BudgetExceeded);
	if (!s.elapsedUs || s.elapsedUs > 120'000)
		return fail(JumpProbeReason::InvalidInput);
	...
	if (!projected.isFinite() || distance(projected, plan.takeoff) > motion.takeoffRadius)
		return fail(JumpProbeReason::OutsideTakeoff);
	const double validatedDistance =
		(std::hypot(double(s.velocity->x), double(s.velocity->y)) + *s.speedLimit) * commandDt;
	if (!sweep(*s.position, projected) || !sweep(touchdown, touchdown, plan.flightHull))
		return fail(result.reason);
	...
	for (std::uint32_t i = 1; i <= segments; ++i)
	{
		...
		if (!representable(px) || !representable(py) || !representable(pz))
			return fail(JumpProbeReason::InvalidInput);
		const auto next =
			i == segments ? touchdown
						  : model::NavVector3{static_cast<float>(px), static_cast<float>(py), static_cast<float>(pz)};
		...
		if (!representable(double(previous.z) + bulge) || !representable(double(next.z) + bulge))
			return fail(JumpProbeReason::InvalidInput);
		upperStart.z =
			std::nextafter(static_cast<float>(double(previous.z) + bulge), std::numeric_limits<float>::infinity());
		...
		if (!upperStart.isFinite() || !upperEnd.isFinite() || double(upperStart.z) - previous.z > hullHeight ||
			double(upperEnd.z) - next.z > hullHeight)
			return fail(JumpProbeReason::InvalidInput);
		const auto hull =
			(time * (i - 1) / segments >= 0.240) ? plan.flightHull : std::optional<runtime::HullDimensions>{};
		if (!sweep(previous, next, hull) || !sweep(upperStart, upperEnd, hull))
			return fail(result.reason);
		previous = next;
		++result.segments;
	}
	...
	proof.origin = *s.position;
	proof.hull = *s.hull;
	proof.velocity = *s.velocity;
	proof.support = source;
	...
	proof.validatedDistance = validatedDistance;
	proof.validForUs = s.elapsedUs;
	proof.predictedLanding = touchdown;
	proof.landingError = trajectory.landingError;
	proof.trajectoryReady = true;
	result.inspection = proof;
	result.touchdown = touchdown;
	result.flightSeconds = time;
	result.landingError = trajectory.landingError;
	result.trajectoryReady = true;
	result.takeoffProof = result.flightProof = result.landingProof = JumpProof::Passed;
	result.provenance = JumpProofProvenance::SweptHull;
	return result;
}
} // namespace astrabot::nav::local
