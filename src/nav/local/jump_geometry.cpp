...
{
	return {
		double(e.northWest.x) - h.minimum.x + radius + margin, double(e.southEast.x) - h.maximum.x - radius - margin,
		double(e.northWest.y) - h.minimum.y + radius + margin, double(e.southEast.y) - h.maximum.y - radius - margin};
}
bool fits(Region r) noexcept
{
...
std::optional<model::NavVector3> point(const model::NavExtent& e, double x, double y, float feetOffset) noexcept
{
	...
	if (!std::isfinite(z) || std::abs(z) > (std::numeric_limits<float>::max)())
		return {};
	p.z = static_cast<float>(z);
	return p;
}
JumpPlan plan(model::NavAreaId source, model::NavAreaId target, model::NavVector3 takeoff, model::NavVector3 landing,
			  std::uint8_t sourceAttributes, std::uint8_t targetAttributes,
			  ...
			  std::uint8_t landingAdvance) noexcept
{
	...
	result.landingArea = landingArea;
	result.landingAdvance = landingAdvance;
	return result;
}
} // namespace
JumpGeometryResult JumpGeometry::derive(const corridor::Corridor& path, Binding binding,
										const runtime::MovementSnapshot& s, JumpLimits motion,
										JumpGeometryLimits limits, bool observedObstacle) noexcept
{
	...
	if (!binding.agent.isValid() || !binding.actor.isValid() || !binding.map.isValid() || !binding.routeGeneration ||
		!s.tick.isValid() || !s.position || !s.position->isFinite() || !s.hull || !s.hull->minimum.isFinite() ||
		...
		!positive(motion.takeoffRadius) || !positive(motion.landingRadius) || !positive(motion.maximumDistance) ||
		!positive(motion.maximumRise) || !positive(limits.preferredDistance) ||
		limits.preferredDistance > motion.maximumDistance || !positive(limits.clearanceMargin))
		return fail(JumpGeometryReason::InvalidInput);
	if (s.agent != binding.agent || s.actor != binding.actor || s.map != binding.map ||
		...
		s.grounded != true || !s.ducked)
		return fail(JumpGeometryReason::InvalidActor);
	if (binding.step >= path.transitions().size())
		return fail(JumpGeometryReason::InvalidStep);
	...
	const auto hints = constraints(jumpEdge ? model::NavTraversalKind::Jump : t.edge.traversal, t.sourceAttributes,
								   t.targetAttributes);
	...
	if (t.edge.external)
	{
		...
		if (!std::isfinite(a.x) || !std::isfinite(a.y) || !std::isfinite(a.z) || !std::isfinite(b.x) ||
			!std::isfinite(b.y) || !std::isfinite(b.z) || !query::containsXY(t.sourceExtent, *s.position))
		...
		const double targetOffset = motion.standingHull && motion.flightHull
										? motion.standingHull->minimum.z - motion.flightHull->minimum.z
										: sourceOffset;
		const double rise = (b.z + targetOffset) - (a.z + sourceOffset);
		if (length <= 0 || length > motion.maximumDistance || (rise < 0 && !motion.flightHull) ||
			rise > motion.maximumRise)
			return fail((rise < 0 && !motion.flightHull) || rise > motion.maximumRise
							? JumpGeometryReason::HeightUnsupported
							: JumpGeometryReason::NoRoom);
		const model::NavVector3 takeoff{static_cast<float>(a.x), static_cast<float>(a.y),
										static_cast<float>(a.z + sourceOffset)};
		const model::NavVector3 landing{static_cast<float>(b.x), static_cast<float>(b.y),
										static_cast<float>(b.z + targetOffset)};
		JumpLandingRegion target{t.edge.target, t.targetExtent, double(b.x), double(b.x),
								 double(b.y),	double(b.y),	1,			 JumpLandingRegionKind::HullMargin};
		return {JumpGeometryReason::None,
				plan(t.edge.source, t.edge.target, takeoff, landing, t.sourceAttributes, t.targetAttributes,
					 motion.flightHull, t.edge.target, 1),
				JumpLandingEnvelope{target, {}}};
	}
	if (!query::containsXY(t.sourceExtent, *s.position))
		return fail(JumpGeometryReason::InvalidActor);
	...
	const auto candidateRegion = [&](const model::NavExtent& e, corridor::AreaFit fit, double radius, bool duck) {
		...
		const double inset = (std::min)({limits.clearanceMargin, (double(e.southEast.x) - e.northWest.x) / 4,
										 (double(e.southEast.y) - e.northWest.y) / 4});
	...
	if (!fits(source) || !fits(targetBounds))
		return fail(JumpGeometryReason::NoRoom);
	JumpLandingRegion target{t.edge.target,
							 t.targetExtent,
							 targetBounds.lowX,
							 targetBounds.highX,
							 targetBounds.lowY,
							 targetBounds.highY,
							 1,
							 t.targetFit == corridor::AreaFit::HullSafe && !hints.targetDuck
								 ? JumpLandingRegionKind::HullMargin
								 : JumpLandingRegionKind::CentreInset};
	...
	if (binding.step + 1 < path.transitions().size())
	{
		...
		if (ordinary)
		{
			const auto nextBounds =
				candidateRegion(next.targetExtent, next.targetFit, motion.landingRadius, nextHints.targetDuck);
			if (fits(nextBounds))
				envelope.successor = JumpLandingRegion{next.edge.target,
													   next.targetExtent,
													   nextBounds.lowX,
													   nextBounds.highX,
													   nextBounds.lowY,
	...
	const double low =
		vertical ? (std::max)(source.lowY, targetBounds.lowY) : (std::max)(source.lowX, targetBounds.lowX);
	const double high =
		vertical ? (std::min)(source.highY, targetBounds.highY) : (std::min)(source.highX, targetBounds.highX);
	if (low > high)
		return fail(JumpGeometryReason::NoRoom);
	...
	const double from =
		std::clamp(boundary - offset, vertical ? source.lowX : source.lowY, vertical ? source.highX : source.highY);
	...
	if (!a || !b || !inside(source, *a) || !inside(targetBounds, *b))
		return fail(JumpGeometryReason::InvalidGeometry);
	...
	if ((rise < 0 && !motion.flightHull) || rise > motion.maximumRise)
		return fail(JumpGeometryReason::HeightUnsupported);
	return {JumpGeometryReason::None,
			plan(t.edge.source, t.edge.target, *a, *b, t.sourceAttributes, t.targetAttributes, motion.flightHull,
				 t.edge.target, 1),
			envelope};
}
} // namespace astrabot::nav::local
