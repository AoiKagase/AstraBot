...
CrouchDecision Crouch::result(CrouchReason reason) const noexcept
{
	...
	out.state = state_;
	out.reason = reason;
	if (observedDuck_ || state_ == CrouchState::Lowering)
		out.intent.duck = ActionRequest::Hold;
	return out;
}
CrouchDecision Crouch::fail(CrouchReason reason) noexcept
{
	state_ = CrouchState::Failed;
	auto out = result(reason);
	out.accepted = out.terminalEvent = true;
	return out;
}
CrouchDecision Crouch::abort() noexcept
{
	if (state_ == CrouchState::Failed || state_ == CrouchState::Aborted)
		return result();
	state_ = CrouchState::Aborted;
	auto out = result(CrouchReason::Cancelled);
	out.accepted = out.terminalEvent = true;
	return out;
}
							  ...
							  runtime::IWorldQueries& port, std::uint32_t reserved, std::uint32_t maximum) noexcept
{
	...
	if (!binding_.agent.isValid() || !binding_.actor.isValid() || !binding_.map.isValid() ||
		!binding_.routeGeneration || !valid(limits_.standing) || !valid(limits_.crouched) ||
		!limits_.transitionTimeoutUs || limits_.standing.minimum.x != limits_.crouched.minimum.x ||
		limits_.standing.maximum.x != limits_.crouched.maximum.x ||
		limits_.standing.minimum.y != limits_.crouched.minimum.y ||
		limits_.standing.maximum.y != limits_.crouched.maximum.y ||
		double(limits_.standing.maximum.z) - limits_.standing.minimum.z <=
			double(limits_.crouched.maximum.z) - limits_.crouched.minimum.z)
	...
	{
		state_ = CrouchState::Aborted;
		auto out = result(CrouchReason::InvalidActor);
		out.accepted = out.terminalEvent = true;
		return out;
	}
	if (!s.tick.isValid() || (tick_.isValid() && !s.tick.isAfter(tick_)))
		return result(CrouchReason::StaleTick);
	if (tick_.isValid() && nowUs <= lastUs_)
		return fail(CrouchReason::InvalidInput);
	tick_ = s.tick;
	lastUs_ = nowUs;
	if (s.ducked)
		observedDuck_ = *s.ducked;
	...
	{
		waiting_ = false;
		state_ = required ? CrouchState::Crouched : CrouchState::Standing;
		auto out = result();
		out.accepted = out.movementAllowed = true;
		return out;
	}
	if (!waiting_)
	{
	...
	if (nowUs - startedUs_ >= limits_.transitionTimeoutUs)
		return fail(CrouchReason::TimedOut);
	...
	if (reserved >= maximum)
	{
		out = fail(CrouchReason::BudgetExceeded);
		return out;
	}
	...
	{
		out = fail(CrouchReason::QueryFailed);
		out.queries = 1;
		return out;
	}
	if (!(reply.stamp == q.stamp) || reply.kind != q.kind)
		out = fail(CrouchReason::StaleQuery);
	else if (reply.error != runtime::QueryError::None || !reply.clearance)
		out = fail(CrouchReason::QueryFailed);
	else if (!reply.clearance->clear)
	{
		if (required)
			out = fail(CrouchReason::Blocked);
		else
		{
			out = result(CrouchReason::Blocked);
			out.accepted = true;
		}
	}
	else
		out.intent.duck = required ? ActionRequest::Hold : ActionRequest::Release;
	out.queries = 1;
	return out;
}
} // namespace astrabot::nav::local
