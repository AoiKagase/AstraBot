...
{
	if (const auto rejected = begin(b, tick, nowUs))
		return *rejected;
...
BlockerDecision BlockerWait::update(const BlockerFeedback& f) noexcept
{
	...
	if (q.agent != b.agent || q.actor != b.actor || q.map != b.map || q.routeGeneration != b.routeGeneration)
		return finish(BlockerAction::Aborted, BlockerReason::InvalidBinding);
	if (const auto rejected = begin(b, q.tick, f.nowUs))
		return *rejected;
	...
	if ((obstacle.kind != runtime::BlockerKind::Geometry && !player && obstacle.kind != runtime::BlockerKind::Other) ||
	...
	return {yield ? BlockerAction::Yield : BlockerAction::InspectAvoidance, BlockerReason::None, true, false};
}
} // namespace astrabot::nav::local
