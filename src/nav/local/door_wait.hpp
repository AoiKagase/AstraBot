...
{
public:
	DoorWait(Binding binding, std::uint64_t timeoutUs) noexcept : binding_(binding), timeoutUs_(timeoutUs)
	{
	}
	DoorWaitDecision update(const DoorWaitFeedback&) noexcept;
	DoorWaitDecision abort() noexcept;
	DoorWaitState state() const noexcept
	{
		return state_;
	}
