...
FrameSchedule IntentPump::beginFrame(const runtime::MovementSnapshot& s, bool immediate) noexcept
{
	...
	{
		retired_ = true;
		stop(PumpReason::InvalidActor);
		return {false, false, reason_, 0};
	}
	if (!s.tick.isValid() || !s.tick.isAfter(tick_))
		return {false, false, PumpReason::DuplicateFrame, 0};
	tick_ = s.tick;
	frameUs_ = s.elapsedUs;
	due_ = false;
	published_ = false;
	taken_ = false;
	...
	if (!started_)
	{
		started_ = true;
		due_ = true;
		nextDecisionUs_ = timeUs_ + decisionPeriodUs;
	...
	if (timeUs_ >= nextDecisionUs_)
	{
		...
		due_ = true;
		return {true, true, PumpReason::None, missed};
	}
	if (immediate)
	{
		due_ = true;
...
bool IntentPump::publish(Binding binding, core::TickId tick, const MovementIntent& intent) noexcept
{
	if (!eligible_ || !due_ || published_ || taken_ || tick != tick_ || binding.agent != binding_.agent ||
		binding.actor != binding_.actor || binding.map != binding_.map ||
		binding.routeGeneration != binding_.routeGeneration || binding.step < binding_.step)
	{
		stop(PumpReason::StaleDecision);
		return false;
	}
	if (!core::Motor::valid(intent))
	{
		stop(PumpReason::InvalidIntent);
		return false;
	}
	binding_ = binding;
	intent_ = intent;
	intentTimeUs_ = timeUs_;
	hasIntent_ = true;
	first_ = true;
	reason_ = PumpReason::None;
	published_ = true;
	return true;
}
PumpOutput IntentPump::take() noexcept
{
	if (!eligible_ || taken_ || retired_)
		return {false, false, {}, reason_, tick_, frameUs_, 0, 0};
	taken_ = true;
	const auto age = hasIntent_ ? timeUs_ - intentTimeUs_ : 0;
	const auto lifetime = hasIntent_ ? freshnessLimit(intent_) : maxIntentAgeUs;
	if (hasIntent_ && age > lifetime)
		stop(PumpReason::StaleIntent);
