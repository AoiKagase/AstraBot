...
inline query::NavQueryResult supportedNavArea(
    const query::NavSpatialIndex& index, model::NavVector3 support,
    ...
    const double lateralTolerance = (std::max)(34.0,
        (std::max)(halfX, halfY) + (std::max)(limits.navTolerance, 18.0));
    // nearestGeometry's radius is 3D; constrain each XY component again below.
    const auto nearby = index.nearestGeometry(support,
        {std::hypot(lateralTolerance, lateralTolerance, vertical), vertical});
    if (!nearby) {
        return nearby;
    }
    if (!*nearby.value) {
    ...
    if (std::abs(p.x - support.x) > lateralTolerance ||
        std::abs(p.y - support.y) > lateralTolerance ||
