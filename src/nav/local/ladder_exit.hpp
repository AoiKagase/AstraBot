...
{
struct LadderClearanceColumn
{
	model::NavVector3 bottom{}, top{};
};
enum class LadderExitReason
{
	None,
	InvalidInput,
	StaleNavigation,
...
struct LadderExitResult
{
	...
	{
		return reason == LadderExitReason::None && value.has_value();
	}
};
...
LadderExitResult planUpperLadderExit(const LadderPlan&, const runtime::MovementSnapshot&, bool touching,
									 double releaseZ, LadderAirPhysics, std::uint8_t commandMsec,
...
LadderExitResult planJumpLadderExit(const LadderPlan&, const runtime::MovementSnapshot&, bool touching,
									LadderAirPhysics, std::uint8_t commandMsec, const query::NavSpatialIndex&,
									core::MapGeneration indexMap,
									std::optional<core::BotCommand> firstCommand = {}) noexcept;
} // namespace astrabot::nav::local
