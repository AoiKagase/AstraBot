...
{
struct JumpGeometryLimits
{
	double preferredDistance{}, clearanceMargin{};
};
enum class JumpGeometryReason
{
	None,
	InvalidInput,
	InvalidActor,
...
enum class JumpLandingRegionKind : std::uint8_t
{
	HullMargin,
	CentreInset,
...
struct JumpLandingRegion
{
	...
	{
		return area.isValid() && extent.isFinite() && lowX <= highX && lowY <= highY && cursorAdvance >= 1 &&
			   cursorAdvance <= 2;
	}
};
...
struct JumpGeometryResult
{
	...
	explicit operator bool() const noexcept
	{
		return reason == JumpGeometryReason::None && plan.has_value();
	}
};
...
public:
	...
	static JumpGeometryResult derive(const corridor::Corridor&, Binding, const runtime::MovementSnapshot&, JumpLimits,
									 JumpGeometryLimits, bool observedObstacle = false) noexcept;
};
} // namespace astrabot::nav::local
