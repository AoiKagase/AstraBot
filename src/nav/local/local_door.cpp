...
bool verified(const runtime::WorldQueryResult& r, const runtime::QueryRequest& q) noexcept
{
	return r.stamp == q.stamp && r.kind == runtime::QueryKind::Door && r.error == runtime::QueryError::None && r.door &&
		   r.door->id;
}
} // namespace
...
std::optional<WalkDecision> LocalDoor::update(const runtime::MovementSnapshot& s, Binding binding,
											  ...
											  std::uint32_t maximum) noexcept
{
	...
	if (!binding.agent.isValid() || !binding.actor.isValid() || !binding.map.isValid() || !binding.routeGeneration ||
		s.agent != binding.agent || s.actor != binding.actor || s.map != binding.map || s.map != indexMap ||
		!s.tick.isValid() || !s.position || !s.position->isFinite() || !s.hull || !desiredTarget.isFinite())
	{
	...
	{
		out.probeReason = ProbeReason::QueryFailed;
		return out;
	}
	if (!verified(r, q))
	{
	...
	if (door.canUse && !door.useView)
		return out;
	if (wait_ && (!same(binding, binding_) || door.id != doorId_))
		reset();
	if (!wait_)
	{
		if (!door.canUse && !door.canTouch)
			return out;
	...
	if (!passive_ || contactSent_)
		return out;

	if (!r.hull || r.hull->startSolid || !r.hull->end.isFinite() || !r.hull->normal.isFinite() ||
	...
	if (length <= 0 || length > probeLimits.maxDistance || std::abs(double(desiredTarget.z) - s.position->z) > 0.1)
	{
		out.probeReason = ProbeReason::InvalidInput;
		return out;
	}
	...
	if (distance < 0 || distance > length || std::abs(hx * uy - hy * ux) > 0.01 ||
		...
		std::abs(distance - length * r.hull->fraction) > 0.02 || r.hull->normal.x * ux + r.hull->normal.y * uy > -0.7 ||
	...
	const auto support = TerrainSampler::locate(s, binding.routeGeneration, index, indexMap, port, supportLimits);
	used += support.queries;
	out.queries += support.queries;
	out.samples = support.samples;
	out.steps = support.steps;
	...
	if (distance > 0.125)
	{
		...
		approachLimits.maxSamples =
			(std::min)(approachLimits.maxSamples,
					   probeLimits.maxSamples > out.samples ? probeLimits.maxSamples - out.samples : 0U);
		const auto travel =
			(std::min)(distance - 0.0625, double(approachLimits.maxSamples) * approachLimits.sampleSpacing);
		const auto x = static_cast<float>(s.position->x + ux * travel);
		const auto y = static_cast<float>(s.position->y + uy * travel);
		const auto approach = TerrainSampler::inspect(s, binding.routeGeneration, support.target->area, x, y, index,
													  indexMap, port, approachLimits);
		used += approach.queries;
		out.queries += approach.queries;
		out.samples += approach.samples;
		out.steps += approach.steps;
		...
		if (!core::Motor::bindLocomotion(out.intent, core::LocomotionMode::Run, double(s.speedLimit.value_or(250)),
										 travel))
		{
			out.intent = {};
			return out;
	...
	out.contact = DoorContact{
		door.id,
		{static_cast<float>(s.position->x + ux * 0.75), static_cast<float>(s.position->y + uy * 0.75), s.position->z}};
	contactSent_ = true;
	return out;
}
} // namespace astrabot::nav::local
