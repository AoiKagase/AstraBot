...
{
	return std::tie(a.from, a.to, a.entry.x, a.entry.y, a.entry.z, a.exit.x, a.exit.y, a.exit.z, a.traversal,
					a.direction, a.additionalCost) == std::tie(b.from, b.to, b.entry.x, b.entry.y, b.entry.z, b.exit.x,
															   b.exit.y, b.exit.z, b.traversal, b.direction,
															   b.additionalCost);
}
inline Error workingBudget(std::size_t areas, std::size_t links, const NavEnrichmentLimits& limits) noexcept
{
	if (links > limits.maxLinks)
		return linkError(K::CountLimitExceeded, F::LinkCount);
	if (links == 0)
		return {};
	std::size_t bytes = 0;
	...
	if (e.isNone())
		e = query::detail::charge(bytes, links, sizeof(std::size_t), maximum);
	if (!e.isNone())
		return linkError(e.kind, F::LinkWorkingBytes);
					  ...
					  const NavTraversalLinkSet& set, bool verifyFingerprint = true)
{
	...
	std::sort(areaOrder.begin(), areaOrder.end(), [&areas](auto a, auto b) { return areas[a].id < areas[b].id; });
	...
	{
		if (!l.sourceId)
			return linkError(K::InvalidValue, F::LinkSourceId);
		if (!l.generation)
			return linkError(K::InvalidValue, F::LinkGeneration);
		if (!l.linkId)
			return linkError(K::InvalidValue, F::LinkId);
		if (!l.from.value)
			return linkError(K::InvalidValue, F::LinkFrom);
		if (!exists(l.from))
			return linkError(K::DanglingReference, F::LinkFrom);
		if (!l.to.value)
			return linkError(K::InvalidValue, F::LinkTo);
		if (!exists(l.to))
			return linkError(K::DanglingReference, F::LinkTo);
		if (l.from == l.to)
			return linkError(K::InvalidValue, F::LinkTo);
		...
		if (!finite(l.entry))
			return linkError(K::InvalidGeometry, F::LinkEntry);
		if (!finite(l.exit))
			return linkError(K::InvalidGeometry, F::LinkExit);
	...
	std::sort(order.begin(), order.end(),
			  [&set](auto a, auto b) { return identity(set.links[a]) < identity(set.links[b]); });
	...
	for (std::size_t begin = 0; begin < order.size();)
	{
		...
		auto end = begin + 1;
		while (end < order.size() && set.links[order[end]].sourceId == first.sourceId)
			++end;
		...
		for (auto key = begin; key < end;)
		{
			...
			if (next > key + 1)
				return linkError(K::DuplicateId, F::LinkId);
			key = next;
		}
		begin = end;
	}
