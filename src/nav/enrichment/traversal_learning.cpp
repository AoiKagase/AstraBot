...
std::uint64_t stableLinkId(const TraversalObservation& observation) noexcept
{
	...
	for (const auto value : observation.fingerprint)
		add(hash, value);
...
{
	return std::tie(left.from, left.to, left.entry.x, left.entry.y, left.entry.z, left.exit.x, left.exit.y, left.exit.z,
					left.traversal, left.direction, left.additionalCost) ==
		   std::tie(right.from, right.to, right.entry.x, right.entry.y, right.entry.z, right.exit.x, right.exit.y,
					right.exit.z, right.traversal, right.direction, right.additionalCost);
}
...
{
	if (from.value == 0U || to.value == 0U || from == to || sequence == 0U || !finitePoint(entry) ||
		!finitePoint(exit) || !model::isKnownTraversalKind(traversal) || traversal == model::NavTraversalKind::Walk ||
		!validDirection(direction) || !finiteNonNegative(additionalCost) || !validActor(actor))
	{
		return false;
	}
	if (direction == NavLinkDirection::Up && !(exit.z > entry.z))
		return false;
	if (direction == NavLinkDirection::Down && !(exit.z < entry.z))
		return false;
...
{
	return maxCandidates != 0U && maxCandidates <= 1024U && finiteNonNegative(maxDistance) && maxDistance > 0.0 &&
		   maxDistance <= 4096.0 && minHumanAttempts != 0U && minHumanAttempts <= 1'000'000U &&
		   finiteNonNegative(minHumanSuccessRate) && minHumanSuccessRate <= 1.0;
}

bool TraversalLearningModel::activate(const NavMapFingerprint& fingerprint, std::uint64_t sourceId,
									  std::uint64_t generation) noexcept
{
	if (!settings_.valid() || sourceId == 0U || generation == 0U)
		return false;
...
TraversalLearningModel::Candidate* TraversalLearningModel::find(std::uint64_t linkId) noexcept
{
	const auto it =
		std::lower_bound(candidates_.begin(), candidates_.end(), linkId,
...
const TraversalLearningModel::Candidate* TraversalLearningModel::find(std::uint64_t linkId) const noexcept
{
	const auto it =
		std::lower_bound(candidates_.begin(), candidates_.end(), linkId,
						 [](const Candidate& candidate, std::uint64_t id) { return candidate.link.linkId < id; });
...
{
	if (candidate.humanAttempts < static_cast<double>(settings_.minHumanAttempts))
		return false;
	const double rate = candidate.humanAttempts > 0.0 ? candidate.humanSuccess / candidate.humanAttempts : 0.0;
	return std::isfinite(rate) && rate >= settings_.minHumanSuccessRate;
}

TraversalLearningUpdate TraversalLearningModel::observe(const TraversalObservation& observation) noexcept
{
	...
	NavTraversalLink link{sourceId_,
						  generation_,
						  linkId,
						  observation.from,
						  observation.to,
	...
	if (candidate == nullptr)
	{
		...
		if (observation.hasNormalConnection || distance(observation.entry, observation.exit) > settings_.maxDistance)
		{
			return {TraversalLearningReason::NonDiscovery, linkId, false};
		}
		...
		try
		{
			...
			const auto it =
				std::lower_bound(candidates_.begin(), candidates_.end(), linkId,
								 [](const Candidate& value, std::uint64_t id) { return value.link.linkId < id; });
	...
	if (observation.actor == TraversalActor::Human)
	{
		...
		if (observation.succeeded)
			candidate->humanSuccess += 1.0;
	}
	else
	{
		candidate->botAttempts += 1.0;
		if (observation.succeeded)
			candidate->botSuccess += 1.0;
	}
	return {TraversalLearningReason::Accepted, linkId, true};
}
...
NavTraversalLinkSet TraversalLearningModel::activeEnrichment() const
{
	...
	for (const auto& candidate : candidates_)
	{
		if (eligible(candidate))
			result.links.push_back(candidate.link);
	}
	return result;
}

bool TraversalLearningModel::experience(std::uint64_t linkId, query::AdaptiveTraversalExperience& result) const noexcept
{
	const auto* candidate = find(linkId);
	if (!candidate)
		return false;
	result = {linkId, candidate->humanAttempts, candidate->humanSuccess, candidate->botAttempts, candidate->botSuccess,
			  0.0};
	return true;
}
