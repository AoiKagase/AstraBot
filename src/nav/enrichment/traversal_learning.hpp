...
{
public:
	explicit TraversalLearningModel(TraversalLearningSettings settings = {}) noexcept : settings_(settings)
	{
	}

	bool activate(const NavMapFingerprint& fingerprint, std::uint64_t sourceId, std::uint64_t generation) noexcept;
	void reset() noexcept;
	...
	const TraversalLearningSettings& settings() const noexcept
	{
		return settings_;
	}
	NavTraversalLinkSet activeEnrichment() const;
	bool experience(std::uint64_t linkId, query::AdaptiveTraversalExperience& result) const noexcept;
