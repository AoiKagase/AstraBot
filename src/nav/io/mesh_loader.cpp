...
{
public:
	static std::shared_ptr<const model::NavMeshSnapshot> build(model::NavFileHeader header,
...
NavMeshLoader::load(ByteView bytes, const NavMeshReadLimits& limits) noexcept
{
	...
	try
	{
		...
		auto block = NavAreaReader::readTracked({bytes.data + context.base, bytes.size - context.base},
												header.value->version, header.value->areaCount, limits.areas, &context);
		if (!block)
		{
		...
		if (end != bytes.size)
			return Result::failure({NavErrorKind::TrailingData, end, NavRecord::RawInput, NavField::RawBytes});
		at = {NavErrorKind::AllocationFailure, 0, NavRecord::RawInput, NavField::RawBytes};
		return Result::success(detail::NavMeshBuilder::build(std::move(*header.value), std::move(block.value->areas)));
	}
