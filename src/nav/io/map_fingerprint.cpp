...
constexpr U rotate(U v, unsigned n) noexcept
{
	return (v >> n) | (v << (32 - n));
}
...
class Sha256
{
	...
	void compress() noexcept
	{
		...
		for (std::size_t i = 0; i < 16; ++i)
			w[i] =
				(U{block[4 * i]} << 24) | (U{block[4 * i + 1]} << 16) | (U{block[4 * i + 2]} << 8) | block[4 * i + 3];
		...
		auto a = state[0], b = state[1], c = state[2], d = state[3], e = state[4], f = state[5], g = state[6],
			 h = state[7];
		for (std::size_t i = 0; i < 64; ++i)
		{
			const U first =
				h + (rotate(e, 6) ^ rotate(e, 11) ^ rotate(e, 25)) + ((e & f) ^ (~e & g)) + constants[i] + w[i];
...
public:
	...
	enrichment::NavMapFingerprint finish(std::uint64_t bytes) noexcept
	{
		...
		for (unsigned i = 0; i < 8; ++i)
			put(static_cast<std::uint8_t>(bits >> (56 - 8 * i)));
...
MapFingerprintResult fingerprintMap(std::istream& input, std::uint64_t limit) noexcept
{
	...
	if (limit > 512ULL * 1024 * 1024)
	{
		result.reason = FingerprintReason::InvalidLimit;
		return result;
	}
	try
	{
		...
		{
			const auto amount =
				static_cast<std::streamsize>((std::min)(limit - result.bytes, std::uint64_t{buffer.size()}));
			input.read(buffer.data(), amount);
			const auto count = input.gcount();
			if (count < 0 || count > amount || input.bad() || (input.fail() && !input.eof()))
				return result;
			for (std::streamsize i = 0; i < count; ++i)
				sha.put(static_cast<std::uint8_t>(buffer[static_cast<std::size_t>(i)]));
		...
		if (!input.eof())
		{
			...
			{
				result.reason = FingerprintReason::SizeLimit;
				return result;
			}
			if (input.bad() || !input.eof())
				return result;
		}
		result.fingerprint = sha.finish(result.bytes);
		result.reason = FingerprintReason::None;
	}
	catch (...)
	{
		result.fingerprint.reset();
		result.reason = FingerprintReason::InputFailure;
	}
	return result;
}
} // namespace astrabot::nav::io
