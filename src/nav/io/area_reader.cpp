...
ReadResult<NavAreaBlock> NavAreaReader::read(ByteView bytes, model::NavVersion version, std::uint32_t areaCount,
											 const NavAreaReadLimits& limits) noexcept
{
	return readTracked(bytes, version, areaCount, limits, nullptr);
}
ReadResult<NavAreaBlock> NavAreaReader::readTracked(ByteView bytes, model::NavVersion version, std::uint32_t areaCount,
													const NavAreaReadLimits& limits,
													detail::DecodeContext* context) noexcept
{
	...
	NavError allocationContext{NavErrorKind::AllocationFailure, 0U, NavRecord::Area, NavField::AreaCount};
	try
	{
		NavAreaBlock block{};
		...
		for (std::uint32_t areaIndex = 0U; areaIndex < areaCount; ++areaIndex)
		{
			...
			const ReadResult<std::uint32_t> areaId = reader.readU32LE(NavRecord::Area, NavField::AreaId);
			if (!areaId)
			{
			...
			const ReadResult<std::uint8_t> attributes = reader.readU8(NavRecord::Area, NavField::Attributes);
			if (!attributes)
			{
			...
			const ReadResult<float> northWestX = reader.readF32LE(NavRecord::Area, NavField::NorthWestExtent);
			if (!northWestX)
			{
			...
			const ReadResult<float> northWestY = reader.readF32LE(NavRecord::Area, NavField::NorthWestExtent);
			if (!northWestY)
			{
			...
			const ReadResult<float> northWestZ = reader.readF32LE(NavRecord::Area, NavField::NorthWestExtent);
			if (!northWestZ)
			{
			...
			const ReadResult<float> southEastX = reader.readF32LE(NavRecord::Area, NavField::SouthEastExtent);
			if (!southEastX)
			{
			...
			const ReadResult<float> southEastY = reader.readF32LE(NavRecord::Area, NavField::SouthEastExtent);
			if (!southEastY)
			{
			...
			const ReadResult<float> southEastZ = reader.readF32LE(NavRecord::Area, NavField::SouthEastExtent);
			if (!southEastZ)
			{
			...
			const ReadResult<float> northEastZ = reader.readF32LE(NavRecord::Area, NavField::NorthEastZ);
			if (!northEastZ)
			{
			...
			const ReadResult<float> southWestZ = reader.readF32LE(NavRecord::Area, NavField::SouthWestZ);
			if (!southWestZ)
			{
			...
			for (std::size_t direction = 0U; direction < area.connections.size(); ++direction)
			{
				...
				const ReadResult<std::uint32_t> connectionCount =
					reader.readU32LE(NavRecord::Connection, NavField::ConnectionCount);
				if (!connectionCount)
				{
				...
				{
					return ReadResult<NavAreaBlock>::failure(
						NavError{NavErrorKind::CountLimitExceeded, static_cast<std::uint64_t>(countOffset),
				...
				allocationContext = NavError{NavErrorKind::AllocationFailure, static_cast<std::uint64_t>(countOffset),
											 NavRecord::Connection, NavField::ConnectionCount};
				area.connections[direction].reserve(*connectionCount.value);
				totalConnections += *connectionCount.value;
				for (std::uint32_t connectionIndex = 0U; connectionIndex < *connectionCount.value; ++connectionIndex)
				{
					const ReadResult<std::uint32_t> connectionId =
						reader.readU32LE(NavRecord::Connection, NavField::ConnectionAreaId);
					if (!connectionId)
					{
						return ReadResult<NavAreaBlock>::failure(connectionId.error);
					}
					area.connections[direction].push_back(model::NavConnection{model::NavAreaId{*connectionId.value}});
				}
			}

			const std::size_t hidingCountOffset = reader.offset();
			const ReadResult<std::uint8_t> hidingCount =
				reader.readU8(NavRecord::HidingSpot, NavField::HidingSpotCount);
			if (!hidingCount)
			{
			...
			{
				return ReadResult<NavAreaBlock>::failure(NavError{NavErrorKind::CountLimitExceeded,
																  static_cast<std::uint64_t>(hidingCountOffset),
																  NavRecord::HidingSpot, NavField::HidingSpotCount});
			}
			allocationContext = NavError{NavErrorKind::AllocationFailure, static_cast<std::uint64_t>(hidingCountOffset),
										 NavRecord::HidingSpot, NavField::HidingSpotCount};
			area.hidingSpots.reserve(*hidingCount.value);
			totalHidingSpots += *hidingCount.value;
			for (std::uint8_t hidingIndex = 0U; hidingIndex < *hidingCount.value; ++hidingIndex)
			{
				model::NavHidingSpot hidingSpot{};
				if (version != model::NavVersion::V1)
				{
					const ReadResult<std::uint32_t> hidingId =
						reader.readU32LE(NavRecord::HidingSpot, NavField::HidingSpotId);
					if (!hidingId)
					{
				...
				const ReadResult<float> hidingX = reader.readF32LE(NavRecord::HidingSpot, NavField::RawBytes);
				if (!hidingX)
				{
					return ReadResult<NavAreaBlock>::failure(hidingX.error);
				}
				hidingSpot.position.x = *hidingX.value;
				const ReadResult<float> hidingY = reader.readF32LE(NavRecord::HidingSpot, NavField::RawBytes);
				if (!hidingY)
				{
					return ReadResult<NavAreaBlock>::failure(hidingY.error);
				}
				hidingSpot.position.y = *hidingY.value;
				const ReadResult<float> hidingZ = reader.readF32LE(NavRecord::HidingSpot, NavField::RawBytes);
				if (!hidingZ)
				{
				...
				if (version != model::NavVersion::V1)
				{
					const ReadResult<std::uint8_t> hidingFlags =
						reader.readU8(NavRecord::HidingSpot, NavField::HidingSpotFlags);
					if (!hidingFlags)
					{
			...
			const ReadResult<std::uint8_t> approachCount = reader.readU8(NavRecord::Approach, NavField::ApproachCount);
			if (!approachCount)
			{
			...
			allocationContext =
				NavError{NavErrorKind::AllocationFailure, static_cast<std::uint64_t>(approachCountOffset),
						 NavRecord::Approach, NavField::ApproachCount};
			area.approaches.reserve(*approachCount.value);
			totalApproaches += *approachCount.value;
			for (std::uint8_t approachIndex = 0U; approachIndex < *approachCount.value; ++approachIndex)
			{
				model::NavApproachRecord approach{};
				const ReadResult<std::uint32_t> here = reader.readU32LE(NavRecord::Approach, NavField::ApproachAreaId);
				if (!here)
				{
					return ReadResult<NavAreaBlock>::failure(here.error);
				}
				approach.here = model::NavAreaId{*here.value};
				const ReadResult<std::uint32_t> previous =
					reader.readU32LE(NavRecord::Approach, NavField::ApproachAreaId);
				if (!previous)
				{
					return ReadResult<NavAreaBlock>::failure(previous.error);
				}
				approach.previous = model::NavAreaId{*previous.value};
				const ReadResult<std::uint8_t> previousToHere =
					reader.readU8(NavRecord::Approach, NavField::ApproachTraversal);
				if (!previousToHere)
				{
					return ReadResult<NavAreaBlock>::failure(previousToHere.error);
				}
				approach.previousToHereHow = *previousToHere.value;
				const ReadResult<std::uint32_t> next = reader.readU32LE(NavRecord::Approach, NavField::ApproachAreaId);
				if (!next)
				{
					return ReadResult<NavAreaBlock>::failure(next.error);
				}
				approach.next = model::NavAreaId{*next.value};
				const ReadResult<std::uint8_t> hereToNext =
					reader.readU8(NavRecord::Approach, NavField::ApproachTraversal);
				if (!hereToNext)
				{
			...
			const ReadResult<std::uint32_t> encounterCount =
				reader.readU32LE(NavRecord::Encounter, NavField::EncounterCount);
			if (!encounterCount)
			{
			...
			if (version != model::NavVersion::V1 && version != model::NavVersion::V2)
			{
				allocationContext =
					NavError{NavErrorKind::AllocationFailure, static_cast<std::uint64_t>(encounterCountOffset),
							 NavRecord::Encounter, NavField::EncounterCount};
			...
			const auto readEncounterVector = [&reader]() -> ReadResult<model::NavVector3> {
				...
				const ReadResult<float> x = reader.readF32LE(NavRecord::Encounter, NavField::RawBytes);
				if (!x)
				{
					return ReadResult<model::NavVector3>::failure(x.error);
				}
				vector.x = *x.value;
				const ReadResult<float> y = reader.readF32LE(NavRecord::Encounter, NavField::RawBytes);
				if (!y)
				{
					return ReadResult<model::NavVector3>::failure(y.error);
				}
				vector.y = *y.value;
				const ReadResult<float> z = reader.readF32LE(NavRecord::Encounter, NavField::RawBytes);
				if (!z)
				{
			...
			for (std::uint32_t encounterIndex = 0U; encounterIndex < *encounterCount.value; ++encounterIndex)
			{
				if (version == model::NavVersion::V1 || version == model::NavVersion::V2)
				{
					const ReadResult<std::uint32_t> from =
						reader.readU32LE(NavRecord::Encounter, NavField::EncounterAreaId);
					if (!from)
					{
						return ReadResult<NavAreaBlock>::failure(from.error);
					}
					const ReadResult<std::uint32_t> to =
						reader.readU32LE(NavRecord::Encounter, NavField::EncounterAreaId);
					if (!to)
					{
					...
					const ReadResult<std::uint8_t> spotCount =
						reader.readU8(NavRecord::Encounter, NavField::EncounterSpotCount);
					if (!spotCount)
					{
					...
					{
						return ReadResult<NavAreaBlock>::failure(
							NavError{NavErrorKind::CountLimitExceeded, static_cast<std::uint64_t>(spotCountOffset),
									 NavRecord::Encounter, NavField::EncounterSpotCount});
					}
					totalEncounterSpots += *spotCount.value;
					for (std::uint8_t spotIndex = 0U; spotIndex < *spotCount.value; ++spotIndex)
					{
						...
						const ReadResult<float> spotT = reader.readF32LE(NavRecord::Encounter, NavField::RawBytes);
						if (!spotT)
						{
				...
				const ReadResult<std::uint8_t> fromDirection =
					reader.readU8(NavRecord::Encounter, NavField::EncounterDirection);
				if (!fromDirection)
				{
					return ReadResult<NavAreaBlock>::failure(fromDirection.error);
				}
				encounter.fromDirection = *fromDirection.value;
				const ReadResult<std::uint32_t> to = reader.readU32LE(NavRecord::Encounter, NavField::EncounterAreaId);
				if (!to)
				{
					return ReadResult<NavAreaBlock>::failure(to.error);
				}
				encounter.to = model::NavAreaId{*to.value};
				const ReadResult<std::uint8_t> toDirection =
					reader.readU8(NavRecord::Encounter, NavField::EncounterDirection);
				if (!toDirection)
				{
				...
				allocationContext =
					NavError{NavErrorKind::AllocationFailure, static_cast<std::uint64_t>(spotCountOffset),
							 NavRecord::Encounter, NavField::EncounterSpotCount};
				...
				for (std::uint8_t spotIndex = 0U; spotIndex < *spotCount.value; ++spotIndex)
				{
					...
					const ReadResult<std::uint32_t> spotId =
						reader.readU32LE(NavRecord::Encounter, NavField::EncounterSpotId);
					if (!spotId)
					{
						return ReadResult<NavAreaBlock>::failure(spotId.error);
					}
					spot.hidingSpotId = *spotId.value;
					const ReadResult<std::uint8_t> spotT =
						reader.readU8(NavRecord::Encounter, NavField::EncounterSpotT);
					if (!spotT)
					{
						return ReadResult<NavAreaBlock>::failure(spotT.error);
					}
			...
			{
				const ReadResult<std::uint16_t> place = reader.readU16LE(NavRecord::Area, NavField::Place);
				if (!place)
				{
