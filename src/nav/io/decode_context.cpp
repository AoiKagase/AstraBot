...
NavError DecodeContext::observe(std::size_t offset, NavRecord record, NavField field, std::uint32_t value) noexcept
{
	...
	const bool keep =
		field == NavField::AreaId || field == NavField::NorthWestExtent || field == NavField::SouthEastExtent ||
		field == NavField::ConnectionCount || field == NavField::ConnectionAreaId || field == NavField::HidingSpotId ||
		field == NavField::ApproachAreaId || (field == NavField::EncounterAreaId && version >= 3) ||
		field == NavField::EncounterDirection || field == NavField::EncounterSpotId || field == NavField::Place;
	if (keep)
	{
		try
		{
