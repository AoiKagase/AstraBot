...
ByteReader::ByteReader(ByteView bytes, detail::DecodeContext* context) noexcept : bytes_(bytes), context_(context)
{
}
NavError ByteReader::observe(NavRecord record, NavField field, std::uint32_t value) noexcept
{
	...
	if (!result.isNone())
		result.offset = static_cast<std::uint64_t>(offset_);
	return result;
}

ReadResult<ByteView> ByteReader::availableBytes(std::size_t length, NavRecord record, NavField field) const noexcept
{
	if (!bytes_.isValid())
	{
...
ReadResult<ByteView> ByteReader::readBytes(std::size_t length, NavRecord record, NavField field) noexcept
{
...
ReadResult<std::uint8_t> ByteReader::readU8(NavRecord record, NavField field) noexcept
{
	...
	if (!error.isNone())
		return ReadResult<std::uint8_t>::failure(error);
	consume(1U);
...
std::uint16_t ByteReader::decodeU16LE(ByteView bytes) noexcept
{
	return static_cast<std::uint16_t>(bytes.data[0]) |
		   static_cast<std::uint16_t>(static_cast<std::uint16_t>(bytes.data[1]) << 8U);
}

std::uint32_t ByteReader::decodeU32LE(ByteView bytes) noexcept
{
	return static_cast<std::uint32_t>(bytes.data[0]) | (static_cast<std::uint32_t>(bytes.data[1]) << 8U) |
		   (static_cast<std::uint32_t>(bytes.data[2]) << 16U) | (static_cast<std::uint32_t>(bytes.data[3]) << 24U);
}

ReadResult<std::uint16_t> ByteReader::readU16LE(NavRecord record, NavField field) noexcept
{
	...
	const auto error = observe(record, field, value);
	if (!error.isNone())
		return ReadResult<std::uint16_t>::failure(error);
	consume(2U);
	return ReadResult<std::uint16_t>::success(value);
}

ReadResult<std::uint32_t> ByteReader::readU32LE(NavRecord record, NavField field) noexcept
{
	...
	const auto error = observe(record, field, value);
	if (!error.isNone())
		return ReadResult<std::uint32_t>::failure(error);
	consume(4U);
	return ReadResult<std::uint32_t>::success(value);
}

ReadResult<float> ByteReader::readF32LE(NavRecord record, NavField field) noexcept
{
	const ReadResult<ByteView> bytes = availableBytes(4U, record, field);
	if (!bytes)
	{
	...
	if (!error.isNone())
		return ReadResult<float>::failure(error);
	consume(4U);
