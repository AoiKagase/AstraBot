...
struct EquipmentSnapshot final
{
	...
	bool hasUsablePrimary() const noexcept
	{
		return primary != WeaponKind::None;
	}
...
{
public:
	BuyStrategyDecision chooseStrategy(const TeamEconomySnapshot& snapshot) const noexcept;
	BuyPlan buildPlan(const TeamEconomySnapshot& snapshot, std::size_t memberIndex) const noexcept;
...
private:
	...
	static bool isKitOwner(const TeamEconomySnapshot& snapshot, std::size_t memberIndex) noexcept;
