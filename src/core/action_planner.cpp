...
std::int32_t clampScore(std::int64_t value) noexcept
{
	...
	return static_cast<std::int32_t>((std::min)(lower, static_cast<std::int64_t>(kMaxScore)));
}
...
{
	return left.action == right.action && left.target == right.target && left.targetArea == right.targetArea &&
		   left.weapon == right.weapon && left.candidateReference == right.candidateReference;
}
...
std::int32_t roleFit(TacticalRole role, WeaponClass weapon) noexcept
{
	...
	case TacticalRole::Entry:
		return weapon == WeaponClass::Rifle || weapon == WeaponClass::SMG || weapon == WeaponClass::Shotgun ||
					   weapon == WeaponClass::MachineGun
				   ? 16
				   : 0;
	case TacticalRole::Support:
		return weapon == WeaponClass::Rifle || weapon == WeaponClass::Shotgun || weapon == WeaponClass::MachineGun ? 16
																												   : 5;
	case TacticalRole::Sniper:
		return weapon == WeaponClass::Sniper ? 24 : weapon == WeaponClass::Rifle ? 8 : 0;
	case TacticalRole::Anchor:
		return weapon == WeaponClass::Rifle || weapon == WeaponClass::Sniper || weapon == WeaponClass::MachineGun ? 14
																												  : 0;
...
{
	return input.weapon.secondaryUsable && closeThreat(input) && input.weapon.activeClass == WeaponClass::Sniper;
}
...
NavigationCandidate bestLocation(const ActionPlannerInput& input) noexcept
{
	...
	bool found = false;
	const auto limit = (std::min)(input.navigationCount, input.navigation.size());
	for (std::size_t i = 0; i < limit; ++i)
	{
		const auto& candidate = input.navigation[i];
		if (!candidate.valid() || !candidate.safe)
			continue;
		if (!found || candidate.exposure < best.exposure ||
			(candidate.exposure == best.exposure &&
			 (candidate.routeCost < best.routeCost ||
			  (candidate.routeCost == best.routeCost && candidate.stableId < best.stableId))))
		{
			best = candidate;
			found = true;
		}
...
bool betterCandidate(const CandidateDecision& left, const CandidateDecision& right) noexcept
{
...
void setExpiry(ActionIntent& intent, const ActionPlannerInput& input, const ActionPlannerSettings& settings,
...
CandidateDecision basicCandidate(ActionType action, const ActionPlannerInput& input,
								 const ActionPlannerSettings& settings, std::int32_t base) noexcept
{
...
{
	if (kind == ObjectiveKind::None && bomb != BombState::None)
		return false;
	if (bomb == BombState::Planted && kind != ObjectiveKind::Bomb)
		return false;
	return true;
}
...
bool EnemyBelief::valid(std::uint64_t nowMicros) const noexcept
{
	...
	if (!known)
		return !directVision && !target.isValid();
	return target.isValid() && finitePoint(lastKnownPosition) && observedMicros <= nowMicros;
}

bool WeaponState::valid() const noexcept
{
	return clipAmmo >= 0 && clipAmmo <= combat::kMaxAmmo && reserveAmmo >= 0 && reserveAmmo <= combat::kMaxAmmo;
}

std::int32_t WeaponState::totalAmmo() const noexcept
{
	return clipAmmo > combat::kMaxAmmo - reserveAmmo ? combat::kMaxAmmo : clipAmmo + reserveAmmo;
}

bool NavigationCandidate::valid() const noexcept
{
	return area.isValid() && finitePoint(position) && std::isfinite(distance) && std::isfinite(routeCost) &&
		   std::isfinite(exposure) && distance >= 0.0 && routeCost >= 0.0 && exposure >= 0.0 && exposure <= 1.0;
}

WeaponObservationError DroppedWeaponObservation::validate(MapGeneration currentMap,
														  perception::RoundGeneration currentRound, TickId currentTick,
														  std::uint64_t nowMicros,
														  std::uint64_t maxAgeMicros) const noexcept
{
	if (reference == 0)
		return WeaponObservationError::InvalidReference;
	if (!weapon.isValid())
		return WeaponObservationError::InvalidWeapon;
	if (!map.isValid() || !round.isValid() || !observedTick.isValid())
	{
		return WeaponObservationError::InvalidIdentity;
	}
	if (map != currentMap)
		return WeaponObservationError::StaleMap;
	if (round != currentRound)
		return WeaponObservationError::StaleRound;
	if (currentTick.isBefore(observedTick))
		return WeaponObservationError::StaleTick;
	...
	if (!finitePoint(position))
		return WeaponObservationError::NonFinitePosition;
	if (!area.isValid())
		return WeaponObservationError::InvalidArea;
	if (estimatedClipAmmo < 0 || estimatedClipAmmo > combat::kMaxAmmo || estimatedReserveAmmo < 0 ||
		estimatedReserveAmmo > combat::kMaxAmmo)
	{
...
bool operator==(const WeaponPickupEvaluation& left, const WeaponPickupEvaluation& right) noexcept
{
	return left.reference == right.reference && left.candidateWeapon == right.candidateWeapon &&
		   left.candidateClass == right.candidateClass && left.upgradeValue == right.upgradeValue &&
		   left.ammoNeed == right.ammoNeed && left.tacticalFit == right.tacticalFit && left.roleFit == right.roleFit &&
...
{
	if (action != requested || !validSituation || !alive)
		return false;
	switch (requested)
	{
	case ActionType::Reload:
...
{
	total = clampScore(static_cast<std::int64_t>(base) + threat + ammunition + objective + navigation + role + urgency -
					   risk);
}

bool ActionPlannerSettings::valid() const noexcept
{
	return maxCandidateAgeMicros != 0 && actionTimeoutMicros != 0 && peekExposureMicros != 0 &&
		   objectiveUrgencyMicros != 0 && maxWeaponCandidates <= kMaxWeaponPickupCandidates &&
		   maxNavigationCandidates <= kMaxNavigationCandidates && maxEvaluations != 0 && actionSwitchMargin >= 0 &&
		   std::isfinite(retreatHealthPercent) && retreatHealthPercent >= 0.0F && retreatHealthPercent <= 100.0F &&
...
{
	if (!settings.valid() || !map.isValid() || !round.isValid() || !tick.isValid() || !player.isValid() ||
		player.slot > perception::kPlayerCapacity || !agent.isValid())
	{
		return false;
	}
	if (!std::isfinite(healthPercent) || healthPercent < 0.0F || healthPercent > 100.0F ||
		!finitePoint(currentPosition) || !enemy.valid(nowMicros) || !weapon.valid() || !objective.valid())
	{
		return false;
	}
	if (navigationCount > navigation.size() || weaponCount > weapons.size())
		return false;
	return true;
}
...
WeaponPickupEvaluation evaluateWeaponPickup(const ActionPlannerInput& input, const WeaponPickupCandidate& candidate,
											const ActionPlannerSettings& settings) noexcept
{
	...
	const auto observationError = candidate.observation.validate(input.map, input.round, input.tick, input.nowMicros,
																 settings.maxCandidateAgeMicros);
	...
	if (!std::isfinite(candidate.distance) || !std::isfinite(candidate.routeCost) ||
		!std::isfinite(candidate.routeDanger) || !std::isfinite(candidate.exposure) || candidate.distance < 0.0 ||
		candidate.routeCost < 0.0 || candidate.routeDanger < 0.0 || candidate.routeDanger > 1.0 ||
		candidate.exposure < 0.0 || candidate.exposure > 1.0 || !candidate.safeRoute)
	{
		result.failure = WeaponAcquisitionFailure::NoRoute;
		return result;
	}
	if (candidate.timeRequiredMicros > input.objective.remainingMicros && input.objective.remainingMicros != 0)
	{
	...
	result.objectiveDelay = input.objective.kind == ObjectiveKind::None
								? 0
								: riskFrom(static_cast<double>(candidate.timeRequiredMicros) / 1'000'000.0, 4.0);
	result.swapCost =
		input.weapon.active.isValid() && input.weapon.activeClass != candidate.observation.weaponClass ? 4 : 1;
	result.totalUtility =
		clampScore(static_cast<std::int64_t>(result.upgradeValue) + result.ammoNeed + result.tacticalFit +
				   result.roleFit - result.travelRisk - result.exposureRisk - result.objectiveDelay - result.swapCost);
										 ...
										 const ActionPlannerSettings& settings) noexcept
{
	...
	const auto limit =
		(std::min)({input.weaponCount, static_cast<std::size_t>(settings.maxWeaponCandidates), input.weapons.size()});
	const auto evaluationLimit = (std::min)(limit, static_cast<std::size_t>(settings.maxEvaluations));
	for (std::size_t i = 0; i < evaluationLimit; ++i)
	{
		...
		if (!evaluation.actionable ||
			(result.found && (evaluation.totalUtility < result.evaluation.totalUtility ||
...
ActionDecision ActionPlanner::invalidDecision(const ActionPlannerInput& input, ActionAbortReason reason) noexcept
{
	clear(reason);
...
ActionDecision ActionPlanner::holdDecision(const ActionPlannerInput& input) noexcept
{
	...
	decision.precondition = {ActionType::Hold,
							 true,
							 true,
							 input.enemy.known,
							 ...
							 input.objective.kind != ObjectiveKind::None,
							 false,
							 secondarySolvesImmediateProblem(input),
							 true};
...
{
	if (!active_ || current_.action != ActionType::AcquireWeapon)
		return true;
	...
	for (std::size_t i = 0; i < limit; ++i)
	{
		if (input.weapons[i].observation.reference == current_.candidateReference &&
			...
			input.weapons[i].observation.validate(input.map, input.round, input.tick, input.nowMicros,
...
bool ActionPlanner::currentStillValid(const ActionPlannerInput& input) const noexcept
{
	...
	case ActionType::Reload:
		return !input.weapon.reloading && input.weapon.canReload && input.weapon.reserveAmmo > 0 &&
	...
	case ActionType::Peek:
		return input.enemy.known && !input.enemy.directVision && input.enemy.target == current_.target;
	case ActionType::Plant:
	...
	case ActionType::RescueHostage:
		return input.objective.kind == ObjectiveKind::Hostage && input.objective.canRescueHostage;
	case ActionType::EscortHostage:
		return input.objective.kind == ObjectiveKind::Hostage && input.objective.canEscortHostage;
	case ActionType::AcquireWeapon:
	{
		const auto limit = (std::min)(input.weaponCount, input.weapons.size());
		for (std::size_t i = 0; i < limit; ++i)
...
{
	if (active_ && reason != ActionAbortReason::None)
		lastAbortReason_ = reason;
	active_ = false;
	current_ = {};
	currentScore_ = {};
	currentPrecondition_ = {};
...
ActionDecision ActionPlanner::decide(const ActionPlannerInput& input) noexcept
{
	...
	if (!input.alive)
		return invalidDecision(input, ActionAbortReason::Dead);
	...
	auto addObjective = [&]() {
		if (input.objective.bomb == BombState::Planted && (input.objective.canDefuse || input.objective.hasDefuseKit))
		{
			...
			candidate.precondition.enoughTime =
				input.objective.remainingMicros == 0 || input.objective.estimatedActionMicros == 0 ||
				input.objective.estimatedActionMicros <= input.objective.remainingMicros;
			candidate.precondition.satisfied = candidate.precondition.enoughTime;
			candidate.score.objective = 140;
			candidate.score.urgency = input.objective.urgent(settings_.objectiveUrgencyMicros) ? 120 : 40;
		...
		if (input.objective.bomb == BombState::Dropped && input.objective.canRecoverBomb)
		{
			...
			candidate.precondition.enoughTime =
				input.objective.remainingMicros == 0 || input.objective.estimatedActionMicros == 0 ||
				input.objective.estimatedActionMicros <= input.objective.remainingMicros;
			candidate.precondition.satisfied = candidate.precondition.safeRoute && candidate.precondition.enoughTime;
			candidate.score.objective = 150;
			candidate.score.navigation = candidate.precondition.safeRoute ? 20 : -100;
			candidate.score.recompute();
			if (candidate.precondition.satisfied)
				add(candidate);
		}
		...
		if (input.objective.kind == ObjectiveKind::Hostage && input.objective.canRescueHostage)
		{
		...
		if (input.objective.kind == ObjectiveKind::Hostage && input.objective.canEscortHostage)
		{
	...
	const auto location = bestLocation(input);
	if (input.healthPercent <= settings_.retreatHealthPercent && input.enemy.pressure && hasLocation(location))
	{
	...
	if (selection.found && !input.objective.urgent(settings_.objectiveUrgencyMicros) && !input.enemy.directVision)
	{
		...
		setExpiry(candidate.intent, input, settings_,
				  (std::max)(settings_.actionTimeoutMicros, pickup.timeRequiredMicros + settings_.actionTimeoutMicros));
		...
		candidate.score.risk = selection.evaluation.travelRisk + selection.evaluation.exposureRisk +
		...
		candidate.acquisition = {pickup.observation.reference, pickup.observation.weapon, pickup.observation.area,
								 pickup.observation.position,  pickup.routeCost,		  input.nowMicros};
	...
	{
		const bool critical = actionPriority(decision.intent.action) > actionPriority(current_.action) + 20 ||
							  decision.score.total > currentScore_.total + settings_.actionSwitchMargin;
		if (!critical && currentPrecondition_.validFor(current_.action) && currentStillValid(input))
		{
		...
		if (transition == ActionAbortReason::None)
		{
			decision.transitionReason =
				critical ? ActionAbortReason::BetterActionCritical : ActionAbortReason::Replaced;
		}
	...
	lastAbortReason_ = ActionAbortReason::None;
	if (!active_ || decision.changed)
		activate(decision);
	return decision;
}

ActionResult ActionPlanner::update(const ActionPlannerInput& input, const ActionObservation& observation) noexcept
{
	if (!settings_.valid() || !input.valid(settings_))
	{
		clear(ActionAbortReason::InvalidInput);
		return {ActionResultState::Rejected, ActionType::NoAction, ActionAbortReason::InvalidInput, 0, true};
	}
	if (!active_)
	{
		...
		{
			return {ActionResultState::Rejected, decision.intent.action, decision.transitionReason, 0, true};
		}
	}
	const auto elapsed = input.nowMicros >= activeSinceMicros_ ? input.nowMicros - activeSinceMicros_ : 0;
	const auto action = current_.action;
	if (observation.actionComplete)
	{
		clear(ActionAbortReason::Completed);
		return {ActionResultState::Complete, action, ActionAbortReason::Completed, elapsed, true};
	}
	if (input.nowMicros >= current_.expiresMicros)
	{
		clear(ActionAbortReason::TimedOut);
		return {ActionResultState::Aborted, action, ActionAbortReason::TimedOut, elapsed, true};
	}
	if (observation.candidatePickedUp)
	{
		clear(ActionAbortReason::CandidatePickedUp);
		return {ActionResultState::Aborted, action, ActionAbortReason::CandidatePickedUp, elapsed, true};
	}
	if (action == ActionType::AcquireWeapon && !observation.candidatePresent)
	{
		clear(ActionAbortReason::CandidateGone);
		return {ActionResultState::Aborted, action, ActionAbortReason::CandidateGone, elapsed, true};
	}
	if (!observation.routeSafe)
	{
		clear(ActionAbortReason::RouteUnsafe);
		return {ActionResultState::Aborted, action, ActionAbortReason::RouteUnsafe, elapsed, true};
	}
	if (!observation.enoughTime)
	{
		clear(ActionAbortReason::InsufficientTime);
		return {ActionResultState::Aborted, action, ActionAbortReason::InsufficientTime, elapsed, true};
	}
	if (observation.objectiveUrgent)
	{
		clear(ActionAbortReason::ObjectiveUrgent);
		return {ActionResultState::Aborted, action, ActionAbortReason::ObjectiveUrgent, elapsed, true};
	}
	if (observation.enemyAppeared)
	{
		clear(ActionAbortReason::EnemyAppeared);
		return {ActionResultState::Aborted, action, ActionAbortReason::EnemyAppeared, elapsed, true};
	}
	if (observation.threatChanged)
	{
		clear(ActionAbortReason::ThreatChanged);
		return {ActionResultState::Aborted, action, ActionAbortReason::ThreatChanged, elapsed, true};
	}
	if (observation.betterActionCritical)
	{
		clear(ActionAbortReason::BetterActionCritical);
		return {ActionResultState::Aborted, action, ActionAbortReason::BetterActionCritical, elapsed, true};
	}
...
const char* actionName(ActionType action) noexcept
{
	...
	case ActionType::RecoverBomb:
		return "RecoverBomb";
	case ActionType::RescueHostage:
		return "RescueHostage";
	case ActionType::EscortHostage:
		return "EscortHostage";
	case ActionType::AcquireWeapon:
...
const char* actionAbortReasonName(ActionAbortReason reason) noexcept
{
	switch (reason)
	{
	case ActionAbortReason::None:
		return "None";
	case ActionAbortReason::InvalidInput:
		return "InvalidInput";
	case ActionAbortReason::Dead:
		return "Dead";
	case ActionAbortReason::Replaced:
		return "Replaced";
	case ActionAbortReason::TimedOut:
		return "TimedOut";
	case ActionAbortReason::EnemyAppeared:
		return "EnemyAppeared";
	case ActionAbortReason::ThreatChanged:
		return "ThreatChanged";
	case ActionAbortReason::ObjectiveUrgent:
		return "ObjectiveUrgent";
	case ActionAbortReason::CandidateGone:
		return "CandidateGone";
	case ActionAbortReason::CandidatePickedUp:
		return "CandidatePickedUp";
	case ActionAbortReason::RouteUnsafe:
		return "RouteUnsafe";
	case ActionAbortReason::InsufficientTime:
		return "InsufficientTime";
	case ActionAbortReason::BetterActionCritical:
		return "BetterActionCritical";
	case ActionAbortReason::NoSafeRoute:
		return "NoSafeRoute";
	case ActionAbortReason::NoTarget:
		return "NoTarget";
	case ActionAbortReason::Completed:
		return "Completed";
	}
	return "Unknown";
}
