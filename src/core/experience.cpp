...
{
	return std::any_of(hash.begin(), hash.end(), [](std::uint8_t value) { return value != 0U; });
}
...
public:
	...
	bool u32(std::uint32_t& value) noexcept
	{
		if (bytes_.size() - offset_ < 4U)
			return false;
		value = static_cast<std::uint32_t>(bytes_[offset_]) | (static_cast<std::uint32_t>(bytes_[offset_ + 1U]) << 8U) |
...
bool readMap(Reader& reader, MapIdentity& map) noexcept
{
	std::uint8_t bspFlag{};
	std::uint8_t navFlag{};
	if (!reader.string(map.name) || !reader.u64(map.bspBytes) || !reader.u8(bspFlag) || bspFlag > 1U ||
		!reader.bytes(map.bspHash.data(), map.bspHash.size()) || !reader.u32(map.navFormatVersion) ||
		!reader.u8(navFlag) || navFlag > 1U || !reader.bytes(map.navHash.data(), map.navHash.size()))
	{
		return false;
	}
...
{
	return reader.u32(area.area) && reader.doubleValue(area.visits) && reader.doubleValue(area.dangerT) &&
		   ...
		   reader.doubleValue(area.botTraffic) && reader.doubleValue(area.humanVisits) &&
		   reader.doubleValue(area.botVisits) && reader.doubleValue(area.damageDealt) &&
		   reader.doubleValue(area.damageReceived) && reader.doubleValue(area.kills) &&
		   reader.doubleValue(area.deaths) && reader.doubleValue(area.attackSuccess) &&
		   reader.doubleValue(area.attackFailure) && area.valid();
}

bool readLegacyArea(Reader& reader, AreaExperience& area) noexcept
{
	if (!reader.u32(area.area))
		return false;
	if (!reader.doubleValue(area.visits) || !reader.doubleValue(area.dangerT) || !reader.doubleValue(area.dangerCT) ||
...
std::vector<std::uint8_t> encode(const ExperienceSnapshot& snapshot)
{
	...
	for (const auto& area : snapshot.areas)
		putArea(payload, area);
	return payload;
}

PersistenceStatus decode(const std::vector<std::uint8_t>& bytes, const MapIdentity& expected,
						 ExperienceSnapshot& snapshot)
{
	if (bytes.size() < kHeaderBytes || !std::equal(std::begin(kMagic), std::end(kMagic) - 1, bytes.begin()))
	{
	...
	if (!header.bytes(magic, sizeof(magic)) || !header.u32(format) || !header.u32(schema) ||
		!header.u64(payloadBytes) || !header.u64(expectedChecksum) || format != kCurrentFormatVersion)
	{
		return format == kCurrentFormatVersion ? PersistenceStatus::Corrupt : PersistenceStatus::UnsupportedVersion;
	}
	if (schema != 1U && schema != kCurrentSchemaVersion)
		return PersistenceStatus::SchemaMismatch;
	...
	if (!readMap(reader, decoded.map) || !reader.u64(decoded.round.value) || !reader.u64(decoded.timeMicros) ||
		!reader.doubleValue(decoded.totals.damageDealt) || !reader.doubleValue(decoded.totals.damageReceived) ||
		!reader.doubleValue(decoded.totals.kills) || !reader.doubleValue(decoded.totals.deaths) ||
		!reader.doubleValue(decoded.totals.roundWins) || !reader.doubleValue(decoded.totals.roundLosses))
	{
		return PersistenceStatus::Corrupt;
	}
	...
	if (reader.remaining() != 0U || !decoded.valid())
		return PersistenceStatus::Corrupt;
	if (decoded.map != expected)
		return PersistenceStatus::MapMismatch;
...
{
	return !name.empty() && name.size() <= kMapNameLimit && name.find('\0') == std::string::npos &&
		   (!hasBspHash || validHash(bspHash)) && (!hasNavHash || validHash(navHash));
}
...
bool ExperienceEvent::valid() const noexcept
{
	...
	const bool actorKindValid = actorKind == ActorKind::Unknown || actorKind == ActorKind::Human ||
								actorKind == ActorKind::Bot || actorKind == ActorKind::OtherBot;
	...
	return map.valid() && round.isValid() && tick.isValid() && kindValid && actorKindValid &&
		   (!requiresArea() || area != 0U) && (!actorRequired || actor.isValid()) && std::isfinite(amount) &&
		   amount >= 0.0 && amount <= 1'000'000.0 &&
...
{
	return finiteNonNegative(humanWeight) && finiteNonNegative(botWeight) && finiteNonNegative(otherBotWeight) &&
		   finiteNonNegative(unknownWeight) && humanWeight <= 100.0 && botWeight <= 100.0 && otherBotWeight <= 100.0 &&
		   unknownWeight <= 100.0 && decayDenominator != 0U && decayNumerator <= decayDenominator &&
		   maxDecayRounds != 0U;
}
...
double ExperienceSettings::weight(ActorKind kind) const noexcept
{
	...
	case ActorKind::Bot:
		return botWeight;
	case ActorKind::OtherBot:
		return otherBotWeight;
...
bool AreaExperience::valid() const noexcept
{
	...
	const std::array values{visits,		   dangerT,		 dangerCT,		encounterRate, deathRate,
							grenadeThreat, sniperThreat, pushSuccess,	retakeSuccess, humanTraffic,
							botTraffic,	   humanVisits,	 botVisits,		damageDealt,   damageReceived,
							kills,		   deaths,		 attackSuccess, attackFailure};
...
void AreaExperience::decay(double factor) noexcept
{
	...
	dangerT *= factor;
	dangerCT *= factor;
	encounterRate *= factor;
	deathRate *= factor;
	grenadeThreat *= factor;
	sniperThreat *= factor;
	pushSuccess *= factor;
	retakeSuccess *= factor;
	humanTraffic *= factor;
	botTraffic *= factor;
	humanVisits *= factor;
	botVisits *= factor;
	damageDealt *= factor;
	damageReceived *= factor;
	kills *= factor;
	deaths *= factor;
	attackSuccess *= factor;
	attackFailure *= factor;
}

bool ExperienceTotals::valid() const noexcept
{
	return finiteNonNegative(damageDealt) && finiteNonNegative(damageReceived) && finiteNonNegative(kills) &&
		   finiteNonNegative(deaths) && finiteNonNegative(roundWins) && finiteNonNegative(roundLosses);
}
...
{
	if (!map.valid() || !round.isValid() || !totals.valid() || areas.size() > kMaxExperienceAreas)
		return false;
	for (std::size_t i = 0; i < areas.size(); ++i)
	{
		if (!areas[i].valid() || (i != 0U && areas[i - 1U].area >= areas[i].area))
			return false;
	}
	return true;
}

bool ExperienceModel::activate(const MapIdentity& map, perception::RoundGeneration round) noexcept
{
	if (!settings_.valid() || !map.valid() || !round.isValid())
		return false;
	try
	{
		map_ = map;
...
{
	if (round.value < round_.value)
		return {ExperienceUpdateReason::StaleRound, 0U, false};
	const auto gap = round.value - round_.value;
	const auto steps = (std::min)(gap, static_cast<std::uint64_t>(settings_.maxDecayRounds));
	const double factor =
		static_cast<double>(settings_.decayNumerator) / static_cast<double>(settings_.decayDenominator);
...
ExperienceUpdateResult ExperienceModel::beginRound(perception::RoundGeneration round, std::uint64_t timeMicros) noexcept
{
	if (!active_)
		return {ExperienceUpdateReason::Inactive, 0U, false};
	if (!round.isValid())
		return {ExperienceUpdateReason::InvalidEvent, 0U, false};
	if (timeMicros < timeMicros_)
		return {ExperienceUpdateReason::StaleTime, 0U, false};
...
{
	if (auto* existing = findArea(area))
		return existing;
	if (areas_.size() >= kMaxExperienceAreas)
		return nullptr;
	try
	{
		const auto it = std::lower_bound(areas_.begin(), areas_.end(), area,
...
ExperienceUpdateResult ExperienceModel::updateArea(const ExperienceEvent& event, AreaExperience& value,
												   double weight) noexcept
{
	...
	case ExperienceEventKind::AreaEntered:
		...
		break;
	case ExperienceEventKind::DamageDealt:
		value.damageDealt += amount * weight;
		totals_.damageDealt += amount * weight;
		break;
	case ExperienceEventKind::DamageReceived:
		value.damageReceived += amount * weight;
		totals_.damageReceived += amount * weight;
		if (event.team == perception::Team::Terrorist)
			value.dangerT += amount * weight;
		if (event.team == perception::Team::CounterTerrorist)
			value.dangerCT += amount * weight;
		break;
	case ExperienceEventKind::Kill:
		value.kills += weight;
		totals_.kills += weight;
		break;
	case ExperienceEventKind::Death:
		value.deaths += weight;
		value.deathRate += weight;
		totals_.deaths += weight;
		break;
	case ExperienceEventKind::Encounter:
...
{
	if (!active_)
		return {ExperienceUpdateReason::Inactive, event.area, false};
	if (!settings_.valid())
		return {ExperienceUpdateReason::InvalidSettings, event.area, false};
	if (!event.valid())
		return {ExperienceUpdateReason::InvalidEvent, event.area, false};
	if (event.map != map_)
		return {ExperienceUpdateReason::MapMismatch, event.area, false};
	if (event.round.value < round_.value)
		return {ExperienceUpdateReason::StaleRound, event.area, false};
	if (event.timeMicros < timeMicros_)
		return {ExperienceUpdateReason::StaleTime, event.area, false};
	if (event.kind == ExperienceEventKind::HumanTraversal && event.actorKind != ActorKind::Human)
	{
		return {ExperienceUpdateReason::InvalidEvent, event.area, false};
	}
	if (event.kind == ExperienceEventKind::BotTraversal && event.actorKind != ActorKind::Bot &&
		event.actorKind != ActorKind::OtherBot)
	{
		return {ExperienceUpdateReason::InvalidEvent, event.area, false};
	}
	if (event.round.value > round_.value)
	{
		...
		if (decay.reason != ExperienceUpdateReason::Accepted)
			return decay;
	}
	else
	{
	...
	if (event.kind == ExperienceEventKind::RoundResult)
	{
		...
		if (event.success)
			totals_.roundWins += weight;
		else
			totals_.roundLosses += weight;
...
{
	if (!settings_.valid() || !snapshot.valid())
		return false;
	if (active_ && snapshot.map != map_)
		return false;
	try
	{
		map_ = snapshot.map;
		round_ = snapshot.round;
		timeMicros_ = snapshot.timeMicros;
		totals_ = snapshot.totals;
		areas_ = snapshot.areas;
		active_ = true;
		return true;
	}
	catch (...)
...
ExperienceSnapshot ExperienceModel::snapshot() const
{
	...
	result.map = map_;
	result.round = round_;
	result.timeMicros = timeMicros_;
	result.totals = totals_;
	result.areas = areas_;
	return result;
}

PersistenceResult BinaryExperienceStore::readFile(const std::string& path, const MapIdentity& expected,
												  ExperienceSnapshot& snapshot) const
{
	std::ifstream input(path, std::ios::binary | std::ios::ate);
	if (!input)
		return {PersistenceStatus::NotFound, false};
...
PersistenceResult BinaryExperienceStore::load(const MapIdentity& map, ExperienceSnapshot& snapshot)
{
	if (path_.empty() || !map.valid())
		return {PersistenceStatus::InvalidPath, false};
	auto result = readFile(path_, map, snapshot);
	if (result.status == PersistenceStatus::Ok)
		return result;
	if (result.status == PersistenceStatus::MapMismatch)
		return quarantine(map);
	if (result.status != PersistenceStatus::Corrupt && result.status != PersistenceStatus::IoFailure &&
		result.status != PersistenceStatus::NotFound)
		return result;
	...
	{
		backup.status = PersistenceStatus::RecoveredBackup;
...
PersistenceResult BinaryExperienceStore::save(const ExperienceSnapshot& snapshot)
{
	...
	if (std::rename(temporaryPath.c_str(), path_.c_str()) != 0)
	{
		std::remove(temporaryPath.c_str());
		if (hadExisting)
			(void)std::rename(backupPath.c_str(), path_.c_str());
		return {PersistenceStatus::IoFailure, false};
	}
...
const char* BinaryExperienceStore::statusName(PersistenceStatus status) noexcept
{
	switch (status)
	{
	case PersistenceStatus::None:
		return "None";
	case PersistenceStatus::Ok:
		return "Ok";
	case PersistenceStatus::NotFound:
		return "NotFound";
	case PersistenceStatus::Migrated:
		return "Migrated";
	case PersistenceStatus::RecoveredBackup:
		return "RecoveredBackup";
	case PersistenceStatus::InvalidPath:
		return "InvalidPath";
	case PersistenceStatus::IoFailure:
		return "IoFailure";
	case PersistenceStatus::Corrupt:
		return "Corrupt";
	case PersistenceStatus::UnsupportedVersion:
		return "UnsupportedVersion";
	case PersistenceStatus::SchemaMismatch:
		return "SchemaMismatch";
	case PersistenceStatus::MapMismatch:
		return "MapMismatch";
	case PersistenceStatus::QuarantineFailure:
...
{
	if (!persistence_ || !model_.active())
		return {PersistenceStatus::InvalidPath, false};
	ExperienceSnapshot snapshot{};
	const auto result = persistence_->load(model_.map(), snapshot);
	if (result.succeeded() && !model_.load(snapshot))
		return {PersistenceStatus::MapMismatch, false};
