...
struct WeaponState final
{
	...
	combat::WeaponSnapshot::WeaponClass activeClass{combat::WeaponSnapshot::WeaponClass::Unknown};
...
struct DroppedWeaponObservation final
{
	...
	combat::WeaponSnapshot::WeaponClass weaponClass{combat::WeaponSnapshot::WeaponClass::Unknown};
	...
	WeaponObservationError validate(MapGeneration currentMap, perception::RoundGeneration currentRound,
...
struct WeaponPickupEvaluation final
{
	...
	combat::WeaponSnapshot::WeaponClass candidateClass{combat::WeaponSnapshot::WeaponClass::Unknown};
	...
	friend bool operator==(const WeaponPickupEvaluation& left, const WeaponPickupEvaluation& right) noexcept;
	friend bool operator!=(const WeaponPickupEvaluation& left, const WeaponPickupEvaluation& right) noexcept
	{
		return !(left == right);
	}
...
struct ActionPlannerSettings final
{
	...
	std::uint32_t maxWeaponCandidates{static_cast<std::uint32_t>(kMaxWeaponPickupCandidates)};
	std::uint32_t maxNavigationCandidates{static_cast<std::uint32_t>(kMaxNavigationCandidates)};
...
{
public:
	explicit ActionPlanner(ActionPlannerSettings settings = {}) noexcept : settings_(settings)
	{
	}

	void reset() noexcept;
	ActionDecision decide(const ActionPlannerInput& input) noexcept;
	ActionResult update(const ActionPlannerInput& input, const ActionObservation& observation) noexcept;

	bool active() const noexcept
	{
		return active_;
	}
	const ActionIntent& currentIntent() const noexcept
	{
		return current_;
	}
	const ActionScore& currentScore() const noexcept
	{
		return currentScore_;
	}
	const ActionPlannerSettings& settings() const noexcept
	{
		return settings_;
	}

private:
	ActionDecision invalidDecision(const ActionPlannerInput& input, ActionAbortReason reason) noexcept;
