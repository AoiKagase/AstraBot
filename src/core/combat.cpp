...
{
	return view.pitch >= kMinPitch && view.pitch <= kMaxPitch && view.yaw >= kMinYaw && view.yaw <= kMaxYaw &&
...
{
	return left.agent == right.agent && left.observer == right.observer && left.map == right.map &&
		   left.round == right.round && left.tick == right.tick && left.timeMicros == right.timeMicros;
}

bool sameStamp(const world::WorldSnapshot& snapshot, const CombatInput& input) noexcept
{
	const perception::Stamp expected{input.agent, input.player, input.map, input.tick, input.timeMicros, input.round};
...
bool calculateAngularError(const perception::Point& eye, const perception::Point& target, const ViewAngles& view,
...
bool calculateAim(const perception::Point& eye, const perception::Point& target, const ViewAngles& current,
				  ViewAngles& result) noexcept
{
	if (!isFinitePoint(target))
		return false;
	const double dx = target.x - eye.x;
	...
	const double desiredPitch = std::atan2(-dz, horizontal) * kDegreesPerRadian;
	const double desiredYaw = std::atan2(dy, dx) * kDegreesPerRadian;
	const double shortestYaw =
		static_cast<double>(current.yaw) + shortestAngle(desiredYaw - static_cast<double>(current.yaw));
	result.pitch =
		static_cast<float>(std::clamp(desiredPitch, static_cast<double>(kMinPitch), static_cast<double>(kMaxPitch)));
...
ViewAngles scanView(const CombatInput& input) noexcept
{
	...
	const auto phase = (input.timeMicros % kViewScanPeriodMicros +
						(static_cast<std::uint64_t>(input.agent.value) * 500'000U) % kViewScanPeriodMicros) %
					   kViewScanPeriodMicros;
	const double yaw = -180.0 + 360.0 * static_cast<double>(phase) / static_cast<double>(kViewScanPeriodMicros);
	result.yaw = static_cast<float>(yaw);
	return result;
}

void preserveWorldMovement(const ViewAngles& from, const ViewAngles& to, BotCommand& command) noexcept
{
	constexpr double radiansPerDegree = 3.14159265358979323846 / 180.0;
	const double delta = (static_cast<double>(from.yaw) - static_cast<double>(to.yaw)) * radiansPerDegree;
...
bool resolveTargetPoint(const CombatInput& input, const CombatDecision& selection, perception::Point& point,
						std::uint64_t& observedMicros) noexcept
{
	...
	else if (selection.source == perception::ObservationSource::TeamReport && input.world.reports != nullptr)
	{
		for (std::size_t i = 0; i < input.world.reports->count; ++i)
		{
			const auto& report = input.world.reports->reports[i].report;
			if (report.target == selection.target && report.identity.source == selection.source)
			{
...
std::uint64_t aimSeed(const CombatInput& input, const CombatDecision& selection, std::uint64_t stream) noexcept
{
...
std::uint64_t reactionReadyAt(std::uint64_t observedMicros, std::uint64_t delayMicros) noexcept
{
	const auto maximum = (std::numeric_limits<std::uint64_t>::max)();
	if (observedMicros > maximum - delayMicros)
		return maximum;
	return observedMicros + delayMicros;
}

bool validVisualIdentity(const perception::ObservationIdentity& identity, const CombatInput& input) noexcept
{
	return identity.source == perception::ObservationSource::Vision && identity.map == input.map &&
		   identity.round == input.round && identity.validAt(input.timeMicros);
}

bool freshVisual(const world::VisualMemory& memory, const CombatInput& input) noexcept
{
	if (memory.lastSeenMicros > input.timeMicros || memory.identity.observedMicros != memory.lastSeenMicros)
	{
		return false;
	}
	return input.timeMicros - memory.lastSeenMicros <= kMaxVisualAgeMicros;
}

bool currentTargetGeneration(const CombatInput& input, PlayerId target) noexcept
{
	if (!validCandidatePlayer(target) || target.slot > input.world.roster.size())
	{
		return false;
	}
	return input.world.roster[target.slot - 1U].player == target;
}

bool validReportIdentity(const world::TeamReport& report, const CombatInput& input) noexcept
{
	return report.origin.source == perception::ObservationSource::Vision && report.origin.map == input.map &&
		   report.origin.round == input.round && report.origin.validAt(input.timeMicros) &&
		   report.identity.source == perception::ObservationSource::TeamReport && report.identity.map == input.map &&
		   report.identity.round == input.round && report.identity.validAt(input.timeMicros) &&
		   report.identity.observedMicros == report.origin.observedMicros &&
		   report.origin.receivedMicros <= report.sentMicros && report.sentMicros <= report.identity.receivedMicros &&
...
bool betterCandidate(const TargetCandidate& left, const TargetCandidate& right) noexcept
{
	...
	if (left.observedMicros != right.observedMicros)
		return left.observedMicros > right.observedMicros;
	if (left.angularError != right.angularError)
		return left.angularError < right.angularError;
...
void noteRelation(RejectionFlags& flags, perception::Relation relation) noexcept
{
	if (relation == perception::Relation::Unknown)
		flags.unknownRelation = true;
	else if (relation == perception::Relation::Self || relation == perception::Relation::Ally)
		flags.ally = true;
}

CombatReason noTargetReason(const RejectionFlags& flags) noexcept
{
	if (flags.unknownRelation)
		return CombatReason::UnknownRelation;
	if (flags.ally)
		return CombatReason::Ally;
	if (flags.stale)
		return CombatReason::StaleTarget;
	if (flags.anonymousSound)
		return CombatReason::AnonymousSound;
...
CombatDecision suppressedTrack(const CombatInput& input, const CombatDecision& aim, CombatReason reason) noexcept
{
	auto decision = aim;
...
const world::VisualMemory* currentVisualFor(const CombatInput& input, PlayerId target) noexcept
{
	...
	if (visual.stamp.agent != input.agent || visual.stamp.observer != input.player || visual.stamp.map != input.map ||
		visual.stamp.round != input.round || visual.stamp.timeMicros > input.timeMicros ||
		input.timeMicros - visual.stamp.timeMicros > kVisualFreshnessMicros ||
		!currentTargetGeneration(input, target) ||
		input.world.relation(input.team, target) != perception::Relation::Opponent)
	{
		return nullptr;
	}
	...
	for (std::size_t i = 0; i < visual.count; ++i)
	{
		...
		if (memory.target != target || !validVisualIdentity(memory.identity, input) ||
			...
			!currentTargetGeneration(input, memory.target) || !isFinitePoint(memory.lastKnownPosition) ||
...
bool inspectVisual(const CombatInput& input, const world::VisualMemory& memory, TargetCandidate& candidate,
				   RejectionFlags& flags) noexcept
{
	...
	if (!validVisualIdentity(memory.identity, input) || memory.lastSeenMicros != memory.identity.observedMicros ||
		memory.lastSeenMicros > input.timeMicros || input.timeMicros - memory.lastSeenMicros > kVisualFreshnessMicros ||
		!currentTargetGeneration(input, memory.target) || !validConfidence(memory.confidence, 1.0) ||
...
bool inspectReport(const CombatInput& input, const world::ReportMemory& memory, TargetCandidate& candidate,
				   RejectionFlags& flags) noexcept
{
	const auto& report = memory.report;
	if (!validCandidatePlayer(report.reporter) || !validCandidatePlayer(report.target) ||
		report.reporter == report.target || report.reporter == report.receiver || report.receiver != input.player ||
	...
	if (reporterRelation != perception::Relation::Ally)
	{
		...
		if (reporterRelation == perception::Relation::Opponent)
			flags.stale = true;
		return false;
	}
	...
	if (!validReportIdentity(report, input) || !validConfidence(memory.confidence, 0.5) ||
...
bool currentVisionThreat(const CombatInput& input, const CombatDecision& aim) noexcept
{
	if (aim.action != CombatAction::Track || aim.source != perception::ObservationSource::Vision ||
		!aim.target.isValid())
	{
		return false;
	}
	const auto* visual = currentVisualFor(input, aim.target);
	return visual != nullptr && input.world.relation(input.team, aim.target) == perception::Relation::Opponent;
}

bool targetDistanceAndAimError(const CombatInput& input, PlayerId target, double& distance, double& aimError) noexcept
{
...
FirePlan boundedBurst(std::uint8_t requested, std::int32_t clipAmmo) noexcept
{
	const auto available =
		static_cast<std::uint8_t>(std::min(std::max(clipAmmo, 0), static_cast<std::int32_t>(kMaxBurstShots)));
...
FirePlan chooseFirePlan(const CombatInput& input, PlayerId target) noexcept
{
	double distance = 0.0;
	double aimError = 0.0;
	if (!targetDistanceAndAimError(input, target, distance, aimError) || input.weapon.clipAmmo <= 0)
	{
		return FirePlan::tap();
	}
	...
	case WeaponClass::MachineGun:
		if (distance >= kLongCombatRange || poorAim)
			return FirePlan::tap();
...
WeaponId preferredSwitchWeapon(const WeaponSnapshot& weapon) noexcept
{
	...
	for (std::size_t i = 0; i < weapon.ownedCount; ++i)
	{
		...
		if (!candidate.isValid() || candidate == weapon.active)
			continue;
		if (!isSwitchableWeaponId(candidate))
			continue;
...
void markAction(AttackLifecycleState& state, const CombatInput& input, CombatAction action) noexcept
{
	state.map = input.map;
...
std::optional<CombatDecision> planWeaponAction(const CombatInput& input, const CombatDecision& aim,
											   AttackLifecycleState& state) noexcept
{
	...
	if (input.weapon.clipAmmo <= input.weapon.reloadClipThreshold && input.weapon.reserveAmmo > 0 &&
		input.weapon.canReload && (input.weapon.clipAmmo == 0 || !threat))
	{
		clearCadence(state);
		clearReaction(state);
	...
	if (input.weapon.clipAmmo <= 0 && input.weapon.reserveAmmo <= 0 && input.weapon.activeClass != WeaponClass::Melee)
	{
	...
	if (input.weapon.clipAmmo <= 0 && input.weapon.reserveAmmo > 0 && !input.weapon.canReload)
	{
		const auto candidate = preferredSwitchWeapon(input.weapon);
		if (input.weapon.canSwitch && candidate.isValid())
		{
...
WeaponValidation WeaponSnapshot::validate() const noexcept
{
	...
	if (static_cast<std::uint8_t>(activeClass) > static_cast<std::uint8_t>(WeaponClass::Melee))
	{
...
bool operator==(const WeaponSnapshot& left, const WeaponSnapshot& right) noexcept
{
	if (left.map != right.map || left.round != right.round || left.tick != right.tick ||
		...
		left.activeClass != right.activeClass || left.ownedCount != right.ownedCount ||
		left.clipAmmo != right.clipAmmo || left.reserveAmmo != right.reserveAmmo ||
		left.reloadClipThreshold != right.reloadClipThreshold || left.reloading != right.reloading ||
...
{
	return reactionDelayMicros <= kMaxReactionDelayMicros && std::isfinite(observationErrorDegrees) &&
		   std::isfinite(predictionErrorDegrees) && std::isfinite(aimNoiseDegrees) && observationErrorDegrees >= 0.0F &&
		   observationErrorDegrees <= kMaxDifficultyErrorDegrees && predictionErrorDegrees >= 0.0F &&
		   predictionErrorDegrees <= kMaxDifficultyErrorDegrees && aimNoiseDegrees >= 0.0F &&
		   aimNoiseDegrees <= kMaxDifficultyErrorDegrees && decisionQuality <= kMaxDecisionQuality;
}
...
DecisionValidation CombatDecision::validate() const noexcept
{
	...
	else
	{
		...
		if (action == CombatAction::Reload && (buttons & static_cast<ButtonMask>(Button::Reload)) == 0U)
		{
...
{
	return action == CombatAction::Fire && (buttons & static_cast<ButtonMask>(Button::Attack)) != 0U;
}
...
CombatInputValidation CombatInput::validate() const noexcept
{
	...
	if (!sameStamp(world.visual->stamp, world.stamp) || !sameStamp(world.sounds->stamp, world.stamp))
	{
		return {CombatInputError::StaleWorldSnapshot};
	}
	if (world.visual->count > world.visual->memories.size() || world.sounds->count > world.sounds->sounds.size())
	{
		return {CombatInputError::InvalidWorldSnapshot};
	}
	...
	if (weapon.map != map || weapon.round != round || weapon.tick != tick || weapon.observedMicros != timeMicros)
	{
		return {CombatInputError::StaleWeapon};
	}
	if (!difficulty.valid())
...
{
	if (input.lock != nullptr && !input.lock->validFor(input))
		input.lock->clear();
	const auto validation = input.validate();
	if (!validation)
		return input.reject();
	if (!input.alive)
		return validNoOp(input, CombatReason::Dead);
	...
	if (selected)
	{
		if (input.lock != nullptr && best.source == perception::ObservationSource::Vision)
		{
			...
			{
				const auto maximum = (std::numeric_limits<std::uint64_t>::max)();
				input.lock->generation = input.lock->generation == maximum ? maximum : input.lock->generation + 1U;
			...
			input.lock->expiresMicros = input.timeMicros > (maximum - kVisualFreshnessMicros)
											? maximum
											: input.timeMicros + kVisualFreshnessMicros;
		}
...
CombatDecision aimTarget(const CombatInput& input) noexcept
{
	...
	{
		if (decision.reason == CombatReason::NoTarget || decision.reason == CombatReason::AnonymousSound)
	...
	const double aimNoise = effectiveError(input.difficulty.aimNoiseDegrees, input.difficulty.decisionQuality) *
	...
	view.pitch = static_cast<float>(std::clamp(static_cast<double>(view.pitch) + totalError,
											   static_cast<double>(kMinPitch), static_cast<double>(kMaxPitch)));
	view.yaw = static_cast<float>(std::clamp(shortestAngle(static_cast<double>(view.yaw) + totalError),
...
FireAuthorization authorizeFire(const CombatInput& input, const CombatDecision& aim,
								AttackLifecycleState previous) noexcept
{
	...
	if (result.nextState.initialized && result.nextState.lastActionTick == input.tick)
	{
		const auto duplicateReason = result.nextState.lastAction == CombatAction::Fire ? CombatReason::DuplicateAttack
	...
	if (const auto weaponAction = planWeaponAction(input, aim, result.nextState); weaponAction.has_value())
	{
		result.decision = *weaponAction;
		if (result.decision.action == CombatAction::Reload || result.decision.action == CombatAction::SwitchWeapon)
		{
	...
	if (aim.action != CombatAction::Track || aim.inputTick != input.tick || aim.validUntilMicros != input.timeMicros ||
		!aim.validateForP5())
	{
		clearCadence(result.nextState);
		clearReaction(result.nextState);
	...
	if ((aim.reason != CombatReason::Accepted && aim.reason != CombatReason::ReactionDelay) ||
	...
	if (!result.nextState.reactionActive || result.nextState.reactionTarget != aim.target)
	{
	...
	const auto reactionReady =
		reactionReadyAt(result.nextState.reactionStartedMicros, input.difficulty.reactionDelayMicros);
	...
	if (result.nextState.cadenceActive &&
		(result.nextState.cadenceTarget != aim.target || result.nextState.cadenceWeapon != input.weapon.active))
	{
		clearCadence(result.nextState);
		clearReaction(result.nextState);
	}
	if (result.nextState.cadenceActive && result.nextState.cadencePauseUntilMicros > input.timeMicros)
	{
		result.decision = suppressedTrack(input, aim, CombatReason::Cooldown);
		return result;
	}
	if (result.nextState.cadenceActive && result.nextState.cadencePauseUntilMicros != 0 &&
	...
	FirePlan plan = result.nextState.cadenceActive ? result.nextState.cadencePlan : chooseFirePlan(input, aim.target);
	if (!plan.valid())
	{
		clearCadence(result.nextState);
		plan = FirePlan::tap();
	...
	if (result.nextState.cadenceActive && plan.pattern == FirePattern::Burst)
	{
		const auto remaining = static_cast<std::uint8_t>(plan.burstShots - result.nextState.cadenceShotsFired);
	...
	decision.targetAgeMicros = input.timeMicros - visual->identity.observedMicros;
	...
	if (plan.pattern == FirePattern::Burst || plan.pattern == FirePattern::FullAuto)
	{
		...
		if (result.nextState.cadenceShotsFired >= cadenceLimit(plan))
		{
			...
			result.nextState.cadencePauseUntilMicros =
				input.timeMicros > maximum - kBurstPauseMicros ? maximum : input.timeMicros + kBurstPauseMicros;
		}
	}
	else
	{
...
CommandCompositionResult composeCommand(const CombatDecision& combat, const BotCommand& navigation) noexcept
{
	...
	const bool directVisionCombat = combat.target.isValid() && combat.source == perception::ObservationSource::Vision &&
									(combat.action == CombatAction::Track || combat.action == CombatAction::Fire);
	if (combat.action == CombatAction::NoOp)
	{
		...
		const bool scan = (combat.reason == CombatReason::NoTarget || combat.reason == CombatReason::AnonymousSound) &&
						  navigation.movement.forward == 0.0F && navigation.movement.side == 0.0F &&
						  navigation.movement.up == 0.0F && navigationActions == 0U;
	...
	if (directVisionCombat)
	{
		// A visible opponent owns the bot's immediate position. Continuing
		// the route while aiming lets the bot walk past an opponent before
		// the next target decision can run. Keep a crouch hold, but remove
		// all translation and traversal buttons from the NAV command.
		command.movement = {};
		constexpr ButtonMask movementButtons =
			static_cast<ButtonMask>(Button::Jump) | static_cast<ButtonMask>(Button::Forward) |
		...
		command.buttons &= ~movementButtons;
	}
	command.buttons = (navigation.buttons & ~combatButtons) | (combat.buttons & combatButtons);
	command.weaponSelect =
		combat.action == CombatAction::SwitchWeapon ? combat.selectedWeapon.value : kNoWeaponSelection;
	if (!command.validate())
	{
		return {{}, CommandCompositionError::InvalidNavigationCommand, false};
	}
