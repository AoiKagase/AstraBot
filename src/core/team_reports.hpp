...
{
enum class ReportReason : std::uint8_t
{
	None,
	NotReady,
	InvalidTime,
...
struct ReportResult
{
	...
	{
		return reason == ReportReason::None && recipients > 0;
	}
};
struct TeamReport
{
...
struct ReportSnapshot
{
	...
	std::array<ReportMemory, 31> reports{};
	std::size_t count{};
};
...
public:
	...
	ReportResult send(const VisualMemoryModel&, PlayerId, PlayerId, std::uint64_t, const std::array<bool, 32>&,
					  const perception::TeamRoster&) noexcept;
	void deliver() noexcept;
	const ReportSnapshot* latest(PlayerId) const noexcept;
	const ReportDiagnostics& diagnostics() const noexcept
	{
		return diagnostics_;
	}
...
private:
	...
	{
		TeamReport report{};
		std::uint64_t reporterEpoch{}, receiverEpoch{}, targetEpoch{};
	};
	...
	struct Epochs
	{
		std::uint64_t reporter{}, receiver{}, target{};
	};
