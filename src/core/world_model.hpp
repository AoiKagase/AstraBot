...
{
enum class WorldReason : std::uint8_t
{
	None,
	InvalidFrame,
	NotCollecting,
...
struct SourceQueueDiagnostics
{
	std::size_t soundPending{};
	std::uint64_t soundOverflow{};
};
...
public:
	...
	bool advance(const MemoryFrame&,
				 const perception::TeamRoster& = {}) noexcept; // Invalidation and decay before staging.
	...
	bool collectingAt(MapGeneration map, perception::RoundGeneration round, TickId tick,
					  std::uint64_t time) const noexcept
	{
		return collecting_ && frame_.map == map && frame_.round == round && frame_.tick == tick &&
			   frame_.timeMicros == time;
	}
	...
	const MemoryFrame* publishedFrame() const noexcept
	{
		return published_ ? &frame_ : nullptr;
	}
	bool setDistribution(PlayerId, PlayerId, const perception::ObservationIdentity&,
						 const PositionDistribution&) noexcept;
	void clearDistributions() noexcept;
	const WorldDiagnostics& diagnostics() const noexcept
	{
		return diagnostics_;
	}
	const VisualMemoryModel& visual() const noexcept
	{
		return visual_;
	}
	const SoundMemoryModel& sounds() const noexcept
	{
		return sounds_;
	}
	const TeamReportModel& reports() const noexcept
	{
		return reports_;
	}
	...
	{
		if (!published_)
			return {ReportReason::NotReady, 0};
		return reports_.send(visual_, reporter, target, now, eligibility, teams);
	}
	void forgetReports(PlayerId player) noexcept
	{
		reports_.forget(player);
	}
	void invalidateReports() noexcept
	{
		reports_.invalidate();
	}
	// Wiring for privileged adapters; LifecycleCoordinator exposes only const WorldModel.
	VisualMemoryModel& visualReducer() noexcept
	{
		return visual_;
	}
	SoundMemoryModel& soundReducer() noexcept
	{
		return sounds_;
	}
...
private:
	...
	{
		PlayerId receiver{};
		perception::SoundObservation sound{};
	};
	struct InputRef
	{
		std::uint16_t index{};
