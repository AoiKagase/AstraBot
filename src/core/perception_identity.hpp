...
struct RoundGeneration
{
	...
	friend constexpr bool operator==(RoundGeneration a, RoundGeneration b) noexcept
	{
	...
	constexpr bool validAt(std::uint64_t now) const noexcept
	{
		return map.isValid() && round.isValid() && sequence != 0 &&
			   (source == ObservationSource::Vision || source == ObservationSource::Sound ||
				source == ObservationSource::TeamReport) &&
			   observedMicros <= receivedMicros && receivedMicros <= now;
	}
};
...
public:
	...
	{
		if (!map.isValid() || map.value <= map_.value)
			return false;
		map_ = map;
		members_ = {};
		generations_ = {};
	...
	{
		if (map != map_ || !map.isValid() || !validPlayer(player))
			return false;
		const auto i = static_cast<std::size_t>(player.slot - 1U);
		if (members_[i].player == player)
			return true;
		if (player.generation.value <= generations_[i].value)
			return false;
		generations_[i] = player.generation;
		members_[i] = {player, Team::Unknown};
		return true;
	}
	bool update(MapGeneration map, PlayerId player, Team team) noexcept
	{
		if (map != map_ || !find(player) || !validTeam(team))
			return false;
		members_[player.slot - 1U].team = team;
		return true;
	}
	void forget(PlayerId player) noexcept
	{
		if (find(player))
			members_[player.slot - 1U] = {};
	}
	void clear() noexcept
	{
		members_ = {};
	} // Retain map/generation high-water marks.
	...
	{
		const auto* a = find(observer);
		const auto* b = find(target);
		if (!a || !b)
			return Relation::Unknown;
		if (observer == target)
			return Relation::Self;
		if (!playing(a->team) || !playing(b->team))
			return Relation::Unknown;
		return a->team == b->team ? Relation::Ally : Relation::Opponent;
	}
	MapGeneration map() const noexcept
	{
		return map_;
	}

private:
	static constexpr bool validPlayer(PlayerId p) noexcept
	{
		return p.isValid() && p.slot <= 32;
	}
	static constexpr bool playing(Team t) noexcept
	{
		return t == Team::Terrorist || t == Team::CounterTerrorist;
	}
	static constexpr bool validTeam(Team t) noexcept
	{
		return playing(t) || t == Team::Unknown || t == Team::Spectator;
	}
	MapGeneration map_{};
