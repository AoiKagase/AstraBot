...
struct ObjectiveTargetId final
{
	...
	friend constexpr bool operator==(ObjectiveTargetId left, ObjectiveTargetId right) noexcept
	{
...
struct ObjectiveAssignment final
{
	...
	bool valid(MapGeneration expectedMap, perception::RoundGeneration expectedRound,
...
struct SharedTeamState final
{
	...
	std::array<ObjectiveAssignment, kMaxObjectiveAssignments> objectiveAssignments{};
...
{
public:
	TeamDecision update(const TeamSnapshot& snapshot, const TeamEvents& events = {}) noexcept;
	void reset() noexcept;
	...
	const SharedTeamState& state() const noexcept
	{
		return state_;
	}
...
private:
	...
	static int roleScore(Role role, const TeamMemberSnapshot& member, const TeamSnapshot& snapshot,
						 double frontline) noexcept;
	static ReassignmentReason reasonFor(const TeamEvents& events) noexcept;
	static void buildObjectiveAssignments(const TeamSnapshot& snapshot, SharedTeamState& state,
