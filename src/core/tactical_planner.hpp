...
struct EnemyBelief final
{
	...
	bool valid(std::uint64_t nowMicros, std::uint64_t maxAgeMicros) const noexcept;
};
...
{
public:
	explicit TacticalPlanner(TacticalPlannerSettings settings = {}) noexcept : settings_(settings)
	{
	}

	void reset() noexcept;
	bool needsReplan(const TacticalContext& context, const ReplanEvents& events = {}) const noexcept;
	TacticalDecision plan(const TacticalContext& context, const ReplanEvents& events = {}) noexcept;

	bool active() const noexcept
	{
		return active_;
	}
	const TacticalIntent& currentIntent() const noexcept
	{
		return current_;
	}
	const TacticalPlannerSettings& settings() const noexcept
	{
		return settings_;
	}

private:
	TacticalDecision invalidDecision(const TacticalContext& context, Reason reason) noexcept;
	TacticalIntent choose(const TacticalContext& context, ReplanTrigger trigger) const noexcept;
