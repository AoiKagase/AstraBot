...
void VisualMemoryModel::forget(PlayerId player) noexcept
{
	...
	for (auto& state : states_)
	{
		...
		{
			if (snapshot.memories[i].target == player)
				++diagnostics_.retired;
			else
				snapshot.memories[kept++] = snapshot.memories[i];
		}
...
bool VisualMemoryModel::advance(const MemoryFrame& input) noexcept
{
	...
	if (!settings_.valid())
	{
		invalidate(MemoryReason::InvalidSettings);
		return false;
	}
	if (!input.map.isValid() || !input.tick.isValid() || !input.round.isValid() ||
		(input.map == frame_.map && input.round.value < frame_.round.value) ||
		(frame_.map.isValid() &&
		 (input.map.value < frame_.map.value || (input.map == frame_.map && (input.tick.value <= frame_.tick.value ||
																			 input.timeMicros < frame_.timeMicros)))))
	{
		invalidate(MemoryReason::InvalidFrame);
		return false;
	}
	...
	for (std::size_t i = 0; i < frame_.players.size(); ++i)
	{
		...
		if (!p.player.isValid() && !p.eligible)
		{
			p = {};
			continue;
		}
		...
		{
			p = {};
			reject(MemoryReason::StaleIdentity);
		}
		else
			generations_[i] = p.player.generation;
	}
	for (std::size_t slot = 0; slot < states_.size(); ++slot)
	{
		...
		if (!owner.eligible || !owner.agent.isValid() || snapshot.stamp.observer != owner.player ||
			snapshot.stamp.agent != owner.agent)
		{
			diagnostics_.retired += snapshot.count;
			state = {};
		}
...
bool VisualMemoryModel::observe(const perception::ObservationBatch& batch) noexcept
{
	...
	{
		reject(MemoryReason::InvalidBatch);
		return false;
	}
	...
	if (snapshot.stamp.observer != stamp.observer || snapshot.stamp.agent != stamp.agent || stamp.map != frame_.map ||
		stamp.round != frame_.round)
	{
		reject(MemoryReason::StaleIdentity);
		return false;
	}
	if (stamp.tick == state.consumed)
	{
		reject(MemoryReason::DuplicateBatch);
		return false;
	}
	if (stamp.tick != frame_.tick || stamp.timeMicros != frame_.timeMicros)
	{
		reject(MemoryReason::StaleBatch);
		return false;
	}
	...
	if (identity.sequence <= state.sequence)
	{
		reject(MemoryReason::StaleBatch);
		return false;
	}
	...
	for (std::size_t i = 0; i < batch.count; ++i)
	{
		...
		if (!valid(observation.target) || observation.target == stamp.observer ||
			!perception::finite(observation.position))
		{
			reject(MemoryReason::InvalidBatch);
			return false;
		}
		...
		if (!target.eligible || target.player != observation.target)
		{
			reject(MemoryReason::StaleIdentity);
			return false;
		}
	...
	for (std::size_t i = 0; i < batch.count; ++i)
	{
		const auto& observation = batch.observations[i];
		std::size_t index = 0;
		while (index < snapshot.count && snapshot.memories[index].target != observation.target)
			++index;
		if (index == snapshot.count)
			++snapshot.count; // At most 31 distinct eligible non-owner slots.
