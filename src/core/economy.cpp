...
std::uint8_t utilityCount(const EquipmentSnapshot& equipment, PurchaseItem item) noexcept
{
	switch (item)
	{
...
std::int32_t minimumAffordable(const BuyRequest& request, std::int32_t money) noexcept
{
	const auto cost = [&](PurchaseItem item) noexcept {
		return BuyPlanner::price(item) * static_cast<std::int32_t>(request.quantity);
	};
	const auto requestedCost = cost(request.item);
	if (requestedCost <= money)
		return requestedCost;
	for (std::size_t i = 0; i < request.fallbackCount; ++i)
	{
		const auto fallbackCost = cost(request.fallbacks[i]);
		if (fallbackCost <= money)
			return fallbackCost;
	}
	return -1;
}

void appendRequest(BuyPlan& plan, PurchaseItem item, std::uint8_t quantity, std::uint8_t priority, bool mandatory,
				   bool preserved, std::array<PurchaseItem, kMaxFallbacks> fallbacks = {},
				   std::size_t fallbackCount = 0) noexcept
{
	if (item == PurchaseItem::None || quantity == 0 || plan.requestCount >= kMaxBuyRequests ||
		fallbackCount > kMaxFallbacks)
	{
		return;
	}
...
void appendUtility(BuyPlan& plan, const PlayerEconomySnapshot& member, PurchaseItem item, std::uint8_t desired,
...
{
	return member.equipment.primary == WeaponKind::Rifle || member.equipment.primary == WeaponKind::Smg ||
...
bool betterKitOwner(const PlayerEconomySnapshot& candidate, const PlayerEconomySnapshot& current) noexcept
{
	const auto candidateDefuser = candidate.role == team::Role::Defuser;
	const auto currentDefuser = current.role == team::Role::Defuser;
	if (candidateDefuser != currentDefuser)
		return candidateDefuser;
	if (candidate.canDefuse != current.canDefuse)
		return candidate.canDefuse;
...
{
	return armor <= 100 && heGrenades <= kMaxUtility && flashbangs <= kMaxUtility && smokeGrenades <= kMaxUtility &&
		   primaryAmmo <= 4096 && secondaryAmmo <= 4096;
}

bool PlayerEconomySnapshot::valid() const noexcept
{
	return player.isValid() && agent.isValid() && playingTeam(team) && role != team::Role::None && money >= 0 &&
		   money <= kMaxMoney && equipment.valid();
}

bool TeamEconomySnapshot::valid() const noexcept
{
	if (!map.isValid() || !round.isValid() || !tick.isValid() || nowMicros == 0 || roundNumber == 0 ||
		phase == RoundPhase::Unknown || !playingTeam(team) || memberCount == 0 || memberCount > members.size() ||
		requestedKitCount > kMaxKitTarget)
	{
		return false;
	}
	if (requestedKitCount > memberCount)
		return false;
	for (std::size_t i = 0; i < memberCount; ++i)
	{
		const auto& member = members[i];
		if (!member.valid() || member.team != team || (!member.connected && member.alive))
		{
			return false;
		}
		for (std::size_t j = 0; j < i; ++j)
		{
			if (members[j].player == member.player || members[j].agent == member.agent)
			{
				return false;
			}
		}
...
{
	if (item == PurchaseItem::None || quantity == 0 || fallbackCount > fallbacks.size())
	{
		return false;
	}
	for (std::size_t i = 0; i < fallbackCount; ++i)
	{
		if (fallbacks[i] == PurchaseItem::None || fallbacks[i] == item)
			return false;
	}
	return true;
}
...
{
	if (!map.isValid() || !round.isValid() || !tick.isValid() || !player.isValid() || role == team::Role::None ||
		strategy == BuyStrategy::None || requestCount > requests.size() || expectedSpend < 0 || remainingMoney < 0 ||
		!accepted)
	{
		return false;
	}
...
{
	if (!valid() || !snapshot.valid() || map != snapshot.map || round != snapshot.round || tick != snapshot.tick)
	{
		return false;
	}
...
{
	if (!accepted || !snapshot.valid() || strategy == BuyStrategy::None || planCount != snapshot.memberCount ||
		kitPlanCount > planCount)
	{
		return false;
	}
	for (std::size_t i = 0; i < planCount; ++i)
	{
		if (!plans[i].valid() || !plans[i].matches(snapshot) || plans[i].strategy != strategy)
		{
			return false;
		}
	}
...
BuyStrategyDecision BuyPlanner::chooseStrategy(const TeamEconomySnapshot& snapshot) const noexcept
{
	...
	for (std::size_t i = 0; i < snapshot.memberCount; ++i)
	{
		...
		if (hasGoodPrimary(member))
			++goodPrimary;
		if (member.money >= 2'500 || hasGoodPrimary(member))
			++halfReady;
		if (member.money >= 4'000 || (hasGoodPrimary(member) && member.money >= 2'000))
		{
			++fullReady;
		}
	}
	std::sort(money.begin(), money.begin() + static_cast<std::ptrdiff_t>(snapshot.memberCount));
	...
	else if (snapshot.lossStreak >= 2 && halfReady >= majority && result.medianMoney >= 1'800)
	{
...
std::size_t BuyPlanner::firstAwpOwner(const TeamEconomySnapshot& snapshot) noexcept
{
	std::size_t owner = snapshot.memberCount;
	for (std::size_t i = 0; i < snapshot.memberCount; ++i)
	{
		const auto& candidate = snapshot.members[i];
		if (!activeMember(candidate) || !candidate.awpCandidate)
			continue;
...
bool BuyPlanner::isKitOwner(const TeamEconomySnapshot& snapshot, std::size_t memberIndex) noexcept
{
	if (snapshot.team != perception::Team::CounterTerrorist || snapshot.requestedKitCount == 0 ||
		memberIndex >= snapshot.memberCount || !activeMember(snapshot.members[memberIndex]) ||
	...
	{
		const auto& candidate = snapshot.members[i];
		if (i == memberIndex || !activeMember(candidate) || !candidate.canDefuse)
			continue;
		if (betterKitOwner(candidate, snapshot.members[memberIndex]))
			++rank;
	}
...
BuyPlan BuyPlanner::buildPlan(const TeamEconomySnapshot& snapshot, std::size_t memberIndex) const noexcept
{
	BuyPlan plan{};
	if (!snapshot.valid() || memberIndex >= snapshot.memberCount || !activeMember(snapshot.members[memberIndex]))
	{
		return plan;
	}
	...
	const bool economyBuy = decision.strategy == BuyStrategy::FullBuy || decision.strategy == BuyStrategy::HalfBuy ||
							decision.strategy == BuyStrategy::ForceBuy || decision.strategy == BuyStrategy::PistolRound;

	if (!economyBuy || decision.strategy == BuyStrategy::Save || decision.strategy == BuyStrategy::Eco)
	{
		...
		plan.preservedUtility =
			member.equipment.heGrenades != 0 || member.equipment.flashbangs != 0 || member.equipment.smokeGrenades != 0;
	}

	if ((economyBuy || decision.strategy == BuyStrategy::Eco) && member.equipment.armor < 100)
	{
		appendRequest(plan, PurchaseItem::Armor, 1, 10, decision.strategy != BuyStrategy::Eco, false);
	}
	...
	{
		appendRequest(plan, PurchaseItem::Helmet, 1, 20, decision.strategy == BuyStrategy::FullBuy, false);
	}
	...
	const bool fullBuyPrimaryUpgrade =
		decision.strategy == BuyStrategy::FullBuy && member.equipment.primary == WeaponKind::Smg;
		...
		(!member.equipment.hasUsablePrimary() || fullBuyPrimaryUpgrade))
	{
		...
		if (member.awpCandidate && awpOwner == memberIndex && decision.strategy == BuyStrategy::FullBuy)
		{
		...
		appendRequest(plan, primary, 1, 30, true, false, fallbacks, fallbackCount);
	}
	...
	if (decision.strategy != BuyStrategy::Eco && decision.strategy != BuyStrategy::Save)
	{
		switch (member.role)
		{
		case team::Role::Entry:
		case team::Role::Trade:
			appendUtility(plan, member, PurchaseItem::Flashbang, decision.strategy == BuyStrategy::PistolRound ? 1 : 2,
						  50);
			break;
	...
	if (isKitOwner(snapshot, memberIndex) && !member.equipment.defuseKit && economyBuy)
	{
	...
	if (member.equipment.hasUsablePrimary() && member.equipment.primaryAmmo < 30 && economyBuy)
	{
		appendRequest(plan, PurchaseItem::Ammo, 1, 60, false, false);
	}
	if (member.equipment.heGrenades != 0 || member.equipment.flashbangs != 0 || member.equipment.smokeGrenades != 0)
	{
		plan.preservedUtility = true;
	}
	return plan;
...
TeamBuyDecision BuyPlanner::planTeam(const TeamEconomySnapshot& snapshot) const noexcept
{
...
const char* BuyPlanner::strategyName(BuyStrategy strategy) noexcept
{
	switch (strategy)
	{
	case BuyStrategy::None:
		return "none";
	case BuyStrategy::PistolRound:
		return "pistol-round";
	case BuyStrategy::Eco:
		return "eco";
	case BuyStrategy::HalfBuy:
		return "half-buy";
	case BuyStrategy::ForceBuy:
		return "force-buy";
	case BuyStrategy::FullBuy:
		return "full-buy";
	case BuyStrategy::Save:
		return "save";
	}
	return "unknown";
}
...
const char* BuyPlanner::strategyReasonName(StrategyReason reason) noexcept
{
	switch (reason)
	{
	case StrategyReason::None:
		return "none";
	case StrategyReason::InvalidInput:
		return "invalid-input";
	case StrategyReason::PistolRound:
		return "pistol-round";
	case StrategyReason::TeamMedianBudget:
		return "team-median-budget";
	case StrategyReason::MajorityReady:
		return "majority-ready";
	case StrategyReason::LossRecovery:
		return "loss-recovery";
	case StrategyReason::CarriedEquipment:
		return "carried-equipment";
	case StrategyReason::LowTeamBudget:
		return "low-team-budget";
	}
	return "unknown";
}
...
const char* BuyPlanner::failureName(BuyFailureReason reason) noexcept
{
	switch (reason)
	{
	case BuyFailureReason::None:
		return "none";
	case BuyFailureReason::InvalidPlan:
		return "invalid-plan";
	case BuyFailureReason::StaleIdentity:
		return "stale-identity";
	case BuyFailureReason::NotEnoughMoney:
		return "not-enough-money";
	case BuyFailureReason::NotInBuyZone:
		return "not-in-buy-zone";
	case BuyFailureReason::BuyTimeExpired:
		return "buy-time-expired";
	case BuyFailureReason::ItemUnavailable:
		return "item-unavailable";
	case BuyFailureReason::AlreadyOwned:
		return "already-owned";
	case BuyFailureReason::InventoryConflict:
		return "inventory-conflict";
	case BuyFailureReason::CommandRejected:
		return "command-rejected";
	case BuyFailureReason::RetryLimit:
		return "retry-limit";
	case BuyFailureReason::Unknown:
		return "unknown";
	}
	return "unknown";
}
