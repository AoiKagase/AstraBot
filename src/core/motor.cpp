...
{
	if (!std::isfinite(limit) || limit < 0 || limit > kMaxMovement)
		return 0;
	if (mode == LocomotionMode::Run)
		return limit;
	if (mode == LocomotionMode::Walk)
		return 0.4 * limit;
	return 0;
}
double Motor::requestedSpeed(const MovementIntent& intent) noexcept
{
	return intent.locomotion == LocomotionMode::Explicit
			   ? intent.speed
			   : locomotionSpeed(intent.locomotion, intent.decisionSpeedLimit);
}
bool Motor::bindLocomotion(MovementIntent& intent, LocomotionMode mode, double limit, double distance) noexcept
{
	if ((mode != LocomotionMode::Run && mode != LocomotionMode::Walk) || !std::isfinite(distance) || distance <= 0)
		return false;
	const double speed = locomotionSpeed(mode, limit);
	if (speed <= 0)
		return false;
	const double lifetime = std::floor(distance * 1000000.0 / speed);
	if (!std::isfinite(lifetime) || lifetime < 1)
		return false;
	intent.speed = 0;
	intent.decisionSpeedLimit = limit;
	intent.locomotion = mode;
	intent.validForUs = static_cast<std::uint64_t>((std::min)(lifetime, double(kMaximumMovementIntentUs)));
	return valid(intent);
}
MotorResult Motor::command(const MovementIntent& intent, ViewAngles observed, float maximum, std::uint64_t frameUs,
						   bool firstFrame) noexcept
{
	if (!valid(intent))
		return {{}, MotorError::InvalidIntent};
	if (!std::isfinite(maximum) || maximum < 0 || !std::isfinite(observed.pitch) || !std::isfinite(observed.yaw) ||
		!std::isfinite(observed.roll))
		return {{}, MotorError::InvalidObservation};
	if (!frameUs)
		return {{}, MotorError::NoElapsedTime};
	...
	set(intent.duck, Button::Duck);
	set(intent.jump, Button::Jump);
	set(intent.use, Button::Use);
	if (speed > 0)
	{
		set(intent.forward, Button::Forward);
		set(intent.back, Button::Back);
	}
