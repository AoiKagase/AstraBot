...
bool validPlayer(PlayerId p) noexcept
{
	return p.isValid() && p.slot <= kPlayerCapacity;
}
...
{
	if (!finite(eye) || !finite(forward) || !finite(end))
...
{
	return intervalMicros > 0 && std::isfinite(maxDistance) && maxDistance > 0 && std::isfinite(fullFovDegrees) &&
		   fullFovDegrees > 0 && fullFovDegrees <= 180;
}
...
void Vision::forget(PlayerId player) noexcept
{
	...
	for (auto& state : states_)
	{
		...
		for (std::size_t i = kept; i < batch.count; ++i)
...
void Vision::update(const InputFrame& input, IVisibilityQueries& queries) noexcept
{
	...
	{
		reset();
		frameReason_ = Reason::InvalidSettings;
		return;
	}
	if (!frame.map.isValid() || !frame.tick.isValid() || !frame.round.isValid() ||
		(frame.map == map_ && frame.round.value < round_.value) ||
		(map_.isValid() && (frame.map.value < map_.value || (frame.map == map_ && (frame.tick.value <= tick_.value ||
																				   frame.timeMicros < timeMicros_)))))
	{
	...
	for (std::size_t i = 0; i < states_.size(); ++i)
	{
		...
		if (!eligible(p, i) || !p.agent.isValid())
			continue;
		auto& state = states_[i];
		if (state.observer != p.player || state.agent != p.agent)
		{
			state = {};
			state.observer = p.player;
			state.agent = p.agent;
	...
	for (std::size_t offset = 0; offset < states_.size(); ++offset)
	{
		...
		if (!state.observer.isValid() || frame.timeMicros < state.nextMicros)
			continue;
		if (frameUpdates_ == kBotsPerFrame)
		{
			++state.diagnostics.deferredFrames;
			continue;
		}
		...
		batch.identity = {frame.map,   frame.round,		 ObservationSource::Vision,
						  ++sequence_, frame.timeMicros, frame.timeMicros};
		Diagnostics d{};
		...
		for (std::size_t j = 0; j < frame.players.size(); ++j)
		{
			...
			if (j == index || !target.alive)
				continue;
			if (!eligible(target, j))
			{
				count(d, Reason::StaleIdentity);
				continue;
			}
			++d.candidates;
			for (const auto point : {target.eye, target.center})
			{
				...
				count(d, reason);
				if (reason == Reason::StaleIdentity || reason == Reason::MissingEngine)
					break;
			}
		}
		state.batch = batch;
