...
{
	return std::isfinite(movement.forward) && std::isfinite(movement.side) && std::isfinite(movement.up);
}
...
{
	return movement.forward >= -kMaxMovement && movement.forward <= kMaxMovement && movement.side >= -kMaxMovement &&
		   movement.side <= kMaxMovement && movement.up >= -kMaxMovement && movement.up <= kMaxMovement;
}
