...
{
	return static_cast<std::uint8_t>(weapon) <= static_cast<std::uint8_t>(combat::WeaponSnapshot::WeaponClass::Melee);
}

bool validApproach(ApproachDirection approach) noexcept
{
	return static_cast<std::uint8_t>(approach) <= static_cast<std::uint8_t>(ApproachDirection::Down);
}
...
bool operator<(const ContextualDangerKey& left, const ContextualDangerKey& right) noexcept
{
	return std::tie(left.area, left.team, left.approach, left.enemyWeaponClass, left.likelyEnemyArea) <
		   std::tie(right.area, right.team, right.approach, right.enemyWeaponClass, right.likelyEnemyArea);
}

bool ContextualDangerObservation::valid() const noexcept
{
	return key.valid() && map.isValid() && finiteNonNegative(risk) && risk <= 1.0 && finiteNonNegative(weight) &&
		   weight > 0.0 && weight <= 1'000'000.0;
}

bool ContextualDangerRecord::valid() const noexcept
{
	return key.valid() && finiteNonNegative(risk) && risk <= 1.0 && finiteNonNegative(observations) &&
		   observations <= 1'000'000.0;
}

ContextualDangerUpdate ContextualDangerModel::observe(const ContextualDangerObservation& observation) noexcept
{
	if (!observation.valid())
		return {ContextualDangerUpdateReason::InvalidObservation, false};
	if (!map_.isValid() || observation.map != map_)
	{
		return {ContextualDangerUpdateReason::WrongMap, false};
	}
	auto it = std::lower_bound(
		records_.begin(), records_.end(), observation.key,
		[](const ContextualDangerRecord& record, const ContextualDangerKey& key) { return record.key < key; });
	...
	const double nextObservations = (std::min)(1'000'000.0, it->observations + observation.weight);
...
{
	if (!map.isValid() || (map_.isValid() && map.value <= map_.value))
		return false;
	try
	{
		records_.clear();
...
const ContextualDangerRecord* ContextualDangerModel::find(const ContextualDangerKey& key) const noexcept
{
	const auto it = std::lower_bound(
		records_.begin(), records_.end(), key,
		[](const ContextualDangerRecord& record, const ContextualDangerKey& value) { return record.key < value; });
	if (it == records_.end() || it->key < key || key < it->key)
		return nullptr;
	return &*it;
}
...
bool OpponentObservation::valid() const noexcept
{
	return player.isValid() && map.isValid() && round.isValid() && tick.isValid() && area != 0U &&
		   ...
		   finiteNonNegative(rushProbability) && rushProbability <= 1.0 && finiteNonNegative(campProbability) &&
		   campProbability <= 1.0 && finiteNonNegative(rotationSpeed) && rotationSpeed <= 1.0;
}

bool OpponentProfile::valid() const noexcept
{
	return player.isValid() && map.isValid() && finiteNonNegative(aggression) && aggression <= 1.0 &&
		   finiteNonNegative(rushProbability) && rushProbability <= 1.0 && finiteNonNegative(campProbability) &&
		   campProbability <= 1.0 && finiteNonNegative(rotationSpeed) && rotationSpeed <= 1.0 &&
...
{
	if (!round.isValid() || (round_.isValid() && round.value <= round_.value))
		return false;
	round_ = round;
	return true;
}
...
void OpponentProfileModel::forget(PlayerId player) noexcept
{
	if (!player.isValid())
		return;
	if (player.slot <= retiredGenerations_.size() && retiredGenerations_[player.slot - 1U] < player.generation)
	{
		retiredGenerations_[player.slot - 1U] = player.generation;
	}
	for (std::size_t i = 0; i < count_; ++i)
	{
		if (profiles_[i].player != player)
			continue;
...
OpponentProfileUpdate OpponentProfileModel::observe(const OpponentObservation& observation) noexcept
{
	if (!observation.valid())
	...
	if (observation.player.slot == 0 || observation.player.slot > retiredGenerations_.size())
	{
		return {OpponentProfileUpdateReason::InvalidObservation, false};
	}
	...
	if (profile == nullptr)
	{
		if (count_ >= profiles_.size())
			return {OpponentProfileUpdateReason::CapacityExceeded, false};
...
WallbangPlan planWallbang(const WallbangInput& input) noexcept
{
	...
	if (!input.belief.valid(input.nowMicros) || !finitePoint(input.belief.lastKnownPosition) ||
		input.maxAgeMicros == 0U || input.minimumConfidence <= 0.0 || input.minimumConfidence > 1.0 ||
		!finiteNonNegative(input.wallThickness) || !std::isfinite(input.materialLoss) || input.materialLoss <= 0.0 ||
		!finiteNonNegative(input.weaponPenetration) || !finiteNonNegative(input.friendlyFireRisk) ||
		input.friendlyFireRisk > 1.0 || !finiteNonNegative(input.expectedDamage) || input.ammoCost <= 0 ||
		input.availableAmmo < 0)
	{
	...
	const double utility =
		effectiveDamage / static_cast<double>(input.ammoCost) - input.friendlyFireRisk * input.expectedDamage;
...
SuppressiveFirePlan planSuppressiveFire(const SuppressiveFireInput& input) noexcept
{
	...
	if (input.regionArea == 0U || !finitePoint(input.region) || !finiteNonNegative(input.regionRadius) ||
		input.regionRadius <= 0.0 || !finiteNonNegative(input.probability) || input.probability > 1.0 ||
	...
	result.utility = input.expectedDamage * input.probability * (1.0 - input.friendlyFireRisk);
