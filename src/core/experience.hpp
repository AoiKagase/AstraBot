...
struct MapIdentity final
{
	...
	friend bool operator==(const MapIdentity& left, const MapIdentity& right) noexcept
	{
...
{
public:
	explicit ExperienceModel(ExperienceSettings settings = {}) noexcept : settings_(settings)
	{
	}

	bool activate(const MapIdentity& map, perception::RoundGeneration round) noexcept;
	void reset() noexcept;
	ExperienceUpdateResult beginRound(perception::RoundGeneration round, std::uint64_t timeMicros) noexcept;
	...
	const MapIdentity& map() const noexcept
	{
		return map_;
	}
	perception::RoundGeneration round() const noexcept
	{
		return round_;
	}
	std::uint64_t timeMicros() const noexcept
	{
		return timeMicros_;
	}
	...
	const ExperienceTotals& totals() const noexcept
	{
		return totals_;
	}
	ExperienceSnapshot snapshot() const;
	const ExperienceSettings& settings() const noexcept
	{
		return settings_;
	}

private:
	ExperienceUpdateResult applyDecayTo(perception::RoundGeneration round, std::uint64_t timeMicros) noexcept;
	...
	ExperienceUpdateResult updateArea(const ExperienceEvent& event, AreaExperience& value, double weight) noexcept;
...
public:
	...
	virtual PersistenceResult load(const MapIdentity& map, ExperienceSnapshot& snapshot) = 0;
...
{
public:
	explicit BinaryExperienceStore(std::string path) noexcept : path_(std::move(path))
	{
	}

	PersistenceResult load(const MapIdentity& map, ExperienceSnapshot& snapshot) override;
...
private:
	PersistenceResult readFile(const std::string& path, const MapIdentity& expected,
							   ExperienceSnapshot& snapshot) const;
...
public:
	...
	ExperiencePipeline(ExperienceSettings settings, IExperiencePersistence* persistence) noexcept
	...
	ExperienceModel& model() noexcept
	{
		return model_;
	}
	const ExperienceModel& model() const noexcept
	{
		return model_;
	}
