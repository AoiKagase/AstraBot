...
constexpr ButtonMask kKnownButtonMask =
	static_cast<ButtonMask>(Button::Attack) | static_cast<ButtonMask>(Button::Jump) |
	static_cast<ButtonMask>(Button::Duck) | static_cast<ButtonMask>(Button::Forward) |
...
struct BotCommand
{
	...
	{
		return left.view == right.view && left.movement == right.movement && left.buttons == right.buttons &&
			   left.weaponSelect == right.weaponSelect && left.impulse == right.impulse && left.msec == right.msec;
	}
	friend bool operator!=(const BotCommand& left, const BotCommand& right) noexcept
	{
		return !(left == right);
