...
// A retained visual may support tracking briefly, but it must not keep a bot
// firing or pursuing a target after the observation has gone stale.
constexpr std::uint64_t kMaxVisualAgeMicros = 300'000;
...
struct WeaponId
{
	...
	static constexpr WeaponId invalid() noexcept
	{
		return {};
	}
...
struct WeaponSnapshot
{
	...
	bool hasAmmunition() const noexcept
	{
...
struct FirePlan
{
	...
	static constexpr FirePlan tap() noexcept
	{
		return {FirePattern::Tap, 1};
	}
	static constexpr FirePlan burst(std::uint8_t shots) noexcept
	{
		return {FirePattern::Burst, shots};
	}
	static constexpr FirePlan fullAuto() noexcept
	{
		return {FirePattern::FullAuto, 0};
	}
...
{
	if (!input.alive || map != input.map || round != input.round || observer != input.player || agent != input.agent ||
		!target.isValid() || !targetGeneration.isValid() || target.generation != targetGeneration ||
		target.slot > input.world.roster.size() || lastConfirmedMicros < acquiredMicros ||
		expiresMicros < lastConfirmedMicros || input.timeMicros < lastConfirmedMicros ||
		input.timeMicros > expiresMicros)
	{
		return false;
	}
	...
	const bool observerPlaying =
		input.team == perception::Team::Terrorist || input.team == perception::Team::CounterTerrorist;
	const bool targetPlaying =
		member.team == perception::Team::Terrorist || member.team == perception::Team::CounterTerrorist;
	return member.player == target && observerPlaying && targetPlaying && member.team != input.team;
}
...
struct AttackLifecycleState
{
	...
	{
		return initialized && map == input.map && round == input.round && player == input.player &&
			   agent == input.agent;
	}
};
