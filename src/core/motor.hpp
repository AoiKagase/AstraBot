...
struct MotorResult
{
	...
	{
		return command.has_value() && error == MotorError::None;
	}
};
class Motor final
{
public:
	...
	static MotorResult command(const MovementIntent&, ViewAngles observedView, float observedSpeedLimit,
...
inline bool Motor::valid(const MovementIntent& i) noexcept
{
	...
	const auto action = [](ActionRequest a) {
		return a == ActionRequest::None || a == ActionRequest::Press || a == ActionRequest::Hold ||
			   a == ActionRequest::Release;
	};
	...
	if (!mode || !finite(i.direction) || !std::isfinite(i.speed) || i.speed < 0 || i.speed > kMaxMovement ||
		...
		!std::isfinite(i.lateralCorrection) || std::abs(i.lateralCorrection) > 1 || (i.view && !finite(*i.view)) ||
		!action(i.duck) || !action(i.jump) || !action(i.use) || !action(i.forward) || !action(i.back))
		return false;
	if (i.validForUs > kMaximumMovementIntentUs ||
		(i.locomotion == LocomotionMode::Explicit && i.decisionSpeedLimit != 0) ||
		(i.locomotion != LocomotionMode::Explicit && (i.speed != 0 || i.decisionSpeedLimit <= 0 || i.validForUs == 0)))
		return false;
	...
	if ((moving(i.forward) && moving(i.back)) || ((moving(i.forward) || moving(i.back)) && requested == 0))
		return false;
