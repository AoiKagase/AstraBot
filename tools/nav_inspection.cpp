...
const char* recordName(NavRecord value)
{
	...
	case NavRecord::None:
		return "None";
	case NavRecord::RawInput:
		return "RawInput";
	case NavRecord::FileHeader:
		return "FileHeader";
	case NavRecord::PlaceDictionary:
		return "PlaceDictionary";
	case NavRecord::Area:
		return "Area";
	case NavRecord::Connection:
		return "Connection";
	case NavRecord::HidingSpot:
		return "HidingSpot";
	case NavRecord::Approach:
		return "Approach";
	case NavRecord::Encounter:
		return "Encounter";
	case NavRecord::TraversalLink:
		return "TraversalLink";
	case NavRecord::Graph:
		return "Graph";
	case NavRecord::Route:
		return "Route";
	}
	return "Unknown";
}
const char* fieldName(NavField value)
{
	...
	case NavField::None:
		return "None";
	case NavField::RawBytes:
		return "RawBytes";
	case NavField::Magic:
		return "Magic";
	case NavField::Version:
		return "Version";
	case NavField::BspSize:
		return "BspSize";
	case NavField::PlaceCount:
		return "PlaceCount";
	case NavField::PlaceLength:
		return "PlaceLength";
	case NavField::PlaceText:
		return "PlaceText";
	case NavField::AreaCount:
		return "AreaCount";
	case NavField::AreaId:
		return "AreaId";
	case NavField::Attributes:
		return "Attributes";
	case NavField::NorthWestExtent:
		return "NorthWestExtent";
	case NavField::SouthEastExtent:
		return "SouthEastExtent";
	case NavField::NorthEastZ:
		return "NorthEastZ";
	case NavField::SouthWestZ:
		return "SouthWestZ";
	case NavField::ConnectionCount:
		return "ConnectionCount";
	case NavField::ConnectionAreaId:
		return "ConnectionAreaId";
	case NavField::HidingSpotCount:
		return "HidingSpotCount";
	case NavField::HidingSpotId:
		return "HidingSpotId";
	case NavField::HidingSpotFlags:
		return "HidingSpotFlags";
	case NavField::ApproachCount:
		return "ApproachCount";
	case NavField::ApproachAreaId:
		return "ApproachAreaId";
	case NavField::ApproachTraversal:
		return "ApproachTraversal";
	case NavField::EncounterCount:
		return "EncounterCount";
	case NavField::EncounterAreaId:
		return "EncounterAreaId";
	case NavField::EncounterDirection:
		return "EncounterDirection";
	case NavField::EncounterSpotCount:
		return "EncounterSpotCount";
	case NavField::EncounterSpotId:
		return "EncounterSpotId";
	case NavField::EncounterSpotT:
		return "EncounterSpotT";
	case NavField::Place:
		return "Place";
	case NavField::GraphBytes:
		return "GraphBytes";
	case NavField::GraphTraversal:
		return "GraphTraversal";
	case NavField::RouteStart:
		return "RouteStart";
	case NavField::RouteGoal:
		return "RouteGoal";
	case NavField::RouteCost:
		return "RouteCost";
	case NavField::RouteHeuristic:
		return "RouteHeuristic";
	case NavField::RouteBytes:
		return "RouteBytes";
	case NavField::LinkCount:
		return "LinkCount";
	case NavField::LinkWorkingBytes:
		return "LinkWorkingBytes";
	case NavField::LinkFingerprint:
		return "LinkFingerprint";
	case NavField::LinkSourceId:
		return "LinkSourceId";
	case NavField::LinkGeneration:
		return "LinkGeneration";
	case NavField::LinkId:
		return "LinkId";
	case NavField::LinkFrom:
		return "LinkFrom";
	case NavField::LinkTo:
		return "LinkTo";
	case NavField::LinkTraversal:
		return "LinkTraversal";
	case NavField::LinkDirection:
		return "LinkDirection";
	case NavField::LinkEntry:
		return "LinkEntry";
	case NavField::LinkExit:
		return "LinkExit";
	case NavField::LinkCost:
		return "LinkCost";
	case NavField::LinkGenerationConflict:
		return "LinkGenerationConflict";
	case NavField::LinkConflict:
		return "LinkConflict";
	}
	return "Unknown";
}
...
#define L(field) out << "limit." #field "=" << p.field << '\n';
	L(mesh.maxInputBytes)
	L(mesh.header.maxAreas)
	L(mesh.header.maxPlaces)
	L(mesh.header.maxPlaceBytes)
	L(mesh.header.maxTotalPlaceBytes)
	L(mesh.areas.maxAreas)
	L(mesh.areas.maxConnectionsPerDirection)
	L(mesh.areas.maxHidingSpotsPerArea)
	L(mesh.areas.maxApproachesPerArea)
	L(mesh.areas.maxEncountersPerArea)
	L(mesh.areas.maxEncounterSpotsPerPath)
	L(mesh.areas.maxTotalConnections)
	L(mesh.areas.maxTotalHidingSpots)
	L(mesh.areas.maxTotalApproaches)
	L(mesh.areas.maxTotalEncounters)
	L(mesh.areas.maxTotalEncounterSpots)
	L(mesh.maxSnapshotBytes)
	L(index.maxAreas)
	L(index.maxNodes)
	L(index.maxIndexBytes)
	L(graph.maxAreas)
	L(graph.maxEdges)
	L(graph.maxGraphBytes)
	L(route.maxExpansions)
...
void vector(std::ostream& out, model::NavVector3 v)
{
	out << v.x << ',' << v.y << ',' << v.z;
}
void metadata(std::ostream& out, const model::NavMeshSnapshot& mesh, std::size_t bytes)
{
	...
	{
		out << "area=" << area.id.value << ",attributes=" << static_cast<unsigned>(area.attributes) << ",nw=";
		vector(out, area.extent.northWest);
		out << ",se=";
		vector(out, area.extent.southEast);
		...
		hiding += area.hidingSpots.size();
		approaches += area.approaches.size();
		encounters += area.encounters.size();
		for (const auto& encounter : area.encounters)
			spots += encounter.spots.size();
	}
	out << "connections=" << connections << "\nhiding_spots=" << hiding << "\napproaches=" << approaches
		<< "\nencounters_retained=" << encounters << "\nencounter_spots_retained=" << spots
		<< "\nlegacy_encounters=" << (header.version <= model::NavVersion::V2 ? "DiscardedNotCounted" : "NotApplicable")
		<< '\n';
}
std::optional<model::NavAreaId> match(std::ostream& out, const char* label, const query::NavSpatialIndex& index,
									  model::NavVector3 point, const Query& q)
{
	...
	if (!result)
	{
		error(out, label, result.error);
		return {};
	}
	if (!*result.value)
	{
		out << label << "_match=None\n";
		return {};
	}
	const auto& hit = **result.value;
	out << label << "_match=" << hit.areaId.value << '\n'
		<< label << "_distance_squared=" << hit.distanceSquared << '\n'
		<< label << "_projected=" << hit.projectedPoint.x << ',' << hit.projectedPoint.y << ',' << hit.projectedPoint.z
		<< '\n';
	return hit.areaId;
}
...
int routes(std::ostream& out, const Options& options, const Profile& profile, const query::NavSpatialIndex& index,
		   const query::NavGraph& graph)
{
	if (!options.query)
	{
		out << "query=NotRequested\n";
		return 0;
	}
	...
	out << "\nroute_total=" << r.total << "\nroute_components=";
	costs(out, r.components);
	out << '\n';
	for (const auto& step : r.steps)
	{
		out << "edge=" << step.edge.source.value << ',' << step.edge.target.value << ','
			<< static_cast<unsigned>(step.edge.direction) << ",traversal=" << static_cast<unsigned>(step.edge.traversal)
			<< ",external=false,total=" << step.total << ",components=";
		costs(out, step.components);
		out << '\n';
	}
...
bool point(const char* const* text, model::NavVector3& value)
{
	double v[3]{};
	for (int i = 0; i < 3; ++i)
		if (!number(text[i], v[i]) || std::abs(v[i]) > std::numeric_limits<float>::max())
			return false;
...
{
	if (!help)
		failure(out, "cli", NavErrorKind::InvalidInput);
...
int run(const Options& options, const Profile& profile, std::ostream& out)
{
	...
	try
	{
		...
		if (!fileLength(options.nav, length))
			return failure(out, "input", NavErrorKind::InvalidInput);
		out << "input_bytes=" << length << '\n';
		if (length > profile.mesh.maxInputBytes)
			return failure(out, "input", NavErrorKind::CountLimitExceeded);
		if (length > static_cast<std::uintmax_t>(std::numeric_limits<std::streamsize>::max()) ||
			length > std::numeric_limits<std::size_t>::max())
			return failure(out, "input", NavErrorKind::OffsetOverflow);
		...
		if (!loaded)
			return error(out, "load", loaded.error);
		...
		if (options.bsp)
		{
			...
			if (!fileLength(*options.bsp, bspBytes))
				return failure(out, "bsp", NavErrorKind::InvalidInput);
			out << "bsp_bytes=" << bspBytes << "\nbsp_size_comparison=";
			if (!mesh.header().bspSize)
				out << "AbsentInVersion";
			else
			{
				bspMismatch = bspBytes != *mesh.header().bspSize;
				out << (bspMismatch ? "Mismatch" : "Match");
			}
			out << '\n';
		}
		else
			out << "bsp_size_comparison=Unverified\n";
		const auto index = query::NavSpatialIndex::build(*loaded.value, profile.index);
		if (!index)
			return error(out, "index", index.error);
		const auto graph = query::NavGraph::build(*loaded.value, profile.graph);
		if (!graph)
			return error(out, "graph", graph.error);
		...
		if (bspMismatch)
			return error(out, "bsp", {NavErrorKind::InvalidValue, 8, NavRecord::FileHeader, NavField::BspSize});
		return out && code == 0 ? 0 : 1;
	}
	catch (const std::bad_alloc&)
	{
		return failure(out, "inspection", NavErrorKind::AllocationFailure);
	}
...
int cli(int argc, const char* const* argv, std::ostream& out)
{
	try
	{
		...
		for (int i = 1; i < argc; ++i)
		{
			...
			else if (arg == "--goal")
			{
				...
				flag = 64;
			if (!flag || (seen & flag) || argc - i - 1 < count)
				return usage(out, false);
			seen |= flag;
			if (flag == 1 || flag == 2)
			{
				if (!argv[i + 1][0] || std::string_view(argv[i + 1]).substr(0, 2) == "--")
					return usage(out, false);
				if (flag == 1)
					options.nav = fs::path(argv[i + 1]);
				else
					options.bsp = fs::path(argv[i + 1]);
			}
			else if (flag == 4)
			{
				if (std::string_view(argv[i + 1]) != "compatibility-v1")
					return usage(out, false);
			}
			else if (flag == 8 || flag == 16)
			{
				if (!point(argv + i + 1, flag == 8 ? q.start : q.goal))
					return usage(out, false);
			}
			else if (!number(argv[i + 1], flag == 32 ? q.radius : q.vertical) ||
					 (flag == 32 ? q.radius : q.vertical) < 0)
				return usage(out, false);
			i += count;
		}
		if (!(seen & 1) || ((seen & 120) != 0 && (seen & 120) != 120))
			return usage(out, false);
		if (seen & 120)
			options.query = q;
	...
	{
		return failure(out, "cli", NavErrorKind::InvalidInput);
	}
}
} // namespace astrabot::tools::inspection
