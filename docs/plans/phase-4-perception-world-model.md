...
`5c3a0be`へ統合済み。P4-03〜P4-09は承認済み計画であり、実装未着手。
...
- **Implementation outline / commit slices:**
  - [ ] 人間を含む全プレイヤーの世代付き所属をTeamInfoから管理し、未知を敵扱いしない。
  - [ ] 検証済みラウンド開始通知でRoundGenerationを更新し、前ラウンドの記憶と待機情報を破棄する。
    ResetHUD単独では全体のラウンド開始を判定しない。
  - [ ] 共通観測識別と元の時刻／受信時刻を追加する。遅延で寿命を延長しない。
  - [ ] 切断・所属・世代・ラウンド変更を位置情報なしで通知し、テスト・報告を揃える。
...
- **Implementation outline / commit slices:**
  - [ ] 標準の音・再生イベントhookを接続し、precache名とイベント番号の対応を管理する。
    固定イベント番号を埋め込まない。
  - [ ] 足音・銃声・爆発について取得経路と対応表を記録する。未知イベントは診断して除外し、
    速度や武器状態から音を捏造しない。
  - [ ] Adapter内で距離・音量・attenuationから受聴可否を判定し、初期幅256 unitsで匿名化する。
    壁の透過・反射は再現したと扱わない。
  - [ ] 固定256件キュー、最大32件/frame、観測者あたり最大16件保持を実装する。
    古い順で処理し、満杯時は新規イベントを破棄して計数する。
  - [ ] 不正値、重複、距離境界、overflow、退役のテストと対応能力の報告を追加する。
...
- **Implementation outline / commit slices:**
  - [ ] 発生経路を確認した煙イベントだけを期限付き領域へ変換する。
    一般的な煙エフェクトをすべてスモークグレネードと解釈しない。
  - [ ] 半径・持続時間を設定値として近似モデルを定義し、有効な煙と交差する視線サンプルを拒否する。
    具体値と対象イベントは出所を確認した対応表へ記録してから実装する。
  - [ ] 観測者宛ての検証済み白色ScreenFadeを効果時間付きで扱い、その間の新規視認を抑制する。
    黒フェード等はフラッシュと扱わない。
  - [ ] 煙領域は最大32件とする。超過時は新規視認を保守的に抑制し、超過状態も期限付きで管理する。
  - [ ] 妨害中の記憶減衰、効果終了、能力不足、マップ／ラウンド退役のテスト・報告を追加する。
...
- **Implementation outline / commit slices:**
  - [ ] 視認記憶と匿名音源のownerと公開snapshotを追加する。
  - [ ] 出所と元の時刻を保持し、音でlastSeenを更新しない。受信数だけで確信度を加算しない。
  - [ ] 視認は1.0／5秒、音は0.5／元の発生時刻から3秒の線形失効を実装する。
  - [ ] 更新順を「無効化→減衰→観測ID順の取り込み→公開」に固定する。
  - [ ] 件数・年齢・出所・拒否理由・キュー超過・遅延の診断と決定的replayを追加する。
...
次は**P4-03の全スライス**。この計画を読み、mainと指示の現状、TeamInfo／ラウンド通知の
既存経路を確認して専用worktreeで開始する。P4-04以降を先取りして実装しない。
