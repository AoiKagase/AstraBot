...
Status: P4-01〜09の実装と適用可能なオフライン検証を完了。
P4-09 `17ae95a`をmainへfast-forward統合後、Windows portable52/52、Metamod67/67、
Linux portable51/51とRelease PE32/x86／指定6 exportの全gateを通過した。
[P4完了報告](../reports/p4-09-offline-gate.md)に証跡・対応能力・制約を記録した。
実機受入は未完了。プロジェクト全体Finishは宣言しない。
...
- **Implementation outline / commit slices:**
  - [x] 報告者が500ms以内に直接視認した相手だけを報告可能とする。
    伝聞の再転送、匿名音の個人への結合を禁止する。
  - [x] 現在同チームの有効なAstraBotだけに配信する。未知所属、敵、別ラウンド、古い世代を拒否する。
  - [x] 明示的な関数／console操作でのみ送信し、常時自動共有しない。
  - [x] 上限確信度0.5、元の観測から5秒で失効する伝聞を直接視認と別に保持する。
    直接視認を優先し、再送や受信時刻で寿命を延長しない。
  - [x] 重複・不正送信・所属変更・切断・出所保持のテストと報告を追加する。
- **Evidence:** [検証報告](../reports/p4-08-team-reports.md)。`f68da7c`をmainへ統合し、全gate完了。
...
- **Implementation outline / commit slices:**
  - [x] 視認→遮蔽→減衰→音→味方報告→再視認を統合replayにする。
  - [x] 煙／フラッシュ、ラウンド／マップ変更、切断、世代再利用、NAVなし、キュー超過を組み合わせる。
  - [x] 1／8／16 Botと8／16／100ms frameのmatrixで、上限、公平性、遅延、メモリを記録する。
  - [x] 既存移動の到着・停止・復旧と並行して検証し、知覚追加による移動回帰を検出する。
  - [x] 全共通gateとマージ後検証を実施し、対応イベント表、未対応能力、既定値、API、ログを報告する。
  - [x] 実機受入を未完了の別表に残し、P4全体の実装・適用可能なオフライン検証完了を判定する。
- **Evidence:** [完了報告](../reports/p4-09-offline-gate.md)。Core/NAV18条件とFake-engine15条件の決定性・独立期待値検査、main全gate完了。
...
P4の実装・適用可能なオフライン検証は完了。[完了報告](../reports/p4-09-offline-gate.md)と
各項目の報告書を次のPhaseの前提にする。後続Phaseやプロジェクト全体Finishは別途判断し、
上記post-Finish実機受入を未チェックのまま保持する。
