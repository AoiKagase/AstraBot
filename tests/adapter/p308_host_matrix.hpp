...
P308HostResult p308HostRun(unsigned actors, std::uint64_t us, unsigned mode, bool mapChange)
{
	...
	for (unsigned epoch = 0; epoch < (mapChange ? 2U : 1U); ++epoch)
	{
		...
		{
			fixture.matrixEntity(slot)->free = slot == 1 ? 0 : 1;
			if (epoch)
				++fixture.matrixEntity(slot)->serialnumber;
		}
		fixture.engine.pfnPEntityOfEntIndex = &captureDoorEntity;
		gDoorActive = false;
		gDoorOpenAtUs = 0;
		gSteeringMode = -1;
		gSimulateCrouch = gCrouchCeiling = false;
		...
		for (unsigned slot = 2; slot <= actors; ++slot)
		{
			...
			players.push_back(created.player);
			auto* entity = fixture.matrixEntity(slot);
			sendVguiMenu(hooks, 11, entity, 2, 1);
			navFrame(fixture, us);
			sendVguiMenu(hooks, 11, entity, 26, 1);
			navFrame(fixture, us);
		...
		for (unsigned i = 0; i < actors; ++i)
		{
			...
			areas.push_back(a);
			areas.push_back(b);
			auto* entity = fixture.matrixEntity(i + 1);
			entity->v = fixture.entity.v;
			entity->v.origin = Vector(mode == 1 ? 20 + static_cast<float>(i) * 4 : 50, y + 50, 36);
		}
		if (mode == 8 || mode == 9)
		{
			gStairHeight = 16;
			areas[1].extent = {{100, 0, 16}, {200, 100, 16}, 16, 16};
			areas[1].targets[3] = {1};
			if (mode == 9)
				fixture.entity.v.origin = Vector(150, 50, 52);
		}
		if (mode == 10)
		{
			gSteeringMode = 0;
		...
		if (mode == 18 || mode == 19)
		{
			...
			gMatrixPlayer = gPlayerObstacle = true;
			gSteeringMode = mode == 18 ? 1 : 2;
		}
		if (mode == 20)
			areas[0].targets[1].clear();
		if (ladder)
		{
			const auto mesh = configureLadderMotionFixture(fixture.engine, &fixture.entity,
														   owner.registry().mapGeneration(), mode >= 16);
			fixture.entity.v.origin = Vector(-49, 32, mode >= 16 ? 164.0f : 36.0f);
			fixture.entity.v.velocity = Vector(0, 0, 0);
			fixture.entity.v.movetype = MOVETYPE_WALK;
		...
		const auto goal = [&](unsigned i) {
			...
			assert(console.trace(players[i])->state ==
				   (diagnostic ? nav::runtime::SessionState::Failed : nav::runtime::SessionState::Ready));
		};
		gActorHullCalls.clear();
		gClientMoves.clear();
		gNavMoves.clear();
		for (unsigned i = 0; i < actors; ++i)
			goal(i);
		if (mode == 20)
		{
			...
			result.marker("unreachable:no-dispatch");
			if (mapChange && epoch == 0)
			{
				retiredMap = owner.registry().mapGeneration();
				retiredPlayers = players;
		...
		if (mode == 2 || mode == 14)
			gFrozenActor = fixture.matrixEntity(1);
		std::vector<std::uint64_t> sequences(actors), decisionTimes(actors), routeGenerations(actors, 1);
		for (unsigned i = 0; i < actors; ++i)
			routeGenerations[i] = console.trace(players[i])->routeGeneration;
		...
		const auto frame = [&] {
			...
			for (unsigned i = 0; i < actors; ++i)
			{
				...
				const unsigned queries = ladder ? ladderMotionQueryCount() - oldLadder
												: gActorHullCalls[const_cast<edict_t*>(entity)] - before;
				assert(queries <= 21);
				result.maxQueries = (std::max)(result.maxQueries, queries);
				result.totalQueries += queries;
				const auto* t = console.motionTrace(players[i]);
				assert(t);
				...
				if (t->sequence != sequences[i] || queries || dispatches[i])
				{
					...
					if (t->selectedEdge)
						line << ':' << t->selectedEdge->source.value << ':' << t->selectedEdge->target.value;
					if (t->decision.target)
						line << ':' << t->decision.target->origin.x << ':' << t->decision.target->origin.y;
					result.event("{\"type\":\"frame\",\"actor\":" + std::to_string(i + 1) +
								 ",\"tick\":" + std::to_string(result.frames) + ",\"data\":\"" + line.str() + "\"}");
				}
				assert(std::abs(entity->v.origin.y - (ladder ? 32.0f : i * 200 + 50.0f)) <
					   ((mode == 18 || mode == 19) ? 50.01f : 20.01f));
				if (!arrivals[i] && t->decision.state == nav::local::WalkState::Arrived)
					arrivals[i] = static_cast<unsigned>(result.frames);
			}
		};
		frame();
		if (mapChange && epoch == 0)
		{
			if (mode >= 5 && mode <= 14)
			{
				...
				{
					frame();
					const auto& d = console.motionTrace(players.front())->decision;
					active = mode == 11 ? (fixture.entity.v.flags & FL_DUCKING) != 0
							 : (mode == 12 || mode == 13)
								 ? (console.motionTrace(players.front())->command.buttons & IN_JUMP) != 0
			...
			if (ladder)
			{
				...
				{
					frame();
					const auto state = console.motionTrace(players.front())->decision.ladderState;
			...
			if (mode == 18 || mode == 19)
			{
				for (unsigned n = 0; n < 500 && !console.motionTrace(players.front())->decision.blocker; ++n)
					frame();
			...
			const auto allPending = [&] {
				...
				{
					const auto* trace = console.motionTrace(player);
					assert(trace);
			...
			for (unsigned n = 0; n < 500 && !allPending(); ++n)
				frame();
			assert(allPending());
			...
			assert(gClientMoves.size() == moves);
			for (const auto player : players)
				assert(!owner.entityFor(player) && !console.trace(player));
			if (ladder)
				assert(!console.ladders());
		...
		if (mode == 3)
		{
			const auto actor = selector(players.front());
			runNav({"astrabot_nav_cancel", actor.c_str()});
		}
		bool finished = false, ladderFault = false;
		for (unsigned n = 0; n < 1000; ++n)
		{
			if (mode == 14 && n * us >= 700000)
				gFrozenActor = nullptr;
			if (mode == 18 && n * us >= 300000)
				gSteeringMode = -1;
			if (mode == 17 && !ladderFault && (console.motionTrace(players.front())->command.buttons & IN_JUMP))
			{
				ladderFault = true;
				invalidateLadderMotionFixture(false);
			}
			frame();
			finished = true;
			for (unsigned i = 0; i < actors; ++i)
			{
				...
				const auto expected = (i == 0 && mode == 2) || mode == 7 || mode == 13 || mode == 17 || mode == 19
										  ? nav::local::WalkState::Failed
									  : i == 0 && mode == 3 ? nav::local::WalkState::Aborted
															: nav::local::WalkState::Arrived;
				if (state != expected)
					finished = false;
				if (i == 0 && mode == 2 &&
					console.motionTrace(players[i])->decision.recovery.state != nav::local::RecoveryState::Aborted)
					finished = false;
			}
			if (finished)
				break;
		...
		if (!finished)
			std::fprintf(stderr,
						 "P3-08 host nonterminal mode=%u actors=%u frame_us=%llu map_change=%u state=%u reason=%u\n",
						 mode, actors, static_cast<unsigned long long>(us), unsigned(mapChange),
						 unsigned(console.motionTrace(players.front())->decision.state),
						 unsigned(console.motionTrace(players.front())->decision.reason));
		assert(finished);
		if (mode == 19)
			assert(console.motionTrace(players.front())->decision.reason == nav::local::WalkReason::DynamicBlocked &&
		...
		if (mode == 13)
		{
			...
			const auto presses = std::count_if(gNavMoves.begin(), gNavMoves.end(),
											   [](const auto& command) { return (command.buttons & IN_JUMP) != 0; });
			assert(presses == 1 && decision.reason == nav::local::WalkReason::JumpFailed &&
				   decision.jumpReason == nav::local::JumpReason::TakeoffTimeout);
		}
		if (mode == 5)
			assert(gDoorUses > 0 && gDoorOpen);
		if (mode == 6)
			assert(gTouchContacts > 0 && gDoorOpen);
		if (mode == 7)
			assert(!gDoorOpen && gDoorUses > 0 &&
				   console.motionTrace(players.front())->decision.doorReason == nav::local::DoorWaitReason::TimedOut);
		if (mode == 11)
			assert(!(fixture.entity.v.flags & FL_DUCKING) && fixture.entity.v.origin.x > 216);
		if (mode == 1 && actors > 1)
			assert(arrivals.front() > arrivals.back());
		if (mode == 2)
		{
			const auto* t = console.motionTrace(players.front());
			assert(t->decision.recovery.state == nav::local::RecoveryState::Aborted &&
				   t->decision.recovery.attempts == 1);
			for (unsigned i = 1; i < actors; ++i)
				assert(arrivals[i] > 0 && arrivals[i] < result.frames);
		}
		if (mode == 4)
		{
			const auto stale = players.front();
			const auto oldSelector = selector(stale);
			owner.clientDisconnect(fixture.matrixEntity(1));
			fixture.matrixSlot = 1;
			++fixture.entity.serialnumber;
			const auto fresh = owner.createBot("AstraBot-Reused", {adapter::cstrike::Team::Terrorist, 1});
			assert(fresh.succeeded() && fresh.player.generation != stale.generation);
			players[0] = fresh.player;
			runNav({"astrabot_goto", "2", oldSelector.c_str()});
			assert(!console.trace(fresh.player));
			...
			sequences[0] = decisionTimes[0] = 0;
			routeGenerations[0] = 1;
			goal(0);
			for (unsigned n = 0;
				 n < 1000 && console.motionTrace(fresh.player)->decision.state != nav::local::WalkState::Arrived; ++n)
				frame();
			assert(console.motionTrace(fresh.player)->decision.state == nav::local::WalkState::Arrived);
			assert(!console.motionTrace(stale));
			result.marker("slot-reused:stale-selector-rejected:fresh-arrived");
		}
		for (unsigned i = 0; i < actors; ++i)
		{
			const auto state = console.motionTrace(players[i])->decision.state;
			const char* outcome = state == nav::local::WalkState::Arrived	? "Arrived"
								  : state == nav::local::WalkState::Aborted ? "Aborted"
																			: "Failed";
			assert(state != nav::local::WalkState::Running);
			result.event("{\"type\":\"terminal\",\"actor\":" + std::to_string(i + 1) +
						 ",\"tick\":" + std::to_string(result.frames) + ",\"outcome\":\"" + outcome + "\"}");
		}
		gFrozenActor = nullptr;
		gSimulateNav = false;
		detach();
...
{
	const char* names[] = {"host-simultaneous",
						   "host-staggered",
						   "host-blocked-actor",
						   "host-cancel",
						   "host-slot-reuse",
						   ...
						   "host-jump",
						   "host-jump-failed",
						   "host-stuck-transient",
						   "host-ladder-up",
						   "host-ladder-down",
						   "host-ladder-failed-dismount",
						   "host-player-transient",
						   "host-player-permanent",
						   "host-unreachable"};
	const char* outcomes[] = {"Arrived",
							  "Arrived",
							  "FailedWithPeerArrival",
							  "CancelledWithPeerArrival",
							  "ReusedAndArrived",
							  "Arrived",
							  "Arrived",
							  "Failed",
		...
		for (unsigned actors : {1U, 8U, 16U})
			for (std::uint64_t us : {8000U, 16000U, 100000U})
				for (bool changed : {false, true})
				{
					...
					const auto one = p308HostRun(actors, us, mode, changed),
							   two = p308HostRun(actors, us, mode, changed);
					if (one.trace != two.trace)
					{
						std::size_t index = 0;
						while (index < one.trace.size() && index < two.trace.size() &&
							   one.trace[index] == two.trace[index])
							++index;
					...
					assert(one.trace == two.trace && one.frames == two.frames && one.totalQueries == two.totalQueries &&
						   one.replans == two.replans && one.setupQueries == two.setupQueries &&
						   one.discoveryQueries == two.discoveryQueries && one.maxQueries == two.maxQueries);
					if (!first)
						out << ',';
					first = false;
					out << "{\"scenario\":\"" << names[mode] << "\",\"frameUs\":" << us << ",\"actors\":" << actors
						<< ",\"variant\":\"" << (changed ? "map-change" : "clean") << "\",\"seed\":308,\"outcome\":\""
						<< outcomes[mode] << "\",\"expectedOutcome\":\"" << outcomes[mode]
						<< "\",\"frames\":" << one.frames << ",\"elapsedUs\":" << one.elapsedUs
						<< ",\"maxQueriesPerActorFrame\":" << one.maxQueries << ",\"totalQueries\":" << one.totalQueries
						<< ",\"replans\":" << one.replans << ",\"setupQueries\":" << one.setupQueries
						<< ",\"discoveryQueries\":" << one.discoveryQueries
						<< ",\"traceLimit\":" << P308HostResult::traceLimit
						<< ",\"traceTruncated\":false,\"replayEqual\":true,\"trace\":[";
