...
void captureLifecycleTrace(const astrabot::debug::LifecycleTrace& trace) noexcept
{
	gLifecycleTraces.push_back(trace);
}

void captureCombatTrace(const astrabot::debug::CombatTrace& trace) noexcept
{
...
int captureArgc()
{
	return static_cast<int>(gCommandArgs.size());
}
...
void captureRunPlayerMove(edict_t* /* entity */, const float* /* viewAngles */, float /* forwardMove */,
						  float /* sideMove */, float /* upMove */, unsigned short /* buttons */, byte /* impulse */,
						  byte /* msec */)
{
	++gRunPlayerMoveCalls;
}
...
void runServerCommand(const std::initializer_list<const char*>& arguments, const char* name)
{
	gCommandArgs.clear();
...
void assertCallbacksEqual(const META_FUNCTIONS& actual, const META_FUNCTIONS& expected)
{
...
void testAttachValidationIsRollbackSafe()
{
	...
	auto noConsole = fixture.engine;
	noConsole.pfnAddServerCommand = nullptr;
...
void testAttachBeforeUserMessagesAreRegistered()
{
	...
	assert(
		!astrabot::adapter::metamod::lifecycleCoordinator().perceptionIdentityDiagnostics().roundNotificationAvailable);
...
void testSuccessfulAttachDoubleAttachAndDetach()
{
	...
	enginefuncs_t soundHooks{};
	int soundVersion = ENGINE_INTERFACE_VERSION;
	assert(fixture.callbacks.pfnGetEngineFunctions_Post(&soundHooks, &soundVersion));
	assert(soundHooks.pfnPrecacheEvent && soundHooks.pfnEmitSound && soundHooks.pfnEmitAmbientSound &&
		   soundHooks.pfnPlaybackEvent);
...
void testEmptyHookTablesAndInterfaceChecks()
{
	...
	assert(entityTable.pfnServerActivate == &astrabot::adapter::metamod::serverActivateHook);
	assert(entityTable.pfnServerDeactivate == &astrabot::adapter::metamod::serverDeactivateHook);
	assert(entityTable.pfnClientDisconnect == &astrabot::adapter::metamod::clientDisconnectHook);
	assert(entityTable.pfnStartFrame == &astrabot::adapter::metamod::startFrameHook);
	...
	assert(engineTable.pfnMessageBegin == &astrabot::adapter::metamod::messageBeginHook);
	assert(engineTable.pfnMessageEnd == &astrabot::adapter::metamod::messageEndHook);
	assert(engineTable.pfnWriteByte == &astrabot::adapter::metamod::writeByteHook);
	assert(engineTable.pfnWriteChar == &astrabot::adapter::metamod::writeCharHook);
	assert(engineTable.pfnWriteShort == &astrabot::adapter::metamod::writeShortHook);
	assert(engineTable.pfnWriteString == &astrabot::adapter::metamod::writeStringHook);
	assert(engineTable.pfnCmd_Args == &astrabot::adapter::metamod::commandArgsHook);
	assert(engineTable.pfnCmd_Argv == &astrabot::adapter::metamod::commandArgvHook);
	assert(engineTable.pfnCmd_Argc == &astrabot::adapter::metamod::commandArgcHook);
...
void testLifecycleHooksAndCoordinatorCleanup()
{
	...
	hooks.pfnStartFrame();
	assert(fixture.globals.mres == MRES_IGNORED);
	assert(astrabot::adapter::metamod::lifecycleCoordinator().registry().currentTick() == astrabot::core::TickId{1});
...
void testTraceSink()
{
	...
	assert(gTraceLines.front() == "astrabot version=0.1.0 adapter=metamod-p interface=5:13 outcome=attached");
...
void testCombatLifecycleRejectsInvalidActorWithTrace()
{
	...
	const auto result = lifecycle.submitCombatDecision({}, {}, {1}, decision, astrabot::core::BotCommand::neutral(8));
...
{
	const std::filesystem::path logDirectory = "astrabot-console-debug-logdir";
