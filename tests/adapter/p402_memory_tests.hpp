...
void memoryLifecycle()
{
	...
	step(fixture, 0.001f);
	assert(latest()->memories[0].lastSeenMicros == fresh.lastSeenMicros);
	...
	fixture.engineGlobals.time = time;
	mode = 1;
	step(fixture, 0.2f);
	...
	step(fixture, 0.2f);
	assert(latest()->count == 1);
	fixture.engineGlobals.time = (std::numeric_limits<float>::quiet_NaN)();
	step(fixture);
	assert(!latest());
	fixture.engineGlobals.time = time + 1;
	mode = 1;
	step(fixture, 0.2f);
	assert(latest() && latest()->count == 0);
	mode = 0;
	...
	for (const unsigned actors : {1U, 8U, 16U})
		for (const float dt : {0.008f, 0.016f, 0.1f})
		{
			...
			for (unsigned i = 0; i < actors; ++i)
			{
				...
				seen[i] = snapshot->memories[0].lastSeenMicros;
			}
			mode = 1;
			human.v.origin.x = 300;
			...
			for (unsigned frame = 0; frame < 40; ++frame)
			{
				...
				for (unsigned i = 0; i < actors; ++i)
				{
					...
					if (batch && batch->stamp.tick == owner.registry().currentTick())
						expectedTraces += vision.diagnostics(observer)->traces;
				}
			...
			{
				const auto* snapshot =
					owner.vision().memory().latest(owner.registry().currentPlayer(static_cast<std::uint16_t>(i + 1)));
