...
{
	auto& owner = astrabot::adapter::metamod::lifecycleCoordinator();
	assert(!owner.world().latest(owner.fakeClient().activePlayer()));
	++during;
}
void reentrant()
{
	for (unsigned mode : {0U, 9U, 10U, 12U})
	{
		...
		during = 0;
		p401::extraTrace = &inspectDuringTrace;
		p401::mode = mode;
		p401::step(fixture, 0.2f);
		assert(during > 0);
		...
		if (mode == 0)
			assert(snapshot && snapshot->visual->count == 1);
		if (mode == 9)
			assert(!snapshot || snapshot->visual->count == 0);
		if (mode == 10 || mode == 12)
			assert(!snapshot);
		detach();
	}
