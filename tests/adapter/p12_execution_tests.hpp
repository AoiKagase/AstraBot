...
void run()
{
	...
	owner.setRuntimeInputProvider([](void*, const adapter::metamod::LifecycleCoordinator&,
									 ...
									 std::size_t) noexcept -> std::size_t { return 0; },
								  nullptr);
	...
	route_test::Area a{1, {{0, 0, 0}, {100, 100, 0}, 0, 0}}, b{2, {{200, 0, 0}, {300, 100, 0}, 0, 0}},
		c{3, {{0, 100, 0}, {100, 200, 0}, 0, 0}}, d{4, {{300, 0, 0}, {400, 100, 0}, 0, 0}};
	a.targets[1] = {2};
	a.targets[2] = {3};
	b.targets[1] = {4};
	...
	assert(failed->failedEdge && failed->failedEdge->source.value == 1 && failed->failedEdge->target.value == 2 &&
		   failed->retryAtUs != 0);
	...
	assert(searchBackoff && searchBackoff->roamSearchBackoff && searchBackoff->roamSearchBackoffRemainingUs != 0 &&
