...
void clientData(const edict_t*, int sendWeapons, clientdata_s* client)
{
	...
	client->iuser3 = 1;
	if (disconnectOnRead)
		adapter::metamod::lifecycleCoordinator().clientDisconnect(&gFixture->entity);
}
...
{
	if (flags)
		captureGround(start, end, flags, observer, result);
	else
		p401::trace(start, end, flags, observer, result);
}
...
bool readObjective(void* context, const adapter::metamod::LifecycleCoordinator&, const adapter::metamod::RuntimeFrame&,
				   core::PlayerId, const edict_t*, adapter::metamod::RuntimeObjectiveObservation& objective,
...
std::size_t produceExperience(void* context, const adapter::metamod::LifecycleCoordinator&,
							  const adapter::metamod::RuntimeFrame& frame, core::PlayerId player, const edict_t*,
...
void sourcesAndRoster()
{
	...
	SourceState state{};
	const adapter::metamod::RuntimeInputSources sources{&readObjective, &produceExperience, &state};
	assert(adapter::metamod::buildRuntimeInputs(owner, frame, &fixture.hookDll, inputs.data(), inputs.size(), nullptr,
												0, sources) == 2);
	...
	for (const auto& input : inputs)
	{
		...
		assert(input.team.members[0].player == firstPlayer || input.team.members[0].player == secondPlayer);
		assert(input.team.members[1].player == firstPlayer || input.team.members[1].player == secondPlayer);
...
void run()
{
	...
	assert(initialStatus.map == frame.map && initialStatus.round == frame.round && initialStatus.tick == frame.tick &&
		   initialStatus.nowMicros == frame.nowMicros);
	...
	{
		if (transport.engineCall)
			assert(transport.dispatchTick.isAfter(transport.commandTick));
	}
	invalidWeapon = false;
	...
	assert(adapter::cstrike::protocol::weaponClass(WEAPON_GALIL) == core::combat::WeaponSnapshot::WeaponClass::Rifle);
	observedWeapon = 20;
	assert(adapter::metamod::buildRuntimeInputs(owner, frame, &fixture.hookDll, input.get(), 1) == 1);
	...
	assert(adapter::metamod::buildRuntimeInputs(owner, frame, &fixture.hookDll, input.get(), 1, &callbackStatus) == 0);
	assert(callbackStatus.reason == adapter::metamod::RuntimeInputBuildReason::MissingUpdateClientData);
