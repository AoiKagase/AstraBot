...
void trace(const float* start, const float* end, int flags, edict_t* observer, TraceResult* result)
{
	...
	*result = {};
	...
	if (mode == 10)
		astrabot::adapter::metamod::lifecycleCoordinator().serverDeactivate();
	if (mode == 11)
		++human.serialnumber;
	if (mode == 12)
		observer->v.health = 0;
...
void setup(Fixture& fixture, enginefuncs_t& hooks, unsigned actors)
{
	...
	for (unsigned slot = 1; slot <= actors; ++slot)
	{
		...
		entity->v.view_ofs = Vector(0, 0, 28);
		entity->v.v_angle = Vector(0, 0, 0);
	...
	human.v.health = 100;
	human.serialnumber = 5;
	human.v.origin = Vector(100, 1, 36);
	human.v.view_ofs = Vector(0, 0, 28);
	human.v.mins = Vector(-16, -16, -36);
	human.v.maxs = Vector(16, 16, 36);
...
void observationsAndRetirement()
{
	...
	for (unsigned m = 0; m <= 8; ++m)
	{
		...
		if (m == 4)
			assert(latest()->observations[0].position.z == 36 && diagnostics()->traces == 2);
		if (m == 6 || m == 7)
			assert(diagnostics()->reason == p::Reason::InvalidTrace);
	}
	mode = 0;
	fixture.engine.pfnTraceLine = nullptr;
	step(fixture, 0.2f);
	...
	mode = 9;
	step(fixture, 0.2f);
	assert(latest()->count == 0 && !owner.registry().currentPlayer(32).isValid());
	mode = 0;
	human.free = 0;
	++human.serialnumber;
	step(fixture, 0.2f);
	...
	mode = 10;
	step(fixture, 0.2f);
	assert(!latest() && !owner.registry().isMapActive());
	mode = 0;
	assert(owner.registry().activateMap(32));
	step(fixture, 0.2f);
	assert(owner.registry().mapGeneration() != oldMap && !latest());
	detach();
}
void matrix()
{
	using namespace astrabot;
	for (const unsigned actors : {1U, 8U, 16U})
		for (const float dt : {0.008f, 0.016f, 0.1f})
		{
			Fixture fixture;
			enginefuncs_t hooks{};
			setup(fixture, hooks, actors);
			auto& owner = adapter::metamod::lifecycleCoordinator();
			std::array<std::uint64_t, 16> before{};
			for (unsigned i = 0; i < actors; ++i)
				before[i] = owner.vision()
								.observations()
								.diagnostics(owner.registry().currentPlayer(static_cast<std::uint16_t>(i + 1)))
								->updates;
			...
			for (unsigned i = 0; i < actors; ++i)
			{
				...
				if (actors >= 8 && dt == 0.1f)
					assert(d->deferredFrames > 0 && d->intervalMicros >= 199000);
			}
			...
			for (unsigned i = 1; i < actors; ++i)
				assert(owner.vision().observations().latest(
					owner.registry().currentPlayer(static_cast<std::uint16_t>(i + 1))));
			detach();
		}
}
void movementCoexistence()
{
	...
	human.v.flags = FL_CLIENT;
	human.v.health = 100;
	human.serialnumber = 22;
	human.v.origin = Vector(150, 80, 36);
	human.v.view_ofs = Vector(0, 0, 28);
	auto& owner = adapter::metamod::lifecycleCoordinator();
	const auto observer = owner.fakeClient().activePlayer();
	const auto moves = gRunPlayerMoveCalls;
	...
	for (unsigned i = 0; i < 400; ++i)
	{
		...
		if (owner.navConsole().motionTrace().decision.state == nav::local::WalkState::Arrived)
			break;
	}
