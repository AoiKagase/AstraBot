...
const char* mapString(int index)
{
	return index == 20 ? activeMap.c_str() : "";
}
...
std::size_t autoLogs()
{
	...
	for (const auto& line : gNavOutput)
		if (line.find("astrabot nav_auto ") == 0)
			++count;
	return count;
}
...
void run()
{
	...
	const auto root =
		...
		("astrabot-p12-map-nav-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
	...
	assert(std::string(owner.mapNavLoadStatus().path.data()) == gameDirectory + "/maps/first_map.nav");
	assert(autoLogs() == 1);
