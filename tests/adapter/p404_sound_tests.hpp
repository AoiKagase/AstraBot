...
void hooksFor(Fixture& fixture, enginefuncs_t& post)
{
	...
	assert(fixture.callbacks.pfnGetEngineFunctions_Post &&
		   fixture.callbacks.pfnGetEngineFunctions_Post(&post, &version));
}
void precache(Fixture& fixture, enginefuncs_t& post, unsigned short index, const char* name)
{
	...
	fixture.globals.orig_ret = nullptr;
...
void publication()
{
	...
	human.v.origin.x = 4000;
	fixture.entity.v.origin.x = 6000;
	mode = 1;
	...
	fixture.globals.status = MRES_SUPERCEDE;
	footstep(post);
	assert(owner.sound().diagnostics().queued == 2);
	fixture.globals.status = MRES_IGNORED;
	step(fixture, 0.1f);
	assert(sounds()->count == 3);
	assert(sounds()->sounds[1].observation.region.x == 1 &&
		   sounds()->sounds[1].observation.kind == p::SoundKind::Explosion);
	assert(sounds()->sounds[2].observation.region.x == 0 &&
		   sounds()->sounds[2].observation.kind == p::SoundKind::Gunshot);
	footstep(post);
	assert(owner.sound().diagnostics().queued == 1);
	p403::roundEvent(hooks);
	assert(owner.sound().diagnostics().queued == 0 && !sounds());
	step(fixture, 0.1f);
	assert(sounds() && sounds()->count == 0);
	footstep(post);
	const auto oldTime = fixture.engineGlobals.time;
	fixture.engineGlobals.time -= 1;
	step(fixture, 0);
	assert(owner.sound().diagnostics().queued == 0 && !sounds());
	fixture.engineGlobals.time = oldTime;
	step(fixture, 0.1f);
	assert(sounds() && sounds()->count == 0);
	footstep(post);
	...
	assert(owner.sound().events().find(413) == adapter::cstrike::EventKind::Unknown);
	detach();
}
void queueAndRetirement()
{
	...
	{
		human.v.origin.x = 100 + static_cast<float>(i) * 0.25f;
		footstep(post);
	}
	...
	for (unsigned frame = 0; frame < 8; ++frame)
	{
		...
		if (frame == 0)
			firstRetainedSequence = snapshot->sounds[0].observation.identity.sequence;
	...
	{
		human.v.origin.x = static_cast<float>(i);
		footstep(post);
	}
	fixture.entity.v.health = 0;
	step(fixture, 0.001f);
...
void matrix()
{
	...
	for (unsigned actors : {1U, 8U, 16U})
		for (float dt : {0.008f, 0.016f, 0.1f})
		{
			Fixture fixture;
			enginefuncs_t hooks{}, post{};
			...
			mode = 1;
			if (actors > 1)
				fixture.matrixEntity(actors)->v.origin.x = 10000;
			...
			{
				const auto* snapshot =
					owner.sound().memory().latest(owner.registry().currentPlayer(static_cast<std::uint16_t>(slot)));
				assert(snapshot && snapshot->count <= 16);
...
void retireDuringVision()
{
	...
	if (retirement == 0)
		owner.clientDisconnect(&gFixture->entity);
	else if (retirement == 1)
		owner.serverDeactivate();
	else
		gFixture->entity.v.health = 0;
}
