...
unsigned calls{};
bool stale{};
edict_t* entityAt(int index)
{
	++calls;
	return index >= 0 && index < 5 ? &entities[index] : nullptr;
}
int entityIndex(const edict_t* e)
{
	return static_cast<int>(e - entities);
}
...
enginefuncs_t reset()
{
	...
	e.pfnIndexOfEdict = entityIndex;
	e.pfnSzFromIndex = nameAt;
	return e;
}
void ladder(int slot)
{
	auto& e = entities[slot];
	e.serialnumber = 7;
	e.v.classname = 1;
	e.v.absmin = Vector(0, 0, 0);
	e.v.absmax = Vector(8, 64, 128);
}
void scans()
{
	auto e = reset();
	auto r = scanLadderCandidates(&e, {1}, 5);
	assert(r && r.candidates.count == 0 && r.candidates.map == core::MapGeneration{1} && calls == 4);
	ladder(1);
	ladder(3);
	entities[2].v.classname = 2;
	entities[4].free = 1;
	entities[4].v.classname = 1;
	calls = 0;
	r = scanLadderCandidates(&e, {2}, 5);
...
void failures()
{
	for (int mode = 0; mode < 10; ++mode)
	{
		...
		if (mode == 0)
			entities[3].v.absmax.z = 0;
		if (mode == 1)
			entities[3].v.absmin.x = (std::numeric_limits<float>::quiet_NaN)();
		if (mode == 2)
			entities[3].v.angles.y = 90;
		if (mode == 3)
			entities[3].v.velocity.x = 1;
		if (mode == 4)
			entities[3].v.avelocity.z = 1;
		if (mode == 5)
		{
			stale = true;
			expected = LadderScanReason::InvalidEntity;
		}
		if (mode == 6)
		{
			e.pfnSzFromIndex = nullptr;
			expected = LadderScanReason::Unavailable;
		}
		if (mode == 7)
		{
			e.pfnIndexOfEdict = nullptr;
			expected = LadderScanReason::Unavailable;
		}
		if (mode == 8)
		{
			e.pfnPEntityOfEntIndex = nullptr;
			expected = LadderScanReason::Unavailable;
		}
		if (mode == 9)
		{
			entities[3].v.classname = 3;
			expected = LadderScanReason::InvalidEntity;
		}
