...
void publication()
{
	...
	const auto reporter = owner.registry().currentPlayer(1), receiver = owner.registry().currentPlayer(2),
			   target = owner.registry().currentPlayer(32);
	sendTeamInfo(hooks, 13, 2, "CT");
	sendTeamInfo(hooks, 13, 2, "TERRORIST");
	p405::fade(hooks, &fixture.secondEntity);
	...
	runNav({"astrabot_report", "1:1", "100,20,30"});
	assert(owner.world().reports().diagnostics().queued == 0);
	sendTeamInfo(hooks, 13, 1, "CT");
	assert(owner.world().latest(receiver)->reports->count == 0);
...
void reentrant()
{
	...
	assert(!owner.report(reporter, target).accepted());
	assert(!disconnectDuringLookup);
	assert(owner.world().reports().diagnostics().queued == 0);
...
void matrix()
{
	for (unsigned actors : {1U, 8U, 16U})
		for (float dt : {0.008f, 0.016f, 0.1f})
		{
			...
			{
				(void)owner.report(reporter, target);
