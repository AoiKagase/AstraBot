...
void fade(enginefuncs_t& hooks, edict_t* target, int color = 255, int flags = 0, int units = 4096)
{
	...
	hooks.pfnWriteShort(units);
	hooks.pfnWriteShort(units);
	hooks.pfnWriteShort(flags);
	hooks.pfnWriteByte(color);
	hooks.pfnWriteByte(color);
	hooks.pfnWriteByte(color);
	hooks.pfnWriteByte(255);
	hooks.pfnMessageEnd();
}
...
{
	p401::setup(fixture, hooks, actors);
...
void flashAndMemory()
{
	...
	assert(owner.vision().observations().latest(actor));
	assert(owner.vision().observations().latest(actor)->count > 0);
	fade(hooks, &fixture.entity, 0);
	fade(hooks, &fixture.entity, 255, 1);
	assert(owner.visualEffects().diagnostics().flashAccepted == 1);
	p401::step(fixture, 0.2f);
	assert(owner.vision().observations().latest(actor)->count > 0);
	fade(hooks, &fixture.entity);
...
void smokeAndSamples()
{
	...
	smoke(hooks, post, 50, 1, 64, 4);
	assert(owner.visualEffects().diagnostics().regions == 1);
	p401::step(fixture, 0.2f);
	assert(p401::traceCalls == 0 && owner.vision().observations().latest(actor)->count == 0);
	assert(owner.vision().memory().latest(actor)->memories[0].lastSeenMicros == before);
	p401::step(fixture, 5);
	assert(owner.vision().memory().latest(actor)->count == 0);
	...
	for (unsigned i = 0; i < 33; ++i)
		smoke(hooks, post, 10000 + static_cast<float>(i) * 300, 0, 0);
	assert(owner.visualEffects().diagnostics().regions == 32 && owner.visualEffects().diagnostics().overflowActive);
	p401::step(fixture, 0.2f);
	assert(owner.vision().observations().latest(actor)->count == 0);
	p401::step(fixture, 22);
	assert(!owner.visualEffects().diagnostics().overflowActive);
	assert(owner.vision().observations().latest(actor)->count == 1);
	smoke(hooks, post);
...
void inject()
{
	...
	if (injection == 0)
		fade(*gEngineHooks, &gFixture->entity);
	else
		smoke(*gEngineHooks, *injectionPost);
}
void reentrant()
{
...
void matrix()
{
	for (unsigned actors : {1U, 8U, 16U})
		for (float dt : {0.008f, 0.016f, 0.1f})
		{
			Fixture fixture;
			enginefuncs_t hooks{}, post{};
			setup(fixture, hooks, post, actors);
			auto& owner = astrabot::adapter::metamod::lifecycleCoordinator();
			smoke(hooks, post);
			...
			{
				const auto* batch = owner.vision().observations().latest(
					owner.registry().currentPlayer(static_cast<std::uint16_t>(slot)));
