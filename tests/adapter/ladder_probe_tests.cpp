...
core::MapGeneration mapNow(const void*) noexcept
{
	return activeMap;
}
edict_t* entityAt(int n)
{
	return n == 1 ? &ladderEntity : n == 0 ? &worldEntity : n == 2 ? &actorEntity : nullptr;
}
int indexOf(const edict_t* e)
{
	return e == &ladderEntity ? 1 : e == &actorEntity ? 2 : 0;
}
const char* classname(int n)
{
	return n == 1 ? "func_ladder" : n == 2 ? "*1" : nullptr;
}
...
void boxTrace(const float* a, const float* b, Box box, int hull, edict_t* hit, TraceResult& out)
{
	...
	if (inside)
	{
		out.fStartSolid = out.fAllSolid = 1;
		out.flFraction = 0;
		out.vecEndPos = Vector(a[0], a[1], a[2]);
		out.pHit = hit;
		return;
	}
	if (enter < 0 || enter > out.flFraction || enter > 1 || leave <= 0)
		return;
	out.flFraction = static_cast<float>(enter);
	out.pHit = hit;
	out.vecPlaneNormal = normal;
	out.vecEndPos = Vector(a[0] + (b[0] - a[0]) * out.flFraction, a[1] + (b[1] - a[1]) * out.flFraction,
						   a[2] + (b[2] - a[2]) * out.flFraction);
}
void begin(const float* b, TraceResult* t)
{
	*t = {};
	t->flFraction = 1;
	t->vecEndPos = Vector(b[0], b[1], b[2]);
	++traces;
}
void inject(const float* a, const float* b, TraceResult* t)
{
	...
	if (fault == 4)
	{
		...
		t->vecEndPos = Vector(b[0], b[1], b[2]);
		t->pHit = nullptr;
		t->fStartSolid = t->fAllSolid = 0;
	}
	if (fault == 5)
	{
		t->fStartSolid = 1;
		t->flFraction = 0;
		t->vecEndPos = Vector(a[0], a[1], a[2]);
	}
	if (fault == 6)
		t->vecPlaneNormal = Vector(0, 0, 1);
	if (fault == 7)
		t->vecEndPos.x += 2;
	if (fault == 8)
		ladderEntity.v.absmax.z += 1;
	if (fault == 9)
		++ladderEntity.v.modelindex;
	if (fault == 10)
		t->pHit = &ladderEntity;
	if (fault == 12)
		actorEntity.v.origin.x += 1;
	if (fault == 13)
		++actorEntity.serialnumber;
	if (fault == 14)
		frameCurrent = false;
	if (fault == 15)
		actorEntity.v.movetype = MOVETYPE_WALK;
	if (fault == 16)
		frameCvars[0].value += 1;
	if (fault == 17)
		frameCvars[1].value += 1;
	if (fault == 18)
		frameCvars[2].value = 100;
	if (fault == 19)
		frameCvars[3].value += 1;
	if (fault == 20)
		actorEntity.v.friction = 0.5f;
#ifdef ASTRABOT_LADDER_HOST_TESTS
	if (fault == 11 && publishing)
		publishing->invalidate(nav::runtime::SessionReason::Cancelled);
#endif
}
void traceModel(const float* a, const float* b, int hull, edict_t* e, TraceResult* t)
{
	assert(e == &ladderEntity && (hull == 0 || hull == 1));
	begin(b, t);
	boxTrace(a, b, {{0, 0, 0}, {8, 64, 128}}, hull, e, *t);
	inject(a, b, t);
}
void traceHull(const float* a, const float* b, int ignore, int hull, edict_t* ignored, TraceResult* t)
{
	assert((ignore == 0 || ignore == 1) && hull == 1 &&
		   (!ignored || ignored == &actorEntity || ignored == motionActor));
	begin(b, t);
	for (const auto& floor : floors)
		boxTrace(a, b, floor, 1, &worldEntity, *t);
	inject(a, b, t);
}
int pointContents(const float* point)
{
	++traces;
	TraceResult dummy{};
	inject(point, point, &dummy);
	if (traces == faultAt && fault == 3)
		return 0;
	for (const auto& floor : floors)
		if (point[0] > floor.low.x && point[0] < floor.high.x && point[1] > floor.low.y && point[1] < floor.high.y &&
			point[2] > floor.low.z && point[2] < floor.high.z)
...
std::shared_ptr<const nav::query::NavSpatialIndex> setup(LadderFace face, LadderExit exit)
{
	...
	ladderEntity.v.classname = 1;
	ladderEntity.v.model = 2;
	ladderEntity.v.modelindex = 1;
	ladderEntity.v.skin = CONTENTS_LADDER;
	ladderEntity.v.absmin = Vector(0, 0, 0);
	ladderEntity.v.absmax = Vector(8, 64, 128);
	traces = 0;
	fault = faultAt = 0;
	activeMap = {1};
	...
	for (unsigned i = 0; i < 2; ++i)
	{
		...
		const float z = f.high.z;
		areas.push_back({i + 1, {{f.low.x, f.low.y, z}, {f.high.x, f.high.y, z}, z, z}});
	}
	auto index = nav::query::NavSpatialIndex::build(route_test::snapshot(areas), {100, 199, 100000});
	assert(index);
	return *index.value;
}
...
{
	enginefuncs_t e{};
	e.pfnPEntityOfEntIndex = entityAt;
	e.pfnIndexOfEdict = indexOf;
	e.pfnSzFromIndex = classname;
	e.pfnTraceModel = traceModel;
	e.pfnTraceHull = traceHull;
	e.pfnCVarGetPointer = frameCvar;
	e.pfnPointContents = pointContents;
	return e;
}
...
void testLadderProbe()
{
	for (auto face : {LadderFace::MinX, LadderFace::MaxX, LadderFace::MinY, LadderFace::MaxY})
		...
		{
			const auto index = setup(face, exit);
			auto e = engine();
			const auto r = inspectLadderPassage({&e, nullptr, mapNow}, {1}, candidate, face, exit, *index, {1}, 2);
			assert(r && r.queries == 12 && traces == 12 && r.passage->bottom.area.value == 1 &&
				   r.passage->top.area.value == 2);
	...
	{
		const auto index = setup(LadderFace::MinX, LadderExit::AcrossTop);
		auto e = engine();
		const auto r = inspectLadderPassage({&e, nullptr, mapNow}, {1}, candidate, LadderFace::MinX,
											LadderExit::AcrossTop, *index, {1}, 2, budget);
		assert(!r && !r.passage && r.reason == LadderProbeReason::BudgetExceeded && traces == budget &&
			   r.queries == budget);
	}
	for (int mode = 1; mode <= 9; ++mode)
	{
		...
		faultAt = 2;
		const auto r = inspectLadderPassage({&e, nullptr, mapNow}, {1}, candidate, LadderFace::MinX,
											LadderExit::AcrossTop, *index, {1}, 2);
		assert(!r && !r.passage && traces == 2);
		const auto expected = mode == 1								? LadderProbeReason::StaleMap
							  : mode == 2 || mode == 8 || mode == 9 ? LadderProbeReason::StaleEntity
							  : mode == 3 || mode == 7				? LadderProbeReason::InvalidTrace
							  : mode == 5							? LadderProbeReason::Blocked
																	: LadderProbeReason::NoFace;
	...
	for (unsigned at : {4U, 6U, 8U, 9U, 10U, 11U, 12U})
	{
		...
		assert(r.reason == (at < 8	  ? LadderProbeReason::NoSupport
							: at < 10 ? LadderProbeReason::NoFace
									  : LadderProbeReason::Blocked));
	}
	...
	for (int mode = 0; mode < 4; ++mode)
	{
		...
		auto e = engine();
		if (mode == 0)
			ladderEntity.v.modelindex = 0;
		if (mode == 1)
			ladderEntity.v.modelindex = 512;
		if (mode == 2)
			ladderEntity.v.model = 1;
		if (mode == 3)
			ladderEntity.v.skin = 0;
		const auto r = inspectLadderPassage({&e, nullptr, mapNow}, {1}, candidate, LadderFace::MinX,
											LadderExit::AcrossTop, *index, {1}, 2);
	...
	fault = 10;
	faultAt = 4;
	auto r = inspectLadderPassage({&e, nullptr, mapNow}, {1}, candidate, LadderFace::MinX, LadderExit::AcrossTop,
								  *index, {1}, 2);
	assert(!r && r.reason == LadderProbeReason::NoSupport);
	traces = 0;
	fault = faultAt = 0;
	r = inspectLadderPassage({&e, nullptr, mapNow}, {1}, candidate, LadderFace::MinX, LadderExit::AcrossTop, *index,
							 {2}, 2);
	assert(!r && r.reason == LadderProbeReason::StaleMap && traces == 0);
	const nav::query::NavSpatialIndex empty;
	r = inspectLadderPassage({&e, nullptr, mapNow}, {1}, candidate, LadderFace::MinX, LadderExit::AcrossTop, empty, {1},
							 2);
...
void testLadderFrame()
{
	for (bool grounded : {false, true})
	{
		...
		assert(batch);
		const auto bound =
			bindLadderPlan({&e, nullptr, mapNow}, {1}, fingerprint, *batch.value, batch.value->links.links.front(), 2);
		assert(bound);
		const V origin =
			grounded ? bound.value->plan.start : V{bound.value->plan.mount.x, bound.value->plan.mount.y, 80};
		const auto resetActor = [&]() {
			actorEntity = {};
			actorEntity.serialnumber = 8;
			frameCurrent = true;
			traces = fault = faultAt = 0;
			activeMap = {1};
			const float values[]{800, 10, 320, 2000};
			for (unsigned i = 0; i < 4; ++i)
			{
				frameCvars[i] = {};
				frameCvars[i].value = values[i];
			}
			ladderEntity.serialnumber = 7;
			auto& v = actorEntity.v;
			v.flags = FL_FAKECLIENT | (grounded ? FL_ONGROUND : 0);
			v.origin = Vector(origin.x, origin.y, origin.z);
			v.mins = Vector(-16, -16, -36);
			v.maxs = Vector(16, 16, 36);
			v.movetype = grounded ? MOVETYPE_WALK : MOVETYPE_FLY;
			v.maxspeed = 250;
			v.friction = 1;
		};
		resetActor();
		nav::runtime::MovementSnapshot s;
		s.agent = frameBinding.agent;
		s.actor = frameBinding.actor;
		s.map = {1};
		s.tick = {6};
		s.kind = nav::runtime::ActorKind::ManagedBot;
		s.connected = s.joined = s.alive = true;
		s.grounded = grounded;
		s.ducked = false;
		s.position = origin;
		s.velocity = s.view = V{};
		...
		const auto run = [&](unsigned budget) {
			return inspectLadderFrame(world, &actorEntity, frameBinding, s, *bound.value, origin, *index, {1}, 3,
									  budget);
		};
		const auto ok = run(4);
		assert(ok && ok.queries == 3 && traces == 3);
		...
		resetActor();
		s.tick = {7};
		assert(run(4).reason == LadderFrameReason::StaleActor && traces == 0);
		s.tick = {6};
		resetActor();
		actorEntity.v.basevelocity.x = 1;
		assert(run(4).reason == LadderFrameReason::StaleActor && traces == 0);
		resetActor();
		actorEntity.v.punchangle.x = 1;
		assert(run(4).reason == LadderFrameReason::StaleActor && traces == 0);
		resetActor();
		actorEntity.v.flags |= FL_FROZEN;
		assert(run(4).reason == LadderFrameReason::StaleActor && traces == 0);
		resetActor();
		frameCvars[0].value = 0;
		assert(run(4).reason == LadderFrameReason::Unavailable && traces == 0);
		resetActor();
		frameCvars[2].value = 120;
		actorEntity.v.gravity = 0.5f;
		const auto scaled = run(4);
		assert(scaled && scaled.value->physics.gravity == 400 && scaled.value->physics.maximumSpeed == 120);
		resetActor();
		assert(run(5).reason == LadderFrameReason::InvalidInput && traces == 0);
		if (!grounded)
		{
			resetActor();
			faultAt = 2;
			fault = 6;
			assert(run(4).reason == LadderFrameReason::WrongFace);
			resetActor();
			faultAt = 2;
			fault = 15;
			assert(run(4).reason == LadderFrameReason::StaleActor);
		}
		else
		{
			resetActor();
			faultAt = 3;
			fault = 10;
			assert(run(4).reason == LadderFrameReason::NoSupport);
		}
		resetActor();
		faultAt = grounded ? 2 : 3;
		fault = 5;
		assert(run(4).reason == LadderFrameReason::Blocked);
		resetActor();
		assert(inspectLadderFrame(world, &actorEntity, frameBinding, s, *bound.value,
								  {origin.x + 97, origin.y, origin.z}, *index, {1}, 3)
					   .reason == LadderFrameReason::InvalidInput &&
			   traces == 0);
		if (grounded)
		{
			...
			const auto inspectMount = [&](unsigned budget) {
				resetActor();
				actorEntity.v.origin = Vector(s.position->x, s.position->y, s.position->z);
				return inspectLadderFrame(world, &actorEntity, frameBinding, s, *bound.value, *s.position, *index, {1},
										  3, budget);
			};
			for (unsigned budget = 0; budget < 4; ++budget)
			{
				const auto r = inspectMount(budget);
				assert(!r && r.queries == budget && r.reason == LadderFrameReason::BudgetExceeded);
			}
			const auto r = inspectMount(4);
			assert(r && r.queries == 4 && !r.value->inspection.support);
			floors[0].high.x = 100; // Independent world floor extends under shaft; NAV does not.
			core::BotCommand down;
			down.msec = 16;
			down.movement.forward = -200;
			...
			assert(command && command.queries == 7 && command.value->floorPointSolid == true &&
				   command.value->prediction);
			...
			for (unsigned budget = 0; budget < 7; ++budget)
			{
				...
				assert(!shortBudget && shortBudget.reason == LadderFrameReason::BudgetExceeded &&
					   shortBudget.queries == budget);
			}
			for (unsigned at = 1; at <= 7; ++at)
				for (int mode : {1, 2, 12, 14, 16})
				{
					const auto stale = inspectCommand(7, at, mode);
					assert(!stale && !stale.value && stale.queries == at);
				}
		...
		else
		{
			...
			core::BotCommand up;
			up.msec = 16;
			up.movement.forward = 200;
			up.buttons = static_cast<core::ButtonMask>(core::Button::Forward);
			auto command = inspectLadderFrame(world, &actorEntity, frameBinding, s, *bound.value, *s.position, *index,
											  {1}, 3, 7, up);
			...
			resetActor();
			core::BotCommand jump;
			jump.msec = 16;
			jump.buttons = static_cast<core::ButtonMask>(core::Button::Jump);
			const auto leap = inspectLadderFrame(world, &actorEntity, frameBinding, s, *bound.value, *s.position,
												 *index, {1}, 3, 7, jump);
			if (!leap)
			{
				std::fprintf(stderr, "host rejected airborne ladder jump\n");
				std::exit(1);
			}
			...
			assert(leap.value->prediction->velocity.x == -270 &&
				   std::abs(leap.value->prediction->velocity.z + 12.8f) < 0.001f);
			resetActor();
			s.position = V{-15, 32, 168};
			s.velocity = V{0, 0, 200};
			actorEntity.v.origin = Vector(-15, 32, 168);
			actorEntity.v.velocity = Vector(0, 0, 200);
			command = inspectLadderFrame(world, &actorEntity, frameBinding, s, *bound.value, *s.position, *index, {1},
										 3, 7, up);
...
void testUpperExitFrame()
{
	...
	{
		const auto index = setup(LadderFace::MinX, exit);
		auto e = engine();
		nav::enrichment::NavMapFingerprint fingerprint{};
		const auto batch = discoverLadderLinks({&e, nullptr, mapNow}, {1}, *index, {1}, fingerprint, 9, 2);
		assert(batch);
		...
		s.kind = nav::runtime::ActorKind::ManagedBot;
		s.connected = s.joined = s.alive = true;
		s.grounded = s.ducked = false;
		s.position = bound.value->plan.dismount;
		s.position->z -= 1;
		s.velocity = s.view = V{};
		s.hull = nav::runtime::HullDimensions{{-16, -16, -36}, {16, 16, 36}};
		s.speedLimit = 250.0f;
		const auto reset = [&]() {
			...
			actorEntity.serialnumber = 8;
			auto& v = actorEntity.v;
			v.flags = FL_FAKECLIENT;
			v.movetype = MOVETYPE_FLY;
			v.maxspeed = 250;
			v.friction = 1;
			v.origin = Vector(s.position->x, s.position->y, s.position->z);
			v.mins = Vector(-16, -16, -36);
			v.maxs = Vector(16, 16, 36);
			const float values[]{800, 10, 320, 2000};
			for (unsigned i = 0; i < 4; ++i)
		...
		const auto inspect = [&](unsigned budget, std::uint8_t msec, unsigned at = 0, int mode = 0) {
			...
			faultAt = at;
			fault = mode;
			return inspectLadderFrame({{&e, nullptr, mapNow}, nullptr, currentFrame}, &actorEntity, frameBinding, s,
									  *bound.value, bound.value->plan.end, *index, {1}, 3, budget, {}, msec);
		};
		for (const auto msec : std::array<std::uint8_t, 3>{8, 16, 100})
		{
			...
			assert(r.value->prediction->command.msec == msec &&
				   r.value->inspection.exitIntent->forward == core::ActionRequest::Hold);
			assert(!r.value->inspection.support); // Future landing is never published as current support.
		}
		const auto good = inspect(21, 16);
		assert(good);
		const auto count = good.queries;
		for (unsigned budget = 0; budget < count; ++budget)
		{
			const auto r = inspect(budget, 16);
			assert(!r && !r.value && r.queries == budget && r.reason == LadderFrameReason::BudgetExceeded);
		}
		for (unsigned at = 1; at <= count; ++at)
			for (int mode : {1, 2, 12, 14, 16})
			{
				const auto r = inspect(21, 16, at, mode);
				assert(!r && !r.value && r.queries == at);
			}
		...
		reset();
		core::BotCommand held;
		held.msec = 16;
		held.view.pitch = -44;
		held.buttons = static_cast<core::ButtonMask>(core::Button::Forward);
		held.movement.forward = 200;
		const auto guarded =
			inspectLadderFrame({{&e, nullptr, mapNow}, nullptr, currentFrame}, &actorEntity, frameBinding, s,
							   *bound.value, bound.value->plan.end, *index, {1}, 3, 21, held, std::uint8_t{16});
		if (!guarded)
		{
			std::fprintf(stderr, "exit forecast cannot validate held first command: %d exit=%d queries=%u\n",
						 int(guarded.reason), int(guarded.exitReason), guarded.queries);
			std::exit(1);
		}
		assert(guarded.value->prediction && guarded.value->prediction->command.view.pitch == -44);
		assert(!guarded.value->inspection
					.exitIntent); // Guarding existing input cannot authorize a different regenerated intent.
	}
}
void testLowerExitFrame()
{
	...
	assert(batch);
	const auto selected =
		std::find_if(batch.value->links.links.begin(), batch.value->links.links.end(),
					 [](const auto& l) { return l.direction == nav::enrichment::NavLinkDirection::Down; });
	assert(selected != batch.value->links.links.end());
	const auto bound = bindLadderPlan({&e, nullptr, mapNow}, {1}, fingerprint, *batch.value, *selected, 2);
	assert(bound);
	nav::runtime::MovementSnapshot s;
	s.agent = frameBinding.agent;
	s.actor = frameBinding.actor;
	...
	s.tick = {6};
	s.kind = nav::runtime::ActorKind::ManagedBot;
	s.connected = s.joined = s.alive = s.grounded = true;
	s.ducked = false;
	s.position = bound.value->plan.dismount;
	s.velocity = s.view = V{};
	s.hull = nav::runtime::HullDimensions{{-16, -16, -36}, {16, 16, 36}};
	...
	const auto inspect = [&](unsigned budget, std::uint8_t msec, unsigned at = 0, int mode = 0) {
		...
		actorEntity.serialnumber = 8;
		auto& v = actorEntity.v;
		v.flags = FL_FAKECLIENT | (*s.grounded ? FL_ONGROUND : 0);
		v.movetype = MOVETYPE_FLY;
		v.maxspeed = 250;
		v.friction = 1;
		v.origin = Vector(s.position->x, s.position->y, s.position->z);
	...
	for (const auto msec : std::array<std::uint8_t, 3>{8, 16, 100})
	{
		...
		if (!r)
		{
			std::fprintf(stderr, "lower exit rejected: reason=%d queries=%u\n", int(r.reason), r.queries);
			std::exit(1);
		}
	...
	for (const auto msec : std::array<std::uint8_t, 3>{8, 16, 100})
	{
		...
		if (!r)
		{
			std::fprintf(stderr, "host rejected lower jump exit candidate\n");
			std::exit(1);
		}
		assert(r.value->inspection.exitIntent && r.value->inspection.exitIntent->jump == core::ActionRequest::Press);
		assert(r.value->prediction &&
			   r.value->prediction->command.buttons == static_cast<core::ButtonMask>(core::Button::Jump));
		assert(!r.value->inspection.support && !r.value->floorPointSolid);
	}
	const auto leap = inspect(21, 16);
	assert(leap && leap.value->jumpExit);
	const auto leapCount = leap.queries;
	for (unsigned budget = 0; budget < leapCount; ++budget)
	{
		const auto r = inspect(budget, 16);
		assert(!r && !r.value && r.queries == budget && r.reason == LadderFrameReason::BudgetExceeded);
	}
	for (unsigned at = 1; at <= leapCount; ++at)
		for (int mode : {1, 2, 12, 14, 16})
		{
			const auto r = inspect(21, 16, at, mode);
			assert(!r && !r.value && r.queries == at);
	...
	floors[0].high.x = 100;
	s.grounded = true;
	s.position = V{-19, 32, 36};
	// Detached and outside NAV, but standing on a measured world floor.
	actorEntity.v.flags = FL_FAKECLIENT | FL_ONGROUND;
	actorEntity.v.movetype = MOVETYPE_WALK;
	actorEntity.v.origin = Vector(-19, 32, 36);
	fault = faultAt = traces = 0;
	const auto approach = inspectLadderFrame({{&e, nullptr, mapNow}, nullptr, currentFrame}, &actorEntity, frameBinding,
											 s, *bound.value, bound.value->plan.end, *index, {1}, 3, 21, {}, {}, true);
	if (!approach)
	{
		std::fprintf(stderr, "measured outside-NAV exit approach unavailable\n");
		std::exit(1);
	}
	assert(approach.value->inspection.worldFloor && approach.value->inspection.groundPathClear == true);
	assert(!approach.value->inspection.support && !approach.value->inspection.exitIntent &&
		   !approach.value->contact.touching);
	...
	const auto groundProof = [&](unsigned budget, unsigned injectAt, int injected) {
		ladderEntity = savedLadder;
		activeMap = {1};
		fault = injected;
		faultAt = injectAt;
		traces = 0;
		return inspectLadderFrame({{&e, nullptr, mapNow}, nullptr, currentFrame}, &actorEntity, frameBinding, s,
								  *bound.value, bound.value->plan.end, *index, {1}, 3, budget, {}, {}, true);
	};
	...
	for (unsigned at = 1; at <= groundCount; ++at)
		for (int change : {1, 2})
			assert(!groundProof(21, at, change));
...
void testLadderDiscovery()
{
	...
	const auto graph =
		nav::query::NavGraph::compose(mesh, fingerprint, batch.value->links, {100, 2048, 1000000}, {2048, 1000000});
	assert(graph && (*graph.value)->edgeCount() == batch.value->passages.size() * 2);
	const auto route = nav::query::NavRouteSearch::search(**graph.value, {{1}, {2}, {2, 100000}, false});
	assert(route);
	const auto corridor = nav::corridor::Corridor::build(**graph.value, *route.value, {16, 16}, {1, 100000, 100});
	if (!corridor)
	{
		std::fprintf(stderr, "published ladder endpoints cannot form standing corridor\n");
		std::exit(1);
	}
	for (std::size_t i = 0; i < batch.value->passages.size(); ++i)
	{
		const auto& up = batch.value->links.links[2 * i];
		const auto& down = batch.value->links.links[2 * i + 1];
		...
		assert(up.direction == nav::enrichment::NavLinkDirection::Up &&
			   down.direction == nav::enrichment::NavLinkDirection::Down);
	...
	for (int mode = 0; mode < 9; ++mode)
	{
		...
		if (mode == 5)
			selected.from = {999};
		if (mode == 6)
			selected.direction = nav::enrichment::NavLinkDirection::Down;
		if (mode == 7)
			selected.traversal = nav::model::NavTraversalKind::Walk;
		if (mode == 8)
			selected.additionalCost = 1;
		...
		assert(r.reason == (mode < 2	? LadderBindingReason::WrongPublication
							: mode == 2 ? LadderBindingReason::MissingLink
										: LadderBindingReason::ChangedLink));
	}
	auto damaged = *batch.value;
	damaged.links.links.pop_back();
	const auto selected = batch.value->links.links.front();
	assert(bindLadderPlan({&e, nullptr, mapNow}, {1}, fingerprint, damaged, selected, 2).reason ==
		   LadderBindingReason::InvalidInput);
	damaged = *batch.value;
	damaged.links.links[1] = selected;
	assert(bindLadderPlan({&e, nullptr, mapNow}, {1}, fingerprint, damaged, selected, 2).reason ==
		   LadderBindingReason::ChangedLink);
	damaged = *batch.value;
	damaged.passages[0].bottom.origin.x += 1;
	assert(bindLadderPlan({&e, nullptr, mapNow}, {1}, fingerprint, damaged, selected, 2).reason ==
		   LadderBindingReason::ChangedLink);
	assert(bindLadderPlan({&e, nullptr, mapNow}, {2}, fingerprint, *batch.value, selected, 2).reason ==
		   LadderBindingReason::WrongPublication);
	auto other = fingerprint;
	++other[0];
	assert(bindLadderPlan({&e, nullptr, mapNow}, {1}, other, *batch.value, selected, 2).reason ==
		   LadderBindingReason::WrongPublication);
	assert(!nav::query::NavGraph::compose(mesh, other, batch.value->links, {100, 2048, 1000000}, {2048, 1000000}));
	const auto old = batch.value;
	++ladderEntity.serialnumber;
	assert(bindLadderPlan({&e, nullptr, mapNow}, {1}, fingerprint, *old, selected, 2).reason ==
		   LadderBindingReason::StaleWorld);
...
void testLadderPublication()
{
	...
	const auto mesh = route_test::snapshot(areas);
	NavConsole console;
	console.configure(&e, nullptr, nullptr);
	...
	assert(console.ladders()->generation > first.generation &&
		   first.links.fingerprint == console.ladders()->links.fingerprint);
...
void simulateLadderMotion(edict_t& actor, const core::BotCommand& command)
{
	...
	if (contact.fStartSolid)
	{
		...
		{
			actor.v.movetype = MOVETYPE_WALK;
			vx = -270;
			vy = 0;
			vz = 0;
			air = true;
		...
		else
		{
			...
			vx = 0;
			vy = speed * std::cos(pitch) * std::sin(yaw);
	...
	{
		actor.v.movetype = MOVETYPE_WALK;
		air = !(actor.v.flags & FL_ONGROUND);
		if (!air)
		{
			vx = command.movement.forward * std::cos(yaw) + command.movement.side * std::sin(yaw);
			vy = command.movement.forward * std::sin(yaw) - command.movement.side * std::cos(yaw);
			vz = 0;
		}
	}
	if (air)
	{
		...
		if (length > 0)
		{
			...
			const double add =
				(std::max)(0.0, (std::min)(10 * wish * dt, (std::min)(wish, 30.0) - (vx * wx + vy * wy) / length));
			vx += add * wx / length;
			vy += add * wy / length;
		}
		vz -= 400 * dt;
	...
	TraceResult hit{};
	hit.flFraction = 1;
	hit.vecEndPos = Vector(to[0], to[1], to[2]);
	for (const auto& floor : floors)
		if (from[2] > floor.high.z + 36.001f || to[2] < from[2])
			boxTrace(from, to, floor, 1, &worldEntity, hit);
	actor.v.origin = hit.vecEndPos;
	if (air)
		vz -= 400 * dt;
	if (hit.flFraction < 1 && hit.vecPlaneNormal.z > 0.7)
	{
		vz = 0;
		actor.v.flags |= FL_ONGROUND;
	}
	actor.v.velocity = Vector(float(vx), float(vy), float(vz));
	actor.v.oldbuttons = command.buttons;
	if (actor.v.flags & FL_ONGROUND)
	{
		const float p[]{actor.v.origin.x, actor.v.origin.y, actor.v.origin.z + 0.05f};
		const float below[]{p[0], p[1], p[2] - 0.1f};
		TraceResult floor{};
		floor.flFraction = 1;
		for (const auto& box : floors)
			boxTrace(p, below, box, 1, &worldEntity, floor);
		if (floor.flFraction == 1)
			actor.v.flags &= ~FL_ONGROUND;
	}
}
std::shared_ptr<const nav::model::NavMeshSnapshot> configureLadderMotionFixture(enginefuncs_t& e, edict_t* actor,
																				core::MapGeneration map, bool down)
{
	(void)setup(LadderFace::MinX, down ? LadderExit::SameFace : LadderExit::AcrossTop);
	activeMap = map;
	motionActor = actor;
	e.pfnPEntityOfEntIndex = [](int slot) -> edict_t* {
		return slot == 0 ? &worldEntity : slot == 1 ? motionActor : slot == 40 ? &ladderEntity : nullptr;
	};
