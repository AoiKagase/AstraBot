...
MessageEvent menu(MessageKind kind, std::uint16_t recipient, std::uint16_t validSlots, std::uint8_t menuType,
				  const char* text)
{
	MessageEvent event{};
	event.kind = kind;
...
void testTokenizer()
{
	...
	assert(astrabot::adapter::cstrike::parseMenuSelect("menuselect 1", command, selection) ==
		   astrabot::adapter::cstrike::CommandParseError::None);
	assert(selection == 1U);
	assert(command.count == 2U);
	assert(std::strcmp(command.args.data(), "1") == 0);

	assert(astrabot::adapter::cstrike::parseMenuSelect("  menuselect \"2\"  ", command, selection) ==
		   astrabot::adapter::cstrike::CommandParseError::None);
	assert(selection == 2U);
	assert(astrabot::adapter::cstrike::tokenizeCommand("menuselect 1; quit", command) ==
		   astrabot::adapter::cstrike::CommandParseError::CommandSeparator);
	assert(astrabot::adapter::cstrike::tokenizeCommand("menuselect \"\"", command) ==
		   astrabot::adapter::cstrike::CommandParseError::None);
	assert(command.tokens[1][0] == '\0');
	assert(astrabot::adapter::cstrike::parseMenuSelect("menuselect", command, selection) ==
		   astrabot::adapter::cstrike::CommandParseError::InvalidMenuselect);
	assert(astrabot::adapter::cstrike::parseMenuSelect("menuselect 10", command, selection) ==
		   astrabot::adapter::cstrike::CommandParseError::InvalidMenuselect);
	assert(astrabot::adapter::cstrike::tokenizeCommand("menuselect \"1", command) ==
		   astrabot::adapter::cstrike::CommandParseError::UnterminatedQuote);

	assert(astrabot::adapter::cstrike::tokenizeCommand("a b c d e", command) ==
		   astrabot::adapter::cstrike::CommandParseError::TooManyTokens);
	assert(astrabot::adapter::cstrike::tokenizeCommand("menuselect 1234567890123456", command) ==
		   astrabot::adapter::cstrike::CommandParseError::TokenTooLong);
	char tooLong[astrabot::adapter::cstrike::kMaxCommandBytes + 1U]{};
	for (std::size_t index = 0; index < astrabot::adapter::cstrike::kMaxCommandBytes; ++index)
	{
		tooLong[index] = 'x';
	}
...
void testMessageDecoder()
{
	...
	decoder.end();
	assert(decoder.lastError() == astrabot::adapter::cstrike::MessageDecodeError::UnknownMenuToken);
	assert(gMessages.size() == 3U);

	decoder.begin(999, 4);
	assert(decoder.lastError() == astrabot::adapter::cstrike::MessageDecodeError::UnknownMessage);
}
...
void testAlreadyAssignedAndInvalidMenu()
{
	JoinState assigned{};
	assert(
		assigned.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::CounterTerrorist, 2}, TickId{1})
			.changed);
	...
	assert(unavailable.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::Terrorist, 4}, TickId{1})
			   .changed);
	...
	assert(invalidRequest
			   .begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{static_cast<Team>(2), 1}, TickId{1})
			   .error == JoinError::InvalidRequest);

	JoinState wrongTeam{};
	assert(wrongTeam.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::Terrorist, 1}, TickId{1})
			   .changed);
...
{
	JoinState state{};
	assert(state.begin(PlayerId{2, Generation{1}}, MapGeneration{1}, JoinRequest{Team::CounterTerrorist, 4}, TickId{1})
			   .changed);

	const MessageEvent teamPrompt = menu(MessageKind::ShowMenu, 2, 0x0002, 0, "#IG_Team_Select");
	...
	const MessageEvent classPrompt = menu(MessageKind::VguiMenu, 2, 0x0008, 27, "");
	...
	assert(unassigned.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::Terrorist, 1}, TickId{1})
			   .changed);
	assert(!unassigned.onMessage(teamInfo(1, "UNASSIGNED"), TickId{1}).changed);
	assert(unassigned.phase() == JoinPhase::WaitingTeamMenu);
	assert(!unassigned.onMessage(menu(MessageKind::VguiMenu, 2, 0x0001, 2, ""), TickId{1}).changed);
	assert(unassigned.phase() == JoinPhase::WaitingTeamMenu);
}
...
void testLegacySpectatorTeamMenusAndSpectatorTeamInfo()
{
	...
	{
		JoinState state{};
		assert(state.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::Terrorist, 1}, TickId{1})
				   .changed);
		const auto action = state.onMessage(menu(MessageKind::ShowMenu, 1, 0x0001, 0, token), TickId{1});
		assert(action.changed);
		assert(state.pendingSelection());
	}

	JoinState spectator{};
	assert(spectator.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::Terrorist, 1}, TickId{1})
			   .changed);
...
{
	JoinState retried{};
	assert(retried.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::Terrorist, 1}, TickId{1})
			   .changed);
	...
	JoinState timeout{};
	assert(timeout.begin(PlayerId{1, Generation{1}}, MapGeneration{1}, JoinRequest{Team::Terrorist, 1}, TickId{1})
			   .changed);
