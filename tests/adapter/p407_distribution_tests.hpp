...
void publication()
{
	...
	auto snapshot = owner.world().latest(actor);
	assert(snapshot && snapshot->distributions[0]);
	assert(snapshot->distributions[0]->count == 1 && snapshot->distributions[0]->areas[0].area == 1);
	p401::mode = 1;
	p401::human.v.origin.x = 3000;
	p401::step(fixture, 0.2f);
	snapshot = owner.world().latest(actor);
	const auto* distribution = snapshot->distributions[0];
	...
	p401::step(fixture, 0.2f);
	distribution = owner.world().latest(actor)->distributions[0];
	assert(distribution && distribution->count == 1);
	...
	assert(!owner.world().latest(actor));
	detach();
}
void matrix()
{
	for (unsigned actors : {1U, 8U, 16U})
		for (float dt : {0.008f, 0.016f, 0.1f})
		{
			...
			route_test::Area a{1, {{0, 0, 0}, {150, 100, 0}, 0, 0}}, b{2, {{200, 0, 0}, {350, 100, 0}, 0, 0}};
			a.targets[1] = {2};
			assert(owner.navConsole().publish(owner.registry().mapGeneration(), route_test::snapshot({a, b})).isNone());
			p401::mode = 1;
			for (unsigned i = 0; i < 40; ++i)
			{
				p401::step(fixture, dt);
				const auto& d = owner.distributions().diagnostics();
