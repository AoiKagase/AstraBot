...
std::shared_ptr<const astrabot::nav::model::NavMeshSnapshot>
configureLadderMotionFixture(enginefuncs_t&, edict_t*, astrabot::core::MapGeneration, bool);
...
{
	gNavCommands[name] = callback;
}
void unexpectedHookRegistration(char*, void (*)())
{
	std::abort();
}
int captureArgc()
{
	return static_cast<int>(gNavArgs.size());
}
const char* captureArgv(int i)
{
	return gNavArgs.at(static_cast<std::size_t>(i)).c_str();
}
...
void captureGround(const float*, const float* end, int, edict_t*, TraceResult* result)
{
	*result = {};
	...
	if (gGroundMissing)
		result->flFraction = 1;
	result->vecEndPos = Vector(end[0], end[1], 0);
...
{
	if (!std::strcmp(name, "sv_gravity"))
		return gMissingJumpGravity ? nullptr : &gJumpGravity;
	if (!std::strcmp(name, "mp_jump_height"))
		return gMissingJumpHeight ? nullptr : &gJumpHeight;
	return nullptr;
}
...
float supportHeight(float x)
{
	return x + 16 > 100 ? gStairHeight : 0;
}
void captureNavHull(const float* start, const float* end, int ignoreMonsters, int hull, edict_t* actor,
					TraceResult* result)
{
	...
	*result = {};
	...
	if (gSimulateCrouch && hull == 1 &&
		(gCloseHeadroom || (gCrouchCeiling && end[0] + 16 > 100 && end[0] - 16 < 200)) && end[2] + 36 > 48)
	{
	...
	if (ignoreMonsters == 1)
	{
		...
		{
			if (start[2] + minimumZ < floor)
				result->fStartSolid = 1;
			else
			{
				result->flFraction = (start[2] + minimumZ - floor) / (start[2] - end[2]);
				result->vecEndPos.z = floor - minimumZ;
				result->vecPlaneNormal = Vector(0, 0, 1);
			}
		}
		if (gInvalidateDuringGround)
	...
	{
		result->flFraction = 0.5f;
		result->vecPlaneNormal = Vector(-1, 0, 0);
	...
	if (gSteeringMode >= 0)
	{
		...
		result->flFraction = h.fraction;
		result->fStartSolid = h.startSolid;
		result->vecEndPos = Vector(h.end.x, h.end.y, h.end.z);
		result->vecPlaneNormal = Vector(h.normal.x, h.normal.y, h.normal.z);
		if (h.fraction < 1)
			result->pHit = gPlayerObstacle ? &gNavPlayer : &gWallWorld;
	}
}
void testNavJumpProbeWorldQueries()
{
	...
	const auto idx = query::NavSpatialIndex::build(
		route_test::snapshot({{1, {{0, 0, 0}, {99, 100, 0}, 0, 0}, {}}, {2, {{100, 0, 0}, {200, 100, 0}, 0, 0}, {}}}),
		{2, 3, 1000000});
	assert(idx);
	...
	s.velocity = model::NavVector3{120, 0, 0};
	s.view = model::NavVector3{};
...
void testNavWorldQueries()
{
	...
	auto idx = query::NavSpatialIndex::build(mesh, {1, 1, 1000000});
	assert(idx);
	runtime::QueryRequest q{{{1}, {2, {3}}, {4}, {5}, 6, 1},
							runtime::QueryKind::SweptHull,
							{20, 50, 36},
							{36, 50, 36},
							runtime::HullDimensions{{-16, -16, -36}, {16, 16, 36}}};
	const auto call = [&] { return astrabot::adapter::cstrike::queryNavWorld(&engine, &entity, idx.value->get(), q); };
	gHullMode = 0;
	gHullCalls = 0;
	auto r = call();
	assert(r.error == runtime::QueryError::None && r.hull && r.hull->fraction == 1);
	assert(r.stamp == q.stamp && gHullKind == 1 && gHullCalls == 1);
	q.hull = runtime::HullDimensions{{-16, -16, -18}, {16, 16, 32}};
	r = call();
	assert(r.hull && gHullKind == 3);
	q.hull->maximum.x = 17;
	r = call();
	assert(r.error == runtime::QueryError::Unavailable && gHullCalls == 2);
	q.hull = runtime::HullDimensions{{-16, -16, -36}, {16, 16, 36}};
	gHullMode = 1;
	r = call();
	assert(r.hull && r.hull->startSolid);
	q.kind = runtime::QueryKind::Clearance;
	r = call();
	assert(r.clearance && !r.clearance->clear);
	gHullMode = 2;
	r = call();
	assert(r.clearance && !r.clearance->clear);
	gHullMode = 0;
	...
	gHullMode = 3;
	r = call();
	assert(r.error == runtime::QueryError::InvalidResult);
	gHullMode = 0;
	q.kind = runtime::QueryKind::Floor;
	q.start = {20, 50, 18};
	q.end = {20, 50, -64};
	r = call();
	assert(r.floor && r.floor->supported && r.floor->height == 0);
	q.end.x = 21;
	assert(call().error == runtime::QueryError::InvalidResult);
	q.end.x = 20;
	q.kind = runtime::QueryKind::Door;
	r = call();
	assert(r.error == runtime::QueryError::None && !r.door);
	struct Port final : runtime::IWorldQueries
	{
		enginefuncs_t* engine;
		edict_t* entity;
	...
	gHullMode = 0;
	engine.pfnTraceHull = nullptr;
	q.kind = runtime::QueryKind::SweptHull;
...
void navTransportTrace(const astrabot::debug::MovementTrace& t) noexcept
{
	gNavTransportTraces.push_back(t);
}
...
void captureHookTables(plid_t /* pluginId */, enginefuncs_t** engineFunctions, DLL_FUNCTIONS** dllFunctions,
...
void captureSetClientKeyValue(int clientIndex, char* infoBuffer, char* key, char* value);
void captureRemoveEntity(edict_t* entity);
qboolean captureCallGameEntity(plid_t pluginId, const char* entityName, entvars_t* variables);
qboolean captureClientConnect(edict_t* entity, const char* name, const char* address, char rejectReason[128]);
...
qboolean unexpectedDirectClientConnect(edict_t* entity, const char* name, const char* address, char rejectReason[128]);
...
int captureGetUserMsgID(plid_t pluginId, const char* messageName, int* size);
...
void captureRunPlayerMove(edict_t* entity, const float* viewAngles, float forwardMove, float sideMove, float upMove,
...
edict_t* captureCreateFakeClient(const char* /* name */)
{
	...
	if (gFixture->matrixSlot)
		return gFixture->matrixEntity(gFixture->matrixSlot);
...
int captureIndexOfEdict(const edict_t* entity)
{
	if (gFixture->matrixSlot)
		for (unsigned slot = 1; slot <= 16; ++slot)
			if (entity == gFixture->matrixEntity(slot))
				return static_cast<int>(slot);
	if (entity == &gNavDoor)
		return 40;
	if (entity == &gNavPlayer)
		return 2;
	if (entity == &gFixture->secondEntity)
		return 2;
	return entity == &gFixture->entity ? gFixture->index : 0;
}
edict_t* captureDoorEntity(int index)
{
	if (gMatrixPlayer && index == 2)
		return &gNavPlayer;
	if (gFixture->matrixSlot && index >= 1 && index <= 16)
		return gFixture->matrixEntity(static_cast<unsigned>(index));
	return index == 40				  ? &gNavDoor
		   : index == gFixture->index ? &gFixture->entity
		   : index == 2				  ? (gFixture->multiClient ? &gFixture->secondEntity : &gNavPlayer)
									  : nullptr;
}
...
void captureLadderDirectory(char* directory)
{
	constexpr char path[] = "nav-ladder-test";
	std::memcpy(directory, path, sizeof(path));
}
const char* captureLadderMapString(int index)
{
	return index == 20 ? "p306_fixture" : "unknown";
}
edict_t* captureDoorSphere(edict_t* previous, const float* origin, float radius)
{
	...
	assert(radius == 64);
	if (gDoorLoop)
		return &gFixture->entity;
	if (gDoorAmbiguous)
		return &gNavCompetitor;
	if (previous == &gNavDoor)
		return nullptr;
	if (!previous)
		return &gFixture->entity;
...
{
	gDoorActive = true;
	gDoorOpen = gDoorLocked = gDoorAmbiguous = gDoorLoop = false;
	gDoorUses = gDoorScans = gTouchContacts = 0;
	gDoorOpenAtUs = 0;
	gNavDoor = {};
	gNavCompetitor = {};
	gNavDoor.serialnumber = 7;
	gNavDoor.v.classname = 10;
	gNavDoor.v.spawnflags = 1 << 8;
	gNavDoor.v.absmin = Vector(100, 0, 0);
	gNavDoor.v.size = Vector(4, 100, 72);
...
void testDoorObservationContracts()
{
	using namespace astrabot;
	Fixture fixture{};
	configureDoor(fixture);
	fixture.entity.v.origin = Vector(50, 50, 36);
	fixture.entity.v.view_ofs = Vector(0, 0, 28);
	nav::runtime::QueryRequest q{{{1}, {1, {1}}, {1}, {1}, 1, 1},
								 nav::runtime::QueryKind::Door,
								 {50, 50, 36},
								 {98, 50, 36},
								 nav::runtime::HullDimensions{{-16, -16, -36}, {16, 16, 36}}};
	const auto query = [&] {
		return adapter::cstrike::queryNavWorld(&fixture.engine, &fixture.entity, nullptr, q, 128);
	};
	auto r = query();
	assert(r.door && !r.door->open && r.door->canUse && r.door->useView);
	const auto id = r.door->id;
	assert(id == ((std::uint64_t{7} << 32) | 40));
	...
	{
		const auto saved = fixture.entity.v.origin;
		const auto oldSize = gNavDoor.v.size;
		if (mode == 0)
			fixture.entity.v.origin.x = 0; // outside 64-unit use search
		if (mode == 1)
			gDoorAmbiguous = true;
		if (mode == 2)
			gDoorLoop = true;
		...
		if (mode == 4)
			gNavDoor.v.size.x = -1;
		if (mode == 5)
			gNavDoor.v.spawnflags = 0;
		if (mode == 6)
			gNavDoor.serialnumber = 8;
		assert(!adapter::cstrike::doorUseView(&fixture.engine, &fixture.entity, id, 128));
		fixture.entity.v.origin = saved;
		gNavDoor.v.size = oldSize;
		gNavDoor.free = 0;
		gNavDoor.v.spawnflags = 1 << 8;
		gNavDoor.serialnumber = 7;
		gDoorAmbiguous = gDoorLoop = false;
	}
	assert(!adapter::cstrike::doorUseView(&fixture.engine, &fixture.entity, id, 40));
	q.doorId = id;
	gDoorOpen = true;
	r = query();
	assert(r.door && r.door->open && !r.door->canUse);
	++gNavDoor.serialnumber;
	assert(!query().door);
	gNavDoor.serialnumber = 7;
	gDoorOpen = false;
	gNavDoor.v.classname = 11;
	r = query();
	assert(r.door && r.door->canUse); // ordinary rotating use door
	gNavDoor.v.classname = 12;
	assert(!query().door);
	gDoorActive = false;
	gDoorOpenAtUs = 0;
}
...
void captureRemoveEntity(edict_t* entity)
{
	...
	{
		gClientRemovals.push_back(entity);
		entity->free = true;
	}
}
...
qboolean captureClientConnect(edict_t* /* entity */, const char* /* name */, const char* /* address */,
							  char /* rejectReason */[128])
{
	return gFixture->connectSucceeds ? 1 : 0;
}

qboolean unexpectedDirectClientConnect(edict_t* /* entity */, const char* /* name */, const char* /* address */,
									   char /* rejectReason */[128])
{
...
void captureClientCommand(edict_t* entity)
{
	...
	{
		gReentrantDispatchResult = astrabot::adapter::metamod::lifecycleCoordinator().dispatchMenuForTest(1);
	}
	...
	if (gFixture->multiClient)
		gClientCommands.push_back({entity, gLastCommandArgv1});
}
...
{
	if (gFixture->matrixSlot)
		return captureIndexOfEdict(entity);
	if (gFixture->multiClient && entity == &gFixture->secondEntity)
		return 2;
						  ...
						  unsigned short buttons, byte impulse, byte msec)
{
	...
	if (gFixture->multiClient &&
		(forwardMove != 0.0F || sideMove != 0.0F || upMove != 0.0F || buttons != 0U || impulse != 0U))
	{
	...
	if (gSimulateNav)
	{
		...
		astrabot::core::BotCommand c{
			{viewAngles[0], viewAngles[1], viewAngles[2]}, {forwardMove, sideMove, upMove}, buttons, impulse, msec};
		gNavMoves.push_back(c);
		assert(c.validate());
		if (forwardMove != 0 || sideMove != 0)
		{
			...
			assert(state != astrabot::nav::local::RecoveryState::Wait &&
				   state != astrabot::nav::local::RecoveryState::Replan &&
		...
		assert((buttons == 0 || (gDoorActive && buttons == IN_USE) || (gSimulateCrouch && buttons == IN_DUCK) ||
				(gSimulateJump && buttons == IN_JUMP)) &&
			   impulse == 0 && upMove == 0 && entity->v.deadflag == DEAD_NO);
		if (gSimulateCrouch)
		{
			const bool duck = (buttons & IN_DUCK) != 0;
			if (!duck && (entity->v.flags & FL_DUCKING))
			{
				assert(!gCloseHeadroom &&
					   !(gCrouchCeiling && entity->v.origin.x + 16 > 100 && entity->v.origin.x - 16 < 200));
			}
			if (duck)
				entity->v.flags |= FL_DUCKING;
			else
				entity->v.flags &= ~FL_DUCKING;
			entity->v.mins.z = duck ? -18.0f : -36.0f;
			entity->v.maxs.z = duck ? 32.0f : 36.0f;
		}
		if (buttons == IN_USE)
		{
			...
			const double px = 102.0 - entity->v.origin.x, py = 50.0 - entity->v.origin.y,
						 pz = 36.0 - entity->v.origin.z;
			...
			const double dot =
				(dx * std::cos(pitch) * std::cos(yaw) + dy * std::cos(pitch) * std::sin(yaw) - dz * std::sin(pitch)) /
		...
		{
			assert(entity->v.flags & FL_ONGROUND);
			assert(entity->v.fuser2 <= 0);
			entity->v.flags &= ~FL_ONGROUND;
			entity->v.velocity.z = static_cast<float>(std::sqrt(1600 * double(gJumpHeight.value)));
			entity->v.fuser2 = 1315.789429f;
		}
		double vx = forwardMove * std::cos(yaw) + sideMove * std::sin(yaw),
			   vy = forwardMove * std::sin(yaw) - sideMove * std::cos(yaw);
		if (gFreezeNav || entity == gFrozenActor)
			vx = vy = 0;
		if (gSimulateJump && !(entity->v.flags & FL_ONGROUND))
		{
			vx = entity->v.velocity.x;
			vy = entity->v.velocity.y;
		}
		entity->v.origin.x += static_cast<float>(vx * dt);
		entity->v.origin.y += static_cast<float>(vy * dt);
		...
		if (gSimulateJump && !(entity->v.flags & FL_ONGROUND))
		{
			...
			{
				entity->v.origin.z = 36;
				entity->v.velocity.z = 0;
				entity->v.flags |= FL_ONGROUND;
				if (gWrongJumpLanding)
					entity->v.origin.x = 50;
			}
		}
		else
		...
		if (gSimulateJump)
		{
			entity->v.velocity.x = static_cast<float>(vx);
			entity->v.velocity.y = static_cast<float>(vy);
			entity->v.fuser2 = (std::max)(0.0f, entity->v.fuser2 - msec);
			entity->v.oldbuttons = buttons;
		}
		if (gSteeringMode >= 0)
		{
		...
		if (gInjectNavDuplicate)
		{
			...
			assert(owner
					   .submitCommand(owner.fakeClient().activePlayer(), owner.registry().mapGeneration(),
									  owner.registry().currentTick(), astrabot::core::BotCommand::neutral(1))
					   .queued());
		}
		if (gDeactivateDuringMove)
		{
			gDeactivateDuringMove = false;
			owner.serverDeactivate();
		}
		if (gReplaceDuringMove)
		{
			gReplaceDuringMove = false;
...
void attach(Fixture& fixture)
{
	...
	astrabot::adapter::metamod::lifecycleCoordinator().setFakeClientTraceSink(&captureFakeTrace);
}
...
{
	attach(fixture);
	runNav({"astrabot_debug", "2"});
	assert(astrabot::adapter::metamod::lifecycleCoordinator().registry().activateMap(32));
}
...
void testSuccessfulCreationAndOpaquePrivateData()
{
	...
	const FakeClientResult result =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-1");
	assert(result.succeeded());
	const astrabot::core::PlayerId expectedPlayer{1, astrabot::core::Generation{1}};
	...
	const FakeClientResult duplicate =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-2");
...
{
	{
		Fixture fixture{};
		activate(fixture);
		const FakeClientResult result = astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create(nullptr);
	...
	}
	{
		...
		activate(fixture);
		const FakeClientResult result =
			astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot");
...
void testInputAndCapacityRejection()
{
	...
	const FakeClientResult tooLongResult =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create(tooLong);
	assert(tooLongResult.error == FakeClientError::InvalidName);
	detach();

	Fixture occupiedFixture{};
	activate(occupiedFixture);
	const auto connected = astrabot::adapter::metamod::lifecycleCoordinator().registry().registerPlayer(1);
	assert(connected);
...
void testMissingFunctionIsRejectedWithoutEngineCall()
{
	...
	const FakeClientResult result = astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot");
	assert(result.error == FakeClientError::NotConfigured);
	assert(fixture.createCalls == 0);
	detach();
}

void sendVguiMenu(enginefuncs_t& hooks, int messageId, edict_t* recipient, int menuType, int validSlots)
{
...
{
	gNavClockUs += us;
	fixture.engineGlobals.frametime = static_cast<float>(us) / 1000000.0f;
	if (gDoorOpenAtUs && gNavClockUs >= gDoorOpenAtUs)
	{
		gDoorOpen = true;
		gDoorOpenAtUs = 0;
	}
...
void prepareNavWalk(Fixture& fixture, enginefuncs_t& hooks, std::uint64_t us = 16000, bool freshMap = false)
{
	...
	gGroundMissing = false;
	gHullMode = 0;
	gInvalidateDuringGround = gInvalidateDuringHull = false;
	gStairHeight = 0;
	gStairCeiling = false;
	int version = ENGINE_INTERFACE_VERSION;
	assert(GetEngineFunctions(&hooks, &version));
	gEngineHooks = &hooks;
	...
	sendVguiMenu(hooks, 11, &fixture.entity, 2, 1);
	navFrame(fixture, us);
	sendVguiMenu(hooks, 11, &fixture.entity, 26, 1);
	navFrame(fixture, us);
	...
	fixture.entity.v.origin = Vector(50, 50, 36);
	fixture.entity.v.v_angle = Vector(0, 90, 0);
	fixture.entity.v.mins = Vector(-16, -16, -36);
	fixture.entity.v.maxs = Vector(16, 16, 36);
	fixture.entity.v.maxspeed = 250;
...
void testNavPlayerQueries()
{
	...
	gNavPlayer = {};
	gNavPlayer.v.flags = 0;
	gNavPlayer.v.solid = SOLID_SLIDEBOX;
	gNavPlayer.serialnumber = 17;
	gPlayerObstacle = true;
	gSteeringMode = 1;
	host::PlayerRegistry players;
	assert(players.activateMap(32));
	assert(players.registerPlayer(1));
	assert(players.registerPlayer(2));
	assert(players.startFrame());
	struct Mapping
	{
		core::PlayerId player{};
		int serial{};
	} mapping{players.currentPlayer(2), 17};
	const adapter::cstrike::NavPlayerResolver resolver{
		&mapping, [](const void* context, edict_t* entity) noexcept {
	...
	fixture.engine.pfnTraceHull = [](const float* start, const float* end, int ignoreMonsters, int hull, edict_t* actor,
									 TraceResult* result) {
	...
	assert(players.disconnectSlot(2));
	assert(players.registerPlayer(2));
	++gNavPlayer.serialnumber;
	mapping = {players.currentPlayer(2), gNavPlayer.serialnumber};
	r = call(&players);
	assert(r.blocker && r.blocker->id != oldId && r.blocker->player->generation.value != oldGeneration);
	r = call(nullptr);
	...
	r = call(&players);
	assert(r.blocker && r.blocker->kind == nav::runtime::BlockerKind::Other && !r.blocker->player);
	++q.stamp.actor.generation.value;
	r = call(&players);
	assert(r.error == nav::runtime::QueryError::InvalidResult && !r.blocker);
	q.stamp.actor = players.currentPlayer(1);
	++q.stamp.map.value;
	r = call(&players);
	assert(r.error == nav::runtime::QueryError::InvalidResult && !r.blocker);
	q.stamp.map = players.mapGeneration();
	++q.stamp.tick.value;
	r = call(&players);
	assert(r.error == nav::runtime::QueryError::InvalidResult && !r.blocker);
	q.stamp.tick = players.currentTick();
	fixture.engine.pfnPEntityOfEntIndex = nullptr;
	assert(call(&players).error == nav::runtime::QueryError::Unavailable);
	gNavPlayer.free = true;
	assert(!call(&players).blocker);
	gNavPlayer = {};
	gPlayerObstacle = false;
	gSteeringMode = -1;
}
void testMultipleManagedClients()
{
	...
	Fixture fixture{};
	fixture.multiClient = true;
	fixture.engine.pfnPEntityOfEntIndex = &captureDoorEntity;
	fixture.entity.serialnumber = 11;
	fixture.secondEntity.serialnumber = 22;
	activate(fixture);
	...
	owner.setMovementClockForTest(&navNow);
	gSimulateNav = false;
	gClientCommands.clear();
	gClientMoves.clear();
	gClientRemovals.clear();
	const auto first = owner.createBot("AstraBot-One", {Team::Terrorist, 1});
	assert(first.succeeded());
	fixture.createSecond = true;
	const auto second = owner.createBot("AstraBot-Two", {Team::CounterTerrorist, 2});
	...
	const auto fragment = [&](edict_t* recipient, int slots, int more, const char* text) {
		hooks.pfnMessageBegin(MSG_ONE, 12, nullptr, recipient);
		hooks.pfnWriteShort(slots);
		hooks.pfnWriteChar(-1);
		hooks.pfnWriteByte(more);
		hooks.pfnWriteString(text);
		hooks.pfnMessageEnd();
	};
	// Only the addressed client consumes its menu and confirmation.
	fragment(&fixture.entity, 1, 1, "#Team_");
	fragment(&fixture.secondEntity, 2, 1, "#Team_");
	fragment(&fixture.entity, 1, 0, "Select");
	assert(!owner.joinState(second.player)->pendingSelection());
	frame();
	assert(gClientCommands.size() == 1 && gClientCommands.back().first == &fixture.entity &&
		   gClientCommands.back().second == "1");
	...
	one.movement.forward = 17;
	two.movement.forward = 53;
	...
	assert(owner.submitCommand(second.player, map, owner.registry().currentTick(), two).error ==
		   adapter::metamod::MovementError::NotJoined);
	frame();
	assert(gClientMoves.size() == 1 && gClientMoves.back().first == &fixture.entity &&
		   gClientMoves.back().second == 17);
	fragment(&fixture.secondEntity, 2, 0, "Select");
	frame();
	assert(gClientCommands.back().first == &fixture.secondEntity && gClientCommands.back().second == "2");
	sendVguiMenu(hooks, 11, &fixture.secondEntity, 27, 2);
	frame();
	sendTeamInfo(hooks, 13, 2, "CT");
	frame();
	assert(owner.joinState(second.player)->phase() == JoinPhase::Joined &&
		   owner.joinState(first.player)->phase() == JoinPhase::Joined);
	gClientMoves.clear();
	assert(owner.submitCommand(second.player, map, owner.registry().currentTick(), two).queued());
	assert(owner.submitCommand(first.player, map, owner.registry().currentTick(), one).queued());
	frame();
	assert(gClientMoves.size() == 2);
	...
	assert(owner.submitCommand(second.player, map, owner.registry().currentTick(), two).queued());
	assert(owner.submitCommand(first.player, map, owner.registry().currentTick(), one).queued());
	fixture.secondEntity.v.deadflag = DEAD_DEAD;
	frame();
	...
	assert(!owner.entityFor(second.player));
	...
	assert(owner.entityFor(first.player) == &fixture.entity &&
		   owner.joinState(first.player)->phase() == JoinPhase::Joined);
	const auto replacement = owner.createBot("AstraBot-Replacement", {Team::Terrorist, 1});
	...
	++fixture.secondEntity.serialnumber;
	const auto fresh = owner.createBot("AstraBot-Fresh", {Team::Terrorist, 1});
	assert(fresh.succeeded());
	hooks.pfnWriteByte(2);
	hooks.pfnWriteString("CT");
	hooks.pfnMessageEnd();
	assert(owner.joinState(fresh.player)->phase() == JoinPhase::WaitingTeamMenu &&
		   !owner.joinState(fresh.player)->teamConfirmed());
	owner.clientDisconnect(&fixture.secondEntity);
	assert(owner.entityFor(first.player) == &fixture.entity && owner.agents().mappingCount() == 1);
	assert(owner.submitCommand(first.player, map, owner.registry().currentTick(), one).queued());
	const auto beforeMap = gClientMoves.size();
	owner.serverDeactivate();
	frame();
...
void testMultipleNavSessions()
{
	...
	for (std::uint64_t us : {8000U, 16000U, 100000U})
		for (int mode = 0; mode < 6; ++mode)
		{
			...
			assert(second.succeeded());
			sendVguiMenu(hooks, 11, &fixture.secondEntity, 2, 1);
			navFrame(fixture);
			sendVguiMenu(hooks, 11, &fixture.secondEntity, 26, 1);
			navFrame(fixture);
			sendTeamInfo(hooks, 13, 2, "TERRORIST");
			navFrame(fixture);
			assert(owner.joinState(second.player)->phase() == adapter::cstrike::JoinPhase::Joined);
			fixture.secondEntity.v = fixture.entity.v;
			...
			runNav({"astrabot_goto", "4", two.c_str()});
			...
			gClientMoves.clear();
			navFrame(fixture, us);
			const auto firstSequence = console.motionTrace(first)->sequence,
					   secondSequence = console.motionTrace(second.player)->sequence;
			// Ambiguous, malformed and stale selectors must not retire either pending command.
			runNav({"astrabot_nav_cancel"});
			runNav({"astrabot_goto", "1", "1:0"});
			runNav({"astrabot_nav_cancel", "2:999"});
			runNav({"astrabot_nav_cancel", "2:1:1"});
			assert(console.motionTrace(first)->sequence == firstSequence &&
				   console.motionTrace(second.player)->sequence == secondSequence);
			if (mode == 1)
				runNav({"astrabot_nav_cancel", one.c_str()});
			if (mode == 2)
				owner.clientDisconnect(&fixture.secondEntity);
			if (mode == 5)
			{
				gInvalidateFromActor = &fixture.entity;
				gInvalidateActor = second.player;
				runNav({"astrabot_goto", "2", one.c_str()});
			...
			{
				const auto firstHulls = gActorHullCalls[&fixture.entity],
						   secondHulls = gActorHullCalls[&fixture.secondEntity];
				navFrame(fixture, us);
				for (const auto player : {first, second.player})
				{
					const auto* t = console.motionTrace(player);
					assert(t);
				...
				assert(std::abs(fixture.entity.v.origin.y - 50) < 0.01f &&
					   std::abs(fixture.secondEntity.v.origin.y - 250) < 0.01f);
				const bool oneArrived = console.motionTrace(first)->decision.state == nav::local::WalkState::Arrived;
				const bool twoArrived =
					console.motionTrace(second.player)->decision.state == nav::local::WalkState::Arrived;
				if ((mode == 1 || oneArrived) && (mode == 2 || mode == 5 || twoArrived))
				{
					arrived = true;
					break;
				}
			...
			if (mode != 2 && mode != 5)
				assert(std::abs(fixture.secondEntity.v.origin.x - 117) <= 1.01f);
			if (mode == 1)
				assert(fixture.entity.v.origin.x == stoppedX);
			if (mode == 2 || mode == 5)
				assert(fixture.secondEntity.v.origin.x == stoppedX);
			if (mode == 4)
			{
				owner.clientDisconnect(&fixture.secondEntity);
				++fixture.secondEntity.serialnumber;
				const auto replacement =
					owner.createBot("AstraBot-NewGeneration", {adapter::cstrike::Team::Terrorist, 1});
				assert(replacement.succeeded() && replacement.player.generation != second.player.generation);
				runNav({"astrabot_goto", "4", two.c_str()});
				assert(!console.trace(replacement.player));
				sendVguiMenu(hooks, 11, &fixture.secondEntity, 2, 1);
				navFrame(fixture);
				sendVguiMenu(hooks, 11, &fixture.secondEntity, 26, 1);
				navFrame(fixture);
				...
				for (int frame = 0; frame < 2000 && console.motionTrace(replacement.player)->decision.state !=
														nav::local::WalkState::Arrived;
					 ++frame)
					navFrame(fixture, us);
	...
	for (std::uint64_t us : {8000U, 16000U, 100000U})
		for (int mode = 0; mode < 4; ++mode)
		{
			...
			for (int frame = 0; frame < 2000; ++frame)
			{
				...
				if (fixture.entity.v.flags & FL_DUCKING)
					lowered = true;
				if (mode == 1 && lowered && fixture.entity.v.origin.x > 120)
				{
					...
					assert(!gNavMoves.empty() && gNavMoves.back().movement == core::Movement{} &&
						   gNavMoves.back().buttons == IN_DUCK);
					finished = true;
					break;
				}
					...
					console.motionTrace().event == adapter::cstrike::MotionEvent::Queued)
				{
					...
					for (std::size_t i = 0; i < console.motionHistoryCount(); ++i)
						releaseRejected = releaseRejected || console.motionHistory(i)->reason ==
																 adapter::cstrike::MotionReason::PostureChanged;
					if (mode == 2)
						gCloseHeadroom = false;
				}
				if (d.state == nav::local::WalkState::Failed)
				{
					assert(mode == 3 && d.reason == nav::local::WalkReason::PostureFailed &&
						   d.postureReason == nav::local::CrouchReason::TimedOut);
					assert(fixture.entity.v.flags & FL_DUCKING);
					finished = true;
					break;
				}
			...
			gSimulateCrouch = gCrouchCeiling = gCloseHeadroom = false;
			gSimulateNav = false;
			detach();
		}
}
void testNavProgressRecovery()
{
	...
	for (std::uint64_t us : {8000U, 16000U, 100000U})
		for (int mode = 0; mode < 2; ++mode)
		{
			...
			{
				if (mode == 1 && elapsed >= 700000)
					gFreezeNav = false;
				const auto queries = gHullCalls;
				navFrame(fixture, us);
				assert(gHullCalls - queries <= 21);
				waiting |= console.motionTrace().decision.recovery.state == nav::local::RecoveryState::Wait;
				if (waiting && !detectedAt)
					detectedAt = elapsed + us;
				if (!finishedAt && console.trace()->routeGeneration == 2 &&
					console.motionTrace().decision.state != nav::local::WalkState::Running)
					finishedAt = elapsed + us;
			}
			assert(waiting);
			assert(detectedAt <= 700000 + us && finishedAt > detectedAt);
			if (mode == 0)
				assert(finishedAt <= 3000000);
			assert(console.motionTrace().dispatched > 0);
			assert(console.motionTrace().decision.state ==
				   (mode == 0 ? nav::local::WalkState::Failed : nav::local::WalkState::Arrived));
			...
			for (std::size_t i = 0; i < console.motionHistoryCount(); ++i)
			{
				...
				if (trace->event == adapter::cstrike::MotionEvent::Decision && trace->decision.terminalEvent)
					++terminalEvents;
			}
			assert(terminalEvents == 1);
			std::printf("P3-07 mode=%d frame_us=%llu detected_us=%llu terminal_us=%llu side_us=%llu reverse_us=%llu\n",
						mode, static_cast<unsigned long long>(us), static_cast<unsigned long long>(detectedAt),
						static_cast<unsigned long long>(finishedAt),
						static_cast<unsigned long long>(
							gRecoveryMotionUs[static_cast<std::size_t>(nav::local::RecoveryState::Sidestep)]),
						static_cast<unsigned long long>(
							gRecoveryMotionUs[static_cast<std::size_t>(nav::local::RecoveryState::Reverse)]));
			...
			if (mode == 0)
			{
				gFreezeNav = false;
				runNav({"astrabot_goto", "4"});
				for (std::uint64_t elapsed = 0; elapsed < 5000000; elapsed += us)
					navFrame(fixture, us);
...
void testNavRecoveryCancellation()
{
	using namespace astrabot;
	for (auto stage :
		 {nav::local::RecoveryState::Wait, nav::local::RecoveryState::Sidestep, nav::local::RecoveryState::Replan})
	{
		Fixture fixture{};
		enginefuncs_t hooks{};
		prepareNavWalk(fixture, hooks);
		...
		{
			navFrame(fixture);
			if (console.motionTrace().decision.recovery.state == stage)
			{
				reached = true;
				break;
			}
		...
		for (std::size_t i = count; i < gNavMoves.size(); ++i)
			assert(gNavMoves[i].movement == core::Movement{});
...
void testNavAutomaticReplan()
{
	...
	for (std::uint64_t us : {8000U, 16000U, 100000U})
		for (int mode = 0; mode < 5; ++mode)
		{
			...
			gNavPlayer = {};
			gNavPlayer.v.flags = FL_CLIENT;
			gNavPlayer.v.solid = SOLID_SLIDEBOX;
			gNavPlayer.serialnumber = 23;
			...
			for (int frame = 0; frame < 2000; ++frame)
			{
				...
				if (console.trace()->routeGeneration == 2)
				{
					...
					assert(console.trace()->route &&
						   console.trace()->route->areas == std::vector<nav::model::NavAreaId>({{1}, {3}, {4}, {2}}));
					assert(console.trace()->route->total == 300);
					if (mode == 1)
						gSteeringMode = 2; // replacement route is blocked too
					if (mode == 1 && console.replan(player)->state() == nav::runtime::ReplanState::Exhausted)
					{
						finished = true;
						break;
					}
					if (mode == 0 && console.motionTrace().decision.state == nav::local::WalkState::Arrived)
					{
						finished = true;
						break;
					}
			...
			if (!finished)
				std::fprintf(
					stderr, "replan mode=%d us=%llu generation=%llu walk=%u reason=%u state=%u pos=(%.3f,%.3f)\n", mode,
					static_cast<unsigned long long>(us),
					static_cast<unsigned long long>(console.trace()->routeGeneration),
					unsigned(console.motionTrace().decision.state), unsigned(console.motionTrace().decision.reason),
					unsigned(console.replan(player)->state()), fixture.entity.v.origin.x, fixture.entity.v.origin.y);
			assert(pending && finished);
			if (mode == 0)
				assert(std::hypot(fixture.entity.v.origin.x - 117, fixture.entity.v.origin.y - expectedGoalY) <= 1.01f);
...
void testNavPlayers()
{
	...
	for (std::uint64_t us : {8000U, 16000U, 100000U})
		for (int mode = 0; mode < 5; ++mode)
		{
			...
			gNavPlayer.serialnumber = 23;
			gPlayerObstacle = true;
			gSteeringMode = mode == 1 ? 2 : 1;
			runNav({"astrabot_goto", "2"});
			const auto start = gNavClockUs;
			bool finished = false, avoided = false, observed = false, cleared = false;
			for (int frame = 0; frame < 1600; ++frame)
			{
				if (mode == 2 && gNavClockUs - start >= 300000)
					gSteeringMode = -1;
				const auto before = gHullCalls;
				navFrame(fixture, us);
				const auto& d = console.motionTrace().decision;
				if (d.tick == owner.registry().currentTick())
					assert(d.queries == unsigned(gHullCalls - before) && d.queries <= 21);
				observed = observed || d.blocker.has_value();
				avoided = avoided || d.avoiding;
				...
				if (d.blocker)
					assert(d.blocker->kind == nav::runtime::BlockerKind::Other && !d.blocker->player);
				if (d.state != nav::local::WalkState::Running)
				{
					if (d.state !=
						(mode == 1 || mode == 4 ? nav::local::WalkState::Failed : nav::local::WalkState::Arrived))
						std::fprintf(
							stderr,
							"host player mode=%d us=%llu state=%u reason=%u probe=%u blocker=%u pos=(%.3f,%.3f)\n",
							mode, static_cast<unsigned long long>(us), unsigned(d.state), unsigned(d.reason),
							unsigned(d.probeReason), unsigned(d.blockerReason), fixture.entity.v.origin.x,
							fixture.entity.v.origin.y);
					assert(d.state ==
						   (mode == 1 || mode == 4 ? nav::local::WalkState::Failed : nav::local::WalkState::Arrived));
					if (mode == 1)
						assert(d.reason == nav::local::WalkReason::DynamicBlocked &&
							   d.blockerReason == nav::local::BlockerReason::TimedOut);
					if (mode == 4)
						assert(d.reason == nav::local::WalkReason::DynamicBlocked &&
							   d.blockerReason == nav::local::BlockerReason::Unavailable);
					finished = true;
					break;
				}
			}
			...
			if (mode != 4)
				assert(observed);
			if (mode == 0 || mode == 3)
				assert(avoided && cleared);
			runNav({"astrabot_nav_status"});
			bool diagnostic = false;
			for (const auto& line : gNavOutput)
				diagnostic = diagnostic || line.find("blocker_action=") != std::string::npos;
			assert(diagnostic);
			gPlayerObstacle = false;
			gSteeringMode = -1;
			gSimulateNav = false;
...
void testMultipleClientDetach()
{
	...
	detach();
	assert(gClientRemovals.size() == 2 && gClientRemovals[0] == &fixture.entity &&
		   gClientRemovals[1] == &fixture.secondEntity);
...
{
	auto& nav = astrabot::adapter::metamod::lifecycleCoordinator().navConsole();
	for (std::size_t i = 0; i < nav.motionHistoryCount(); ++i)
		if (nav.motionHistory(i)->reason == reason)
			return true;
	return false;
}
void testStandardJumpPhysics()
{
	using namespace astrabot;
	enginefuncs_t engine{};
	engine.pfnCVarGetPointer = &captureJumpCvar;
	edict_t entity{};
	entity.v.movetype = MOVETYPE_WALK;
	entity.v.mins = Vector(-16, -16, -36);
	entity.v.maxs = Vector(16, 16, 36);
	const nav::local::Binding binding{{1}, {2, {3}}, {4}, 5, 6};
	gJumpGravity.value = 800;
	gJumpHeight.value = 45;
	...
	p = adapter::cstrike::standardJumpPhysics(&engine, &entity, binding, {7});
	assert(p && p->gravity == 400);
	entity.v.gravity = 0;
	gJumpHeight.value = 64;
	p = adapter::cstrike::standardJumpPhysics(&engine, &entity, binding, {7});
	assert(p && p->verticalImpulse == 320);
	gMissingJumpHeight = true;
	p = adapter::cstrike::standardJumpPhysics(&engine, &entity, binding, {7});
	assert(p && p->verticalImpulse < 269);
	gMissingJumpHeight = false;
	gJumpHeight.value = 45;
	for (int mode = 0; mode < 4; ++mode)
	{
		entity.v.movetype = MOVETYPE_WALK;
		entity.v.waterlevel = 0;
		entity.v.basevelocity = Vector(0, 0, 0);
		gMissingJumpGravity = false;
		if (mode == 0)
			gMissingJumpGravity = true;
		if (mode == 1)
			entity.v.waterlevel = 1;
		if (mode == 2)
			entity.v.basevelocity.x = 1;
		if (mode == 3)
...
void testJumpPhysicsAssessment()
{
	...
	gJumpHeight.value = 45;
	gMissingJumpGravity = gMissingJumpHeight = false;
	const auto inspect = [&]() { return adapter::cstrike::assessStandardJumpPhysics(&engine, &entity, binding, {7}); };
	auto p = inspect();
	assert(p && p.reason == JumpPhysicsReason::None && p.posture == JumpActorPosture::Standing && p.physics);
	const auto ticket = *p.physics;
	auto current = ticket;
	assert(adapter::cstrike::validateJumpPhysicsTicket(ticket, current, binding, {7}) == JumpPhysicsReason::None);
	current.gravity = std::nextafter(current.gravity, (std::numeric_limits<double>::infinity)());
	assert(adapter::cstrike::validateJumpPhysicsTicket(ticket, current, binding, {7}) == JumpPhysicsReason::None);
	for (int i = 0; i < 4; ++i)
		current.gravity = std::nextafter(current.gravity, (std::numeric_limits<double>::infinity)());
	...
	current = ticket;
	current.verticalImpulse += 1;
	assert(adapter::cstrike::validateJumpPhysicsTicket(ticket, current, binding, {7}) ==
		   JumpPhysicsReason::ImpulseChanged);
	current = ticket;
	current.crouchSpeedMultiplier += 0.1;
	assert(adapter::cstrike::validateJumpPhysicsTicket(ticket, current, binding, {7}) ==
		   JumpPhysicsReason::CrouchMultiplierChanged);
	current = ticket;
	current.standingHull->maximum.x += 1;
	assert(adapter::cstrike::validateJumpPhysicsTicket(ticket, current, binding, {7}) ==
	...
	assert(adapter::cstrike::validateJumpPhysicsTicket(ticket, ticket, binding, {8}) == JumpPhysicsReason::TicketStale);
	entity.v.flags |= FL_DUCKING;
	p = inspect();
	...
	entity.v.movetype = MOVETYPE_FLY;
	assert(inspect().reason == JumpPhysicsReason::MoveType);
	entity.v.movetype = MOVETYPE_WALK;
	entity.v.waterlevel = 1;
	assert(inspect().reason == JumpPhysicsReason::Water);
	entity.v.waterlevel = 0;
	entity.v.flags |= FL_WATERJUMP;
		...
		for (int mode = 0; mode <= 10; ++mode)
			for (std::uint64_t us : {8000U, 16000U, 100000U})
			{
				if (multiple && mode)
					continue;
				Fixture fixture{};
				fixture.engine.pfnCVarGetPointer = &captureJumpCvar;
				gJumpGravity.value = 800;
				gJumpHeight.value = 45;
				gMissingJumpGravity = gMissingJumpHeight = false;
				enginefuncs_t hooks{};
				...
				fixture.entity.v.velocity = Vector(0, 0, 0);
				auto& owner = adapter::metamod::lifecycleCoordinator();
				auto& console = owner.navConsole();
				route_test::Area a{1, {{0, 0, 0}, {100, 100, 0}, 0, 0}},
					b{2, {{100, 0, 0}, {200, 100, 0}, 0, 0}, {}, 2}, c{3, {{200, 0, 0}, {300, 100, 0}, 0, 0}};
				a.targets[1] = {2};
				if (multiple)
					b.targets[1] = {3};
				assert(console
						   .publish(owner.registry().mapGeneration(),
									route_test::snapshot(multiple ? std::vector<route_test::Area>{a, b, c}
																  : std::vector<route_test::Area>{a, b}))
						   .isNone());
				gSimulateJump = true;
				gMissJump = mode == 2;
				gWrongJumpLanding = mode == 3;
				...
				for (int frame = 0; frame < 2000; ++frame)
				{
					...
					if (press && !injected)
					{
						...
						if (mode == 6)
							fixture.entity.v.velocity.x = 0;
						if (mode == 7)
							gJumpGravity.value = 600;
						if (mode == 8)
							fixture.entity.v.fuser2 = 200;
						if (mode == 9)
							fixture.entity.v.waterlevel = 1;
					}
					if (!(fixture.entity.v.flags & FL_ONGROUND))
					{
						if (mode == 4)
						{
							...
							assert(!gNavMoves.empty() && gNavMoves.back().buttons == 0 &&
								   gNavMoves.back().movement == core::Movement{});
							done = true;
							break;
						}
						if (mode == 10)
					...
					const auto before = gHullCalls;
					navFrame(fixture, us);
					assert(gHullCalls - before <= 21);
					const auto& trace = console.motionTrace();
					if (trace.decision.state == nav::local::WalkState::Arrived)
					{
						assert(mode == 0);
						done = true;
						break;
					}
					if (trace.decision.state == nav::local::WalkState::Failed ||
						trace.decision.state == nav::local::WalkState::Aborted)
					{
						if (mode == 0)
							std::fprintf(stderr,
										 "host jump failed us=%llu frame=%d reason=%u jump=%u probe=%u motion=%u "
										 "pos=(%.3f,%.3f,%.3f)\n",
										 static_cast<unsigned long long>(us), frame, unsigned(trace.decision.reason),
										 unsigned(trace.decision.jumpReason), unsigned(trace.decision.jumpProbeReason),
										 unsigned(trace.reason), fixture.entity.v.origin.x, fixture.entity.v.origin.y,
										 fixture.entity.v.origin.z);
						assert(mode != 0);
						done = true;
						break;
					}
				...
				unsigned presses = 0;
				for (const auto& command : gNavMoves)
				...
				else if (mode == 2 || mode == 3 || mode == 4 || mode == 10)
					assert(presses == 1);
				else
					assert(presses == 0);
				gSimulateJump = gMissJump = gWrongJumpLanding = false;
				gSimulateNav = false;
				gHullMode = 0;
				gInvalidateDuringHull = false;
				detach();
			}
}
	...
	for (bool down : {false, true})
		for (std::uint64_t us : {8000U, 16000U, 100000U})
			for (unsigned fault = 0; fault < 4; ++fault)
			{
				...
				auto& owner = adapter::metamod::lifecycleCoordinator();
				auto& console = owner.navConsole();
				const auto mesh = configureLadderMotionFixture(fixture.engine, &fixture.entity,
															   owner.registry().mapGeneration(), down);
				fixture.entity.v.origin = Vector(-49, 32, down ? 164.0f : 36.0f);
				fixture.entity.v.velocity = Vector(0, 0, 0);
				fixture.entity.v.movetype = MOVETYPE_WALK;
				fixture.entity.v.friction = 1;
				assert(console.publish(owner.registry().mapGeneration(), mesh).isNone());
				std::istringstream bsp("abc");
				const adapter::cstrike::LadderWorld world{
					&fixture.engine, &owner, [](const void* context) noexcept {
						return static_cast<const adapter::metamod::LifecycleCoordinator*>(context)
							->registry()
							.mapGeneration();
					}};
				...
				for (unsigned i = 0; i < 1200; ++i)
				{
					...
					if (ladderMotionQueryCount() - queriesBefore > 21)
					{
						std::fprintf(stderr, "ladder budget actual=%u down=%d us=%llu frame=%u state=%u recorded=%u\n",
									 ladderMotionQueryCount() - queriesBefore, down,
									 static_cast<unsigned long long>(us), i,
									 unsigned(console.motionTrace().decision.ladderState.value_or(
										 nav::local::LadderState::Failed)),
									 console.motionTrace().decision.queries);
						std::exit(1);
					}
					const auto state = console.motionTrace().decision.ladderState;
					climbed = climbed ||
							  state == (down ? nav::local::LadderState::ClimbDown : nav::local::LadderState::ClimbUp);
					exited = exited || state == nav::local::LadderState::Exit;
					if (fault && !injected && (console.motionTrace().command.buttons & IN_JUMP))
					{
						injected = true;
						if (fault < 3)
							invalidateLadderMotionFixture(fault == 2);
						else
							console.invalidate(nav::runtime::SessionReason::Cancelled);
					}
					if (console.motionTrace().decision.state != nav::local::WalkState::Running)
						break;
				}
				const auto& d = console.motionTrace().decision;
				if (!fault && d.state != nav::local::WalkState::Arrived)
				{
					std::fprintf(stderr, "direction=%d us=%llu position=%.3f %.3f %.3f\n", down,
								 static_cast<unsigned long long>(us), fixture.entity.v.origin.x,
								 fixture.entity.v.origin.y, fixture.entity.v.origin.z);
					std::fprintf(stderr,
								 "host ladder not connected: walk=%u reason=%u ladder=%u bind=%u frame=%u flags=%d\n",
								 unsigned(d.state), unsigned(d.reason), unsigned(d.ladderReason),
								 unsigned(console.motionTrace().ladderBindingReason),
								 unsigned(console.motionTrace().ladderFrameReason), fixture.entity.v.flags);
					std::exit(1);
				}
				assert(climbed && exited && console.motionTrace().dispatched > 0);
				const auto jumps = std::count_if(gNavMoves.begin(), gNavMoves.end(),
												 [](const auto& c) { return (c.buttons & IN_JUMP) != 0; });
				assert(jumps == (!fault && down ? 1 : 0));
				if (fault)
				{
					assert(injected && d.state != nav::local::WalkState::Arrived);
				}
				gSimulateLadder = false;
				gSimulateNav = false;
				detach();
			}
}
void testNavWalkArrival()
{
	...
	for (std::uint64_t us : {8000U, 16000U, 100000U})
		for (const char* goal : {"1", "2", "3", "4"})
		{
			...
			auto& owner = adapter::metamod::lifecycleCoordinator();
			auto& console = owner.navConsole();
			runNav({"astrabot_goto", goal});
			assert(console.trace()->state == nav::runtime::SessionState::Ready);
			...
			for (int frame = 0; frame < 2000; ++frame)
			{
				...
				if (t.decision.state == nav::local::WalkState::Failed ||
					t.decision.state == nav::local::WalkState::Aborted)
				{
					std::fprintf(stderr, "host Walk failure goal=%s frame_us=%llu reason=%u probe=%u motion=%u\n", goal,
								 static_cast<unsigned long long>(us), unsigned(t.decision.reason),
								 unsigned(t.decision.probeReason), unsigned(t.reason));
					assert(false);
				}
				assert(t.decision.queries <= 21 && t.decision.samples <= 4 &&
					   console.motionHistoryCount() <= console.motionHistoryLimit);
				if (t.event == adapter::cstrike::MotionEvent::Dispatched)
					assert(t.dispatchTick.isAfter(t.commandTick));
				if (t.decision.state == nav::local::WalkState::Arrived)
				{
					arrived = true;
					break;
				}
			...
			assert(console.motionTrace().decision.support &&
				   console.motionTrace().decision.support->area.value == unsigned(goal[0] - '0'));
			navFrame(fixture, us);
			assert(!gNavMoves.empty() && gNavMoves.back().movement == core::Movement{});
			const auto count = gNavMoves.size();
			...
			if (goal[0] == '4' && us == 8000)
				assert(console.motionHistoryCount() == console.motionHistoryLimit);
			gSimulateNav = false;
			detach();
		}
}
void testNarrowNavGoal()
{
	...
	for (bool missing : {false, true})
	{
		...
		auto& owner = adapter::metamod::lifecycleCoordinator();
		auto& console = owner.navConsole();
		route_test::Area a{1, {{0, 0, 0}, {100, 100, 0}, 0, 0}}, b{2, {{100, 40, 0}, {120, 60, 0}, 0, 0}};
		a.targets[1] = {2};
		...
		for (int i = 0; i < 500; ++i)
		{
			...
			if (d.state == nav::local::WalkState::Failed || d.state == nav::local::WalkState::Aborted)
				break;
		}
		assert(arrived != missing);
		if (missing)
			assert(fixture.entity.v.origin.x < 100);
		gGroundMissing = false;
		gSimulateNav = false;
		detach();
	}
...
void testNavStairs()
{
	...
	for (int mode = 0; mode < 4; ++mode)
		for (std::uint64_t us : {8000U, 16000U, 100000U})
		{
			...
			auto& owner = adapter::metamod::lifecycleCoordinator();
			auto& console = owner.navConsole();
			gStairHeight = mode == 3 ? 19.0f : 16.0f;
			gStairCeiling = mode == 2;
			route_test::Area a{1, {{0, 0, 0}, {100, 100, 0}, 0, 0}},
			...
			for (int i = 0; i < 1000; ++i)
			{
				...
				navFrame(fixture, us);
				const auto& d = console.motionTrace().decision;
				stepped = stepped || d.steps > 0;
				assert(d.queries <= 21);
				if (d.tick == owner.registry().currentTick())
					assert(d.queries == static_cast<unsigned>(gHullCalls - traces));
				else
					assert(gHullCalls == traces);
				if (d.state != nav::local::WalkState::Running)
				{
					terminal = true;
					if (mode < 2)
						assert(d.state == nav::local::WalkState::Arrived && d.support &&
							   d.support->floor.height == (descending ? 0 : 16));
					else
						assert(d.state == nav::local::WalkState::Failed && fixture.entity.v.origin.x < 84);
					break;
				}
			}
			assert(terminal);
			...
			navFrame(fixture, us);
			navFrame(fixture, us);
			for (auto i = count; i < gNavMoves.size(); ++i)
				assert(gNavMoves[i].movement == core::Movement{});
			gSimulateNav = false;
			gStairHeight = 0;
			gStairCeiling = false;
...
void testNavDoors()
{
	...
	for (int mode = 0; mode < 9; ++mode)
		for (std::uint64_t us : {8000U, 16000U, 100000U})
		{
			...
			fixture.entity.v.view_ofs = Vector(0, 0, 28);
			gDoorLocked = mode == 1;
			gDoorAmbiguous = mode == 2;
			gDoorLoop = mode == 8;
			if (mode == 7)
				gNavDoor.v.spawnflags = 0; // touch-only cannot be invented as Use capability
			runNav({"astrabot_goto", "2"});
			...
			for (int frame = 0; frame < 2000; ++frame)
			{
				...
				if (d.state != nav::local::WalkState::Running)
				{
					terminal = true;
					if (mode == 0)
						assert(d.state == nav::local::WalkState::Arrived && gDoorUses == 1 && waiting);
					else
					{
						...
						if (mode == 1 || mode == 4)
						{
							...
							assert(gNavClockUs - firstWait >= 1000000 &&
								   gNavClockUs - firstWait <= 1000000 + us + 40000);
						}
					}
					break;
				}
			...
			const auto presses = gDoorUses;
			navFrame(fixture, us);
			navFrame(fixture, us);
			assert(gDoorUses == presses);
			gDoorActive = false;
			gDoorOpenAtUs = 0;
			gSimulateNav = false;
			detach();
...
void testNavTouchDoors()
{
	...
	for (int mode = 0; mode < 9; ++mode)
		for (std::uint64_t us : {8000U, 16000U, 100000U})
		{
			...
			auto& console = owner.navConsole();
			gNavDoor.v.spawnflags = 0;
			gNavDoor.v.solid = SOLID_BSP;
			gDoorLocked = mode == 1;
			if (mode == 5)
				gNavDoor.v.targetname = 12; // requires another activator; never touch on speculation
			runNav({"astrabot_goto", "2"});
			bool terminal = false, pulse = false, rejected = false;
			std::uint64_t started = 0, nextFrameUs = us;
			for (int frame = 0; frame < 2000; ++frame)
			{
				const auto traces = gHullCalls;
				const auto guards = console.motionTrace().contactGuardQueries;
				const auto actualUs = nextFrameUs;
				nextFrameUs = us;
				navFrame(fixture, actualUs);
				const auto& t = console.motionTrace();
				const auto& d = t.decision;
				rejected = rejected || motionReason(adapter::cstrike::MotionReason::DoorChanged);
				if (d.doorState == nav::local::DoorWaitState::Waiting && !started)
					started = gNavClockUs;
				...
				if (d.tick == owner.registry().currentTick())
					...
					assert(static_cast<unsigned>(gHullCalls - traces) == added);
					...
					std::hypot(t.command.movement.forward, t.command.movement.side) > 0)
				{
					...
					if (mode == 2)
						++gNavDoor.serialnumber;
					if (mode == 3)
						gNavDoor.v.targetname = 12;
					if (mode == 4)
						runNav({"astrabot_nav_cancel"});
					if (mode == 6)
						nextFrameUs = 160000; // queued contact expires; it must not be retried
					if (mode == 7)
						nextFrameUs = 1000; // actual pulse would not reach the contact plane
					if (mode == 8)
				...
				if (d.state != nav::local::WalkState::Running)
				{
					terminal = true;
					if (mode == 0)
						assert(d.state == nav::local::WalkState::Arrived && gTouchContacts == 1);
					else
					{
						assert(d.state == nav::local::WalkState::Failed || d.state == nav::local::WalkState::Aborted);
			...
			if (mode == 2 || mode == 3 || mode == 7)
				assert(rejected);
			const auto contacts = gTouchContacts;
			navFrame(fixture, us);
			navFrame(fixture, us);
			assert(gTouchContacts == contacts);
			gDoorActive = false;
			gDoorOpenAtUs = 0;
			gSimulateNav = false;
			detach();
		...
		for (std::uint64_t us : {8000U, 16000U, 100000U})
		{
			...
			for (int frame = 0; frame < 4000; ++frame)
			{
				...
				if (d.tick == owner.registry().currentTick())
					...
					assert(gHullCalls == traces);
				narrow = narrow || d.narrow;
				corrected = corrected || std::abs(d.intent.lateralCorrection) > 0.001;
				avoided = avoided || d.avoiding;
				if (d.state != nav::local::WalkState::Running)
				{
					terminal = true;
					if (mode < 2)
					{
						if (d.state != nav::local::WalkState::Arrived)
							std::fprintf(stderr,
										 "steering mode=%d us=%llu reason=%u recovery=%u projected=%g pos=(%g,%g)\n",
										 mode, static_cast<unsigned long long>(us), unsigned(d.reason),
										 unsigned(d.recovery.state), d.recovery.projected,
										 double(fixture.entity.v.origin.x), double(fixture.entity.v.origin.y));
						assert(d.state == nav::local::WalkState::Arrived);
					}
					else
						assert(d.state == nav::local::WalkState::Failed && d.intent.speed == 0);
					break;
				}
			}
			assert(terminal);
...
void testNavWalkCancellationAndGuards()
{
	...
	for (int mode = 0; mode < 15; ++mode)
	{
		...
		if (mode == 0)
		{
			...
			assert(gNavMoves.size() == count);
			navFrame(fixture);
			assert(gNavMoves.size() == count + 1 && gNavMoves.back().movement == core::Movement{});
			navFrame(fixture);
			assert(gNavMoves.size() == count + 1 && fixture.entity.v.origin.x == position.x);
		...
		else if (mode == 1)
		{
			const auto generation = console.trace()->routeGeneration;
			runNav({"astrabot_goto", "1"});
			assert(console.trace()->routeGeneration == generation + 1);
			navFrame(fixture);
			assert(gNavMoves.size() == count);
			navFrame(fixture);
		...
		{
			gInvalidateDuringHull = true;
			for (int i = 0; i < 6 && console.trace(); ++i)
				navFrame(fixture);
			assert(!console.trace());
			gInvalidateDuringHull = false;
			const auto before = gNavMoves.size();
			navFrame(fixture);
			navFrame(fixture);
			for (auto i = before; i < gNavMoves.size(); ++i)
				assert(gNavMoves[i].movement == core::Movement{});
		}
		else if (mode == 9)
		{
			owner.serverDeactivate();
			navFrame(fixture);
			assert(gNavMoves.size() == count && !console.trace());
		}
		else if (mode == 10)
		{
			owner.clientDisconnect(&fixture.entity);
			navFrame(fixture);
			assert(gNavMoves.size() == count && !console.trace());
		}
		else if (mode == 11)
		{
			gInjectNavDuplicate = true;
			navFrame(fixture);
			...
			navFrame(fixture);
			assert(gNavMoves.back().movement == core::Movement{});
		}
		else if (mode == 12)
		{
			const auto hulls = gHullCalls;
			navFrame(fixture, 120000);
		...
		else if (mode == 13)
		{
			...
			navFrame(fixture);
			assert(gNavMoves.size() == count + 1);
		}
		else
		{
			const auto generation = console.trace()->routeGeneration;
			gReplaceDuringMove = true;
			navFrame(fixture);
			...
			assert(console.motionHistoryCount() == 1 &&
				   console.motionHistory(0)->event == adapter::cstrike::MotionEvent::Dispatched);
...
void testMessageDrivenJoinAndCommandContext()
{
	...
	fixture.engineGlobals.maxEntities = 3;
	activate(fixture);
	astrabot::adapter::metamod::lifecycleCoordinator().setJoinTraceSink(&captureJoinTrace);
	gJoinTraces.clear();
	enginefuncs_t hooks{};
	...
	const FakeClientResult created =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-Join");
	assert(created.succeeded());
	assert(astrabot::adapter::metamod::lifecycleCoordinator()
			   .requestJoin({astrabot::adapter::cstrike::Team::Terrorist, 1})
			   .changed);
	...
	const std::string bspBytes(123, 'x'); // Matches this synthetic NAV's BSP size.
	{
		...
		assert(file.rdbuf()->sputn(bspBytes.data(), static_cast<std::streamsize>(bspBytes.size())) ==
...
void testJoinFailureCleanupAndCommandContextReentry()
{
	...
	const FakeClientResult created =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-Failure");
	assert(created.succeeded());
	...
	const FakeClientResult fallbackCreated =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-Fallback");
	assert(fallbackCreated.succeeded());
	fallbackFixture.userId = 0;
	assert(astrabot::adapter::metamod::lifecycleCoordinator()
			   .requestJoin({astrabot::adapter::cstrike::Team::Terrorist, 1})
			   .changed);
	sendVguiMenu(hooks, 11, &fallbackFixture.entity, 2, 0x0001);
	astrabot::adapter::metamod::lifecycleCoordinator().startFrame();
	sendVguiMenu(hooks, 11, &fallbackFixture.entity, 26, 0x0000);
...
void testCounterTerroristPrimaryJoinRequest()
{
	...
	assert(astrabot::adapter::metamod::lifecycleCoordinator().joinState().request().classNumber == 2U);
...
void testJoinFallbackRequiresGameDllProgress()
{
	...
	gRemovalTraces.clear();
	const FakeClientResult created = coordinator.fakeClient().create("AstraBot-Timeout");
	assert(created.succeeded());
	assert(coordinator.requestJoin({astrabot::adapter::cstrike::Team::Terrorist, 1}).changed);
	for (int frame = 0; frame < 128; ++frame)
	{
		coordinator.startFrame();
	}
	assert(coordinator.joinState().phase() == astrabot::adapter::cstrike::JoinPhase::Failed);
	assert(coordinator.joinState().error() == astrabot::adapter::cstrike::JoinError::Timeout);
	assert(fixture.serverCommandCalls == 1);
	assert(fixture.serverExecuteCalls == 1);
...
void testJoinMenuEmittedDuringPutInServer()
{
	...
	const FakeClientResult created = coordinator.fakeClient().create("AstraBot-PutInServer-Menu");
	assert(created.succeeded());
	const auto started = coordinator.requestJoin({astrabot::adapter::cstrike::Team::Terrorist, 1});
	assert(started.changed);
	assert(coordinator.joinState().pendingSelection());
	assert(coordinator.joinState().phase() == astrabot::adapter::cstrike::JoinPhase::WaitingTeamMenu);
	...
	assert(coordinator.joinState().phase() == astrabot::adapter::cstrike::JoinPhase::Joined);
	coordinator.startFrame();
...
void testJoinFallbackTimeoutCleansUpWithoutGameDllProgress()
{
	...
	coordinator.setRemovalTraceSink(&captureRemovalTrace);
	gRemovalTraces.clear();
	const FakeClientResult created = coordinator.fakeClient().create("AstraBot-Timeout-Fallback");
	assert(created.succeeded());
	assert(coordinator.requestJoin({astrabot::adapter::cstrike::Team::Terrorist, 1}).changed);
	for (int frame = 0; frame < 128; ++frame)
	{
...
void testExplicitRemovalKickAndDisconnectAcknowledge()
{
	...
	activate(fixture);
	const FakeClientResult created =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-Remove");
	assert(created.succeeded());
...
void testRemovalCancelsJoinBeforeKick()
{
	...
	gGameDllCommandCalls = 0;
	const FakeClientResult created =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-JoiningRemove");
	assert(created.succeeded());
	...
	assert(result.succeeded());
	assert(coordinator.joinState().phase() == astrabot::adapter::cstrike::JoinPhase::Cancelled);
	assert(coordinator.fakeClient().removalPending());
	assert(coordinator.fakeClient().activeEntity() != nullptr);
...
void testRemovalFallbackCleansInvalidUserId()
{
	...
	activate(fixture);
	const FakeClientResult created =
		astrabot::adapter::metamod::lifecycleCoordinator().fakeClient().create("AstraBot-FallbackRemove");
	assert(created.succeeded());
	...
	assert(coordinator.fakeClient().activeEntity() == nullptr);
	assert(gRemovalTraces.size() == 2);
	assert(coordinator.status().lastRemovalError == astrabot::debug::RemovalError::InvalidUserId);
	detach();
}
...
void testMapDeactivateReplaysPrimaryCreate()
{
	...
	coordinator.startFrame();
	const astrabot::core::MapGeneration firstMap = coordinator.registry().mapGeneration();
	const astrabot::core::PlayerId firstPlayer = coordinator.fakeClient().activePlayer();
	...
	assert(coordinator.fakeClient().activeEntity() == nullptr);
	assert(coordinator.agents().mappingCount() == 0);
	assert(coordinator.joinState().phase() == astrabot::adapter::cstrike::JoinPhase::Idle);

	coordinator.serverActivate(32);
	coordinator.startFrame();
	assert(coordinator.registry().mapGeneration() != firstMap);
	assert(coordinator.fakeClient().activePlayer().isValid());
	assert(coordinator.fakeClient().activePlayer().generation != firstPlayer.generation);
...
void testMapReplayPreservesExplicitCounterTerroristRequest()
{
	...
	attach(fixture);
	auto& coordinator = astrabot::adapter::metamod::lifecycleCoordinator();
	coordinator.queuePrimaryCreate({astrabot::adapter::cstrike::Team::CounterTerrorist, 3});

	coordinator.serverActivate(32);
	coordinator.startFrame();
	assert(coordinator.joinState().request().team == astrabot::adapter::cstrike::Team::CounterTerrorist);
...
void testExternalDisconnectResetsJoinedState()
{
	...
	activate(fixture);
	auto& coordinator = astrabot::adapter::metamod::lifecycleCoordinator();
	const FakeClientResult created = coordinator.fakeClient().create("AstraBot-DisconnectJoined");
	assert(created.succeeded());
	assert(coordinator.requestJoin({astrabot::adapter::cstrike::Team::Terrorist, 1}).changed);
	enginefuncs_t hooks{};
	int engineVersion = ENGINE_INTERFACE_VERSION;
...
void testRemovalStopsPendingMovement()
{
	...
	const FakeClientResult created = coordinator.fakeClient().create("AstraBot-MovementRemove");
	assert(created.succeeded());
	assert(coordinator.requestJoin({astrabot::adapter::cstrike::Team::Terrorist, 1}).changed);
	sendVguiMenu(hooks, 11, &fixture.entity, 2, 0x0001);
	coordinator.startFrame();
	...
	coordinator.startFrame();
	astrabot::core::BotCommand command = astrabot::core::BotCommand::neutral(100);
	command.movement.forward = 100.0F;
	const auto queued = coordinator.submitCommand(created.player, coordinator.registry().mapGeneration(),
												  coordinator.registry().currentTick(), command);
...
void testRepeatedCreateAfterDisconnectUpdatesGeneration()
{
	...
	activate(fixture);
	auto& coordinator = astrabot::adapter::metamod::lifecycleCoordinator();
	const FakeClientResult first = coordinator.fakeClient().create("AstraBot-Reuse-1");
	assert(first.succeeded());
	coordinator.clientDisconnect(&fixture.entity);
	const FakeClientResult second = coordinator.fakeClient().create("AstraBot-Reuse-2");
	assert(second.succeeded());
...
void testDetachDirectlyCleansActiveEntityOnce()
{
	...
	activate(fixture);
	auto& coordinator = astrabot::adapter::metamod::lifecycleCoordinator();
	const FakeClientResult created = coordinator.fakeClient().create("AstraBot-Detach");
	assert(created.succeeded());
	assert(Meta_Detach(PT_ANYTIME, PNL_COMMAND) != 0);
	assert(fixture.disconnectCalls == 1);
	assert(fixture.removeCalls == 1);
...
#include "p12_execution_tests.hpp"
#include "p12_map_nav_tests.hpp"
...
#endif
	if (argc == 3 && std::strcmp(argv[1], "--p308-output") == 0)
		return runP308HostMatrix(argv[2]);
	if (argc == 2 && std::strcmp(argv[1], "--p401-tests") == 0)
	{
		p401::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--runtime-input-tests") == 0)
	{
		runtime_input_test::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p12-execution-tests") == 0)
	{
		p12_execution_test::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p12-map-nav-tests") == 0)
	{
		p12_map_nav_test::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p402-tests") == 0)
	{
		p402::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p403-tests") == 0)
	{
		p403::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p404-tests") == 0)
	{
		p404::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p405-tests") == 0)
	{
		p405::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p406-tests") == 0)
	{
		p406::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p407-tests") == 0)
	{
		p407::run();
		return 0;
	}
	if (argc == 2 && std::strcmp(argv[1], "--p408-tests") == 0)
	{
		p408::run();
		return 0;
	}
	if (argc == 3 && std::strcmp(argv[1], "--p409-output") == 0)
	{
		p409::run(argv[2]);
		return 0;
	}
