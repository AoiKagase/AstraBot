...
void captureRunPlayerMove(edict_t* entity, const float* angles, float forward, float side, float up,
...
{
	return std::chrono::steady_clock::time_point(std::chrono::microseconds(gClockUs));
}

void captureMovementTrace(const astrabot::debug::MovementTrace& trace) noexcept
{
...
struct Fixture final
{
	...
	BotCommand command() const
	{
		...
		value.buttons = static_cast<astrabot::core::ButtonMask>(Button::Forward | Button::Jump);
		value.impulse = 4;
		return value;
	}
	...
	astrabot::adapter::metamod::MovementResult dispatch(TickId tick = TickId{2})
	{
		return movement.dispatchAtFrameEnd(JoinPhase::Joined, player, &entity, map, tick);
	}
};
...
void testJoinedActorReceivesNeutralHeartbeat()
{
	...
	assert(gCalls.front().forward == 0.0F && gCalls.front().side == 0.0F && gCalls.front().up == 0.0F &&
		   gCalls.front().buttons == 0U && gCalls.front().impulse == 0U && gCalls.front().msec == 10U);
	assert(gTraces.back().source == astrabot::debug::MovementTraceSource::Idle);
	assert(gTraces.back().physical.valid);
	assert(gTraces.back().physical.beforeOriginX == gTraces.back().physical.afterOriginX);
	assert(gTraces.back().physical.beforeOriginY == gTraces.back().physical.afterOriginY);
...
void testRemovalPendingSuppressesMovement()
{
	...
	const auto queued = fixture.movement.dispatchAtFrameEnd(JoinPhase::Joined, fixture.player, &fixture.entity,
															fixture.map, TickId{2}, true);
	assert(queued.rejected() && queued.error == MovementError::NotJoined);
	assert(gCalls.empty());

	const auto idle = fixture.movement.dispatchAtFrameEnd(JoinPhase::Joined, fixture.player, &fixture.entity,
														  fixture.map, TickId{3}, true);
...
void testManagedBindingAndEdictIdentityRejectWithoutFallback()
{
	...
	movement.beginFrame();
	const auto unbound =
		movement.dispatchAtFrameEnd(JoinPhase::Joined, fixture.player, &fixture.entity, fixture.map, TickId{2});
	...
	gClockUs += 10000U;
	movement.beginFrame();
	const auto queued = movement.submit(fixture.player, fixture.map, fixture.registry.currentTick(), fixture.command());
...
void testMsecQuantizationAndAbiConversion()
{
	...
	const auto result =
		fixture.movement.submit(fixture.player, fixture.map, fixture.registry.currentTick(), fixture.command());
	...
	assert(call.up == 8.0F);
	assert(call.buttons ==
		   static_cast<unsigned short>(static_cast<astrabot::core::ButtonMask>(Button::Forward | Button::Jump)));
	...
	assert(gTraces[1].physical.beforeOriginX == gTraces[1].physical.afterOriginX);
	assert(gTraces[1].physical.beforeVelocityX == gTraces[1].physical.afterVelocityX);
	...
	assert(gTraces[1].buttons ==
		   static_cast<std::uint16_t>(static_cast<astrabot::core::ButtonMask>(Button::Forward | Button::Jump)));
...
void testLongIdleDispatchPulsesViewActivity()
{
	...
	assert(fixture.movement.submit(fixture.player, fixture.map, fixture.registry.currentTick(), command).queued());
...
void testForgetClearsFrameTraceCaches()
{
	Fixture fixture{};
	fixture.armAndAdvance(16500U);
	assert(fixture.movement.submit(fixture.player, fixture.map, fixture.registry.currentTick(), fixture.command())
			   .queued());
	assert(fixture.dispatch().dispatched());
	assert(fixture.movement.frameQueueTrace(fixture.player).outcome == MovementOutcome::Queued);
	assert(fixture.movement.frameDispatchTrace(fixture.player).outcome == MovementOutcome::Dispatched);

	fixture.movement.forget(fixture.player);
	assert(fixture.movement.frameQueueTrace(fixture.player).outcome == MovementOutcome::None);
	assert(fixture.movement.frameDispatchTrace(fixture.player).outcome == MovementOutcome::None);
	assert(fixture.movement.frameRejectionTrace(fixture.player).outcome == MovementOutcome::None);
}
...
void testOneCallAndPendingClear()
{
	...
	assert(gCalls.size() == 1);
	assert(fixture.dispatch(TickId{3}).outcome == MovementOutcome::None);
	const auto duplicate = fixture.movement.submit(fixture.player, fixture.map, tick, fixture.command());
...
void testExactPendingCancellation()
{
	...
	auto reused = f.player;
	++reused.generation.value;
...
void testSubmissionRejectionAndGenerationChecks()
{
	...
	invalid.msec = 0;
	assert(fixture.movement.submit(fixture.player, fixture.map, tick, invalid).rejected());
	assert(gTraces.back().error == MovementError::RegistryRejected);
	assert(fixture.movement.submit(fixture.player, fixture.map, TickId{2}, valid).rejected());
	assert(fixture.movement.submit(fixture.player, MapGeneration{99}, tick, valid).rejected());

	const PlayerId stale{fixture.player.slot, astrabot::core::Generation{fixture.player.generation.value + 1U}};
...
void testDispatchGuardsAndCleanup()
{
	...
	assert(fixture.movement.submit(fixture.player, fixture.map, nextTick, fixture.command()).queued());
	...
	assert(disconnected.movement.submit(disconnected.player, disconnected.map, disconnectedTick, disconnected.command())
			   .queued());
	...
	assert(inactive.movement.submit(inactive.player, inactive.map, inactiveTick, inactive.command()).queued());
...
void testSuppressedDeadActorStillReceivesNeutralHeartbeat()
{
	...
	const auto result = fixture.movement.dispatchAtFrameEnd(JoinPhase::Joined, fixture.player, &fixture.entity,
															fixture.map, TickId{2}, false, true);
...
{
	Fixture fixture{};
	fixture.armAndAdvance(16000);
	const auto second = fixture.registry.registerPlayer(2).event.player;
	edict_t other{};
	auto command = fixture.command();
	command.movement.forward = 25;
	const auto tick = fixture.registry.currentTick();
	assert(fixture.movement.submit(second, fixture.map, tick, command).queued());
	assert(fixture.movement.submit(fixture.player, fixture.map, tick, fixture.command()).queued());
	assert(fixture.movement
			   .dispatchAtFrameEnd(JoinPhase::WaitingTeamMenu, fixture.player, &fixture.entity, fixture.map, {2})
			   .error == MovementError::NotJoined);
	assert(gCalls.empty());
