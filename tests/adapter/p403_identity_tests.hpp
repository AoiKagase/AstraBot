...
void roundEvent(enginefuncs_t& hooks, int destination = MSG_SPEC, int first = 0, int second = 0,
				edict_t* recipient = nullptr)
{
	hooks.pfnMessageBegin(destination, 14, nullptr, recipient);
	hooks.pfnWriteByte(first);
	hooks.pfnWriteByte(second);
	hooks.pfnMessageEnd();
}
void reentrantRound()
{
	p401::extraTrace = nullptr;
	roundEvent(*gEngineHooks);
}
void reentrantTeam()
{
	p401::extraTrace = nullptr;
	sendTeamInfo(*gEngineHooks, 13, 32, "SPECTATOR");
}
void teamsAndRounds()
{
	...
	step(fixture, 0.001f);
	assert(latest()->memories[0].identity.sequence == sequence);
	extraTrace = &reentrantRound;
	step(fixture, 0.2f);
	...
	hooks.pfnWriteByte(32);
	++human.serialnumber;
	hooks.pfnWriteString("TERRORIST");
	hooks.pfnMessageEnd();
	...
	hooks.pfnMessageBegin(MSG_ONE, 15, nullptr, &fixture.entity);
	hooks.pfnMessageEnd(); // Unregistered ResetHUD-like notification.
	...
	assert(!owner.teams().find(observer) && !latest());
	detach();
}
void run()
{
