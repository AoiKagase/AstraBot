...
a::WeaponPickupCandidate dropped(std::uint32_t reference, c::WeaponSnapshot::WeaponClass weaponClass)
{
...
void testWeaponAcquisitionUtilityAndLifetime()
{
	...
	assert(a::evaluateWeaponPickup(state, stale).failure == a::WeaponAcquisitionFailure::StaleObservation);
	...
	assert(a::evaluateWeaponPickup(state, stale).failure == a::WeaponAcquisitionFailure::InvalidObservation);
	...
	assert(a::evaluateWeaponPickup(urgent, urgent.weapons[0]).failure == a::WeaponAcquisitionFailure::ObjectiveUrgent);
}
