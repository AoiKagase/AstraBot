...
int main()
{
	...
	assert(a::soundSample("weapons/unknown.wav") == p::SoundKind::Unknown &&
		   a::soundSample(nullptr) == p::SoundKind::Unknown);
	...
	assert(catalog.bind(5, "events/createsmoke.sc") == a::EventBinding::Registered &&
		   catalog.find(5) == a::EventKind::Smoke);
	...
	assert(catalog.find(413) == a::EventKind::Unknown);
	assert(!catalog.supports(a::EventKind::Smoke));
	for (std::uint16_t i = 1; i <= a::EventCatalog::capacity; ++i)
		assert(catalog.bind(i, "events/ak47.sc") == a::EventBinding::Registered);
