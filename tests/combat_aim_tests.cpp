...
void addVisual(Fixture& fixture, p::Point position, double confidence = 1.0, std::uint64_t observed = now,
			   std::uint64_t sequence = 1)
{
	fixture.visual.memories[0] = {target, position, observed, confidence, visionIdentity(observed, observed, sequence)};
	fixture.visual.count = 1;
}

void addReport(Fixture& fixture, p::Point position, double confidence = 0.5, std::uint64_t observed = now,
			   std::uint64_t sequence = 1)
{
	w::TeamReport report{};
