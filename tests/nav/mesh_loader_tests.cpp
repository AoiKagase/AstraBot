		 ...
		 diagnostics::NavField field)
{
	...
	if (r.error.kind != k || r.error.offset != offset || r.error.field != field)
		std::fprintf(stderr, "expected %u/%llu/%u, got %u/%llu/%u\n", unsigned(k),
					 static_cast<unsigned long long>(offset), unsigned(field), unsigned(r.error.kind),
					 static_cast<unsigned long long>(r.error.offset), unsigned(r.error.field));
...
void traversalDecodeAndBudget()
{
	...
	for (unsigned version = 1; version <= 5; ++version)
	{
		...
		auto l = limits();
		l.maxSnapshotBytes =
			sizeof(model::NavMeshSnapshot) + 2 * sizeof(model::NavAreaRecord) + 4 * sizeof(model::NavConnection);
		if (version == 5)
	...
	for (unsigned version = 1; version <= 5; ++version)
	{
		...
		if (!result)
		{
			std::fprintf(stderr, "v%u zero identity/reference rejected: kind=%u field=%u offset=%llu\n", version,
						 unsigned(result.error.kind), unsigned(result.error.field),
		...
		if (version >= 2)
			assert(*area.hidingSpots.front().id == 0);
		...
		if (version >= 3)
			assert(area.encounters.front().spots.front().hidingSpotId == 0);
		for (const auto& span : fixture.spans)
		{
...
void tacticalAndConnections()
{
	...
	for (auto o : {84U, 88U, 93U, 102U, 107U})
	{
		...
		if (o < 100)
			...
			bad(b, K::InvalidValue, o, F::EncounterAreaId);
	}
	b = original;
	set(b, 113, 999);
	...
	auto l = limits();
	l.maxSnapshotBytes = sizeof(model::NavMeshSnapshot) + sizeof(model::NavAreaRecord) + sizeof(model::NavHidingSpot) +
						 sizeof(model::NavApproachRecord) + sizeof(model::NavEncounterRecord) +
						 sizeof(model::NavEncounterSpot);
...
void legacyAndLimits()
{
	...
	l = limits();
	l.maxSnapshotBytes = sizeof(model::NavMeshSnapshot) + sizeof(model::NavAreaRecord) + sizeof(std::string) + 2;
	assert(load(b, l));
	--l.maxSnapshotBytes;
	assert(load(b, l).error.kind == K::CountLimitExceeded);
