...
namespace
{
constexpr local::WalkLimits limits{
	{21, 4, 48, 16, 18, 18, 64, 4, 2, 0.7}, 160, 1, 1, 3, 0, 0, 12, 8, 5, 25, {120000, 400000, 3000000}};
struct World final : runtime::IWorldQueries
{
	...
	runtime::HullObservation sweep(model::NavVector3 a, model::NavVector3 b) const
	{
		...
		float shift = 0;
		if (mode == 10 || mode == 11)
			shift = static_cast<float>((std::min)(10.0, double(now) / 1000000 * 5)) * (mode == 10 ? 1.0f : -1.0f);
		a.x -= shift;
		b.x -= shift;
	...
	{
		if (q.stamp.tick.value != lastTick)
		{
			lastTick = q.stamp.tick.value;
			ordinal = mode == 6 ? 2U : 0U;
		}
		assert(q.stamp.ordinal == ++ordinal);
		++calls;
		...
		else if (q.kind == runtime::QueryKind::Blocker)
		{
			...
			if (mode == 3)
				r.blocker = runtime::BlockerObservation{123, runtime::BlockerKind::Other, {}};
			if (mode == 4)
			{
				r.blocker.reset();
			...
			{
				r.blocker->id = now;
				r.blocker->player->generation.value = static_cast<std::uint32_t>(now);
			}
		}
		else
			assert(false);
...
std::vector<unsigned> scenario(int mode, std::uint64_t frameUs)
{
	...
	assert(index);
	const auto graph = query::NavGraph::build(mesh, {2, 1, 1000000});
	assert(graph);
	...
	s.view = model::NavVector3{0, 90, 0};
	s.hull = runtime::HullDimensions{{-16, -16, -36}, {16, 16, 36}};
	s.speedLimit = 250.0f;
	s.elapsedUs = frameUs;
	auto profile = limits;
	if (mode == 8)
		profile.probe.maxQueries = 5;
	if (mode == 9)
		profile.blocker.yieldUs = profile.blocker.timeoutUs;
	World world(mode);
	local::Walk walk(bind, path.value, {150, 50, 0}, profile);
	local::IntentPump pump(bind);
	std::optional<core::BotCommand> pending;
	bool finished = false, yielded = false, avoided = false, cleared = false;
	std::vector<unsigned> trace;
	for (std::uint64_t tick = 1; tick < 3000; ++tick)
	{
		s.tick = {tick};
		world.now = tick * frameUs;
		if (pending)
		{
			assert(pending->buttons == 0);
			const auto old = *s.position;
			const double yaw = pending->view.yaw * 3.14159265358979323846 / 180, dt = double(pending->msec) / 1000;
			s.position->x += static_cast<float>(
				(pending->movement.forward * std::cos(yaw) + pending->movement.side * std::sin(yaw)) * dt);
			s.position->y += static_cast<float>(
				(pending->movement.forward * std::sin(yaw) - pending->movement.side * std::cos(yaw)) * dt);
		...
		if (schedule.decisionDue)
		{
			...
			assert(d.queries == world.calls - before + reserved && d.queries <= profile.probe.maxQueries &&
				   d.samples <= 4);
			...
			cleared = cleared || d.blockerAction == local::BlockerAction::ReinspectPassage;
			if (d.blockerAction == local::BlockerAction::Yield ||
				d.blockerAction == local::BlockerAction::ReinspectPassage)
				...
				if (mode == 0 || mode == 2 || mode == 3 || mode == 6 || mode == 10 || mode == 11)
				{
					if (d.state != local::WalkState::Arrived)
						std::fprintf(stderr, "reactive mode=%d us=%llu reason=%u probe=%u action=%u blocker=%u\n", mode,
									 static_cast<unsigned long long>(frameUs), unsigned(d.reason),
									 unsigned(d.probeReason), unsigned(d.blockerAction), unsigned(d.blockerReason));
					assert(d.state == local::WalkState::Arrived);
				}
				else
					assert(d.state == local::WalkState::Failed && d.intent.speed == 0);
				if (mode == 1 || mode == 7)
				{
					assert(d.reason == local::WalkReason::DynamicBlocked &&
						   d.blockerReason == local::BlockerReason::TimedOut);
		...
		if (output.emit)
		{
			...
			assert(motor);
			pending = motor.command;
		}
	}
	assert(finished);
	if (mode == 0 || mode == 2 || mode == 6 || mode == 10 || mode == 11)
		assert(yielded && cleared);
	if (mode == 0 || mode == 3 || mode == 6 || mode == 10 || mode == 11)
		assert(avoided && cleared);
	// Navigation and selected static route remain immutable across dynamic observations.
	const auto again = query::NavRouteSearch::search(**graph.value, {{1}, {2}, {2, 1000000}, false});
	assert(again);
...
int main()
{
	for (std::uint64_t us : {8000U, 16000U, 100000U})
		for (int mode = 0; mode < 12; ++mode)
			assert(scenario(mode, us) == scenario(mode, us));
}
