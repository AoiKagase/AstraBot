...
{
	if (!yes)
		throw std::runtime_error("route session check failed");
}
...
void consoleValues()
{
	...
	result->status = query::NavRouteStatus::Complete;
	result->total = 1.25;
	result->steps.resize(70);
	runtime::DecisionTrace trace;
	trace.route = result;
	trace.state = runtime::SessionState::Ready;
	std::vector<std::string> first, second;
	debug::printNavTrace(trace, &collect, &first);
	debug::printNavTrace(trace, &collect, &second);
...
struct FakeQueries final : IWorldQueries
{
	...
	WorldQueryResult query(const QueryRequest& request) override
	{
		...
		if (stale)
			++result.stamp.routeGeneration;
		switch (staleField)
		{
		case 1:
			result.stamp.agent = {99};
			break;
		case 2:
			result.stamp.actor.generation = {99};
			break;
		case 3:
			result.stamp.map = {99};
			break;
		case 4:
			result.stamp.tick = {99};
			break;
		case 5:
			++result.stamp.ordinal;
			break;
		case 6:
			result.kind = QueryKind::Floor;
			break;
		default:
			break;
		}
		if (!unknown)
			result.ground = GroundedAreaObservation{current, FloorObservation{0, {0, 0, 1}, true}};
		return result;
	}
};
MovementSnapshot snapshot()
{
	...
	s.joined = true;
	s.grounded = true;
	s.position = model::NavVector3{1, 1, 0};
	return s;
}
...
void externalOwnership()
{
	...
	options.limits = {3, 1000000};
	std::weak_ptr<const model::NavMeshSnapshot> weak;
	{
		auto mesh = route_test::snapshot({{1, {{0, 0, 0}, {2, 2, 0}, 0, 0}}, {2, {{0, 0, 10}, {2, 2, 10}, 10, 10}}});
		weak = mesh;
...
void groundTolerance()
{
	...
	for (double tolerance : {-1.0, std::numeric_limits<double>::infinity(), std::numeric_limits<double>::quiet_NaN()})
	{
		...
		check(session.request(s, {2}, graph(), port, options).reason == SessionReason::InvalidSnapshot &&
			  port.calls == calls);
	}
}
...
int main()
{
	try
	{
		...
		check(update.reason == SessionReason::ExpansionLimit && !session.executable() &&
			  session.trace().route->areas.empty());
		options.diagnosticPartial = true;
		update = session.request(s, {3}, g, port, options);
		check(update.reason == SessionReason::ExpansionLimit && !session.executable() &&
			  !session.trace().route->areas.empty());
		options.limits = {3, 1000000};
		options.diagnosticPartial = false;
		options.maxWorldQueries = 0;
		const auto calls = port.calls;
		update = session.request(s, {3}, g, port, options);
		...
		unknown = s;
		unknown.position->x = std::numeric_limits<float>::quiet_NaN();
		check(session.request(unknown, {3}, g, port, options).reason == SessionReason::InvalidSnapshot);
		...
		s.tick = {5};
		check(session.request(s, {3}, g, port, options).reason == SessionReason::MapChanged);
		session = RouteSession(s.agent, s.actor, s.map);
		check(session.request(s, {3}, g, port, options).accepted);
		next = s;
		next.tick = {6};
		next.actor.generation = {2};
		check(session.observe(next).reason == SessionReason::ActorChanged);
		s.tick = {7};
		check(session.request(s, {3}, g, port, options).reason == SessionReason::ActorChanged);
		session = RouteSession(s.agent, s.actor, s.map);
		check(session.request(s, {3}, g, port, options).accepted);
		next = s;
		next.tick = {8};
		...
		other.agent = {2};
		other.actor = {2, {1}};
		RouteSession second(other.agent, other.actor, other.map);
		FakeQueries otherPort;
