...
struct Fixture
{
	...
	explicit Fixture(bool south = false, bool narrowTarget = false)
	{
		...
		if (south)
			b.extent = {{0, 125, -75}, {100, 325, -75}, -75, -75};
		if (narrowTarget)
		{
			b.extent.northWest.y = 37.5F;
			b.extent.southEast.y = 62.5F;
		}
		...
		const auto spatial = query::NavSpatialIndex::build(mesh, {2, 3, 1000000});
		assert(spatial);
		index = *spatial.value;
	}
};
WalkLimits limits()
{
	WalkLimits l{{21, 4, 48, 16, 18, 18, 128, 4, 2, 0.7}, 120, 1, 1, 1};
	l.drop = DropLimits{};
	return l;
}
runtime::MovementSnapshot actor()
{
	...
	s.ducked = false;
	s.position = model::NavVector3{96, 50, 36};
...
struct World final : runtime::IWorldQueries
{
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		...
		if (q.kind == runtime::QueryKind::GroundedArea)
		{
			...
			r.clearance = runtime::ClearanceObservation{!blocked};
		else
		{
...
JumpPhysics physics(const runtime::MovementSnapshot& s)
{
	return {binding, s.tick, 800, 0};
}
void observedLifecycle()
{
	...
	assert(core::Motor::requestedSpeed(start.intent) > 0 && start.intent.jump == ActionRequest::None &&
		   walk.step() == 0);
	++s.tick.value;
	s.grounded = false;
	s.elapsedUs = 200000;
	s.position = model::NavVector3{117 + s.velocity->x * 0.2F, 50, 20};
	s.velocity->z = -160;
	...
	++s.tick.value;
	s.grounded = true;
	s.position = model::NavVector3{145, 50, -39};
	s.velocity->z = 0;
...
void narrowTargetRequiresPhysicalSupport()
{
	...
	for (int mode = 0; mode < 3; ++mode)
	{
		auto s = actor();
		World world(*f.index);
		world.blocked = mode == 1;
		world.missing = mode == 2;
		Walk walk(binding, f.path, {175, 50, -75}, limits());
...
void rejectsUnprovenMotion()
{
	...
	for (int mode = 0; mode < 7; ++mode)
	{
		...
		if (mode == 4)
			world.missing = true;
		if (mode == 5)
			l.drop->maximumFall = 64;
		if (mode == 6)
			l.drop->maximumGap = 24;
...
void exactMicroSourceDrop()
{
	...
	const auto graph = query::NavGraph::build(mesh, {2, 1, 1000000});
	assert(graph);
	const auto route = query::NavRouteSearch::search(**graph.value, {{2036}, {141}, {2, 1000000}, false});
	assert(route);
	const auto built = corridor::Corridor::build(**graph.value, *route.value, {16, 16}, {1, 1000000, 1},
												 corridor::PortalPolicy::AllowMicroTransit);
	assert(built);
	...
	{
		World world(**spatial.value);
		world.microSource = true;
		world.missingSource = unsupported;
		auto s = actor();
		s.position = model::NavVector3{-162.5F, -1429, 36};
...
{
	Fixture south(true);
	assert(south.path->transitions()[0].edge.direction == 2);
	observedLifecycle();
	rejectsUnprovenMotion();
	landingRequiresObservedFlight();
	exactMicroSourceDrop();
	narrowTargetRequiresPhysicalSupport();
}
