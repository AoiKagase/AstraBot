...
struct Fixture
{
	...
	Fixture(const std::vector<route_test::Area>& areas, std::uint32_t start, std::uint32_t goal)
	{
		...
		auto g = query::NavGraph::build(mesh, {10, 20, 1000000});
		assert(g);
		graph = *g.value;
		auto i = query::NavSpatialIndex::build(mesh, {10, 19, 1000000});
		assert(i);
		index = *i.value;
		auto route = query::NavRouteSearch::search(*graph, {{start}, {goal}, {10, 1000000}, false});
		assert(route);
		auto c = corridor::Corridor::build(*graph, *route.value, {16, 16}, {10, 1000000, 20});
		assert(c);
		corridor = c.value;
	}
};
struct World final : runtime::IWorldQueries
{
	...
	std::optional<model::NavAreaId> area(model::NavVector3 p) const
	{
		for (const auto& a : areas)
			if (p.x >= a.extent.northWest.x && p.x <= a.extent.southEast.x && p.y >= a.extent.northWest.y &&
				p.y <= a.extent.southEast.y)
				return model::NavAreaId{a.id};
		return {};
	}
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		assert(q.stamp.ordinal ==
			   (calls.empty() || calls.back().stamp.tick != q.stamp.tick ? 1 : calls.back().stamp.ordinal + 1));
		calls.push_back(q);
		...
		if (mode == 5)
			++r.stamp.routeGeneration;
		if (mode == 6)
			throw std::runtime_error("world unavailable");
		...
		if (q.kind == runtime::QueryKind::GroundedArea)
		{
			...
			if (mode == 2)
				r.ground->floor->supported = false;
		}
		else if (q.kind == runtime::QueryKind::Floor)
		{
			r.floor = floor;
			if (!area(q.start) || mode == 3)
				r.floor->supported = false;
			if (mode == 4)
				r.floor->height = -32;
		}
...
{
	std::size_t step;
	local::WalkState state;
	local::PrimitiveEvent event;
	model::NavVector3 position;
};
struct DoorWorld final : runtime::IWorldQueries
{
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		if (q.kind == runtime::QueryKind::Door)
		{
			...
			if (throws)
				throw std::runtime_error("door unavailable");
			runtime::WorldQueryResult r;
			r.stamp = q.stamp;
			r.kind = q.kind;
			r.error = runtime::QueryError::None;
			...
			{
				r.door->canUse = false;
				r.door->canTouch = true;
				r.door->useView.reset();
				r.hull = runtime::HullObservation{(83.96875f - q.start.x) / (q.end.x - q.start.x),
												  {83.96875f, q.start.y, q.start.z},
												  {-1, 0, 0},
												  false};
			}
			if (stale)
		...
		if (!open && missingFloor && q.start.x >= 84 && q.kind == runtime::QueryKind::Floor)
			r.floor.reset();
		return r;
	}
};
void doors()
{
	...
	{
		DoorWorld world(areas);
		auto s = actor();
		auto profile = limits;
		profile.doorTimeoutUs = 100000;
		world.missingFloor = mode == 1;
		world.stale = mode == 4;
		world.throws = mode == 5;
		if (mode == 6)
			profile.probe.maxQueries = 7;
		...
		if (mode == 4 || mode == 5 || mode == 6)
		{
			...
			if (mode == 6)
				assert(d.probeReason == local::ProbeReason::BudgetExceeded && d.queries == 7);
			continue;
		}
		assert(d.state == local::WalkState::Running && d.intent.use == local::ActionRequest::Press &&
			   d.intent.speed == 0);
		...
		assert(d.doorState == local::DoorWaitState::Clear && d.state == local::WalkState::Running &&
			   d.intent.speed == 0);
		if (mode == 7)
			world.open = false;
		++s.tick.value;
		d = walk.update(s, *f.index, s.map, world, 120000);
		if (mode == 7)
			assert(d.state == local::WalkState::Failed && d.doorReason == local::DoorWaitReason::Reblocked);
		else
			assert(d.state == local::WalkState::Running && core::Motor::requestedSpeed(d.intent) > 0 &&
				   d.intent.use == local::ActionRequest::None);
	}
}
void touchAndReservedQueries()
{
	...
	auto s = actor();
	auto profile = limits;
	profile.probe.maxQueries = 21;
	profile.doorTimeoutUs = 1000000;
	profile.touchTimeoutUs = 3000000;
	DoorWorld world(areas);
	world.touch = true;
	world.missingFloor = true;
	local::Walk walk(binding(), f.corridor, {150, 50, 0}, profile);
	int contacts = 0;
	bool arrived = false;
	for (std::uint64_t tick = 1; tick < 200; ++tick)
	{
		...
		runtime::QueryRequest guard{{s.agent, s.actor, s.map, s.tick, binding().routeGeneration, 1},
		...
		{
			assert(++contacts == 1);
			s.position->x = 83.96875f;
			world.open = true;
		}
		else
			s.position->x += static_cast<float>(d.intent.direction.x * core::Motor::requestedSpeed(d.intent) * 0.040);
	...
	s = actor();
	DoorWorld empty(areas);
	local::Walk exhausted(binding(), f.corridor, {150, 50, 0}, profile);
	const auto d = exhausted.update(s, *f.index, s.map, empty, 0, 21);
	assert(d.state == local::WalkState::Failed && d.probeReason == local::ProbeReason::BudgetExceeded &&
		   empty.world.calls.empty());
}
std::vector<Trace> simulate(const std::vector<route_test::Area>& areas, std::uint32_t start, std::uint32_t goalArea,
							model::NavVector3 from, model::NavVector3 goal, std::uint64_t frameUs)
{
	Fixture f(areas, start, goalArea);
	World world(areas);
	auto s = actor(from);
	s.elapsedUs = frameUs;
	local::Walk walk(binding(), f.corridor, goal, limits);
	local::IntentPump pump(binding());
	std::optional<core::BotCommand> pending;
	core::TickId queued{};
	...
	for (std::uint64_t tick = 1; tick <= 4000; ++tick)
	{
		...
		const auto schedule = pump.beginFrame(s);
		...
		if (schedule.decisionDue)
		{
			...
			if (d.state != local::WalkState::Running && d.state != local::WalkState::Arrived)
				std::fprintf(
					stderr,
					"Walk failure goal=%u frame=%llu tick=%llu step=%zu reason=%d probe=%d position=(%g,%g,%g)\n",
					goalArea, static_cast<unsigned long long>(frameUs), static_cast<unsigned long long>(tick),
					d.binding.step, static_cast<int>(d.reason), static_cast<int>(d.probeReason), double(s.position->x),
					double(s.position->y), double(s.position->z));
			...
			{
				++completions;
				assert(d.support && d.support->area == f.corridor->transitions()[d.binding.step].edge.target);
			}
			assert(pump.publish(d.binding, s.tick, d.intent));
			if (d.state == local::WalkState::Arrived)
			{
				assert(d.terminalEvent && d.support && d.support->area == model::NavAreaId{goalArea});
				assert(std::hypot(double(goal.x) - s.position->x, double(goal.y) - s.position->y) <=
					   limits.arrivalTolerance);
				assert(completions == f.corridor->transitions().size());
				const auto output = pump.take();
				assert(output.emit && output.intent.speed == 0);
				++s.tick.value;
				const auto count = world.calls.size();
				auto again = walk.update(s, *f.index, s.map, world);
				assert(!again.accepted && !again.terminalEvent && again.intent.speed == 0 &&
					   count == world.calls.size());
				return trace;
			}
		}
		auto output = pump.take();
		assert(output.emit);
	...
	assert(false && "bounded simulation did not arrive");
	return {};
}
void arrivals()
{
	...
	for (std::uint64_t frameUs : {8000U, 16000U, 100000U})
	{
		for (std::uint32_t goal : {1U, 2U, 3U, 4U})
		{
			const model::NavVector3 end = goal == 1	  ? model::NavVector3{70, 50, 0}
										  : goal == 2 ? model::NavVector3{170, 50, 0}
										  : goal == 3 ? model::NavVector3{150, 170, 0}
													  : model::NavVector3{270, 170, 0};
			const auto a = simulate(path, 1, goal, {50, 50, 36}, end, frameUs);
			const auto b = simulate(path, 1, goal, {50, 50, 36}, end, frameUs);
			assert(a.size() == b.size());
			for (std::size_t i = 0; i < a.size(); ++i)
				assert(a[i].step == b[i].step && a[i].state == b[i].state && a[i].event == b[i].event &&
					   a[i].position == b[i].position);
		}
	}
	auto reverse = path;
	reverse[3].targets = {};
	reverse[3].targets[3] = {3};
	reverse[2].targets = {};
	reverse[2].targets[0] = {2};
	reverse[1].targets = {};
	reverse[1].targets[3] = {1};
	simulate(reverse, 4, 1, {270, 150, 36}, {30, 30, 0}, 16000);
}
void stops()
{
	const auto areas = zigzag();
	Fixture f(areas, 1, 2);
	...
	local::Walk wrongGoal(binding(), f.corridor, {50, 50, 0}, limits);
	s = actor();
	world.calls.clear();
	...
	local::Walk off(binding(), f.corridor, {170, 50, 0}, limits);
	world.calls.clear();
	s.position = model::NavVector3{150, 150, 36};
...
void measuredCompletion()
{
	...
	s.position->x = 101;
	auto d = walk.update(s, *f.index, s.map, world);
	assert(d.state == local::WalkState::Running && walk.step() == 0 && core::Motor::requestedSpeed(d.intent) > 0 &&
		   d.target);
	...
	assert(d.state == local::WalkState::Running &&
		   core::Motor::requestedSpeed(d.intent) > 0); // exhaustion is not arrival
	++s.tick.value;
	s.position->x = 170;
	s.grounded = false;
	d = walk.update(s, *f.index, s.map, world);
...
void crouchCrossing()
{
	for (int mode = 0; mode < 3; ++mode)
	{
		...
		local::Walk walk(binding(), f.corridor, goal, profile);
		bool arrived = false, ducked = false;
		for (std::uint64_t tick = 1; tick < 500; ++tick)
		{
			...
			if (d.state == local::WalkState::Arrived)
			{
				...
				assert(s.ducked == (mode == 1));
				arrived = true;
				break;
			}
			const auto command = core::Motor::command(d.intent, {}, 250, 40000, true);
			assert(command);
			const bool duck = (command.command->buttons & static_cast<core::ButtonMask>(core::Button::Duck)) != 0;
			s.ducked = duck;
			s.hull = duck ? profile.crouch.crouched : profile.crouch.standing;
			s.position->z = -s.hull->minimum.z;
			ducked = ducked || duck;
...
void recoverySafety()
{
	...
	for (int mode = 0; mode < 7; ++mode)
	{
		...
		local::RecoveryDecision r;
		r.state = local::RecoveryState::Sidestep;
		r.forward = {1, 0, 0};
		if (mode == 1)
			world.mode = 3; // Missing future floor.
		if (mode == 2)
			world.mode = 5; // Stale stamped response.
		if (mode == 3)
			s.grounded = false;
		if (mode == 4)
			s.position = nav::model::NavVector3{20, 95, 36}; // Center leaves the route area.
		if (mode == 5)
			r.state = local::RecoveryState::Reverse;
		if (mode == 6)
			world.mode = 1; // Swept hull collision.
		...
		if (mode == 0 || mode == 5)
		{
			...
			if (mode == 0)
				assert(d.intent.direction.y == 1); // Stable even actor chooses left on tie.
			else
				assert(d.intent.direction.x == -1);
		}
		else
			assert(!d.target && d.intent.speed == 0);
		++s.tick.value;
	...
	World world(areas);
	auto s = actor();
	local::Walk walk(binding(), f.corridor, {117, 50, 0}, limits);
	local::RecoveryDecision r;
	r.state = local::RecoveryState::Reverse;
	r.forward = {1, 0, 0};
	...
	cause.reason = local::WalkReason::DoorBlocked;
	cause.doorId = 42;
	assert(local::observedStuckCause(cause) == local::StuckCause::DoorBlocked);
	cause = {};
	cause.probeReason = local::ProbeReason::Blocked;
	assert(local::observedStuckCause(cause) == local::StuckCause::GeometryBlocked);
	cause = {};
	cause.reason = local::WalkReason::LadderFailed;
	cause.ladderReason = local::LadderReason::Fall;
	assert(local::observedStuckCause(cause) == local::StuckCause::TraversalFailed);
	cause = {};
	cause.blocker.emplace();
	cause.blocker->kind = runtime::BlockerKind::Player;
...
void narrowRecoveryUsesPhysicalQueries()
{
	...
	for (int mode = 0; mode < 6; ++mode)
	{
		...
		r.forward = {1, 0, 0};
		if (mode == 1)
			world.mode = 1; // Physical wall must still reject.
		if (mode == 2)
			world.mode = 3; // Missing floor must still reject.
		if (mode == 4)
			r.state = local::RecoveryState::Reverse;
		if (mode == 5)
		{
...
void invalidAndBudgets()
{
	...
	World world(areas);
	auto bad = limits;
	bad.probe.maxQueries = 2;
	local::Walk budget(binding(), f.corridor, {170, 50, 0}, bad);
	budget.update(s, *f.index, s.map, world);
	++s.tick.value;
	...
	{
		bad = limits;
		bad.finalApproachRange = value;
		local::Walk invalid(binding(), f.corridor, {170, 50, 0}, bad);
		const auto count = world.calls.size();
	...
	areas[0].attributes = 1;
	Fixture crouch(areas, 1, 2);
	World constrained(areas);
	...
	Fixture same(areas, 1, 1);
	World noFloor(areas);
	noFloor.mode = 2;
	local::Walk atGoal(binding(), same.corridor, {50, 50, 0}, limits);
	s = actor();
	assert(atGoal.update(s, *same.index, s.map, noFloor).state == local::WalkState::Failed);
	World hinted(areas);
	local::Walk sameHint(binding(), same.corridor, {70, 50, 0}, limits);
...
{
	auto a = square(5, 1175, 0), b = square(6, 1275, 0);
	a.targets[1] = {6};
	const std::vector<route_test::Area> areas{a, b};
	Fixture f(areas, 5, 6);
	World world(areas);
	auto s = actor({1187.742F, 50, 36});
...
void preciseGoalSegment()
{
	for (bool single : {false, true})
	{
		...
		std::vector<route_test::Area> areas =
			single ? std::vector<route_test::Area>{a} : std::vector<route_test::Area>{a, b};
		Fixture f(areas, 1, single ? 1 : 2);
		World world(areas);
		auto profile = limits;
		profile.probe.maxQueries = 21;
		profile.sideProbeDistance = 12;
		profile.narrowMargin = 8;
		profile.minimumCrossingDistance = 5;
		...
		for (const auto& q : world.calls)
			assert(q.start.y == 50 && q.end.y == 50);
	}
}
void insufficientMovementProof()
{
	...
	World world(areas);
	auto profile = limits;
	profile.probe.maxDistance = 1;
	profile.sideProbeDistance = 0;
...
int main()
{
	...
	invalidAndBudgets();
	doors();
	touchAndReservedQueries();
	crouchCrossing();
	recoverySafety();
	crossingUsesCenterMembership();
	narrowRecoveryUsesPhysicalQueries();
	supportedBoundaryOrigin();
	insufficientMovementProof();
}
