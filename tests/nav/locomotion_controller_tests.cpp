...
struct Fixture
{
	...
	explicit Fixture(std::uint32_t goal = 3)
	{
		...
		const auto graph = query::NavGraph::build(mesh, {10, 20, 1000000});
		assert(graph);
		const auto spatial = query::NavSpatialIndex::build(mesh, {10, 19, 1000000});
		assert(spatial);
		index = *spatial.value;
...
void lateralDriftRequiresFreshPhysicalGuard()
{
	...
	for (const auto& q : world.calls)
		if (q.kind == runtime::QueryKind::SweptHull && q.start.y == 52 && q.end.x > q.start.x)
			freshSweep = true;
	assert(freshSweep);
	world.wall = true;
	world.calls.clear();
...
void blockedForwardProbeAttemptsBothDetourSides()
{
	...
	std::size_t swept = 0;
	for (const auto& q : world.calls)
		if (q.kind == runtime::QueryKind::SweptHull && q.start.x != q.end.x)
			++swept;
