...
template <class Operation> void measure(std::uint32_t areas, const char* name, unsigned batch, Operation operation)
{
...
int main(int argc, char** argv)
{
	...
	for (std::uint32_t count : {128U, 1024U})
	{
		...
		out.close();
		check(bool(out), "benchmark fixture output");
		auto caps = limits();
		caps.maxInputBytes = 1048576;
		caps.header.maxAreas = count;
		caps.areas.maxAreas = count;
