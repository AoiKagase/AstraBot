...
void bad(const Bytes& bytes, K kind, const Span& span)
{
	...
	{
		std::fprintf(stderr, "expected %u/%zu/%u/%u got %u/%llu/%u/%u\n", unsigned(kind), span.offset,
					 unsigned(span.record), unsigned(span.field), unsigned(r.error.kind),
					 static_cast<unsigned long long>(r.error.offset), unsigned(r.error.record),
...
void values(const Fixture& f, unsigned v, bool full)
{
	...
	if (full)
		assert(route.value->steps[0].edge.direction == 0);
}
void corruptions(const Fixture& f, unsigned version)
{
	...
	for (const auto& s : f.spans)
	{
		for (std::size_t n = s.offset; n < s.offset + s.width; ++n)
			bad(Bytes(f.bytes.begin(), f.bytes.begin() + static_cast<std::ptrdiff_t>(n)), K::EndOfInput, s);
		auto b = f.bytes;
		if (s.floating)
		{
			set(b, s, 0x7FC00000);
			bad(b, K::NonFiniteFloat, s);
			set(b, s, 0x7F800000);
			bad(b, K::NonFiniteFloat, s);
			continue;
		}
		...
		case F::AreaId:
			kind = K::InvalidValue;
			break;
		case F::ApproachAreaId:
			replacement = 999;
			kind = K::DanglingReference;
			break;
		case F::Version:
			replacement = 6;
			kind = K::UnsupportedValue;
			break;
		case F::AreaCount:
		case F::ConnectionCount:
		case F::HidingSpotCount:
		case F::ApproachCount:
		case F::EncounterCount:
		case F::EncounterSpotCount:
		case F::PlaceCount:
			replacement = 255;
			kind = K::CountLimitExceeded;
			break;
		case F::PlaceLength:
			kind = K::InvalidValue;
			break;
		case F::ConnectionAreaId:
		case F::EncounterSpotId:
			replacement = 999;
			kind = K::DanglingReference;
			break;
		...
		case F::PlaceText:
			b[s.offset] = 0;
			bad(b, K::InvalidValue, s);
			b = f.bytes;
			b[s.offset + s.width - 1] = 1;
			bad(b, K::InvalidValue, s);
			break;
		default:
			break;
		...
		if (kind != K::None)
		{
			...
			const auto earlier = std::find_if(f.spans.begin(), f.spans.end(), [&s](const Span& p) {
...
void boundariesAndPrecedence()
{
	...
	cap = limits();
	cap.maxSnapshotBytes = sizeof(nav::model::NavMeshSnapshot) + sizeof(nav::model::NavAreaRecord);
	assert(load(f.bytes, cap));
	...
	for (unsigned n = 0; n < 64; ++n)
	{
		...
		places.bytes.insert(places.bytes.end(), 255, 0xFF);
		places.bytes.push_back(0);
	}
	...
	for (unsigned which = 0; which < 3; ++which)
	{
		cap = limits();
		if (which == 0)
			--cap.header.maxPlaces;
		if (which == 1)
			--cap.header.maxPlaceBytes;
		if (which == 2)
			--cap.header.maxTotalPlaceBytes;
	...
	{
		bytes = full.bytes;
		const Span s{14, 2, R::PlaceDictionary, F::PlaceLength, false};
		set(bytes, s, length);
		bad(bytes, length > 256 ? K::CountLimitExceeded : K::InvalidValue, s);
	}
}
} // namespace
int main(int argc, char** argv)
{
	...
	for (unsigned v = 1; v <= 5; ++v)
	{
		for (bool full : {false, true})
		{
			...
			{
				const auto path =
					std::filesystem::path(argv[1]) / ("v" + std::to_string(v) + (full ? "-full.nav" : "-minimal.nav"));
			...
			std::printf("v%u %s bytes=%zu exact-prefixes=%zu fields=%zu OK\n", v, full ? "full" : "minimal",
						f.bytes.size(), f.bytes.size(), f.spans.size());
		}
	}
