...
query::NavCostDecision customCost(const query::NavCostContext& c,
                                  const void* context) {
    auto& state=*const_cast<BasePolicyContext*>(
        static_cast<const BasePolicyContext*>(context));
    assert(context==state.expected);
    ++state.costCalls;
    return {false,{c.geometricDistance+7,3,0,0,0}};
}

double customHeuristic(const query::NavHeuristicContext& c,
                       const void* context) {
    auto& state=*const_cast<BasePolicyContext*>(
        static_cast<const BasePolicyContext*>(context));
    assert(context==state.expected);
...
int main() {
    ...
    a.targets[1]={2};
    a.targets[2]={3};
    const auto graph=query::NavGraph::build(route_test::snapshot({a,b,c}),
        {3,2,1'000'000});
    assert(graph);
    ...
    const query::NavDirectedEdge failed{{1},{2},1,
        model::NavTraversalKind::Walk,{}};
    const query::NavDirectedEdge sibling{{1},{3},2,
        model::NavTraversalKind::Walk,{}};
    ...
    const auto decision=composedPolicy.cost({failed,source,target,100},
        composedPolicy.context);
    assert(decision.components.distance==107 && decision.components.traversal==3);
    assert(composedPolicy.heuristic &&
        composedPolicy.heuristic({source,target,100},composedPolicy.context)==50);
    ...
    const auto defaultCost=defaultPolicy.cost({sibling,source,siblingTarget,125},
    ...
    auto changed=identity; ++changed.routeGeneration;
    assert(!lease.holds(changed));
    changed=identity; ++changed.round.value;
    assert(!lease.holds(changed));
    lease.release();
    ...
    assert(replanPolicy.cost({sibling,source,siblingTarget,80},replanPolicy.context)
        .components.distance==87);
    assert(replanPolicy.heuristic &&
        replanPolicy.heuristic({source,siblingTarget,80},replanPolicy.context)==40);
