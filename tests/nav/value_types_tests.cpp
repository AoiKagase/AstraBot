...
void testFiniteGeometry()
{
	...
	const NavVector3 nanValue{std::numeric_limits<float>::quiet_NaN(), 0.0F, 0.0F};
	const NavVector3 infiniteValue{0.0F, std::numeric_limits<float>::infinity(), 0.0F};
...
int main()
{
	auto owned = astrabot::nav::diagnostics::ReadResult<std::unique_ptr<int>>::success(std::make_unique<int>(42));
	...
	for (const auto kind : {NavTraversalKind::Walk, NavTraversalKind::Crouch, NavTraversalKind::Jump,
							NavTraversalKind::Ladder, NavTraversalKind::Drop})
	{
