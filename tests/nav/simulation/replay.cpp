...
struct Trace
{
	...
	{
		require(events.size() < traceLimit, "trace bound exceeded");
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		...
		if (q.kind == runtime::QueryKind::GroundedArea)
		{
			...
			for (auto i = areas.begin(); i != areas.end(); ++i)
				if (q.start.x >= i->extent.northWest.x && q.start.x <= i->extent.southEast.x && q.start.y >= 0 &&
					q.start.y <= 100)
				{
					id = model::NavAreaId{i->id};
					break;
				}
...
{
	return std::string("{\"type\":\"") + kind + "\",\"actor\":" + std::to_string(actor) +
...
void routeTrace(Trace& t, const runtime::DecisionTrace& d)
{
	...
	out << prefix("route", d.agent.value, d.tick.value) << ",\"map\":" << d.map.value << ",\"state\":" << int(d.state)
		<< ",\"reason\":" << int(d.reason) << ",\"edges\":[";
	bool first = true;
	if (d.route)
		for (const auto& s : d.route->steps)
		{
			if (!first)
				out << ',';
			first = false;
...
std::string expectedOutcome(const std::string& scenario, bool mapChange)
{
	...
	if (mapChange)
		return "cancelled-fresh-arrived";
...
Result replay(const std::string& scenario, std::uint64_t delta, std::uint32_t count, bool mapChange)
{
	...
	for (std::uint32_t id = 1; id <= 3; ++id)
	{
		...
		if (id < 3)
			areas.back().targets[1] = {id + 1};
	}
	if (scenario == "crouch-walk-release")
		areas[1].attributes = 1;
	if (scenario == "unreachable-non-execution")
		areas[1].targets[1].clear();
	auto mesh = route_test::snapshot(areas);
	auto graphResult = query::NavGraph::build(mesh, {10, 20, 1000000});
	require(bool(graphResult), "graph");
	auto indexResult = query::NavSpatialIndex::build(mesh, {10, 19, 1000000});
	require(bool(indexResult), "index");
	const auto graph = *graphResult.value;
	const auto index = *indexResult.value;
	World world(areas);
	Result result;
	std::vector<Actor> actors(count);
	...
	const auto initialize = [&](Actor& a, std::uint32_t id, std::uint64_t tick, std::uint32_t map) {
		a = Actor{};
		auto& s = a.observation;
		s.agent = {id};
		s.actor = {static_cast<std::uint16_t>(id), {map}};
		s.map = {map};
		s.tick = {tick};
		s.elapsedUs = delta;
		...
		s.hull = profile().crouch.standing;
		s.speedLimit = 250.0f;
		...
		result.queries += world.calls;
		result.maxQueries = std::max(result.maxQueries, world.calls);
		...
		if (routeOnly)
		{
			...
			require(a.session->trace().reason == (scenario == "partial-non-execution"
													  ? runtime::SessionReason::ExpansionLimit
													  : runtime::SessionReason::Unreachable),
					"route reason");
			result.trace.add(prefix("terminal", id, tick) + ",\"map\":" + std::to_string(map) + ",\"outcome\":\"" +
							 (scenario == "partial-non-execution" ? "ExpansionLimit" : "Unreachable") + "\"}");
			a.done = true;
			return;
		}
		...
		for (const auto& portal : corridor.value->transitions())
		{
			...
			o << prefix("portal", id, tick) << ",\"edge\":[" << portal.edge.source.value << ','
			  << portal.edge.target.value << "],\"source\":[" << portal.sourceLow.x << ',' << portal.sourceLow.y << ','
			  << portal.sourceHigh.x << ',' << portal.sourceHigh.y << "],\"target\":[" << portal.targetLow.x << ','
			  << portal.targetLow.y << ',' << portal.targetHigh.x << ',' << portal.targetHigh.y << "]}";
	...
	for (std::uint32_t id = 1; id <= count; ++id)
		initialize(actors[id - 1], id, 1, 1);
	bool changed = false;
	for (std::uint32_t frame = 1; frame <= frameLimit; ++frame)
	{
		result.frames = frame;
		result.elapsed = std::uint64_t(frame) * delta;
		...
		if (mapChange && !changed && (routeOnly || frame == 3))
		{
			for (std::uint32_t id = 1; id <= count; ++id)
			{
				auto& a = actors[id - 1];
				if (!routeOnly)
					require(a.queued.has_value() && !a.done, "map change must retire active queued motion");
				if (a.queued)
					result.trace.add(prefix("receipt", id, tick) + ",\"outcome\":\"map-invalidated\",\"queuedTick\":" +
									 std::to_string(a.queuedTick) + "}");
				a.observation.map = {2};
				a.observation.tick = {tick};
				...
				if (a.walk)
				{
					...
					require(!a.pump->beginFrame(a.observation).accepted && !a.pump->take().emit,
							"old pump survived map");
				}
				result.trace.add(prefix("map-cancel", id, tick) +
								 ",\"reason\":" + std::to_string(int(a.session->trace().reason)) + "}");
		...
		for (std::uint32_t id = 1; id <= count; ++id)
		{
			...
			if (a.queued)
			{
				...
				s.position->x +=
					float((command.movement.forward * std::cos(yaw) + command.movement.side * std::sin(yaw)) * seconds);
				s.position->y +=
					float((command.movement.forward * std::sin(yaw) - command.movement.side * std::cos(yaw)) * seconds);
				s.ducked = (command.buttons & static_cast<core::ButtonMask>(core::Button::Duck)) != 0;
				if (s.ducked == true)
					a.duckDispatched = true;
				else if (a.duckDispatched)
					a.duckReleased = true;
				if (scenario == "crouch-walk-release" && s.position->x > 116 && s.position->x < 184)
				{
					require(s.ducked == true, "crossed low passage without observed crouch");
					a.crossedCrouched = true;
				}
			...
			const auto schedule = a.pump->beginFrame(s);
			require(schedule.accepted, "pump frame rejected");
			...
			world.calls = 0;
			world.blocked = scenario == "blocked-floor" && !changed && (!mapChange || frame >= 4);
			if (schedule.decisionDue)
			{
				...
				o << prefix("decision", id, tick) << ",\"step\":" << a.walk->step() << ",\"state\":" << int(d.state)
				  ...
				  << ",\"queries\":" << world.calls;
				if (d.target)
					o << ",\"target\":[" << d.target->origin.x << ',' << d.target->origin.y << ',' << d.target->origin.z
					  << ']';
				o << '}';
				result.trace.add(o.str());
				...
				{
					if (world.blocked)
						require(d.state == local::WalkState::Failed && d.reason == local::WalkReason::ProbeFailed,
								"blocked typed outcome");
					else
					{
						if (d.state != local::WalkState::Arrived)
							throw std::runtime_error(scenario + " frame " + std::to_string(frame) + " state " +
													 std::to_string(int(d.state)) + " reason " +
													 std::to_string(int(d.reason)) + " probe " +
													 std::to_string(int(d.probeReason)));
						require(std::abs(s.position->x - 250) <= 4 && std::abs(s.position->y - 50) <= 4,
								"unobserved arrival");
						require(s.ducked == false, "crouch release missing");
						if (scenario == "crouch-walk-release")
							require(a.duckDispatched && a.crossedCrouched && a.duckReleased,
									"missing crouch hold/release dispatch");
					}
			...
			result.queries += world.calls;
			result.maxQueries = std::max(result.maxQueries, world.calls);
			const auto out = a.pump->take();
			require(out.emit, "missing motor frame");
			...
			o.imbue(std::locale::classic());
			o << std::setprecision(9);
			o << prefix("command", id, tick) << ",\"map\":" << s.map.value << ",\"movement\":["
			  << motor.command->movement.forward << ',' << motor.command->movement.side << ','
			  << motor.command->movement.up << "],\"view\":[" << motor.command->view.pitch << ','
			  << motor.command->view.yaw << ',' << motor.command->view.roll
			  << "],\"buttons\":" << motor.command->buttons << ",\"msec\":" << unsigned(motor.command->msec) << '}';
			result.trace.add(o.str());
			if (!a.done)
			{
				a.queued = motor.command;
				a.queuedTick = tick;
				a.queuedMap = s.map;
			}
			require(!a.pump->take().emit, "duplicate command");
		}
		if (allDone && (!mapChange || changed))
			break;
		require(frame < frameLimit, "frame bound");
	}
	for (const auto& a : actors)
		require(a.done, "missing terminal actor");
	if (routeOnly)
		result.outcome = mapChange ? "cancelled-fresh-non-execution" : scenario;
	else
		result.outcome = mapChange					   ? "cancelled-fresh-arrived"
						 : scenario == "blocked-floor" ? "probe-failed"
													   : "arrived";
	return result;
}
} // namespace
int main(int argc, char** argv)
{
	try
	{
		...
		if (argc == 3)
		{
			...
			require(bool(file), "cannot open output");
		}
		std::ostream& output = argc == 3 ? file : std::cout;
		output << "{\"schemaVersion\":1,\"producer\":\"portable\",\"results\":[";
		bool first = true;
		for (const std::string scenario : {"floor-arrival", "crouch-walk-release", "blocked-floor",
										   "partial-non-execution", "unreachable-non-execution"})
			for (const std::uint64_t delta : {8000ULL, 16000ULL, 100000ULL})
				for (const std::uint32_t count : {1U, 8U, 16U})
					for (bool change : {false, true})
					{
						std::cerr << "case=" << scenario << " frameUs=" << delta << " actors=" << count
								  << " mapChange=" << change << " seed=308\n";
						const auto a = replay(scenario, delta, count, change);
						const auto b = replay(scenario, delta, count, change);
						if (a.trace.events != b.trace.events)
						{
							std::size_t i = 0;
							while (i < a.trace.events.size() && i < b.trace.events.size() &&
								   a.trace.events[i] == b.trace.events[i])
								++i;
						...
						require(a.outcome == b.outcome && a.frames == b.frames && a.queries == b.queries,
								"replay metrics mismatch");
						const auto expected = expectedOutcome(scenario, change);
						require(a.outcome == expected, "scenario expected outcome mismatch");
						if (!first)
							output << ',';
						first = false;
						output << "{\"scenario\":\"" << scenario << "\",\"frameUs\":" << delta
							   << ",\"actors\":" << count << ",\"variant\":\"" << (change ? "map-change" : "clean")
							   << "\",\"seed\":308,\"outcome\":\"" << a.outcome << "\",\"expectedOutcome\":\""
							   << expected << "\",\"frames\":" << a.frames << ",\"elapsedUs\":" << a.elapsed
							   << ",\"maxQueriesPerActorFrame\":" << a.maxQueries << ",\"totalQueries\":" << a.queries
							   << ",\"replans\":0,\"traceLimit\":" << traceLimit
							   << ",\"traceTruncated\":false,\"replayEqual\":true,\"trace\":[";
						for (std::size_t i = 0; i < a.trace.events.size(); ++i)
						{
							if (i)
								output << ',';
							output << a.trace.events[i];
						}
						output << "]}";
					}
		output << "]}\n";
	...
	{
		std::cerr << "portable replay: " << e.what() << '\n';
		return 1;
	}
}
