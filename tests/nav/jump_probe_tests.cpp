...
{
	const auto r = query::NavSpatialIndex::build(
		route_test::snapshot({{1, {{0, 0, 0}, {99, 200, 0}, 0, 0}, {}},
							  {2, {{100, 0, height}, {200, 200, height}, height, height}, {}}}),
		{2, 3, 1000000});
	assert(r);
	return *r.value;
}
struct Box
{
	model::NavVector3 minimum{}, maximum{};
};
...
bool intersects(const runtime::QueryRequest& q, Box box)
{
	...
	const double mn[]{double(box.minimum.x) - q.hull->maximum.x, double(box.minimum.y) - q.hull->maximum.y,
					  double(box.minimum.z) - q.hull->maximum.z};
	const double mx[]{double(box.maximum.x) - q.hull->minimum.x, double(box.maximum.y) - q.hull->minimum.y,
					  double(box.maximum.z) - q.hull->minimum.z};
	for (unsigned axis = 0; axis < 3; ++axis)
	{
		const double d = b[axis] - a[axis];
		if (d == 0)
		{
			if (a[axis] <= mn[axis] || a[axis] >= mx[axis])
				return false;
		}
		else
		{
...
struct World final : runtime::IWorldQueries
{
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		...
		{
			r.ground = runtime::GroundedAreaObservation{
				model::NavAreaId{q.start.x < 100 ? 1U : 2U},
		...
		else
		{
			...
			for (const auto box : obstacles)
				if (intersects(q, box))
				{
					r.hull->fraction = 0;
					if (obstacleKind)
						r.blocker = runtime::BlockerObservation{1, *obstacleKind, {}};
				}
		}
		if (q.stamp.ordinal == faultAt)
		{
			...
			case 1:
				...
				break;
			case 2:
				r.kind = runtime::QueryKind::Floor;
				break;
			case 3:
			...
			case 9:
				...
				break;
			case 10:
				r.ground->floor->height = 1;
				break;
			case 11:
				r.hull.reset();
				break;
			case 12:
				r.hull->startSolid = true;
				break;
			case 13:
				r.hull->fraction = std::numeric_limits<float>::quiet_NaN();
				break;
			case 14:
				r.hull->end.y++;
				break;
...
JumpPhysics physics()
{
	return {binding, actor().tick, 800, 268.3281573};
}
...
{
	World world, replay;
	const auto r = launch(world), again = launch(replay);
	assert(r && again && r.inspection->step == binding.step && r.inspection->velocity == actor().velocity);
	assert(r.queries == 19 && r.segments == 7 && world.calls.size() == 19 && r.touchdown && r.flightSeconds > 0.67 &&
		   r.flightSeconds < 0.68);
	assert(r.inspection->queries == r.queries && r.inspection->flightClear == true);
	for (std::size_t i = 0; i < world.calls.size(); ++i)
	{
		assert(world.calls[i].stamp == replay.calls[i].stamp && world.calls[i].start == replay.calls[i].start &&
			   world.calls[i].end == replay.calls[i].end);
	}
	...
	{
		const auto& low = world.calls[5 + segment * 2];
		const auto& high = world.calls[6 + segment * 2];
	...
	exact.maxQueries = 19;
	World full;
	assert(launch(full, exact));
	auto eight = limits;
	eight.maxSegmentSeconds = 0.09;
	World maximum;
	const auto bounded = launch(maximum, eight);
	assert(bounded && bounded.queries == 21 && bounded.segments == 8);
	exact.maxQueries = 18;
	World over;
	const auto exhausted = launch(over, exact);
	assert(!exhausted && exhausted.reason == JumpProbeReason::BudgetExceeded && over.calls.size() == 3);
	World obstacle;
	obstacle.obstacles.push_back({{80, 20, 0}, {90, 80, 18}});
	assert(launch(obstacle));
	World wall;
	wall.obstacleKind = runtime::BlockerKind::Geometry;
	wall.obstacles.push_back({{80, 20, 0}, {90, 80, 80}});
	const auto staticBlock = launch(wall);
	assert(staticBlock.reason == JumpProbeReason::Blocked && staticBlock.flightProof == JumpProof::StaticBlocked &&
		   staticBlock.provenance == JumpProofProvenance::StaticBsp);
	World dynamic;
	dynamic.obstacleKind = runtime::BlockerKind::Player;
	...
	World ceiling;
	ceiling.obstacleKind = runtime::BlockerKind::Geometry;
...
void supportedTransitions()
{
	...
	{
		World world;
		world.targetHeight = height;
		world.obstacles.push_back({{100, 0, -100}, {200, 200, height}});
		auto jump = plan;
		jump.landing.z += height;
		const auto r =
			JumpProbe::launch(actor(), binding, jump, motion, physics(), limits, *index(height), binding.map, world);
		assert(r && r.touchdown && r.touchdown->z == 36 + height && r.flightSeconds < 0.67);
	}
	World rotated;
	auto s = actor();
	s.velocity = model::NavVector3{84.8528137f, 84.8528137f, 0};
	s.view->y = 45;
	auto jump = plan;
	jump.landing = {104, 104, 36};
...
void landingSupportFallbacks()
{
	...
	assert(plannedEvidence.role == JumpSupportRole::PlannedLanding &&
		   plannedEvidence.initialReason == ProbeReason::AllSolid && plannedEvidence.fallbackAttempted &&
		   plannedEvidence.fallbackAccepted && plannedEvidence.reason == ProbeReason::None &&
		   plannedEvidence.fallbackTrace);
	...
	assert(predictedEvidence.role == JumpSupportRole::PredictedLanding &&
		   predictedEvidence.initialReason == ProbeReason::AllSolid && predictedEvidence.fallbackAttempted &&
		   predictedEvidence.fallbackAccepted && predictedEvidence.reason == ProbeReason::None &&
		   predictedEvidence.fallbackTrace);
	...
	assert(rejectedEvidence.initialReason == ProbeReason::AllSolid && rejectedEvidence.fallbackAttempted &&
		   !rejectedEvidence.fallbackAccepted && rejectedEvidence.reason == ProbeReason::AllSolid);
}
void failures()
{
	const JumpProbeReason errors[]{JumpProbeReason::None,
								   JumpProbeReason::StaleQuery,
								   JumpProbeReason::StaleQuery,
								   JumpProbeReason::QueryUnavailable,
								   JumpProbeReason::BudgetExceeded,
								   JumpProbeReason::QueryFailed,
								   JumpProbeReason::QueryUnavailable,
								   JumpProbeReason::InvalidResult,
								   JumpProbeReason::WrongArea,
								   JumpProbeReason::ActorNotGrounded,
								   JumpProbeReason::FloorHeightMismatch,
								   JumpProbeReason::InvalidResult,
	...
	for (int fault = 1; fault <= 14; ++fault)
	{
		World world;
		world.fault = fault;
		world.faultAt = fault <= 9 ? 1U : (fault == 10 ? 3U : 6U);
		const auto r = launch(world);
		assert(!r && !r.inspection && r.reason == errors[fault]);
	...
	for (int mode = 0; mode < 14; ++mode)
	{
		...
		if (mode == 2)
			p.gravity = 0;
		if (mode == 3)
			p.verticalImpulse = std::numeric_limits<double>::infinity();
		if (mode == 4)
			s.velocity->x = 99;
		if (mode == 5)
			s.velocity->y = 11;
	...
	auto p = physics();
	p.verticalImpulse = 400;
	World far;
	assert(JumpProbe::launch(actor(), binding, plan, motion, p, limits, *index(), binding.map, far).reason ==
		   JumpProbeReason::CannotLand);
	auto l = limits;
	l.maxSegments = 6;
	World segments;
...
void controllerConsumesRealQueries()
{
	...
	for (unsigned tick = 10; tick < 13; ++tick)
	{
		...
		p.tick = s.tick;
		World world;
		const auto probe = JumpProbe::launch(s, binding, plan, motion, p, limits, *index(), binding.map, world);
		assert(probe);
	...
	{
		SimpleJump other(binding, plan, motion);
		World world;
		auto proof = launch(world).inspection;
		if (changedStep)
			++proof->step;
		else
			proof->velocity->x++;
...
void currentPhysicsCapabilities()
{
	auto s = actor();
	JumpPhysics p{
		binding, s.tick, 800, std::sqrt(72000.0), *s.hull, runtime::HullDimensions{{-16, -16, -18}, {16, 16, 32}}};
	...
	p.tick = {};
	assert(!deriveJumpLimits(motion, p, s, hints));
}
void launchProofDoesNotRequireVelocityReadiness()
{
	...
	assert(proof && proof.takeoffProof == JumpProof::Passed && proof.flightProof == JumpProof::Passed &&
...
int main()
{
	currentPhysicsCapabilities();
	clearAndEnvelope();
	supportedTransitions();
	landingSupportFallbacks();
	failures();
	controllerConsumesRealQueries();
	launchProofDoesNotRequireVelocityReadiness();
	trajectoryUsesMeasuredHorizontalVelocity();
}
