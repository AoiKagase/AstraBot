...
std::shared_ptr<const corridor::Corridor> path(std::vector<route_test::Area> areas,
                                             std::uint32_t goal) {
    ...
    assert(route);
    const auto result=corridor::Corridor::build(**graph.value,*route.value,{16,16},
        {100,1000000,1000},corridor::PortalPolicy::AllowMicroTransit);
    assert(result);
    return result.value;
}
void closeBoundariesAndMissingSupport() {
    auto a=area(1,0), b=area(2,100,4), c=area(3,104,4), d=area(4,108);
    a.targets[1]={2}; b.targets[1]={3}; c.targets[1]={4};
...
void specialIsBarrier() {
    ...
    a.targets[1]={2}; b.targets[1]={3}; c.targets[1]={4}; c.attributes=2;
...
void bendDoesNotCutCorner() {
    ...
    c.extent.northWest.y=100; c.extent.southEast.y=200;
...
void boundaryRecoveryDoesNotSkipSpecial() {
    auto a=area(1,0), b=area(2,100), c=area(3,200);
    a.targets[1]={2}; b.targets[1]={3}; b.attributes=2;
    local::PathFollower follower(path({a,b,c},3),{280,50,0});
...
void specialBackDriftRequiresForwardPortalCrossing() {
    auto a=area(1,0), b=area(2,100), c=area(3,100);
    c.extent.northWest.y=100; c.extent.southEast.y=200; c.attributes=2;
    a.targets[1]={2}; b.targets[2]={3};
    local::PathFollower follower(path({a,b,c},3),{150,180,0});
...
void denseRouteSupportsThreeHundredUnitLookAhead() {
    ...
    for (std::uint32_t id=1; id<=25; ++id) {
        ...
        if (id>1) areas[id-2].targets[1]={id};
    }
...
int main() {
    specialBackDriftRequiresForwardPortalCrossing(); nearbyUnrelatedAreaCannotSupplyRecoveryTarget();
    denseRouteSupportsThreeHundredUnitLookAhead();
    pointPortalUsesCardinalOrientation();
    closeBoundariesAndMissingSupport(); driftDoesNotRewind();
    specialIsBarrier(); sameAreaAndInvalidPosition(); bendDoesNotCutCorner();
