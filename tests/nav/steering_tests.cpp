...
void scenario(int mode, std::uint64_t frameUs)
{
	auto a = route_test::Area{1, {{0, 0, 0}, {100, 100, 0}, 0, 0}},
		 b = route_test::Area{2, {{100, 0, 0}, {200, 100, 0}, 0, 0}};
	a.targets[1] = {2};
	...
	assert(route);
	const auto corridor = corridor::Corridor::build(**graph.value, *route.value, {16, 16}, {1, 1000000, 2});
	assert(corridor);
	...
	s.position = model::NavVector3{50, mode == 0 ? 49.0f : 50.0f, 36};
	...
	if (mode == 6)
		profile.sideProbeDistance = (std::numeric_limits<double>::quiet_NaN)();
	if (mode == 7)
		profile.narrowMargin = 0;
	if (mode == 8)
		profile.maxAvoidanceDecisions = 0;
	if (mode == 9)
		profile.maxAvoidanceDecisions = 1;
	World world(mode);
	local::Walk walk(bind, corridor.value, {150, 50, 0}, profile);
	local::IntentPump pump(bind);
	std::optional<core::BotCommand> pending;
	bool finished = false, narrow = false, corrected = false, avoided = false;
	for (std::uint64_t tick = 1; tick < 4000; ++tick)
	{
		...
		if (pending)
		{
			...
			if (mode == 0)
				assert(s.position->y >= 48 && s.position->y <= 52);
			pending.reset();
		}
		...
		if (schedule.decisionDue)
		{
			...
			const auto d = walk.update(s, **index.value, s.map, world, pump.timeUs());
