...
std::vector<std::uint8_t> makeHeader(std::uint32_t version, std::uint32_t areaCount = 1U,
									 const std::vector<std::string>& places = {}, std::uint32_t bspSize = 0x12345678U)
{
...
void testMinimalVersionsAndOptionalFields()
{
	for (std::uint32_t version = 1U; version <= 5U; ++version)
	{
		const std::vector<std::string> places =
			version == 5U ? std::vector<std::string>{"MID", "\xFF\x01"} : std::vector<std::string>{};
...
void testWrongMagicAndUnsupportedVersions()
{
	...
	wrongMagic[0] = 0U;
	const auto magicResult = NavFileReader::readHeader(ByteView{wrongMagic.data(), wrongMagic.size()}, normalLimits());
	assert(!magicResult);
...
void testTruncationReportsFieldOffset()
{
	...
	{
		const auto result = NavFileReader::readHeader(ByteView{bytes.data(), length}, normalLimits());
		assert(!result);
...
void assertTruncatedField(const std::vector<std::uint8_t>& bytes, std::size_t length, std::uint64_t offset,
						  NavField field)
{
	assert(length <= bytes.size());
	auto truncated = bytes;
	truncated.resize(length);
	const auto result = NavFileReader::readHeader(ByteView{truncated.data(), truncated.size()}, normalLimits());
	assert(!result);
	assert(result.error.kind == NavErrorKind::EndOfInput);
...
void testAreaCountLimits()
{
	...
	const auto zeroResult = NavFileReader::readHeader(ByteView{zeroAreas.data(), zeroAreas.size()}, normalLimits());
	assert(!zeroResult);
	...
	const auto limitResult =
		NavFileReader::readHeader(ByteView{tooManyAreas.data(), tooManyAreas.size()}, normalLimits());
	assert(!limitResult);
...
void testPlaceValidationAndLimits()
{
	...
	const auto placeCountResult = NavFileReader::readHeader(ByteView{tooManyPlaces.data(), tooManyPlaces.size()},
	...
	zeroLength[14] = 0U;
	zeroLength[15] = 0U;
	const auto zeroLengthResult =
		NavFileReader::readHeader(ByteView{zeroLength.data(), zeroLength.size()}, normalLimits());
	...
	const auto unterminatedResult =
		NavFileReader::readHeader(ByteView{unterminated.data(), unterminated.size()}, normalLimits());
	...
	const std::vector<std::uint8_t> embeddedNul = makeHeader(5U, 1U, {embeddedPlace});
	const auto embeddedNulResult =
		NavFileReader::readHeader(ByteView{embeddedNul.data(), embeddedNul.size()}, normalLimits());
	...
	const std::vector<std::uint8_t> tooLong = makeHeader(5U, 1U, {std::string(16U, '1')});
	const auto tooLongResult = NavFileReader::readHeader(ByteView{tooLong.data(), tooLong.size()}, normalLimits());
...
void testAllocationBudgetRejectsPartialHeader()
{
	...
	const auto result = NavFileReader::readHeader(ByteView{bytes.data(), bytes.size()}, NavReadLimits{4U, 4U, 16U, 3U});
	assert(!result);
