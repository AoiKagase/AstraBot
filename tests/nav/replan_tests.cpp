...
int main()
{
	...
	const auto graph = query::NavGraph::build(mesh, {4, 5, 1000000});
	assert(graph);
	...
	assert(original && original.value->steps.size() == 1);
	...
	assert(executableDetour && executableDetour.value->areas == std::vector<model::NavAreaId>({{1}, {3}, {4}, {2}}));
	execution.begin(); // new route does not forget structurally invalid edges
	assert(execution.blocked(edge));
	const auto executionReverse =
		query::NavRouteSearch::search(**graph.value, {{2}, {1}, {4, 1000000}, false}, excluded.policy());
	...
	const auto policy = attempt.consume(binding, {2}, 102);
	assert(policy && attempt.attempts() == 1);
	const auto detour = search(policy->policy());
	assert(detour && detour.value->areas == std::vector<model::NavAreaId>({{1}, {3}, {4}, {2}}));
	assert(detour.value->total == 300 && original.value->total == 100);
	const auto reverse =
		query::NavRouteSearch::search(**graph.value, {{2}, {1}, {4, 1000000}, false}, policy->policy());
	assert(reverse && reverse.value->steps.size() == 1); // directional fact
	auto later = binding;
	++later.routeGeneration;
	...
	auto external = edge;
	external.external = enrichment::NavTraversalLink{};
	external.external->sourceId = 5;
	external.external->generation = 7;
	external.external->linkId = 9;
