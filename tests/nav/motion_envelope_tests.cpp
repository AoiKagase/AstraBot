...
int main() {
    ...
    s.agent={1}; s.actor={1,{1}}; s.map={1}; s.tick={2};
    ...
    envelope.binding={s.agent,s.actor,s.map,1,0}; envelope.tick={1};
    envelope.origin={0,0,36}; envelope.target={40,0,36};
    envelope.hull=*s.hull; envelope.maximumDisplacement=64;
    ...
    s.actor.generation={2}; assert(!envelope.matches(s,10000));
    s.actor.generation={1}; assert(!envelope.matches(s,120001));
    s.position=nav::model::NavVector3{65,0,36}; assert(!envelope.matches(s,10000));
    s.position=nav::model::NavVector3{5,2,36};
    s.alive=false; assert(!envelope.matches(s,10000));
}
