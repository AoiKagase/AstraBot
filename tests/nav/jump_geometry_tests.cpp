...
Fixture fixture(std::uint8_t direction = 1, float height = 0, float width = 100, std::uint8_t attributes = 2)
{
	...
	route_test::Area b{
		2,
		{{xs[direction], ys[direction], height}, {xs[direction] + 100, ys[direction] + width, height}, height, height},
		{},
		attributes};
	a.targets[direction] = {2};
	b.targets[(direction + 2U) % 4U] = {1};
	const auto mesh = route_test::snapshot({a, b});
	const auto graph = query::NavGraph::build(mesh, {2, 2, 1000000});
	assert(graph);
	const auto route = query::NavRouteSearch::search(**graph.value, {{1}, {2}, {2, 1000000}, false});
	assert(route);
	const auto path = corridor::Corridor::build(**graph.value, *route.value, {16, 16}, {1, 1000000, 2});
	assert(path);
	const auto index = query::NavSpatialIndex::build(mesh, {2, 3, 1000000});
	assert(index);
...
void geometry()
{
	for (std::uint8_t direction = 0; direction < 4; ++direction)
	{
		...
		const auto r = JumpGeometry::derive(*f.path, binding, actor(), motion, {80, 1});
		assert(r && r.plan->source == model::NavAreaId{1} && r.plan->target == model::NavAreaId{2});
		assert(r.plan->sourceAttributes == 0 && r.plan->targetAttributes == 2 && r.plan->takeoff.z == 36 &&
			   r.plan->landing.z == 36);
		const double dx = double(r.plan->landing.x) - r.plan->takeoff.x,
					 dy = double(r.plan->landing.y) - r.plan->takeoff.y;
		assert(std::hypot(dx, dy) == 80);
		assert((direction == 0 && dy < 0) || (direction == 1 && dx > 0) || (direction == 2 && dy > 0) ||
			   (direction == 3 && dx < 0));
	...
	auto s = actor();
	s.position->y = 1;
	auto r = JumpGeometry::derive(*f.path, binding, s, motion, {80, 1});
	assert(r && r.plan->takeoff.y == 33 && r.plan->landing.y == 33);
	s.position->y = 99;
	r = JumpGeometry::derive(*f.path, binding, s, motion, {80, 1});
	assert(r && r.plan->takeoff.y == 67);
	const auto upper = fixture(1, 32);
	r = JumpGeometry::derive(*upper.path, binding, actor(), motion, {80, 1});
	assert(r && r.plan->landing.z - r.plan->takeoff.z == 32);
	const auto high = fixture(1, 33);
	assert(JumpGeometry::derive(*high.path, binding, actor(), motion, {80, 1}).reason ==
		   JumpGeometryReason::HeightUnsupported);
	const auto down = fixture(1, -1);
	assert(JumpGeometry::derive(*down.path, binding, actor(), motion, {80, 1}).reason ==
		   JumpGeometryReason::HeightUnsupported);
	const auto narrow = fixture(1, 0, 65);
	assert(JumpGeometry::derive(*narrow.path, binding, actor(), motion, {80, 1}).reason == JumpGeometryReason::NoRoom);
	...
	for (const auto hint : unsupportedHints)
	{
		...
		assert(JumpGeometry::derive(*unsupported.path, binding, actor(), motion, {80, 1}).reason ==
			   JumpGeometryReason::UnsupportedTransition);
	}
	auto wrong = binding;
	wrong.step = 1;
	...
	auto enormous = motion;
	enormous.takeoffRadius = std::numeric_limits<double>::infinity();
...
void selectedStepAndExternal()
{
	...
	const auto graph = query::NavGraph::build(mesh, {3, 2, 1000000});
	assert(graph);
	const auto route = query::NavRouteSearch::search(**graph.value, {{1}, {3}, {3, 1000000}, false});
	assert(route);
	...
	enrichment::NavMapFingerprint fingerprint{};
	fingerprint[0] = 1;
	...
	b.targets[1].clear();
	const auto external =
		query::NavGraph::compose(route_test::snapshot({a, b}), fingerprint, links, {2, 1, 1000000}, {1, 1000000});
	assert(external);
	const auto er = query::NavRouteSearch::search(**external.value, {{1}, {2}, {2, 1000000}, false});
	assert(er);
	const auto ep = corridor::Corridor::build(**external.value, *er.value, {16, 16}, {1, 1000000, 1});
	assert(ep);
	assert(JumpGeometry::derive(*ep.value, binding, actor(), motion, {80, 1}).reason ==
		   JumpGeometryReason::UnsupportedTransition);
}
struct World final : runtime::IWorldQueries
{
	...
	unsigned staleOrdinal{};
	bool blocked{}, missing{};
	...
	{
		calls.push_back(q);
		assert(q.stamp.ordinal == calls.size());
		runtime::WorldQueryResult r;
		r.stamp = q.stamp;
		r.kind = q.kind;
		r.error = runtime::QueryError::None;
		if (q.stamp.ordinal == staleOrdinal)
			--r.stamp.ordinal; // Must not match the helper's lower ordinal.
		if (q.kind == runtime::QueryKind::GroundedArea || q.kind == runtime::QueryKind::Floor)
		{
			...
			if (match && *match.value && !missing)
			{
				...
				if (q.kind == runtime::QueryKind::Floor)
					r.floor = floor;
				else
					r.ground = runtime::GroundedAreaObservation{p.areaId, floor};
			}
		}
		else
		{
...
{
	const auto f = fixture();
	const auto p = JumpGeometry::derive(*f.path, binding, actor(), motion, {80, 1});
	assert(p);
	auto s = actor();
	s.position->x = 20;
	World world(*f.index);
	...
	{
		world.calls.clear();
		world.staleOrdinal = ordinal;
		proof = JumpProbe::prepare(s, binding, *p.plan, motion, groundLimits, *f.index, binding.map, world);
		assert(!proof && proof.reason == JumpProbeReason::StaleQuery && world.calls.size() == ordinal);
	}
	world.staleOrdinal = 0;
	world.calls.clear();
	world.blocked = true;
	assert(JumpProbe::prepare(s, binding, *p.plan, motion, groundLimits, *f.index, binding.map, world).reason ==
		   JumpProbeReason::Blocked);
	world.blocked = false;
	world.calls.clear();
	auto low = groundLimits;
	low.maxQueries = 3;
	assert(JumpProbe::prepare(s, binding, *p.plan, motion, low, *f.index, binding.map, world).reason ==
		   JumpProbeReason::BudgetExceeded);
	...
	world.calls.clear();
	auto bad = *p.plan;
	bad.takeoff = {140, 50, 36};
	bad.landing = {190, 50, 36};
	s.position->x = 90;
	...
	s.position = p.plan->landing;
	world.calls.clear();
	proof = JumpProbe::land(s, binding, *p.plan, motion, groundLimits, *f.index, binding.map, world);
	assert(proof && proof.queries == 1 && proof.inspection->support->area == p.plan->target &&
		   !proof.inspection->flightClear);
	world.calls.clear();
	world.missing = true;
	assert(JumpProbe::land(s, binding, *p.plan, motion, groundLimits, *f.index, binding.map, world).reason ==
		   JumpProbeReason::QueryUnavailable);
	world.missing = false;
	world.calls.clear();
	s.position = p.plan->takeoff;
	assert(JumpProbe::land(s, binding, *p.plan, motion, groundLimits, *f.index, binding.map, world).reason ==
		   JumpProbeReason::WrongArea);
}
void pipeline()
{
	for (std::uint8_t direction = 0; direction < 4; ++direction)
	{
		...
		for (std::uint64_t us : {8000U, 16000U, 100000U})
		{
			...
			if (direction == 3)
				s.position->x = 80;
			const auto geometry = JumpGeometry::derive(*f.path, binding, s, motion, {80, 1});
			assert(geometry);
			const auto plan = *geometry.plan;
			SimpleJump jump(binding, plan, motion);
			unsigned presses = 0;
			bool complete = false;
			double vz = 0;
			...
			for (std::uint64_t tick = 1; tick < 700; ++tick)
			{
				...
				s.elapsedUs = us;
				World world(*f.index);
				JumpFeedback feedback{binding, s, tick * us, {}, dispatch};
				dispatch.reset();
				if (s.grounded == true)
				{
					...
					else if (jump.state() == JumpState::Accelerate &&
							 std::hypot(s.velocity->x, s.velocity->y) >= motion.minimumSpeed)
						proof = JumpProbe::launch(s, binding, plan, motion, {binding, s.tick, 800, 268.3281573},
												  {21, 8, 0.1, 2, 2}, *f.index, binding.map, world);
					else
						proof =
							JumpProbe::prepare(s, binding, plan, motion, groundLimits, *f.index, binding.map, world);
					if (!proof)
						std::fprintf(stderr, "probe dt=%llu tick=%llu reason=%u x=%.6f\n",
									 static_cast<unsigned long long>(us), static_cast<unsigned long long>(tick),
									 unsigned(proof.reason), s.position->x);
					assert(proof && world.calls.size() <= 21);
				...
				if (d.state == JumpState::Complete)
				{
					assert(d.terminalEvent && presses == 1);
					complete = true;
					break;
				}
				const auto cmd = core::Motor::command(d.intent, {s.view->x, s.view->y, s.view->z}, 250, us, true);
				assert(cmd);
				...
				const double vx =
					cmd.command->movement.forward * std::cos(yaw) + cmd.command->movement.side * std::sin(yaw);
				const double vy =
					cmd.command->movement.forward * std::sin(yaw) - cmd.command->movement.side * std::cos(yaw);
				s.position->x += static_cast<float>(vx * dt);
				s.position->y += static_cast<float>(vy * dt);
				...
				if (s.grounded == false)
				{
					s.position->z += static_cast<float>(vz * dt - 400 * dt * dt);
					vz -= 800 * dt;
					if (s.position->z <= 36)
					{
						s.position->z = 36;
...
{
	geometry();
	selectedStepAndExternal();
	preparationAndLanding();
	pipeline();
}
