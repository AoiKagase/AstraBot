...
runtime::MovementSnapshot actor() {
    ...
    s.agent=b.agent; s.actor=b.actor; s.map=b.map; s.tick={1};
    s.elapsedUs=40'000; s.kind=runtime::ActorKind::ManagedBot;
...
struct DoorWorld final : runtime::IWorldQueries {
    ...
    runtime::WorldQueryResult query(const runtime::QueryRequest& q) override {
        ...
        r.stamp=q.stamp; r.kind=q.kind; r.error=error;
...
std::shared_ptr<const query::NavSpatialIndex> index() {
    ...
    const auto built=query::NavSpatialIndex::build(route_test::snapshot({area}),
        {2,3,1'000'000});
    assert(built);
    return *built.value;
}
...
int main() {
    ...
    const auto press=localDoor.update(s,binding(),{90,50,36},*spatial,s.map,
        limits,world,0,used,4);
    ...
    ++s.tick.value;
    const auto waiting=localDoor.update(s,binding(),{90,50,36},*spatial,s.map,
        limits,world,1'000,used,4);
    assert(waiting && waiting->intent.use==local::ActionRequest::None);

    ++s.tick.value;
    world.door.open=true;
    const auto clear=localDoor.update(s,binding(),{90,50,36},*spatial,s.map,
        limits,world,2'000,used,4);
    assert(clear && clear->doorState==local::DoorWaitState::Clear);

    localDoor.reset(); world.door.open=false; used=0; s.tick={10};
    assert(localDoor.update(s,binding(),{90,50,36},*spatial,s.map,
        limits,world,10,used,2)->intent.use==local::ActionRequest::Press);
    ++s.tick.value;
    const auto timedOut=localDoor.update(s,binding(),{90,50,36},*spatial,s.map,
        limits,world,10+local::LocalDoor::timeoutUs,used,2);
    assert(timedOut && timedOut->reason==local::WalkReason::DoorBlocked &&
        timedOut->doorReason==local::DoorWaitReason::TimedOut);

    localDoor.reset(); world.error=runtime::QueryError::Unavailable;
    used=0; s.tick={20};
    const auto unknown=localDoor.update(s,binding(),{90,50,36},*spatial,s.map,
        limits,world,0,used,1);
    assert(unknown && unknown->disposition==local::MotionDisposition::Hold &&
        unknown->state==local::WalkState::Running &&
        unknown->probeReason==local::ProbeReason::QueryUnavailable);

    localDoor.reset(); world.error=runtime::QueryError::None;
    world.door={42,false,false,std::nullopt,true};
    used=0; s.tick={30};
    const auto unsupportedContact=localDoor.update(s,binding(),{90,50,36},
        *spatial,s.map,limits,world,0,used,1);
