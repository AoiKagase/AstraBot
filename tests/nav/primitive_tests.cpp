...
corridor::Transition transition(model::NavTraversalKind kind = model::NavTraversalKind::Walk)
{
	...
	t.sourceLow = t.targetLow = {100, 16, 0};
	t.sourceHigh = t.targetHigh = {100, 84, 0};
	return t;
}
bool neutral(const MovementIntent& i)
{
	return i.direction.x == 0 && i.direction.y == 0 && i.direction.z == 0 && i.speed == 0 && i.lateralCorrection == 0 &&
		   !i.view && i.duck == ActionRequest::None && i.jump == ActionRequest::None && i.use == ActionRequest::None;
}
Feedback feedback()
{
	Feedback f;
...
void lifecycle()
{
	...
	auto t = transition();
	auto r = p.enter(binding(), t, {10});
	assert(r.accepted && r.event == PrimitiveEvent::Entered && neutral(r.intent));
	assert(!p.enter(binding(), t, {10}).accepted);
	auto f = feedback();
	f.intent.direction = {1, 0, 0};
	f.intent.speed = 100;
	f.intent.jump = ActionRequest::Press;
	r = p.update(f);
	assert(r.accepted && r.event == PrimitiveEvent::None && r.intent.jump == ActionRequest::Press);
	r = p.update(f);
	assert(!r.accepted && r.reason == PrimitiveReason::StaleUpdate && neutral(r.intent));
	f.tick = {12};
	f.progress = Progress::Complete;
	f.supportedArea = model::NavAreaId{2};
	f.supportVerified = true;
	r = p.update(f);
	assert(r.accepted && r.event == PrimitiveEvent::Complete && neutral(r.intent));
	f.tick = {13};
...
void staleAndTerminal()
{
	...
	{
		Primitive p;
		assert(p.enter(binding(), transition(), {10}).accepted);
		auto f = feedback();
		switch (field)
		{
		case 0:
		...
		r = p.abort();
		assert(r.accepted && r.event == PrimitiveEvent::Aborted && neutral(r.intent));
		assert(p.abort().event == PrimitiveEvent::None);
	}
	for (int mode = 0; mode < 4; ++mode)
	{
		...
		if (mode == 1)
		{
			f.supportedArea = model::NavAreaId{1};
			f.supportVerified = true;
		}
		if (mode == 2)
			f.supportedArea = model::NavAreaId{2};
		if (mode == 3)
			f.progress = Progress::Failed;
		auto r = p.update(f);
		assert(r.accepted && r.event == PrimitiveEvent::Failed && neutral(r.intent));
...
void dispatchAndOwnership()
{
	...
	{
		Primitive p;
		auto t = transition(kind);
		t.edge.external = enrichment::NavTraversalLink{
			7, 8, 9, {1}, {2}, {25, 25, 0}, {125, 25, 0}, kind, enrichment::NavLinkDirection::Forward, 0};
	...
	{
		Primitive p;
		auto r = p.enter(binding(), transition(kind), {10});
...
void badInputs()
{
	...
	{
		Primitive q;
		assert(q.enter(binding(), transition(), {10}).accepted);
		auto f = feedback();
		if (mode == 0)
			f.intent.speed = std::numeric_limits<double>::quiet_NaN();
		if (mode == 1)
			f.intent.direction.x = std::numeric_limits<double>::infinity();
		if (mode == 2)
			f.intent.speed = -1;
		if (mode == 3)
			f.intent.use = static_cast<ActionRequest>(255);
		if (mode == 4)
			f.progress = static_cast<Progress>(255);
		if (mode == 5)
			f.intent.lateralCorrection = 1.1;
		if (mode == 6)
			f.intent.speed = 401;
		if (mode == 7)
			f.intent.view = astrabot::core::IntentVector{0, 0, std::numeric_limits<double>::quiet_NaN()};
		if (mode == 8)
			f.intent.speed = 1; // nonzero speed needs a direction
		const auto r = q.update(f);
		assert(r.event == PrimitiveEvent::Failed && neutral(r.intent));
...
void corridorReplay()
{
	...
	a.targets[1] = {2};
	b.targets[2] = {3};
	auto g = query::NavGraph::build(route_test::snapshot({a, b, c}), {3, 2, 1000000});
	assert(g);
	auto r = query::NavRouteSearch::search(**g.value, {{1}, {3}, {3, 1000000}, false});
	assert(r);
	auto corridor = corridor::Corridor::build(**g.value, *r.value, {16, 16}, {2, 1000000, 2});
	assert(corridor);
	...
	for (int replay = 0; replay < 2; ++replay)
	{
		...
		for (std::size_t step = 0; step < 2; ++step)
		{
			...
			f.tick = {12};
			f.progress = Progress::Complete;
			f.supportedArea = t.edge.target;
			f.supportVerified = true;
			auto terminal = p.update(f);
			assert(terminal.event == PrimitiveEvent::Complete);
