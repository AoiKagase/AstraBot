...
int main(int argc, char** argv)
{
	try
	{
		...
		for (int stage = 0; stage < 3; ++stage)
		{
			limited = profile;
			if (stage == 0)
				limited.mesh.maxSnapshotBytes = 1;
			if (stage == 1)
				limited.index.maxIndexBytes = 1;
			if (stage == 2)
				limited.graph.maxGraphBytes = 1;
			std::ostringstream out;
