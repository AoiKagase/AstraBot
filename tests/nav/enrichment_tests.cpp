...
auto snapshot()
{
	return route_test::snapshot({area(2, 10), area(1, 0), area(3, 20)});
}
NavTraversalLink up(std::uint64_t id = 1)
{
	return {1, 1, id, {1}, {2}, {1, 1, 0}, {1, 1, 10}, model::NavTraversalKind::Ladder, NavLinkDirection::Up, 2};
}
NavTraversalLinkSet links()
{
	return {fingerprint(), {up()}};
}
...
void shapeAndIdentity()
{
	...
	set.links[0].sourceId = 2;
	set.links[0].generation = 9;
...
void individualValidation()
{
	...
	const auto check = [](auto mutate, K kind, F field) {
		...
		mutate(candidate.links[0]);
		failure(compose(candidate), kind, field);
	};
	...
	check([](auto& l) { l.traversal = static_cast<model::NavTraversalKind>(255); }, K::UnsupportedValue,
		  F::LinkTraversal);
	check([](auto& l) { l.direction = static_cast<NavLinkDirection>(255); }, K::UnsupportedValue, F::LinkDirection);
	for (auto number : {std::numeric_limits<double>::infinity(), -std::numeric_limits<double>::infinity(),
	...
	for (auto kind : {model::NavTraversalKind::Walk, model::NavTraversalKind::Crouch, model::NavTraversalKind::Jump,
					  model::NavTraversalKind::Ladder, model::NavTraversalKind::Drop})
	{
		set = links();
		set.links[0].traversal = kind;
...
void relationalValidation()
{
	...
	set = links();
	auto invalid = up(2);
	invalid.additionalCost = -1;
	set.links.push_back(invalid);
	set.links[0].to = {99};
...
void budgets()
{
	auto snap = snapshot();
	auto set = links();
	failure(NavGraph::compose({}, fingerprint(), set, graphLimits, linkLimits), K::InvalidInput, F::GraphBytes,
			diagnostics::NavRecord::Graph);
	failure(NavGraph::compose(snap, fingerprint(), set, graphLimits, {0, 1000000}), K::CountLimitExceeded,
			F::LinkCount);
	const auto working = (snap->areas().size() + set.links.size()) * sizeof(std::size_t);
	failure(NavGraph::compose(snap, fingerprint(), set, graphLimits, {1, working - 1}), K::CountLimitExceeded,
			F::LinkWorkingBytes);
	...
	failure(NavGraph::compose(snap, fingerprint(), set, {3, 1, bytes - 1}, linkLimits), K::CountLimitExceeded,
			F::GraphBytes, diagnostics::NavRecord::Graph);
	failure(NavGraph::compose(snap, fingerprint(), set, {3, 0, 1000000}, linkLimits), K::CountLimitExceeded,
			F::ConnectionCount, diagnostics::NavRecord::Graph);
	failure(NavGraph::compose(snap, fingerprint(), set, {2, 10, 1000000}, linkLimits), K::CountLimitExceeded,
			F::AreaCount, diagnostics::NavRecord::Graph);
	set.links.clear();
...
void ladderRoutes()
{
	auto snap = snapshot();
	auto set = links();
	auto base = NavGraph::build(snap, graphLimits);
	assert(base);
	...
	assert(r.value->total == 12 && r.value->components.distance == 10 && r.value->components.traversal == 2);
	assert((r.value->areas == std::vector<model::NavAreaId>{{1}, {2}}));
	assert(r.value->steps.size() == 1 && r.value->steps[0].edge.external);
	assert(r.value->steps[0].edge.external->sourceId == 1 && r.value->steps[0].edge.external->generation == 1 &&
	...
	down.from = {2};
	down.to = {1};
	std::swap(down.entry, down.exit);
	down.direction = NavLinkDirection::Down;
	down.additionalCost = 5;
	set.links.push_back(down);
	g = compose(set);
	assert(g);
	r = search(**g.value, 2, 1);
...
void parallelLinksAndPolicies()
{
	...
	assert(r && r.value->total == 13 && r.value->components.distance == 1 && r.value->components.traversal == 3 &&
		   r.value->components.danger == 4 && r.value->components.experience == 5);
	assert(r.value->steps[0].edge.external->linkId == 2);
	auto a = area(1, 0);
	a.targets[3] = {2};
	auto snap = route_test::snapshot({a, area(2, 10)});
	set.links[0].additionalCost = set.links[1].additionalCost = 0;
	g = NavGraph::compose(snap, fingerprint(), set, graphLimits, linkLimits);
	assert(g);
	...
	auto base = NavGraph::build(snap, graphLimits);
	assert(base);
	const auto before = search(**base.value, 1, 2);
	assert(before);
	assert(before.value->total == r.value->total && before.value->areas == r.value->areas &&
		   before.value->steps[0].edge.direction == r.value->steps[0].edge.direction);
	set.links.clear();
	g = NavGraph::compose(snap, fingerprint(), set, graphLimits, {0, 0});
	assert(g);
	r = search(**g.value, 1, 2);
...
void partialAndOverflow()
{
	...
	auto g = compose(set);
	assert(g);
	auto req = request(1, 3);
	req.limits.maxExpansions = 1;
	...
	for (auto& l : set.links)
		l.additionalCost = std::numeric_limits<double>::max();
	g = compose(set);
	assert(g); // individually finite; accumulated route must reject
	r = search(**g.value, 1, 3);
...
void sameRoute(const NavRouteResult& a, const NavRouteResult& b)
{
	const auto components = [](const auto& x, const auto& y) {
		assert(x.distance == y.distance && x.traversal == y.traversal && x.danger == y.danger &&
	...
	assert(a.metrics.expansions == b.metrics.expansions && a.metrics.examinedEdges == b.metrics.examinedEdges &&
		   a.metrics.relaxations == b.metrics.relaxations && a.metrics.reopens == b.metrics.reopens &&
		   a.metrics.peakOpen == b.metrics.peakOpen);
	for (std::size_t i = 0; i < a.steps.size(); ++i)
	{
		...
		if (x.edge.external)
			sameLink(*x.edge.external, *y.edge.external);
	}
}
void workingOverflowAndPreflight()
{
	...
	for (auto cap : {std::size_t{0}, maximum})
	{
		...
		assert((mult == diagnostics::NavError{K::OffsetOverflow, 0, diagnostics::NavRecord::TraversalLink,
											  F::LinkWorkingBytes}));
		assert(add == mult);
	}
	auto snap = snapshot();
	auto set = links();
	auto previous = compose(set);
	assert(previous);
	const auto exact = (*previous.value)->logicalBytes();
	const auto check = [&](NavGraphLimits gl, NavEnrichmentLimits el, K kind, F field, diagnostics::NavRecord record) {
		failAfter = 0;
	...
	check(graphLimits, {0, maximum}, K::CountLimitExceeded, F::LinkCount, diagnostics::NavRecord::TraversalLink);
	check(graphLimits, {1, 0}, K::CountLimitExceeded, F::LinkWorkingBytes, diagnostics::NavRecord::TraversalLink);
	check({3, 1, exact - 1}, linkLimits, K::CountLimitExceeded, F::GraphBytes, diagnostics::NavRecord::Graph);
	set.fingerprint[0] = 0;
	check(graphLimits, linkLimits, K::InvalidValue, F::LinkFingerprint, diagnostics::NavRecord::TraversalLink);
...
void failureSweeps()
{
	...
	auto snap = snapshot();
	auto set = links();
	auto built = compose(set);
	assert(built);
	const auto previous = *built.value;
	const auto saved = search(*previous, 1, 2);
	assert(saved);
	bool success = false;
	for (std::size_t n = 0; n < 32; ++n)
	{
		...
		failAfter = maximum;
		auto retained = search(*previous, 1, 2);
		assert(retained);
	...
	for (bool partial : {false, true})
	{
		...
		const auto expected = NavRouteSearch::search(*previous, req);
		assert(expected);
		for (std::size_t n = 0; n < 32; ++n)
		{
			failAfter = n;
...
void lifetimeAndConcurrentReads()
{
	...
	static_assert(
		std::is_same_v<decltype(std::declval<GraphResult>().value)::value_type, std::shared_ptr<const NavGraph>>);
	...
	NavRouteResult saved;
	{
		...
		assert(r);
		graph = *r.value;
		set.links[0].sourceId = 999;
		set.links[0].additionalCost = 999;
		auto route = search(*graph, 1, 2);
		assert(route);
		saved = *route.value;
...
void validationMatrix()
{
	...
	for (auto member : {&NavLinkPoint::x, &NavLinkPoint::y, &NavLinkPoint::z})
	{
		for (auto point : {&NavTraversalLink::entry, &NavTraversalLink::exit})
		{
			...
			failure(compose(set), K::InvalidGeometry, point == &NavTraversalLink::entry ? F::LinkEntry : F::LinkExit);
		}
	}
	...
	auto other = set.links[0];
	other.additionalCost = 0.0;
	...
	auto secondSource = up();
	secondSource.sourceId = 2;
	set.links.push_back(secondSource);
	++secondSource.generation;
	set.links.push_back(secondSource);
	for (int n = 0; n < 4; ++n)
	{
		std::rotate(set.links.begin(), set.links.begin() + 1, set.links.end());
...
int main()
{
	...
	budgets();
	ladderRoutes();
	parallelLinksAndPolicies();
	partialAndOverflow();
	workingOverflowAndPreflight();
	failureSweeps();
	lifetimeAndConcurrentReads();
	validationMatrix();
