...
NavRouteRequest request(std::uint32_t start = 1, std::uint32_t goal = 4, std::size_t cap = 100, bool partial = false)
{
	return {{start}, {goal}, {cap, 1000000}, partial};
}
void corridor(const NavRouteResult& r, std::initializer_list<model::NavAreaId> ids, double total)
{
...
NavCostDecision unit(const NavCostContext&, const void*)
{
...
void heapOrderingAndDecreaseKey()
{
	...
	const auto cost = [](const NavCostContext& c, const void*) -> NavCostDecision {
		return {
			false,
			{c.edge.source.value == 1 ? (c.edge.target.value == 2 ? 1.0 : 2.0) : (c.edge.source.value == 2 ? 2.0 : 1.0),
			 0, 0, 0}};
	};
	...
	const auto varied = [](const NavCostContext& c, const void*) -> NavCostDecision {
		...
		return {false,
				{c.edge.source.value == 1 ? costs[c.edge.target.value - 2] : (c.edge.source.value == 8 ? 0.0 : 2.0), 0,
				 0, 0}};
	};
...
{
	bool block{false};
	mutable std::array<unsigned, 5> heuristicCalls{};
};
...
void reopeningAndBlocking()
{
	...
	for (std::size_t i = 1; i <= 4; ++i)
		assert(state.heuristicCalls[i] == 1);
	state = {true, {}};
...
NavCostDecision zero(const NavCostContext&, const void*)
{
	return {};
}
...
void numericAndExceptionBaseline()
{
	...
	{
		for (auto components : {NavCostComponents{bad, 0, 0, 0}, {0, bad, 0, 0}, {0, 0, bad, 0}, {0, 0, 0, bad}})
			failure(NavRouteSearch::search(*g, request(1, 3), {&components, numericCost, nullptr}), K::InvalidValue,
					F::RouteCost);
		failure(NavRouteSearch::search(*g, request(1, 1), {&bad, nullptr, numericHeuristic}), K::InvalidValue,
				F::RouteHeuristic);
	}
	double nonzeroGoal = 1;
	failure(NavRouteSearch::search(*g, request(1, 1), {&nonzeroGoal, nullptr, numericHeuristic}), K::InvalidValue,
			F::RouteHeuristic);
	...
	failure(NavRouteSearch::search(*g, request(1, 3), {&large, numericCost, largeH}), K::InvalidValue,
			F::RouteHeuristic);
	const auto badDiscoveredH = [](const NavHeuristicContext& c, const void*) -> double {
		return c.area.id.value == 2 ? -1 : 0;
	};
	failure(NavRouteSearch::search(*g, request(1, 3), {nullptr, unit, badDiscoveredH}), K::InvalidValue,
			F::RouteHeuristic);
	// Opt-in partial output must not turn a numeric error into a usable route.
	const NavCostComponents overflow{huge, huge, 0, 0};
	failure(NavRouteSearch::search(*g, request(1, 3, 1, true), {&overflow, numericCost, nullptr}), K::InvalidValue,
			F::RouteCost);
	const auto throwsCost = [](const NavCostContext&, const void*) -> NavCostDecision {
		throw std::runtime_error("cost");
	};
	failure(NavRouteSearch::search(*g, request(1, 3), {nullptr, throwsCost, nullptr}), K::PolicyFailure, F::RouteCost);
	const auto throwsHeuristic = [](const NavHeuristicContext&, const void*) -> double {
		throw std::length_error("heuristic");
	};
	failure(NavRouteSearch::search(*g, request(1, 1), {nullptr, nullptr, throwsHeuristic}), K::PolicyFailure,
			F::RouteHeuristic);
	const auto allocation = [](const NavCostContext&, const void*) -> NavCostDecision { throw std::bad_alloc{}; };
	failure(NavRouteSearch::search(*g, request(1, 3), {nullptr, allocation, nullptr}), K::AllocationFailure,
			F::RouteCost);
	const auto goalAllocation = [](const NavHeuristicContext&, const void*) -> double { throw std::bad_alloc{}; };
	failure(NavRouteSearch::search(*g, request(1, 1), {nullptr, nullptr, goalAllocation}), K::AllocationFailure,
			F::RouteHeuristic);
...
void capAndBudgetBaseline()
{
	...
	failure(NavRouteSearch::search(*g, {{1}, {2}, {1, 0}, true}), K::CountLimitExceeded, F::RouteBytes);
}
void metrics(const NavRouteResult& r, std::size_t expansions, std::size_t edges, std::size_t relaxations,
			 std::size_t reopens, std::size_t peak)
{
...
NavCostDecision ancestorCost(const NavCostContext& c, const void* p)
{
	...
	if (from == 1)
		return {false, {to == 2 ? 3.0 : 1.0, 0, 0, 0}};
	if (from == 4)
		return {state.blockGoal, {100, 0, 0, 0}};
	return {false, {1, 0, 0, 0}};
}
double ancestorH(const NavHeuristicContext& c, const void*)
{
	return c.area.id.value == 3 ? 5 : 0;
}
void staleAncestorEvidence()
{
	...
	for (const auto& step : r.value->steps)
		assert(step.total == 1 && step.components.distance == 1);
...
void sameRoute(const NavRouteResult& a, const NavRouteResult& b)
{
	...
	metrics(a, b.metrics.expansions, b.metrics.examinedEdges, b.metrics.relaxations, b.metrics.reopens,
			b.metrics.peakOpen);
...
void concurrentSearches()
{
	...
	const auto cost = [](const NavCostContext& ctx, const void* p) -> NavCostDecision {
		...
		return {false, ctx.edge.direction == state.direction ? state.components : NavCostComponents{100, 0, 0, 0}};
	};
	...
	for (auto& workerResults : results)
		workerResults.reserve(runs);
	...
	for (std::size_t i = 0; i < results.size(); ++i)
	{
		...
		for (const auto& r : results[i])
			sameRoute(r, serial[i]);
	}
	assert(failAfter == noFailure && !probeAllocations);
	std::puts(
		"concurrent routes: 2 workers x 100; full routes/edges/components/metrics match serial; failpoint disabled");
}
...
void snapshotLifetimeAndSpatialComposition()
{
	...
	assert(route);
	sameRoute(*route.value, saved);
	g.reset();
	assert(lifetime.expired());
	sameRoute(*route.value, saved); // Result owns its corridor and cost evidence.
	std::puts("snapshot lifetime: caller/bytes destroyed; containing+nearest compose; graph retains snapshot; route "
			  "survives graph");
}
void queryByteBoundaries()
{
	...
	while (low < high)
	{
		...
		auto req = request();
		req.limits.maxWorkingBytes = mid;
		auto r = NavRouteSearch::search(*g, req, {nullptr, unit, nullptr});
		if (r)
			high = mid;
		else
		{
			failure(r, K::CountLimitExceeded, F::RouteBytes);
			low = mid + 1;
		}
	}
	...
	for (auto req :
		 {request(), request(1, 4, 0, true), request(1, 4, 0), request(1, 4, 1, true), request(1, 1, 0), request(4, 1)})
	{
		CallCounts calls;
		...
		{
			req.limits.maxWorkingBytes = budget;
			calls = {};
			auto under = [&] {
	...
	CallCounts calls;
	auto bad = request(0, 4);
	bad.limits.maxWorkingBytes = 0;
	auto r = [&] {
		AllocationProbe probe(0);
...
void allocationFailureSweeps()
{
	...
	const auto checkRetained = [&] {
		...
		assert(again);
		sameRoute(*again.value, saved);
	};
	bool success = false;
	for (std::size_t n = 0; n < 64; ++n)
	{
		auto r = [&] {
			AllocationProbe probe(n);
		...
		if (r)
		{
			...
			sameRoute(*route.value, saved);
			std::printf(
				"graph OOM sweep: %zu failed allocation sites then success; previous route/graph/snapshot retained\n",
				n);
			success = true;
			break;
		}
		...
		assert(
			(r.error == diagnostics::NavError{K::AllocationFailure, 0, diagnostics::NavRecord::Graph, F::GraphBytes}));
	}
	assert(success);
	unsigned scenario = 0;
	for (auto req :
		 {request(), request(1, 4, 1, true), request(1, 4, 0, true), request(1, 1, 0), request(4, 1), request(1, 4, 0)})
	{
		...
		for (std::size_t n = 0; n < 64; ++n)
		{
			...
			if (r)
			{
				...
				if (!r.value->areas.empty())
					assert(reconstructionFailures >= 2);
				else
					assert(reconstructionFailures == 0);
...
NavCostDecision throwingCost(const NavCostContext& c, const void* p)
{
	...
	++state.costs;
	if (state.fromCost && c.source.id.value == state.at)
		throwAt(state);
	return {false, {1, 0, 0, 0}};
}
double throwingH(const NavHeuristicContext& c, const void* p)
{
	const auto& state = *static_cast<const ThrowPolicy*>(p);
	++state.heuristics;
	if (!state.fromCost && c.area.id.value == state.at)
		throwAt(state);
	return 0;
}
	...
	for (bool allocation : {false, true})
	{
		for (bool cost : {false, true})
		{
			for (std::uint32_t at : {1U, 2U, 3U, 4U})
			{
				...
				auto r = NavRouteSearch::search(*g, request(1, 4, 100, true), {&p, throwingCost, throwingH});
				failure(r, allocation ? K::AllocationFailure : K::PolicyFailure,
						cost ? F::RouteCost : F::RouteHeuristic);
				if (cost)
					assert(p.costs == at);
				else
					assert(p.costs == (at == 4 ? 0U : at - 1));
...
void numericFailureStages()
{
	...
	for (double bad : {-1.0, -std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(),
					   std::numeric_limits<double>::quiet_NaN()})
	{
		for (auto components : {NavCostComponents{bad, 0, 0, 0}, {0, bad, 0, 0}, {0, 0, bad, 0}, {0, 0, 0, bad}})
		{
			InvalidPolicy p{components, 0, 0};
			failure(NavRouteSearch::search(*g, request(1, 4, 100, true), {&p, cost, h}), K::InvalidValue, F::RouteCost);
		}
		for (std::uint32_t at : {1U, 3U, 4U})
		{
			InvalidPolicy p{{1, 0, 0, 0}, bad, at};
			failure(NavRouteSearch::search(*g, request(1, 4, 100, true), {&p, cost, h}), K::InvalidValue,
					F::RouteHeuristic);
		}
	}
	InvalidPolicy nonzeroGoal{{1, 0, 0, 0}, 1, 4};
	failure(NavRouteSearch::search(*g, request(1, 4, 0, true), {&nonzeroGoal, cost, h}), K::InvalidValue,
			F::RouteHeuristic);
...
void metricSizeBoundaries()
{
	...
	assert((overflow == diagnostics::NavError{K::OffsetOverflow, 0, diagnostics::NavRecord::Route, F::None}));
...
int main()
{
	...
	static_assert(std::is_same_v<decltype(std::declval<NavGraph&>().area(0)), const model::NavAreaRecord&>);
	static_assert(std::is_same_v<decltype(std::declval<const NavGraph&>().area(0)), const model::NavAreaRecord&>);
	static_assert(std::is_same_v<decltype(std::declval<const NavGraph&>().edge(0)), const NavDirectedEdge&>);
	trivialAndDirected();
