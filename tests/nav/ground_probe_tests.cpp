...
auto index()
{
	auto mesh = route_test::snapshot(
		{{1, {{0, 0, 0}, {100, 100, 0}, 0, 0}, {}}, {2, {{0, 0, 100}, {100, 100, 100}, 100, 100}, {}}});
	auto r = query::NavSpatialIndex::build(mesh, {2, 3, 1000000});
	assert(r);
	return *r.value;
}
...
struct Script final : runtime::IWorldQueries
{
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		...
		if (q.kind == runtime::QueryKind::GroundedArea)
		{
			...
			{
				r.ground->area.reset();
				r.ground->floor->status = runtime::FloorObservationStatus::NavContainmentMissing;
			}
		}
		else if (q.kind == runtime::QueryKind::Floor)
		{
			...
			if (mode == 6)
				r.floor->normal.z = 0.1f;
			if (mode == 7)
				r.floor->height = std::numeric_limits<float>::quiet_NaN();
			if (mode == 8)
				r.floor->height = 10; // support exists but NAV floor disagrees
			if (mode == 13 && q.end.x == 36)
				r.floor->height = 10; // Intermediate NAV miss only.
		}
...
void successReplayAndFloors()
{
	...
	Script wrong;
	assert(local::GroundProbe::inspect(s, 7, {2}, 52, 50, *index(), {4}, wrong, limits).reason ==
		   local::ProbeReason::FloorHeightMismatch);
}
void failures()
{
	const local::ProbeReason reasons[] = {local::ProbeReason::None,
										  local::ProbeReason::StaleQuery,
										  local::ProbeReason::QueryFailed,
										  local::ProbeReason::QueryUnavailable,
										  local::ProbeReason::UnsafeDrop,
										  local::ProbeReason::ActorNotGrounded,
										  local::ProbeReason::UnsupportedFloor,
										  local::ProbeReason::InvalidResult,
										  local::ProbeReason::NoArea,
										  local::ProbeReason::Blocked,
										  local::ProbeReason::Blocked,
										  local::ProbeReason::InvalidResult};
	for (int mode = 1; mode <= 11; ++mode)
	{
		...
		Script p;
		p.mode = mode;
		const auto r = inspect(p);
		assert(!r && !r.target && r.reason == reasons[mode]);
		assert(p.calls.size() <= limits.maxQueries);
	}
	Script physicalOnly;
	physicalOnly.mode = 8;
				   ...
				   local::GroundProbeLimits{9, 1, 64, 16, 18, 18, 64, 4, 2, 0.7}})
	{
		Script p;
		auto r = inspect(p, l);
		assert(r.reason == local::ProbeReason::BudgetExceeded && p.calls.empty());
	}
	Script p;
	auto l = limits;
	l.sampleSpacing = 0;
	...
	l = limits;
	l.maxStepUp = std::numeric_limits<double>::max();
	assert(inspect(p, l).reason == local::ProbeReason::InvalidInput && p.calls.size() == 1);
	p.calls.clear();
	auto s = actor();
	s.grounded = false;
	assert(local::GroundProbe::inspect(s, 7, {1}, 52, 50, *index(), {4}, p, limits).reason ==
		   local::ProbeReason::ActorGroundFlagMismatch);
	s = actor();
	s.kind = runtime::ActorKind::Human;
	assert(local::GroundProbe::inspect(s, 7, {1}, 52, 50, *index(), {4}, p, limits).reason ==
		   local::ProbeReason::InvalidInput);
	assert(local::GroundProbe::inspect(actor(), 7, {1}, 52, 50, *index(), {3}, p, limits).reason ==
		   local::ProbeReason::StaleNavigation);
...
struct Stairs final : runtime::IWorldQueries
{
	...
	{
		calls.push_back(q);
		assert(q.stamp.ordinal == calls.size() && q.navTolerance == 18);
		runtime::WorldQueryResult r;
		r.stamp = q.stamp;
		r.kind = q.kind;
		r.error = runtime::QueryError::None;
		if (q.kind == runtime::QueryKind::GroundedArea)
			r.ground =
				runtime::GroundedAreaObservation{model::NavAreaId{1}, runtime::FloorObservation{from, {0, 0, 1}, true}};
		else if (q.kind == runtime::QueryKind::Floor)
			r.floor = runtime::FloorObservation{to, {0, 0, 1}, true};
		else
		{
			r.hull = runtime::HullObservation{1, q.end, {0, 0, 0}, false};
			if (q.stamp.ordinal == 3 || q.stamp.ordinal == static_cast<std::uint32_t>(block))
				r.hull->fraction = 0.5f;
			if (q.stamp.ordinal == static_cast<std::uint32_t>(stale))
				++r.stamp.tick.value;
		}
		return r;
	}
...
void stairProbes()
{
	...
	Stairs boundary;
	boundary.to = 18;
	assert(run(boundary, l));
	const auto d = run(down, l);
	assert(d && d.steps == 1 && d.queries == 6 && d.target->origin.z == 36);
	for (int ordinal : {4, 5, 6})
	{
		Stairs p;
		p.block = ordinal;
		const auto failure = run(p, l);
		assert(!failure && failure.reason == local::ProbeReason::Blocked && failure.steps == 0);
		assert(failure.queries == static_cast<std::uint32_t>(ordinal));
	}
	Stairs stale;
	stale.stale = 4;
	assert(run(stale, l).reason == local::ProbeReason::StaleQuery);
	Stairs budget;
	auto small = l;
	small.maxQueries = 5;
	const auto exhausted = run(budget, small);
	assert(exhausted.reason == local::ProbeReason::BudgetExceeded && exhausted.queries == 3);
	// A riser can block the hull while the sampled floor is still level.
	Stairs flat;
	flat.to = 0;
	const auto levelStep = run(flat, l);
	...
	Stairs high;
	high.to = 19;
	assert(run(high, l).reason == local::ProbeReason::InvalidResult && high.calls.size() == 2);
}
} // namespace
...
{
	auto s = actor();
	s.position = model::NavVector3{44, 50, 54};
	Script p;
	p.height = 18;
	auto allowance = limits;
	allowance.navTolerance = 18;
	...
	Script budget;
	allowance.maxQueries = 1;
	assert(local::inspectGroundFrame(s, 7, {1}, {1}, 46, 50, *index(), s.map, budget, allowance).reason ==
		   local::ProbeReason::BudgetExceeded);
}
int main()
{
	intermediateNavMissKeepsPhysicalTarget();
	frameGround();
	successReplayAndFloors();
	failures();
	stairProbes();
	Script port;
	auto s = actor();
