...
struct World final : runtime::IWorldQueries
{
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		++calls;
		last = q;
		assert(q.kind == runtime::QueryKind::Clearance && q.start == q.end && q.hull);
...
void hints()
{
	using T = model::NavTraversalKind;
	using R = local::ConstraintReason;
	...
	for (unsigned bit : {16U, 32U, 64U, 128U})
		assert(local::constraints(T::Walk, 0, static_cast<std::uint8_t>(bit)).reason == R::UnknownAttributes);
	for (auto kind : {T::Ladder, T::Drop, static_cast<T>(255)})
		assert(local::constraints(kind, 0, 0).reason == R::UnsupportedTraversal);
}
void posture()
{
	for (std::uint64_t frame : {8000U, 16000U, 100000U})
	{
		auto s = actor();
		World world;
		local::Crouch crouch(binding, limits);
		...
		++s.tick.value;
		s.ducked = true; // flag without observed crouched hull is insufficient
		d = crouch.update(s, true, frame, world, 0, 1);
		assert(!d.movementAllowed && d.queries == 0);
		++s.tick.value;
		s.hull = limits.crouched;
		s.position->z = 18;
		d = crouch.update(s, true, 2 * frame, world, 0, 1);
		assert(d.movementAllowed && d.state == local::CrouchState::Crouched &&
			   d.intent.duck == core::ActionRequest::Hold);
		...
		assert(!d.movementAllowed && d.reason == local::CrouchReason::Blocked &&
			   d.intent.duck == core::ActionRequest::Hold);
...
void failures()
{
	for (int mode = 0; mode < 8; ++mode)
	{
		...
		World world;
		local::Crouch crouch(binding, limits);
		auto d = crouch.update(s, true, 0, world, 0, 1);
		assert(d.accepted);
		++s.tick.value;
		s.ducked = true;
		s.hull = limits.crouched;
		...
		{
			d = crouch.abort();
			assert(d.terminalEvent && !crouch.abort().terminalEvent);
		}
		else
			d = crouch.update(s, false, 2, world, 0, mode == 7 ? 0U : 1U);
		if (mode == 4)
		{
			assert(d.state == local::CrouchState::Raising);
			++s.tick.value;
			d = crouch.update(s, false, 2 + limits.transitionTimeoutUs, world, 0, 1);
			assert(d.reason == local::CrouchReason::TimedOut);
		}
		assert(d.terminalEvent && !d.movementAllowed && d.intent.speed == 0 &&
			   d.intent.duck == core::ActionRequest::Hold);
