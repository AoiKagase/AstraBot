...
void testLittleEndianReads()
{
	const std::uint8_t bytes[] = {
		0x7FU, 0x34U, 0x12U, 0x78U, 0x56U, 0x34U, 0x12U, 0x00U, 0x00U, 0x80U, 0x3FU,
	};
...
void testRangeOverflow()
{
	...
	const auto overflow =
		reader.readBytes(std::numeric_limits<std::size_t>::max(), NavRecord::Area, NavField::RawBytes);
	assert(!overflow);
...
void testNonFiniteFloatDoesNotAdvance()
{
	...
	const auto result = reader.readF32LE(NavRecord::Area, NavField::NorthEastZ);
...
void testDeterministicErrors()
{
	...
	const auto firstResult = first.readU32LE(NavRecord::Connection, NavField::ConnectionAreaId);
	const auto secondResult = second.readU32LE(NavRecord::Connection, NavField::ConnectionAreaId);
