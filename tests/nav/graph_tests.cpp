...
void chargeBoundaries()
{
	...
	const auto checkFailure = [](std::size_t before, std::size_t count, std::size_t size, std::size_t cap, K kind) {
		auto used = before;
		const auto e = query::detail::charge(used, count, size, cap);
		assert((e == diagnostics::NavError{kind, 0, diagnostics::NavRecord::Graph, diagnostics::NavField::GraphBytes}));
		assert(used == before);
	};
...
void constructionShapeSweeps()
{
	...
	for (const auto& areas :
		 {std::vector<route_test::Area>{{1, extent}, {2, extent}}, std::vector<route_test::Area>{{1, extent}}, dense})
	{
		...
		const auto checkRetained = [&] {
			...
			for (std::size_t i = 0; i < count; ++i)
			{
				...
				for (auto edge = previous->edgeBegin(i); edge < previous->edgeEnd(i); ++edge)
				{
					...
					if (target >= e.source.value)
						++target; // loader forbids self edges
					assert(e.target.value == target);
				}
			}
			if (edges != 0)
				assert(snap->areas()[0].connections[0][0].target == model::NavAreaId{16});
		};
		...
		assert((under.error == diagnostics::NavError{K::CountLimitExceeded, 0, diagnostics::NavRecord::Graph,
		...
		for (std::size_t n = 0; n < 64; ++n)
		{
			...
			assert((attempt.error == diagnostics::NavError{K::AllocationFailure, 0, diagnostics::NavRecord::Graph,
														   diagnostics::NavField::GraphBytes}));
		}
		assert(succeeded);
	}
...
void runtimeOverlayAugment()
{
	...
	links.links.push_back({0x415354524153484FULL,
						   1,
						   1,
						   {1},
						   {2},
...
int main()
{
	...
	route_test::Area huge{8, {{-maximum, -maximum, maximum}, {maximum, maximum, maximum}, maximum, maximum}};
