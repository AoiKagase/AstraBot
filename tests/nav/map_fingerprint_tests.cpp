...
void check(const std::string& value, const char* expected)
{
	for (auto limit : {static_cast<std::uint64_t>(value.size()), std::uint64_t{2000000}})
	{
		...
		const auto r = fingerprintMap(input, limit);
...
int main()
{
	...
	{
		std::istringstream input(std::string(4097, 'a'));
		const auto r = fingerprintMap(input, cap);
	...
	std::istringstream limit("abc");
	assert(fingerprintMap(limit, 512ULL * 1024 * 1024 + 1).reason == FingerprintReason::InvalidLimit);
	assert(limit.tellg() == 0);
	FailingBuffer buffer;
	std::istream interrupted(&buffer);
