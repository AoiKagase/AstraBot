...
struct Fixture
{
	...
	void integer(std::uint32_t value, std::size_t width, R record, F field, bool floating = false)
	{
...
{
	return {65536, {128, 64, 256, 16384}, {128, 128, 64, 64, 64, 64, 4096, 4096, 4096, 4096, 4096}, 4194304};
}
...
inline Fixture fixture(unsigned version, bool full)
{
	...
	if (version >= 4)
		f.integer(123, 4, R::FileHeader, F::BspSize);
	if (version == 5)
	{
	...
	for (unsigned id = 1; id <= (full ? 2U : 1U); ++id)
	{
		...
		for (float v : {x + 2, 2.f, z})
			f.real(v, R::Area, F::SouthEastExtent);
		f.real(z, R::Area, F::NorthEastZ);
		f.real(z, R::Area, F::SouthWestZ);
		for (unsigned direction = 0; direction < 4; ++direction)
		{
			f.integer(full ? 1U : 0U, 4, R::Connection, F::ConnectionCount);
			if (full)
				f.integer(3U - id, 4, R::Connection, F::ConnectionAreaId);
		}
		f.integer(full ? 1U : 0U, 1, R::HidingSpot, F::HidingSpotCount);
		if (full)
		{
			if (version >= 2)
				f.integer(100 + id, 4, R::HidingSpot, F::HidingSpotId);
			for (float v : {x + 1, 1.f, z})
				f.real(v, R::HidingSpot, F::RawBytes);
			if (version >= 2)
				f.integer(0xFE, 1, R::HidingSpot, F::HidingSpotFlags);
		}
		...
		if (full && version <= 2)
		{
			...
			for (unsigned i = 0; i < 6; ++i)
				f.real(0, R::Encounter, F::RawBytes);
			f.integer(1, 1, R::Encounter, F::EncounterSpotCount);
			for (float v : {1.f, 2.f, 3.f, .5f})
				f.real(v, R::Encounter, F::RawBytes);
		}
		else if (full)
		{
		...
		if (version == 5)
			f.integer(full ? 1U : 0U, 2, R::Area, F::Place);
	}
	return f;
}
} // namespace evidence
