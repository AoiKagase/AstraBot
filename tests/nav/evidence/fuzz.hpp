...
inline Mutation mutate(std::size_t index)
{
	...
	Mutation m{fixture(version, full).bytes, "v" + std::to_string(version) + (full ? "-full" : "-minimal")};
	...
	for (unsigned step = 0; step < steps; ++step)
	{
		...
		m.recipe += ";" + std::to_string(op) + ":" + std::to_string(pos) + ":" + std::to_string(value);
		...
		if (op == 2 && m.bytes.size() < 65536)
			m.bytes.insert(m.bytes.begin() + static_cast<std::ptrdiff_t>(pos), static_cast<std::uint8_t>(value));
...
inline void byteOracle(const Bytes& b, std::size_t index)
{
	...
	for (unsigned step = 0; step < 256; ++step)
	{
		...
		if (enough && op < 4)
		{
			...
			{
				bits += b[offset + j] * scale;
				scale *= 256U;
			}
		}
		bool success = false;
		nav::diagnostics::NavError error;
		const auto scalar = [&](const auto& result, auto expected) {
			success = bool(result);
			error = result.error;
			if (success)
				check(*result.value == expected, "independent scalar decode");
			else
				check(!result.value, "reader partial result");
		};
		float real = 0;
		std::memcpy(&real, &bits, 4);
		if (op == 0)
			scalar(reader.readU8(), bits);
		if (op == 1)
			scalar(reader.readU16LE(), bits);
		if (op == 2)
			scalar(reader.readU32LE(), bits);
		if (op == 3)
			scalar(reader.readF32LE(), real);
		if (op == 4)
		{
			...
			if (success)
			{
				...
				check(!r.value, "slice partial result");
		}
		const K expected = !enough ? K::EndOfInput : op == 3 && !std::isfinite(real) ? K::NonFiniteFloat : K::None;
		check(success == (expected == K::None), "reader success");
		if (success)
		{
			check(error.isNone(), "success error");
			offset += width;
		}
		else
			check(error == nav::diagnostics::NavError{expected, offset, R::RawInput, F::RawBytes},
				  "reader exact error");
		check(reader.offset() == offset && reader.remaining() == b.size() - offset &&
				  reader.atEnd() == (offset == b.size()),
			  "reader position");
	}
}
...
public:
	explicit CandidateJournal(const std::filesystem::path& dir)
		: bytes_(dir / "current.case", std::ios::binary | std::ios::trunc), recipe_(dir / "replay.txt", std::ios::trunc)
	{
	}
	void save(const Mutation& m, std::size_t index)
	{
		...
		bytes_.write(reinterpret_cast<const char*>(m.bytes.data()), static_cast<std::streamsize>(m.bytes.size()));
		bytes_.flush();
		check(bool(bytes_), "write candidate");
		recipe_.seekp(0);
		recipe_ << "seed=0xA208 case=" << index << "\n" << m.recipe << "\nEND\n";
		recipe_.flush();
		check(bool(recipe_), "write recipe");
	}
};
inline void runFuzz(std::size_t count, const std::filesystem::path& requested, std::size_t first = 0)
{
