...
inline std::string snapshotState(const nav::model::NavMeshSnapshot& mesh)
{
	...
	std::size_t logical = sizeof(nav::model::NavMeshSnapshot) + areas.size() * sizeof(nav::model::NavAreaRecord) +
	...
	for (const auto& s : h.places)
	{
		...
		put(s.size());
		for (char c : s)
			put(static_cast<unsigned>(static_cast<unsigned char>(c)));
	}
	check(placeBytes <= 16384, "Place aggregate");
	logical += placeBytes;
	...
	for (const auto& a : areas)
	{
		...
		{
			check(s.id.has_value() == (version >= 2) && s.flags.has_value() == (version >= 2), "hiding version");
			if (s.id)
				check(hidingIds.insert(*s.id).second, "unique hiding");
		}
	}
	...
	{
		put(a.id.value);
		put(unsigned(a.attributes));
		point(a.extent.northWest);
		point(a.extent.southEast);
		check(std::isfinite(a.extent.northEastZ) && std::isfinite(a.extent.southWestZ), "corner Z");
		put(a.extent.northEastZ);
		put(a.extent.southWestZ);
		check(a.extent.northWest.x < a.extent.southEast.x && a.extent.northWest.y < a.extent.southEast.y, "rectangle");
		for (const auto& direction : a.connections)
		{
			check(direction.size() <= 128, "direction count");
			put(direction.size());
			connections += direction.size();
			...
			for (const auto& c : direction)
			{
				...
				put(c.target.value);
				put(unsigned(c.traversal));
			}
		}
		check(a.hidingSpots.size() <= 64 && a.approaches.size() <= 64 && a.encounters.size() <= 64, "nested count");
		hiding += a.hidingSpots.size();
		approaches += a.approaches.size();
		...
		for (const auto& p : a.approaches)
		{
			for (auto id : {p.here, p.previous, p.next})
				if (id.value != 0)
					reference(id);
			put(p.here.value);
			put(p.previous.value);
			put(p.next.value);
			put(unsigned(p.previousToHereHow));
			put(unsigned(p.hereToNextHow));
		}
		...
		for (const auto& e : a.encounters)
		{
			...
			put(e.from.value);
			put(e.to.value);
			put(unsigned(e.fromDirection));
			put(unsigned(e.toDirection));
			put(e.spots.size());
			spots += e.spots.size();
		...
		if (a.place)
		{
			check(*a.place <= h.places.size(), "area Place");
			put(*a.place);
		}
	}
	check(connections <= 4096 && hiding <= 4096 && approaches <= 4096 && encounters <= 4096 && spots <= 4096,
		  "aggregate counts");
	logical += connections * sizeof(nav::model::NavConnection) + hiding * sizeof(nav::model::NavHidingSpot) +
			   approaches * sizeof(nav::model::NavApproachRecord) +
