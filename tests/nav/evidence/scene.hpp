...
inline Scene scene(std::uint32_t count)
{
	...
	for (std::uint32_t i = 0; i < count; ++i)
	{
		...
		s.patches.push_back(
			{i + 1, float(i % 4U) * 3.f, float((i / 4U) % 4U) * 3.f, z, z + float(i % 3U), z - float(i % 2U), z + 2});
		if (i + 1 < count)
			s.arcs.push_back({i + 1, i + 2, 1});
		if (i + 4 < count)
			s.arcs.push_back({i + 1, i + 5, 2});
		if (i % 3 == 0 && i > 0)
			s.arcs.push_back({i + 1, i, 0});
	}
	return s;
}
inline Fixture encode(const Scene& s)
{
	...
	{
		const auto& p = *it;
		f.integer(p.id, 4, R::Area, F::AreaId);
		f.integer(0, 1, R::Area, F::Attributes);
		for (float v : {p.x, p.y, p.z00})
			f.real(v, R::Area, F::NorthWestExtent);
		for (float v : {p.x + 2, p.y + 2, p.z11})
			f.real(v, R::Area, F::SouthEastExtent);
		f.real(p.z10, R::Area, F::NorthEastZ);
		f.real(p.z01, R::Area, F::SouthWestZ);
		for (unsigned d = 0; d < 4; ++d)
		{
			const auto count = std::count_if(s.arcs.begin(), s.arcs.end(),
											 [&](const Arc& a) { return a.from == p.id && a.direction == d; });
...
inline std::optional<Match> linear(const Scene& s, nav::model::NavVector3 q, double radius, double vertical,
								   bool containing)
{
	...
	for (const auto& p : s.patches)
	{
		...
		if (std::abs(dz) > vertical || (!containing && std::sqrt(squared) > radius))
			continue;
	...
	for (std::size_t n = 0; n < s.patches.size(); ++n)
	{
		...
		for (std::size_t i = 0; i < s.patches.size(); ++i)
			if (!visited[i] && (selected == s.patches.size() || distance[i] < distance[selected]))
				selected = i;
		if (selected == s.patches.size() || !std::isfinite(distance[selected]))
			break;
		visited[selected] = true;
		for (const auto& arc : s.arcs)
			if (arc.from == s.patches[selected].id)
			{
				const auto to =
					std::find_if(s.patches.begin(), s.patches.end(), [&arc](const Patch& p) { return p.id == arc.to; });
...
{
	return std::abs(a - b) <= 1e-10 * std::max({1., std::abs(a), std::abs(b)});
}
} // namespace evidence
