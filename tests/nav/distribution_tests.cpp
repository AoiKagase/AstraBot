...
std::shared_ptr<const q::DistributionTopology> topology(const std::vector<route_test::Area>& areas, unsigned map = 1)
{
	...
	auto index = q::NavSpatialIndex::build(mesh, {1000, 1999, 1000000});
	assert(graph && index);
	auto result = q::DistributionTopology::build({map}, *graph.value, *index.value);
	assert(result);
	return result;
}
struct Fixture
{
	...
	explicit Fixture(std::shared_ptr<const q::DistributionTopology> t, unsigned observers = 1) : nav(std::move(t))
	{
		...
		for (std::uint16_t i = 1; i <= observers; ++i)
			frame.players[i - 1] = {{i, {1}}, {i}, true};
	...
	void step(std::uint64_t dt, bool see = false, double x = 10)
	{
		++frame.tick.value;
		frame.timeMicros += dt;
		assert(world->advance(frame));
		if (see)
			for (const auto& player : frame.players)
				if (player.agent.isValid())
				{
					p::ObservationBatch batch{};
					batch.stamp = {player.agent, player.player, frame.map, frame.tick, frame.timeMicros, frame.round};
	...
	const w::PositionDistribution* distribution(PlayerId actor = observer) const
	{
		...
		assert(s && s->visual->count == 1);
		return s->distributions[0];
	}
};
double weight(const w::PositionDistribution& d, unsigned id)
{
	double sum = d.unknownMass;
	for (std::size_t i = 0; i < d.count; ++i)
		sum += d.areas[i].weight;
	assert(std::abs(sum - 1) < 1e-12);
	for (std::size_t i = 0; i < d.count; ++i)
		if (d.areas[i].area == id)
			return d.areas[i].weight;
	return 0;
}
void branchAndCycle()
{
	...
	a.targets[1] = {3, 2};
	Fixture f(topology({c, a, b}));
	f.step(0, true);
	assert(f.distribution()->available && weight(*f.distribution(), 1) == 1);
	f.step(199999);
	assert(weight(*f.distribution(), 1) == 1);
	f.step(1);
	assert(weight(*f.distribution(), 1) == 0.5 && weight(*f.distribution(), 2) == 0.25 &&
		   weight(*f.distribution(), 3) == 0.25);
	f.step(200000);
	assert(weight(*f.distribution(), 1) == 0.25 && weight(*f.distribution(), 2) == 0.375);
	f.step(1, true, 210);
	assert(weight(*f.distribution(), 2) == 1 && f.distribution()->count == 1);
	f.step(600000);
	assert(weight(*f.distribution(), 2) == 1); // isolated area and no reverse edge
	b.targets[3] = {1};
	a.targets[1] = {2};
	f.nav = topology({b, a});
	f.step(0, true);
	f.step(400000);
	assert(weight(*f.distribution(), 1) == 0.5 && weight(*f.distribution(), 2) == 0.5);
}
void truncationAndFairness()
{
	...
	for (unsigned i = 2; i <= 1000; ++i)
		areas[0].targets[1].push_back(i);
	Fixture f(topology(areas), 16);
	f.step(0, true);
	f.step(200000);
	...
	for (std::uint16_t i = 1; i <= 16; ++i)
		assert(f.distribution({i, {1}})->updatedMicros == 1000000);
	for (unsigned i = 0; i < 70; ++i)
		f.step(1000);
	for (std::uint16_t i = 1; i <= 16; ++i)
	{
		const auto* d = f.distribution({i, {1}});
		assert(d && d->count == 32 && d->updatedMicros == 1200000);
...
{
	auto a = area(1), b = area(2), c = area(3);
	a.targets[1] = reverse ? std::vector<std::uint32_t>{3, 2} : std::vector<std::uint32_t>{2, 3};
	b.targets[0] = {1};
	c.targets[0] = {2};
...
void pendingRetirement()
{
	...
	--bad.timeMicros;
	++bad.tick.value;
	assert(!f.world->advance(bad));
	f.model->update(*f.world, f.nav);
	assert(!f.world->latest(observer));
	f.step(1);
	assert(f.world->latest(observer)->visual->count == 0);
}
void retirement()
{
	Fixture f({});
	f.step(0, true);
	assert(!f.distribution());
	f.nav = topology({area(1)});
	f.step(1);
	assert(f.distribution()->available);
	auto invalid = *f.distribution();
	const auto identity = f.world->latest(observer)->visual->memories[0].identity;
	invalid.areas[0].weight = 0.5;
	assert(!f.world->setDistribution(observer, target, identity, invalid));
	invalid = *f.distribution();
	invalid.updatedMicros = f.frame.timeMicros + 1;
	assert(!f.world->setDistribution(observer, target, identity, invalid));
	const auto revision = f.distribution()->navRevision;
	f.nav = topology({area(2)});
	f.step(1);
	assert(!f.distribution()->available && f.distribution()->navRevision != revision);
	...
	f.step(1, true);
	assert(!f.distribution());
	f.nav = topology({area(1)}, 2);
	f.step(1);
	assert(f.distribution()->available);
	f.frame.players[0].player.generation = {2};
	f.step(1);
	assert(!f.world->latest(observer));
}
int main()
