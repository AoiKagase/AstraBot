...
struct World final : runtime::IWorldQueries
{
	...
	runtime::WorldQueryResult query(const runtime::QueryRequest& q) override
	{
		...
		else
		{
			...
			if (stairs && q.end.x > q.start.x && q.end.z != q.start.z)
				r.hull->fraction = 0.5f;
			// Physical head clearance blocks the upward leg.
			if (ceiling && q.end.z > q.start.z && q.start.x == q.end.x)
				r.hull->fraction = 0.25f;
		}
		return r;
	}
};
...
void boundedNavBoundaryFallback()
{
	...
	auto mesh =
		route_test::snapshot({{1, {{0, 0, 0}, {30, 100, 0}, 0, 0}, {}}, {2, {{40, 0, 0}, {100, 100, 0}, 0, 0}, {}}});
	auto gaps = query::NavSpatialIndex::build(mesh, {2, 3, 1000000});
	assert(gaps);
	...
	auto raised = query::NavSpatialIndex::build(mesh, {1, 1, 1000000});
	assert(raised);
	auto strict = limits;
	strict.navTolerance = 2;
