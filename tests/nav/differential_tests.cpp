...
{
	for (double vertical : {0., 1., 20.})
		for (double radius : {0., 2., 100.})
		{
			for (bool containing : {true, false})
			{
				const auto expected = linear(s, point, radius, vertical, containing);
				const auto actual =
					containing ? index.containing(point, vertical) : index.nearestGeometry(point, {radius, vertical});
				...
				if (expected)
				{
					...
					check(close(a.projectedPoint.x, expected->point[0]) &&
							  ...
							  close(a.projectedPoint.z, expected->point[2]),
						  "projection");
				}
			}
		}
...
int main()
{
	...
	s.patches[1] = s.patches[0];
	s.patches[1].id = 2;
	s.arcs.erase(std::remove_if(s.arcs.begin(), s.arcs.end(), [](const Arc& a) { return a.from == 32 || a.to == 32; }),
				 s.arcs.end());
	...
	for (unsigned i = 0; i < 128; ++i)
		compare(s, **index.value, {float(i % 17) - 3, float((i * 7) % 19) - 4, float((i * 11) % 23) - 5});
	...
	for (std::size_t start = 0; start < s.patches.size(); ++start)
	{
		...
		{
			const auto actual = q::NavRouteSearch::search(
				**graph.value, {{s.patches[start].id}, {s.patches[goal].id}, {128, 4194304}, false});
			check(bool(actual), "route result");
			const auto& route = *actual.value;
			if (!std::isfinite(expected[goal]))
			{
				check(route.status == q::NavRouteStatus::Unreachable && route.areas.empty() && route.steps.empty(),
					  "unreachable");
				++unreachable;
				continue;
			}
			...
			check(!route.areas.empty() && route.areas.front().value == s.patches[start].id &&
					  route.areas.back().value == s.patches[goal].id && route.steps.size() + 1 == route.areas.size(),
				  "corridor");
			double sum = 0;
			for (std::size_t i = 0; i < route.steps.size(); ++i)
			{
				...
				check(arc != s.arcs.end() && !step.edge.external && step.edge.source == route.areas[i] &&
						  step.edge.target == route.areas[i + 1],
					  "route edge");
				...
				check(close(cost, step.total) && close(cost, step.components.distance) &&
						  ...
						  step.components.experience == 0,
					  "step components");
				sum += cost;
			}
			check(close(sum, route.total) && close(route.components.distance, sum) && route.components.traversal == 0 &&
					  route.components.danger == 0 && route.components.experience == 0,
				  "total components");
			++complete;
		}
	}
