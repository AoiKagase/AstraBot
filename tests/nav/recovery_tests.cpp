...
struct Replay
{
	Recovery recovery{};
	Binding b = binding();
	RecoveryEdge edge{{11}, {2036}};
	std::uint64_t tick = 1, now = 0;
	Replay()
	{
		assert(recovery.bindRoute(b, edge));
	}
...
void detection()
{
	Replay r;
	for (int i = 0; i < 4; ++i)
		assert(r.frame().state == RecoveryState::Monitoring);
	auto d = r.frame();
	assert(d.state == RecoveryState::Wait && d.commandedUs == 500000);
	assert(d.cause == StuckCause::Unknown && d.symptom == StuckSymptom::NoProgress);
	Replay crouch;
	for (int i = 0; i < 9; ++i)
		assert(crouch.frame({}, ExpectedProgress::Crouch).state == RecoveryState::Monitoring);
	assert(crouch.frame({}, ExpectedProgress::Crouch).state == RecoveryState::Wait);
	Replay progressing;
	for (int i = 0; i < 100; ++i)
		assert(progressing.frame({float(i), 0, 0}, ExpectedProgress::Crouch).state == RecoveryState::Monitoring);
	...
	Replay oscillating;
	for (int i = 0; i < 10; ++i)
		d = oscillating.frame({i % 2 ? -3.0f : 0.0f, 0, 0}, ExpectedProgress::Walk, true, {i % 2 ? -1.0 : 1.0, 0, 0},
							  50000);
	...
	for (int i = 0; i < 30; ++i)
	{
		...
		assert(alternating.recovery.report({alternating.b,
											{alternating.tick},
											{alternating.tick + 1},
											{start, 0, 0},
											{i % 2 ? -1.0 : 1.0, 0, 0},
											100000,
											ExpectedProgress::Walk,
											true}));
		alternating.tick += 2;
		alternating.now += 100000;
		d = alternating.recovery.observe(alternating.b, {alternating.tick}, alternating.now, {end, 0, 0});
		detected |= d.state != RecoveryState::Monitoring;
	}
	assert(detected);
	Replay lateral;
	lateral.recovery.replanned();
	for (int i = 0; i < 30; ++i)
	{
...
void pausesAndStaleFeedback()
{
	for (auto expected : {ExpectedProgress::Pause, ExpectedProgress::Walk})
	{
		Replay r;
		for (int i = 0; i < 30; ++i)
			assert(r.frame({}, expected, expected == ExpectedProgress::Pause).state == RecoveryState::Monitoring);
		assert(r.recovery.decision().commandedUs == 0);
	}
	Replay r;
	r.frame();
	const auto saved = r.recovery.decision().commandedUs;
	auto wrong = r.b;
...
void finiteReplanAndReset()
{
	...
	for (const auto next : {RecoveryState::Sidestep, RecoveryState::Reverse, RecoveryState::Replan})
	{
		r.now = d.deadlineUs;
		r.tick += 2;
		d = r.recovery.observe(r.b, {r.tick}, r.now, {});
		assert(d.state == next);
	}
	r.recovery.replanned();
	++r.b.routeGeneration;
	assert(r.recovery.bindRoute(r.b));
	for (int i = 0; i < 4; ++i)
		assert(r.frame().state == RecoveryState::Monitoring);
	d = r.frame();
	assert(d.state == RecoveryState::Aborted && d.terminalEvent && d.attempts == 1);
	for (int i = 0; i < 10; ++i)
		assert(!r.frame().terminalEvent);
	// Even a further route replacement cannot resurrect a terminal goal.
	++r.b.routeGeneration;
	assert(r.recovery.bindRoute(r.b));
	assert(r.frame().state == RecoveryState::Aborted);
	Replay resumed;
	resumed.recovery.replanned();
	++resumed.b.routeGeneration;
	assert(resumed.recovery.bindRoute(resumed.b));
	resumed.frame();
	d = resumed.frame({5, 0, 0});
	assert(d.measuredProgress && d.attempts == 0);
	...
	const auto policy = plan.consume(binding(), {2}, 100000);
	...
	for (auto cause : {StuckCause::DoorBlocked, StuckCause::PlayerBlocked, StuckCause::GeometryBlocked,
					   StuckCause::TraversalFailed, StuckCause::Unknown})
	{
		Recovery terminal;
		assert(terminal.abort(cause).terminalEvent);
...
void edgeScopedBinding()
{
	...
	Replay changed;
	for (int i = 0; i < 4; ++i)
		assert(changed.frame().state == RecoveryState::Monitoring);
	...
	for (int i = 0; i < 4; ++i)
		assert(differentSource.frame().state == RecoveryState::Monitoring);
	...
	for (int i = 0; i < 5; ++i)
	{
		assert(rejected.recovery.report({rejected.b,
										 {rejected.tick},
										 {rejected.tick + 1},
										 {},
										 {1, 0, 0},
...
#endif
	detection();
	pausesAndStaleFeedback();
	finiteReplanAndReset();
	edgeScopedBinding();
}
