...
void rates()
{
	...
	{
		IntentPump p(binding());
		std::uint32_t decisions = 0, frames = 0, presses = 0;
		for (std::uint64_t t = 1; t <= 1000000 / delta; ++t)
		{
			auto s = snapshot(t, delta);
			auto schedule = p.beginFrame(s);
			assert(schedule.accepted);
			if (schedule.decisionDue)
			{
				++decisions;
				assert(p.publish(binding(), s.tick, moving()));
			}
			auto out = p.take();
			assert(out.emit && out.tick == s.tick && out.frameUs == delta);
			++frames;
		...
		if (delta <= 40000)
			assert(decisions >= 24 && decisions <= 25);
		else
			assert(decisions == frames);
	}
}
void freshness()
{
	IntentPump p(binding());
	auto s = snapshot(1, 10000);
	...
	auto out = p.take();
	assert(out.emit && out.intent.speed == 100 && !out.firstFrame && out.intentAgeUs == 120000);
	s = snapshot(3, 1);
	assert(p.beginFrame(s).accepted);
	out = p.take();
	assert(out.emit && out.reason == PumpReason::StaleIntent && out.intent.speed == 0 &&
		   out.intent.jump == ActionRequest::None);
	s = snapshot(4, 40000);
	assert(p.beginFrame(s).decisionDue);
	assert(p.publish(binding(), s.tick, moving()));
	assert(p.take().intent.speed == 100);
	p.submissionRejected();
	s = snapshot(5, 1);
	assert(p.beginFrame(s).accepted);
	out = p.take();
	assert(out.emit && out.intent.speed == 0 && out.reason == PumpReason::SubmissionRejected);
	s = snapshot(6, 0);
	assert(p.beginFrame(s).accepted && !p.take().emit);
	s = snapshot(7, 1000000);
	schedule = p.beginFrame(s);
...
void invalidation()
{
	...
	auto bad = moving();
	bad.speed = -1;
	assert(!p.publish(binding(), s.tick, bad));
	assert(p.take().reason == PumpReason::InvalidIntent);
	s = snapshot(2, 10000);
	s.alive = false;
	assert(!p.beginFrame(s).accepted && !p.take().emit);
	s = snapshot(3, 10000);
	assert(!p.beginFrame(s).accepted); // retired route never revives
...
void provedLifetime()
{
	...
	auto out = p.take();
	assert(out.intentLifetimeUs == 20000 && out.intent.locomotion == astrabot::core::LocomotionMode::Run);
	s = snapshot(2, 10000);
	assert(p.beginFrame(s).accepted);
	out = p.take();
	...
	assert(p.beginFrame(s).accepted);
	out = p.take();
	assert(out.reason == PumpReason::StaleIntent && out.intent.locomotion == astrabot::core::LocomotionMode::Explicit &&
		   out.intent.speed == 0);
}
} // namespace
int main()
