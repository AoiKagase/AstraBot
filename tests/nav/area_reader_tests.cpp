...
void overwriteU32LE(std::vector<std::uint8_t>& bytes, std::size_t offset, std::uint32_t value)
{
...
NavAreaReadLimits normalLimits()
{
	return NavAreaReadLimits{
		1U, 2U, 2U, 2U, 2U, 2U, 4U, 2U, 2U, 2U, 2U,
	};
}
...
void assertTruncatedAt(const std::vector<std::uint8_t>& bytes, NavVersion version, std::size_t offset, NavField field)
{
	const auto result = NavAreaReader::read(ByteView{bytes.data(), offset}, version, 1U, normalLimits());
	assert(!result);
...
void testPublicAreaRecordContract()
{
	const NavAreaRecord area{
		NavAreaId{7U}, 0x05U, NavExtent{}, {}, {}, {}, {}, std::optional<std::uint16_t>{static_cast<std::uint16_t>(3U)},
	};
	assert(area.id == NavAreaId{7U});
	assert(area.place.has_value() && *area.place == 3U);

	const auto result = NavAreaReader::read(ByteView{nullptr, 0U}, NavVersion::V1, 0U, NavAreaReadLimits{});
	assert(!result);
}
...
void testV1AreaBaseAndConnections()
{
	...
	const auto result = NavAreaReader::read(ByteView{bytes.data(), bytes.size()}, NavVersion::V1, 1U, normalLimits());
	assert(result);
...
void testV1AreaBaseTruncation()
{
	...
	const std::vector<std::pair<std::size_t, NavField>> fields{
		{0U, NavField::AreaId},			  {4U, NavField::Attributes},		 {5U, NavField::NorthWestExtent},
		{9U, NavField::NorthWestExtent},  {13U, NavField::NorthWestExtent},	 {17U, NavField::SouthEastExtent},
		{21U, NavField::SouthEastExtent}, {25U, NavField::SouthEastExtent},	 {29U, NavField::NorthEastZ},
		{33U, NavField::SouthWestZ},	  {37U, NavField::ConnectionCount},	 {41U, NavField::ConnectionAreaId},
		{45U, NavField::ConnectionCount}, {49U, NavField::ConnectionAreaId}, {53U, NavField::ConnectionAreaId},
		{57U, NavField::ConnectionCount}, {61U, NavField::ConnectionCount},	 {65U, NavField::ConnectionAreaId},
	};
...
void testV2Hiding()
{
	...
	const auto result = NavAreaReader::read(ByteView{bytes.data(), bytes.size()}, NavVersion::V2, 1U, normalLimits());
	assert(result);
	assert(result.value->bytesConsumed == bytes.size());
...
void testHidingAndApproachTruncation()
{
	...
	const std::vector<std::pair<std::size_t, NavField>> v1Fields{
		{69U, NavField::HidingSpotCount},	{70U, NavField::RawBytes},			{74U, NavField::RawBytes},
		{78U, NavField::RawBytes},			{82U, NavField::ApproachCount},		{83U, NavField::ApproachAreaId},
		{87U, NavField::ApproachAreaId},	{91U, NavField::ApproachTraversal}, {92U, NavField::ApproachAreaId},
	...
	const std::vector<std::pair<std::size_t, NavField>> v2Fields{
		{69U, NavField::HidingSpotCount}, {70U, NavField::HidingSpotId}, {74U, NavField::RawBytes},
		{78U, NavField::RawBytes},		  {82U, NavField::RawBytes},	 {86U, NavField::HidingSpotFlags},
...
void testV3Encounter()
{
	...
	const auto result = NavAreaReader::read(ByteView{bytes.data(), bytes.size()}, NavVersion::V3, 1U, normalLimits());
	assert(result);
	assert(result.value->bytesConsumed == bytes.size());
...
void testV4AndV5PlaceEntries()
{
	...
	const auto v4Result = NavAreaReader::read(ByteView{v4.data(), v4.size()}, NavVersion::V4, 1U, normalLimits());
	assert(v4Result);
	...
	const auto v5Result = NavAreaReader::read(ByteView{v5.data(), v5.size()}, NavVersion::V5, 1U, normalLimits());
	assert(v5Result);
...
void testEncounterAndPlaceTruncation()
{
	...
	const std::vector<std::pair<std::size_t, NavField>> legacyFields{
		{71U, NavField::EncounterCount},  {75U, NavField::EncounterAreaId},
		{79U, NavField::EncounterAreaId}, {83U, NavField::RawBytes},
		{87U, NavField::RawBytes},		  {91U, NavField::RawBytes},
		{95U, NavField::RawBytes},		  {99U, NavField::RawBytes},
		{103U, NavField::RawBytes},		  {107U, NavField::EncounterSpotCount},
		{108U, NavField::RawBytes},		  {112U, NavField::RawBytes},
		{116U, NavField::RawBytes},		  {120U, NavField::RawBytes},
	};
	...
	const std::vector<std::pair<std::size_t, NavField>> modernFields{
		{71U, NavField::EncounterCount},  {75U, NavField::EncounterAreaId},	   {79U, NavField::EncounterDirection},
		{80U, NavField::EncounterAreaId}, {84U, NavField::EncounterDirection}, {85U, NavField::EncounterSpotCount},
		{86U, NavField::EncounterSpotId}, {90U, NavField::EncounterSpotT},	   {91U, NavField::EncounterSpotId},
...
void testEncounterLimits()
{
	...
	auto result = NavAreaReader::read(ByteView{modern.data(), modern.size()}, NavVersion::V3, 1U, noAreaEncounters);
	assert(!result);
	...
	result = NavAreaReader::read(ByteView{modern.data(), modern.size()}, NavVersion::V3, 1U, noTotalEncounters);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	assert(result.error.offset == 71U);
	assert(result.error.field == NavField::EncounterCount);
	...
	result = NavAreaReader::read(ByteView{modern.data(), modern.size()}, NavVersion::V3, 1U, oneSpot);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	...
	result = NavAreaReader::read(ByteView{modern.data(), modern.size()}, NavVersion::V3, 1U, oneTotalSpot);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	assert(result.error.offset == 85U);
	assert(result.error.field == NavField::EncounterSpotCount);
...
void testGlobalLimitsAndTransactionalFailures()
{
	...
	const auto zeroArea = NavAreaReader::read(ByteView{base.data(), base.size()}, NavVersion::V1, 0U, normalLimits());
	assert(!zeroArea);
	...
	const auto tooManyAreas =
		NavAreaReader::read(ByteView{base.data(), base.size()}, NavVersion::V1, 2U, normalLimits());
	assert(!tooManyAreas);
	...
	auto result = NavAreaReader::read(ByteView{base.data(), base.size()}, NavVersion::V1, 1U, noConnections);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	...
	result = NavAreaReader::read(ByteView{base.data(), base.size()}, NavVersion::V1, 1U, noTotalConnections);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	assert(result.error.offset == 37U);
	assert(result.error.field == NavField::ConnectionCount);
	...
	result = NavAreaReader::read(ByteView{hiding.data(), hiding.size()}, NavVersion::V1, 1U, noHiding);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	...
	result = NavAreaReader::read(ByteView{hiding.data(), hiding.size()}, NavVersion::V1, 1U, noTotalHiding);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	assert(result.error.offset == 69U);
	assert(result.error.field == NavField::HidingSpotCount);
	...
	result = NavAreaReader::read(ByteView{hiding.data(), hiding.size()}, NavVersion::V1, 1U, noApproaches);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	...
	result = NavAreaReader::read(ByteView{hiding.data(), hiding.size()}, NavVersion::V1, 1U, noTotalApproaches);
	assert(!result);
	assert(result.error.kind == NavErrorKind::CountLimitExceeded);
	assert(result.error.offset == 82U);
	assert(result.error.field == NavField::ApproachCount);

	const auto invalidBytes = NavAreaReader::read(ByteView{nullptr, 1U}, NavVersion::V1, 1U, normalLimits());
	assert(!invalidBytes);
	...
	const auto nonFiniteResult =
		NavAreaReader::read(ByteView{nonFinite.data(), nonFinite.size()}, NavVersion::V1, 1U, normalLimits());
	...
	const auto secondAreaFailure = NavAreaReader::read(ByteView{base.data(), base.size()}, NavVersion::V1, 2U,
													   NavAreaReadLimits{
														   2U,
														   2U,
														   2U,
	...
	const auto allocationResult =
		NavAreaReader::read(ByteView{allocationFailurePayload.data(), allocationFailurePayload.size()}, NavVersion::V1,
							1U, allocationLimits);
