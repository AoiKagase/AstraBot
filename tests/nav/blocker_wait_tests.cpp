...
namespace
{
Binding binding(std::uint16_t slot = 2)
{
	return {{1}, {slot, {1}}, {1}, 1, 0};
}
BlockerFeedback feedback(Binding b, std::uint64_t tick, std::uint64_t now)
{
	...
	f.observed.kind = runtime::QueryKind::Blocker;
...
void expiryAndBudget()
{
	...
	BlockerWait wait(b, {120, 200, 1000});
...
void priorityAndClear()
{
	...
	auto f = feedback(a, 1, 100);
	f.observed.blocker->player = b.actor;
	...
	BlockerWait geometry(b, {120, 200, 1000});
	f = feedback(b, 1, 0);
...
void invalidations()
{
	for (int mode = 0; mode < 11; ++mode)
	{
		...
		if (mode == 5)
			f.observed.blocker.reset();
		if (mode == 6)
			f.observed.blocker->player.reset();
		if (mode == 7)
			f.observed.blocker->player = b.actor;
		if (mode == 8)
			f.nowUs = 0;
		...
		if (mode == 10)
			f.observed.blocker->player = core::PlayerId{b.actor.slot, {2}};
		...
		if (mode == 9)
			assert(d.reason == BlockerReason::Unavailable);
		assert(!wait.abort().terminalEvent);
	}
	for (const auto limits : {BlockerLimits{0, 200, 1000}, BlockerLimits{120, 0, 1000}, BlockerLimits{120, 1000, 1000},
							  BlockerLimits{1001, 200, 1000}})
	{
...
{
	BlockerWait wait(binding(), {120, 200, 1000});
	std::vector<BlockerAction> trace;
