...
JumpFeedback feedback(runtime::MovementSnapshot s, std::uint64_t time)
{
	...
	proof.stamp = {s.agent, s.actor, s.map, s.tick, binding.routeGeneration, 0};
	proof.queries = 5;
	...
	if (s.grounded == true)
		proof.support = GroundedTarget{*s.position, area, {0, {0, 0, 1}, true}};
	...
	proof.validatedDistance = 32;
	proof.validForUs = 120000;
	f.inspection = proof;
...
core::TickId press(SimpleJump& jump, runtime::MovementSnapshot& s)
{
	...
	{
		s.tick = {i};
		const auto d = jump.update(feedback(s, i * 40000));
		assert(d.accepted);
		if (i == 3)
		{
			assert(d.intent.jump == ActionRequest::Press && d.state == JumpState::Takeoff);
			return d.pressTick;
		}
...
void physics()
{
	...
	{
		SimpleJump jump(binding, plan, limits);
		auto s = actor();
		s.view->y = 90;
		s.velocity = model::NavVector3{};
		s.position->x = 30;
		const double dt = double(us) / 1000000;
		bool airborne = false, landed = false, complete = false;
		unsigned presses = 0;
		...
		for (std::uint64_t tick = 1; tick < 600; ++tick)
		{
			...
			if (d.state == JumpState::Failed || d.state == JumpState::Aborted)
				std::fprintf(stderr, "jump us=%llu tick=%llu reason=%u pos=(%.3f,%.3f,%.3f)\n",
							 ...
							 unsigned(d.reason), s.position->x, s.position->y, s.position->z);
			...
			{
				assert(landed && d.terminalEvent);
				complete = true;
				break;
			}
			const auto motor = core::Motor::command(d.intent, {s.view->x, s.view->y, s.view->z}, 250, us, true);
			assert(motor);
			const auto replay = core::Motor::command(d.intent, {}, 250, us, false);
			assert(replay);
			...
			{
				assert(++presses == 1 && s.grounded == true);
				vz = 268.3281573;
				s.grounded = false;
		...
		assert(complete && airborne && presses == 1 &&
			   std::abs(s.position->x - plan.landing.x) <= limits.landingRadius);
...
void failures()
{
	for (bool dispatched : {false, true})
	{
		SimpleJump jump(binding, plan, limits);
		auto s = actor();
		const auto p = press(jump, s);
		s.tick = {4};
		auto f = feedback(s, 160000);
	...
	for (int mode = 0; mode < 10; ++mode)
	{
		...
		s.position->z = 45;
		auto f = feedback(s, 160000);
		f.dispatch = JumpDispatch{binding, p, {4}, true};
		if (mode == 0)
			f.dispatch.reset();
		if (mode == 1)
			f.dispatch->dispatched = false;
		if (mode == 2)
			++f.dispatch->commandTick.value;
		if (mode == 3)
		{
		...
		if (mode == 4)
		{
			++f.inspection->stamp.tick.value;
		}
		auto d = jump.update(f);
		if (mode < 5)
			assert(d.terminalEvent && (d.state == JumpState::Failed || d.state == JumpState::Aborted));
		else
		{
			assert(d.state == JumpState::Airborne);
			s.tick = {5};
			if (mode == 5)
				d = jump.update(feedback(s, 160000 + limits.airborneTimeoutUs));
			else if (mode == 6)
				d = jump.abort();
			else
			{
				s.grounded = true;
				s.position = model::NavVector3{134, 50, 36};
				f = feedback(s, 200000);
				if (mode == 7)
					f.inspection->support->area = {1};
				if (mode == 8)
					f.inspection->support.reset();
				d = jump.update(f);
				if (mode == 9)
				{
					assert(d.state == JumpState::Recover);
					++s.tick.value;
					s.grounded = false;
					d = jump.update(feedback(s, 240000));
					assert(d.reason == JumpReason::LostSupport);
				}
			}
	...
	for (int mode = 0; mode < 5; ++mode)
	{
		SimpleJump jump(binding, plan, limits);
		auto s = actor();
		auto f = feedback(s, 40000);
		assert(jump.update(f).state == JumpState::Align);
		++s.tick.value;
		assert(jump.update(feedback(s, 80000)).state == JumpState::Accelerate);
		++s.tick.value;
		f = feedback(s, 120000);
		if (mode == 0)
			f.inspection->flightClear = false;
		if (mode == 1)
			f.inspection->takeoffClear.reset();
		if (mode == 2)
			f.inspection->landingClear = false;
		if (mode == 3)
			f.inspection->origin.x++;
		if (mode == 4)
			f.movement.ducked = true;
		const auto d = jump.update(f);
		assert(d.state == JumpState::Failed && d.intent.jump != ActionRequest::Press);
	}
	auto forbidden = plan;
	forbidden.sourceAttributes = 8;
	SimpleJump noJump(binding, forbidden, limits);
	assert(noJump.update(feedback(actor(), 1)).reason == JumpReason::InvalidInput);
	SimpleJump noTakeoff(binding, plan, limits);
	auto s = actor();
	const auto p = press(noTakeoff, s);
	s.tick = {4};
	auto f = feedback(s, 120000 + limits.takeoffTimeoutUs);
	f.dispatch = JumpDispatch{binding, p, {4}, true};
...
void accelerationRequiresLaunchProofBeforePress()
{
	...
	{
		s.tick = {tick};
		auto f = feedback(s, tick * 40000);
		f.inspection->flightClear.reset();
		f.inspection->landingClear.reset();
		f.inspection->velocity.reset();
		const auto d = jump.update(f);
		assert(d.accepted && d.intent.jump != ActionRequest::Press);
		if (tick == 3)
			assert(d.state == JumpState::Accelerate && d.intent.locomotion == core::LocomotionMode::Run &&
				   d.intent.speed == 0);
	}
	s.tick = {4};
	s.velocity = model::NavVector3{120, 0, 0};
...
void readinessAndAttemptRegistry()
{
	...
	s.tick = {1};
	assert(jump.update(feedback(s, 40000)).state == JumpState::Align);
	s.tick = {2};
	assert(jump.update(feedback(s, 80000)).state == JumpState::Accelerate);
	s.tick = {3};
	auto d = jump.update(feedback(s, 120000));
	assert(d.state == JumpState::Accelerate && !d.terminalEvent && d.readiness == JumpReadiness::UnderSpeed &&
		   d.intent.locomotion == core::LocomotionMode::Run);
	s.velocity = model::NavVector3{120, 23, 0};
	s.tick = {4};
	d = jump.update(feedback(s, 160000));
	assert(d.state == JumpState::Accelerate && d.readiness == JumpReadiness::LateralError && !d.terminalEvent);
	s.velocity = model::NavVector3{190, 0, 0};
	s.tick = {5};
	d = jump.update(feedback(s, 200000));
	...
	++slow.tick.value;
	assert(limited.update(feedback(slow, 80000)).state == JumpState::Accelerate);
	++slow.tick.value;
	d = limited.update(feedback(slow, 120000));
	...
	const JumpAttemptKey first{binding.agent, binding.actor, binding.map,
							   plan.source,	  plan.target,	 model::NavTraversalKind::Jump};
	auto sibling = first;
	sibling.target = {3};
	...
	auto stampedActor = actor();
	stampedActor.tick = {1};
	auto stale = feedback(stampedActor, 1240);
	stale.inspection->attemptId = owned.attemptId + 1;
...
int main()
{
	physics();
	failures();
	accelerationRequiresLaunchProofBeforePress();
	readinessAndAttemptRegistry();
	crouchJumpConstraints();
}
