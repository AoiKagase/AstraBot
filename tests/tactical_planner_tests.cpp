...
t::TacticalRoute route(t::RouteStyle style, std::uint32_t id, std::uint64_t eta = 1'000'000)
{
...
t::TacticalContext context()
{
	...
	result.self = {kSelf, {1}, p::Team::CounterTerrorist, t::RolePreference::Any, {0.0, 0.0, 0.0}, {100}, 100.0F,
				   100,	  true};
	result.objective = {};
...
void addRoute(t::TacticalContext& state, t::RouteStyle style, std::uint32_t id, std::uint64_t eta = 1'000'000)
{
...
{
	return {player, {areaId}, {static_cast<double>(areaId), 0.0, 0.0}, 0.95, 0, true, false};
}
...
void testAutonomousRoamAndPriorityContracts()
{
	...
	assert(explicitDecision.intent.target.area == astrabot::nav::model::NavAreaId{500});
}
} // namespace
