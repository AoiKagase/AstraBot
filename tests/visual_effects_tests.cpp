...
int main()
{
	...
	for (unsigned i = 0; i < 32; ++i)
		assert(effects.smoke({static_cast<double>(i) * 30, 0, 0}));
	...
	assert(effects.flash(actor, 100));
	effects.forget(actor);
	assert(effects.blocked(actor, {0, 0, 0}, {1, 0, 0}) == p::Reason::None);
	assert(effects.smoke({0, 0, 0}));
	assert(effects.advance({1}, {2}, 2100));
	assert(effects.diagnostics().regions == 0);
	assert(effects.smoke({0, 0, 0}));
	assert(effects.advance({2}, {1}, 0));
	assert(effects.diagnostics().regions == 0);
	assert(effects.advance({2}, {1}, 100));
	assert(effects.smoke({0, 0, 0}));
	...
	a::VisualEffects invalid({0, 1});
	assert(!invalid.advance({1}, {1}, 0));
	a::VisualEffects invalidTime({1, 0});
	assert(!invalidTime.advance({1}, {1}, 0));
