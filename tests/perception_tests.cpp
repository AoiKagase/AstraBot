...
void advance(InputFrame& f, std::uint64_t dt = 100000)
{
	++f.tick.value;
	f.timeMicros += dt;
}
void roundIdentity()
{
	...
	v.beginRound({2});
	assert(!v.latest(observer));
	advance(f);
	v.update(f, port);
	assert(!v.latest(observer) && v.frameReason() == Reason::InvalidFrame);
	f.round = {2};
	v.update(f, port);
	assert(v.latest(observer));
...
void geometryAndPublication()
{
	...
	Vision narrow({200000, 50, 60});
	Port clear;
	auto shortRange = frame();
	...
	Vision bad({0, 4096, 90});
	bad.update(shortRange, clear);
...
void scheduling()
{
	for (const unsigned actors : {1U, 8U, 16U, 32U})
		for (const std::uint64_t dt : {8000ULL, 16000ULL, 100000ULL})
		{
			InputFrame f{};
			f.map = {1};
			f.tick = {1};
			...
			for (std::size_t i = 0; i < 32; ++i)
				f.players[i] = {{static_cast<std::uint16_t>(i + 1), {1}},
								{i < actors ? static_cast<std::uint32_t>(i + 1) : 0U},
								true,
								{static_cast<double>(i), 0, 16},
