...
struct Scenario final
{
	...
	Scenario(astrabot::core::PlayerId player = {1, {1}}, astrabot::core::PlayerId targetPlayer = {2, {1}},
	...
	void refresh()
	{
		const p::Stamp stamp{input.agent, input.player, input.map, input.tick, input.timeMicros, input.round};
		visual.stamp = stamp;
		if (visual.count != 0)
		{
			visual.memories[0].target = target;
...
void testReloadSwitchAndStaleLifecycle()
{
	...
	assert((reload.decision.buttons & static_cast<astrabot::core::ButtonMask>(astrabot::core::Button::Reload)) != 0U);
...
void testTargetReplacementAndAllyGate()
{
	...
	scenario.input.world.roster[replacement.slot - 1U] = {replacement, p::Team::Terrorist};
	scenario.next();
	...
	scenario.input.world.roster[replacement.slot - 1U] = {replacement, p::Team::CounterTerrorist};
	scenario.next();
	...
	for (const auto load : {1U, 8U, 16U})
	{
		for (const auto intervalMs : {8U, 16U, 100U})
		{
			...
			for (unsigned pass = 0; pass < 2; ++pass)
			{
				...
				{
					const auto bot = astrabot::core::PlayerId{static_cast<std::uint16_t>(index * 2U + 1U), {1}};
					const auto enemy = astrabot::core::PlayerId{static_cast<std::uint16_t>(index * 2U + 2U), {1}};
					Scenario scenario{bot, enemy, {index + 1U}};
					scenario.input.weapon.activeClass = c::WeaponSnapshot::WeaponClass::Pistol;
					...
					(pass == 0 ? firstActions : secondActions)[index] = result.decision.action;
				}
			}
			for (unsigned index = 0; index < load; ++index)
			{
