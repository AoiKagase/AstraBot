...
void assertReload(const c::FireAuthorization& result)
{
	...
	assert(result.decision.buttons == static_cast<astrabot::core::ButtonMask>(astrabot::core::Button::Reload));
...
void assertFirePattern(const c::FireAuthorization& result, c::FirePattern pattern, std::uint8_t burstShots)
{
...
{
	{
		...
		const auto pending = c::authorizeFire(fixture.input, fixture.aim, result.nextState);
		...
		const auto completed = c::authorizeFire(fixture.input, fixture.aim, pending.nextState);
	...
	}
	{
		...
		fixture.input.weapon.reserveAmmo = 24;
		fixture.input.weapon.canReload = true;
		fixture.aim = c::CombatDecision::noOp(fixture.input.tick, c::CombatReason::NoTarget);
		const auto result = c::authorizeFire(fixture.input, fixture.aim, {});
		assertReload(result);
	}
	{
...
{
	{
		...
		const auto duplicate = c::authorizeFire(fixture.input, fixture.aim, result.nextState);
...
void testCadenceIsBoundedAndReevaluated()
{
	...
	const auto resumed = c::authorizeFire(fixture.input, fixture.aim, paused.nextState);
...
void testCadenceInterruptionAndDeterminism()
{
	...
	assert(firstResult.nextState.cadenceShotsFired == secondResult.nextState.cadenceShotsFired);

	first.advance();
	first.visual.count = 0;
	const auto interrupted = c::authorizeFire(first.input, first.aim, firstResult.nextState);
