...
int main()
{
	...
	const auto colors = [&]() {
		for (unsigned i = 0; i < 4; ++i)
			decoder.writeByte(255);
	};
	decoder.begin(15, 1);
	decoder.writeShort(4096);
	decoder.writeShort(8192);
	decoder.writeShort(0);
	colors();
	decoder.end();
	...
	assert(decoder.lastEvent().kind == a::MessageKind::None &&
		   decoder.lastError() == a::MessageDecodeError::UnexpectedField);
	decoder.reset();
	decoder.begin(15, 1);
	decoder.writeShort(65536);
	decoder.end();
	assert(decoder.lastEvent().kind == a::MessageKind::None &&
		   decoder.lastError() == a::MessageDecodeError::InvalidShape);
	decoder.reset();
	decoder.begin(15, 1);
	decoder.writeShort(1);
	decoder.writeShort(1);
	...
	decoder.writeShort(1);
	decoder.writeShort(0);
	decoder.writeByte(256);
	decoder.end();
	assert(decoder.lastEvent().kind == a::MessageKind::None);
	decoder.configure({11, 12, 13, 14, 0}, nullptr, nullptr);
	decoder.begin(15, 1);
