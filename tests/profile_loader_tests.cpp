...
int main()
{
	...
	if (!check(loader.loadText(validText, std::strlen(validText), &catalog) ==
			ProfileLoadResult::Loaded, "valid profile text loads"))
	{
		return 1;
	}
	...
	if (!check(catalog.selectNamed("Quoted Alpha", ProfileTeam::CounterTerrorist, &selected) ==
			ProfileSelectionResult::Selected && selected.skill == 90 &&
			selected.aggression == 80, "quoted profile fields are preserved"))
	{
		return 1;
	}
	...
	if (!check(boundedLoader.loadText(tooLargeText, std::strlen(tooLargeText), &catalog) ==
			ProfileLoadResult::TooLarge && catalog.size() == 2U,
			"oversized input is rejected before publication"))
	{
		return 1;
	}

	const char boundedProfilesText[] =
		"One { Skill = 10 }\n"
		"Two { Skill = 20 }\n";
	const ProfileLoadLimits profileLimits = {sizeof(boundedProfilesText), 1U};
	ProfileLoader profileCountLoader(profileLimits);
	if (!check(profileCountLoader.loadText(
			boundedProfilesText, std::strlen(boundedProfilesText), &catalog) ==
			ProfileLoadResult::CatalogFull && catalog.size() == 2U,
	...
	if (!check(loader.loadFile("missing-profile.db", &catalog) ==
			ProfileLoadResult::MissingFile, "missing profile file is explicit"))
	{
		return 1;
	}
