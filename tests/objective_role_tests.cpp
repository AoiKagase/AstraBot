...
BotAgentId agent(std::uint32_t value)
{
	return {value};
}

t::ObjectiveTargetId targetId(EntityId value, std::uint32_t generation = 1)
{
	return {value, Generation{generation}};
}

t::TeamSnapshot snapshot(std::size_t count = 5,
						 astrabot::core::perception::Team side = astrabot::core::perception::Team::CounterTerrorist)
{
	...
	for (std::size_t i = 0; i < count; ++i)
	{
		...
		result.members[i] = {player(slot),
							 agent(static_cast<std::uint32_t>(i + 1)),
...
std::size_t countKind(const t::TeamDecision& decision, t::ObjectiveAssignmentKind kind)
{
	std::size_t count = 0;
...
const t::ObjectiveAssignment* findKind(const t::TeamDecision& decision, t::ObjectiveAssignmentKind kind)
{
	for (std::size_t i = 0; i < decision.shared.objectiveAssignmentCount; ++i)
	{
		if (decision.shared.objectiveAssignments[i].kind == kind)
...
bool hasAssignment(const t::TeamDecision& decision, t::ObjectiveAssignmentKind kind, PlayerId playerId,
				   t::ObjectiveTargetId target = {})
{
	for (std::size_t i = 0; i < decision.shared.objectiveAssignmentCount; ++i)
	{
		const auto& assignment = decision.shared.objectiveAssignments[i];
		if (assignment.kind == kind && assignment.player == playerId && assignment.target == target)
		{
			return true;
		}
	}
...
void assertNoDuplicateExclusive(const t::TeamDecision& decision)
{
	for (std::size_t i = 0; i < decision.shared.objectiveAssignmentCount; ++i)
	{
		...
		for (std::size_t j = 0; j < i; ++j)
		{
			...
			assert(!(right.exclusive && left.kind == right.kind && left.playerTarget == right.playerTarget &&
...
void testObjectiveContractsRejectInvalidAndStaleTargets()
{
	...
	for (std::size_t i = 0; i < decision.shared.objectiveAssignmentCount; ++i)
	{
		assert(decision.shared.objectiveAssignments[i].valid(input.map, input.round, input.nowMicros));
	}
}
...
void testVipEscortAndInterceptAssignments()
{
	...
	for (std::size_t i = 0; i < input.memberCount; ++i)
	{
		if (input.members[i].player == deadEscort)
			input.members[i].alive = false;
	}
	t::TeamEvents events{};
	events.botDied = true;
	...
	assert(!hasAssignment(replacement, t::ObjectiveAssignmentKind::VipEscort, deadEscort));
...
void testHostagesGetDistinctOwnershipAndFollowReassignment()
{
	...
	for (std::size_t i = 0; i < input.memberCount; ++i)
	{
		if (input.members[i].player == deadRescuer)
			input.members[i].alive = false;
	}
	t::TeamEvents events{};
	...
	assert(!hasAssignment(replacement, t::ObjectiveAssignmentKind::HostageRescuer, deadRescuer));

	input = snapshot();
	input.objective = {t::ObjectivePhase::Escort, {}, 0, 0, true};
	input.objective.family = t::ObjectiveFamily::HostageRescue;
	input.objective.routeArea = 77;
	input.objective.targets[0] = {targetId(10), {}, player(3), true, true, false};
	input.objective.targetCount = 1;
	events = {};
	events.hostageOwnershipChanged = true;
	const auto following = director.update(input, events);
	assert(hasAssignment(following, t::ObjectiveAssignmentKind::HostageEscort, player(3), targetId(10)));
}
...
void testEscapeAssignmentsReplanForBothSides()
{
	...
	for (std::size_t i = 0; i < input.memberCount; ++i)
	{
		if (input.members[i].player == deadRunner)
			input.members[i].alive = false;
	}
	t::TeamEvents events{};
	...
	assert(!hasAssignment(replacement, t::ObjectiveAssignmentKind::EscapeRunner, deadRunner));
...
void testMapRoundGenerationAndInputOrderAreSafe()
{
	...
	assert(first.shared.objectiveAssignmentCount == transitioned.shared.objectiveAssignmentCount);
	...
	assert(beforeReuse.shared.objectiveAssignmentCount == afterReuse.shared.objectiveAssignmentCount);
	...
	assert(leftDecision.shared.objectiveAssignmentCount == rightDecision.shared.objectiveAssignmentCount);
	for (std::size_t i = 0; i < leftDecision.shared.objectiveAssignmentCount; ++i)
	{
		const auto& assignment = leftDecision.shared.objectiveAssignments[i];
		bool found = false;
		for (std::size_t j = 0; j < rightDecision.shared.objectiveAssignmentCount; ++j)
		{
			const auto& candidate = rightDecision.shared.objectiveAssignments[j];
			if (candidate.kind == assignment.kind && candidate.player == assignment.player &&
