...
t::TeamSnapshot snapshot(std::size_t count = 5)
{
	...
	for (std::size_t i = 0; i < count; ++i)
	{
		...
		result.members[i] = {
			...
			i == 0 ? t::Personality::Aggressive : (i == 1 ? t::Personality::Supportive : t::Personality::Balanced),
			{i == 0 ? t::PlanIntent::Entry : (i == 1 ? t::PlanIntent::Support : t::PlanIntent::Attack), 0},
			false,
			false,
			true,
			true};
...
bool hasRole(const t::TeamDecision& decision, t::Role role)
{
	...
	{
		if (decision.shared.assignments[i].role == role)
			return true;
	}
	return false;
}
...
{
	auto input = snapshot();
	input.observations[0] = {input.members[0].player, player(31), {400.0, 20.0, 0.0},
							 input.nowMicros,		  0.8F,		  t::SharedObservationSource::DirectVision};
	input.observationCount = 1;
	input.proposals[0] = {input.members[1].agent, t::PlanIntent::Support, 42, 3, true};
