...
void contextualDangerIsBoundedAndDeterministic()
{
	...
	assert(model
			   .observe({{7, p::Team::Terrorist, l::ApproachDirection::East, c::WeaponSnapshot::WeaponClass::Rifle, 9},
						 {1},
						 0.8,
						 1.0,
						 100})
			   .reason == l::ContextualDangerUpdateReason::WrongMap);
	...
	assert(model.observe({key, {1}, 0.2, 1.0, 90}).reason == l::ContextualDangerUpdateReason::StaleObservation);
...
void opponentProfilesAreMapSessionLocal()
{
	...
	const l::OpponentObservation observation{{2, {1}}, {1}, {1}, {1}, 12, c::WeaponSnapshot::WeaponClass::Rifle,
											 0.8,	   0.7, 0.1, 0.4};
	...
	assert(model.observe(nextRound).reason == l::OpponentProfileUpdateReason::RetiredGeneration);

	const l::OpponentObservation replacement{{2, {2}}, {1}, {2}, {1}, 21, c::WeaponSnapshot::WeaponClass::Pistol,
											 0.6,	   0.3, 0.2, 0.5};
...
e::TraversalObservation traversal(std::uint64_t sequence, bool succeeded,
