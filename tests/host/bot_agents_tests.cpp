...
{
	BotAgentRegistry registry;
	assert(registry.bind(PlayerId::invalid(), MapGeneration{1}).error == BotAgentError::InvalidPlayer);
	assert(registry.bind(PlayerId{1, Generation{1}}, MapGeneration::invalid()).error == BotAgentError::InvalidMap);
	assert(registry.unbind(PlayerId::invalid()).error == BotAgentError::InvalidPlayer);

	const auto binding = registry.bind(PlayerId{1, Generation{1}}, MapGeneration{1});
	assert(binding);
	registry.reset();
	assert(registry.mappingCount() == 0);
