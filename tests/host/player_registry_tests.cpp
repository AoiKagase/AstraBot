...
void testCommandAcceptanceAndRejection()
{
	...
	const auto invalid = registry.submitCommand(player, TickId{2}, BotCommand::neutral(0));
	assert(!invalid);
	...
	const auto staleGeneration = registry.submitCommand(stalePlayer, TickId{2}, valid);
...
void testMapCleanupAndInactiveRejection()
{
	...
	assert(registry.submitCommand(player, TickId{1}, BotCommand::neutral(16)).error == HostError::NotMapActive);
}
