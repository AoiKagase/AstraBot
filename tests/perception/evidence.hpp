...
struct Row
{
	...
	void sample(const char* phase, const w::WorldModel& world, const q::DistributionModel& model,
				astrabot::core::PlayerId actor, astrabot::core::PlayerId target, std::size_t traces = 0,
				std::size_t audience = 0, std::size_t queue = 0)
	{
		assert(events.size() < 32768);
		const auto snapshot = world.latest(actor);
		assert(snapshot);
		const auto& s = *snapshot;
		const auto known = s.known(target);
		const auto& wd = world.diagnostics();
		const auto& rd = world.reports().diagnostics();
		const auto& dd = model.diagnostics();
		const w::ReportMemory* report = nullptr;
		for (std::size_t i = 0; i < s.reports->count; ++i)
			if (s.reports->reports[i].report.target == target)
				report = &s.reports->reports[i];
		...
		out << "{\"phase\":\"" << phase << "\",\"actor\":" << actor.slot << ",\"generation\":" << actor.generation.value
			...
			<< ",\"visual\":" << s.visual->count << ",\"sounds\":" << s.sounds->count
			<< ",\"reports\":" << s.reports->count
			...
			<< ",\"reporter\":" << (known ? known->reporter.slot : 0) << ",\"visualAge\":" << s.oldestVisualAgeMicros
			<< ",\"soundAge\":" << s.oldestSoundAgeMicros << ",\"reportAge\":" << s.oldestReportAgeMicros
			<< ",\"delay\":" << s.maxReceiptDelayMicros << ",\"processed\":" << wd.frameProcessed
			<< ",\"visualVisits\":" << world.visual().diagnostics().frameVisits
			<< ",\"soundVisits\":" << world.sounds().diagnostics().frameVisits << ",\"reportVisits\":" << rd.frameVisits
			<< ",\"deliveries\":" << rd.frameDelivered << ",\"reportQueue\":" << rd.queued
			<< ",\"connections\":" << dd.frameConnections << ",\"mappings\":" << dd.frameMappings
			<< ",\"distributionVisits\":" << dd.frameVisits << ",\"distributionDelay\":" << dd.maxDelayMicros
			<< ",\"traces\":" << traces << ",\"audience\":" << audience << ",\"soundQueue\":" << queue
		...
		for (std::size_t i = 0; i < s.visual->count; ++i)
			if (s.visual->memories[i].target == target)
				distribution = s.distributions[i];
		if (distribution)
			for (std::size_t i = 0; i < distribution->count; ++i)
			{
				if (i)
					out << ',';
				out << '[' << distribution->areas[i].area << ',' << distribution->areas[i].weight << ']';
			}
		out << "],\"unknown\":" << (distribution ? distribution->unknownMass : 0) << '}';
...
inline void writeEvidence(const char* path, const char* producer, const std::vector<Row>& rows)
{
	...
	assert(out);
	out << "{\"schemaVersion\":1,\"producer\":\"" << producer << "\",\"worldBytes\":" << sizeof(w::WorldModel)
		<< ",\"distributionBytes\":" << sizeof(q::DistributionModel) << ",\"results\":[";
	for (std::size_t i = 0; i < rows.size(); ++i)
	{
		...
		for (std::size_t j = 0; j < row.events.size(); ++j)
		{
			if (j)
				out << ',';
			out << row.events[j];
