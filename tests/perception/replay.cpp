...
struct Replay
{
	...
	Replay(unsigned actors, unsigned us, bool hasNav) : row{"timeline", actors, us, hasNav, {}}
	{
		...
		{
			f.players[i - 1] = {{i, {1}}, {i}, true};
			eligible[i - 1] = true;
			assert(teams.bind(f.map, {i, {1}}));
			assert(teams.update(f.map, {i, {1}}, p::Team::Terrorist));
		}
		...
		if (hasNav)
		{
			...
			auto graph = e::q::NavGraph::build(mesh, {1000, 4000, 1000000});
			auto index = e::q::NavSpatialIndex::build(mesh, {1000, 1999, 1000000});
			assert(graph && index);
			nav = e::q::DistributionTopology::build(f.map, *graph.value, *index.value);
			assert(nav);
		}
	}
	p::ObservationBatch batch(PlayerId viewer, double x)
	{
		...
		b.stamp = {{viewer.slot}, viewer, f.map, f.tick, f.timeMicros, f.round};
		b.identity = {f.map, f.round, p::ObservationSource::Vision, f.tick.value, f.timeMicros, f.timeMicros};
		b.count = 1;
		b.observations[0] = {target, {x, 10, 64}};
		return b;
	}
	void step(std::uint64_t delta, bool see = false, double x = 100, bool sound = false)
	{
		...
		{
			if (see && slot != 2)
				assert(world->stage(batch({slot, {1}}, x)));
			if (sound)
				assert(world->stage({slot, {1}}, {{f.map, f.round, p::ObservationSource::Sound, f.tick.value,
												   f.timeMicros - delta, f.timeMicros},
												  p::SoundKind::Footstep,
												  {0, 0, 0}}));
		}
		assert(world->publish());
		distribution->update(*world, nav);
	}
	void record(const char* phase)
	{
		for (std::uint16_t slot = 1; slot <= row.actors; ++slot)
			row.sample(phase, *world, *distribution, {slot, {1}}, target);
	}
	void advanceTo(std::uint64_t elapsed)
	{
		...
		{
			step((std::min)(static_cast<std::uint64_t>(row.frameUs), end - f.timeMicros));
			record("decay");
		}
	}
	e::Row run()
	{
		step(0, true);
		record("sight");
		const auto old = batch({1, {1}}, 100);
		step(100000);
		record("occluded");
		step(100000, false, 100, true);
		record("sound");
		...
		if (nav)
			for (std::uint16_t slot = 1; slot <= row.actors; ++slot)
				if (slot != 2)
				{
					const auto* d = world->latest({slot, {1}})->distributions[0];
					assert(d && d->updatedMicros >= 1200000);
				}
		advanceTo(2500000);
		record("half");
		advanceTo(5000000);
		record("expired");
		step(row.frameUs, true, 210);
		record("resee");
		++f.round.value;
		step(row.frameUs);
		record("round");
		++target.generation.value;
		f.players[31].player = target;
		step(row.frameUs);
		// Old observations cannot resurrect after a round and slot reuse.
		++f.tick.value;
		++f.timeMicros;
		assert(world->advance(f, teams));
		assert(world->stage(old));
		assert(world->publish());
		distribution->update(*world, nav);
		record("reuse");
		++f.map.value;
		...
		for (std::uint16_t slot = 1; slot <= row.actors; ++slot)
		{
			assert(teams.bind(f.map, {slot, {1}}));
			assert(teams.update(f.map, {slot, {1}}, p::Team::Terrorist));
		}
