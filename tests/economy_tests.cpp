...
PlayerId player(std::uint16_t slot)
{
	return {slot, {1}};
}
...
e::TeamEconomySnapshot snapshot(std::size_t count = 5)
{
	...
	{
		const auto role =
			i == 0 ? t::Role::Entry : (i == 1 ? t::Role::Support : (i == 2 ? t::Role::Trade : t::Role::Lurk));
		result.members[i] = {player(static_cast<std::uint16_t>(i + 1)),
...
void testTeamStrategiesUseMedianAndLossState()
{
	...
	for (std::size_t i = 1; i < input.memberCount; ++i)
		input.members[i].money = 800;
...
void testTeamKitAndUtilityCoordination()
{
	...
	for (std::size_t i = 2; i < input.memberCount; ++i)
		input.members[i].team = input.team;
...
void testExecutionVerifiesInventoryAndUsesBoundedFallback()
{
	...
	const auto upgradeResult = executor.execute(upgradePlan, context(smgInventory), upgradeOperations);
